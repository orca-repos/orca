...
static auto ansiColor(uint code) -> QColor
{
...
auto AnsiEscapeCodeHandler::parseText(const FormattedText &input) -> QList<FormattedText>
{
        ...
        if (code >= TextColorStart && code <= TextColorEnd) {
          ...
          case RgbBackgroundColor:
            ...
            case 2:
              ...
              if ((i + 3) < numbers.size()) {
                (code == RgbTextColor) ? charFormat.setForeground(QColor(numbers.at(i + 1).toInt(), numbers.at(i + 2).toInt(), numbers.at(i + 3).toInt())) : charFormat.setBackground(QColor(numbers.at(i + 1).toInt(), numbers.at(i + 2).toInt(), numbers.at(i + 3).toInt()));
                setFormatScope(charFormat);
              }
              i += 3;
              break;
...
auto AnsiEscapeCodeHandler::endFormatScope() -> void
{
  m_previousFormatClosed = true;
}

auto AnsiEscapeCodeHandler::setFormatScope(const QTextCharFormat &charFormat) -> void
{
