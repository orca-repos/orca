...
ORCA_UTILS_EXPORT auto winErrorMessage(unsigned long error) -> QString
{
  ...
  const int len = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, error, 0, (LPTSTR)&lpMsgBuf, 0, NULL);
  if (len) {
...
#ifdef Q_OS_WIN
static inline auto msgCannotLoad(const char *lib, const QString &why) -> QString
{
  return QString::fromLatin1("Unable load %1: %2").arg(QLatin1String(lib), why);
}

static inline auto msgCannotResolve(const char *lib) -> QString
{
...
ORCA_UTILS_EXPORT auto winGetDLLVersion(WinDLLVersionType t, const QString &name, QString *errorMessage) -> QString
{
  ...
  using VerQueryValueWProtoType = BOOL (APIENTRY*)(LPVOID, LPWSTR lpSubBlock, LPVOID, PUINT);
  ...
  case WinDLLFileVersion:
    QTextStream(&rc) << HIWORD(versionInfo->dwFileVersionMS) << '.' << LOWORD(versionInfo->dwFileVersionMS) << '.' << HIWORD(versionInfo->dwFileVersionLS) << '.' << LOWORD(versionInfo->dwFileVersionLS);
    break;
  case WinDLLProductVersion:
    QTextStream(&rc) << HIWORD(versionInfo->dwProductVersionMS) << '.' << LOWORD(versionInfo->dwProductVersionMS) << '.' << HIWORD(versionInfo->dwProductVersionLS) << '.' << LOWORD(versionInfo->dwProductVersionLS);
    break;
  }
  return rc;
  #endif
...
ORCA_UTILS_EXPORT auto is64BitWindowsBinary(const FilePath &binaryIn) -> bool
{
...
#ifdef Q_OS_WIN
  : silenceErrorMode(SEM_NOGPFAULTERRORBOX | SEM_NOOPENFILEERRORBOX | SEM_FAILCRITICALERRORS), originalErrorMode(SetErrorMode(silenceErrorMode))
#endif
{}
