...
auto TreeItem::prependChild(TreeItem *item) -> void
{
  insertChild(0, item);
}
...
auto TreeItem::insertChild(int pos, TreeItem *item) -> void
{
...
auto TreeItem::insertOrderedChild(TreeItem *item, const std::function<bool (const TreeItem *, const TreeItem *)> &cmp) -> void
{
  auto where = std::lower_bound(begin(), end(), item, cmp);
  insertChild(int(where - begin()), item);
}

auto TreeItem::removeChildAt(int pos) -> void
{
...
auto TreeItem::removeChildren() -> void
{
  if (childCount() == 0)
    return;
  if (m_model) {
...
auto TreeItem::sortChildren(const std::function<bool(const TreeItem *, const TreeItem *)> &cmp) -> void
{
  if (m_model) {
...
auto TreeItem::update() -> void
{
  if (m_model) {
    QModelIndex idx = index();
    emit m_model->dataChanged(idx.sibling(idx.row(), 0), idx.sibling(idx.row(), m_model->m_columnCount - 1));
  }
}

auto TreeItem::updateAll() -> void
{
  if (m_model) {
    QModelIndex idx = index();
...
auto TreeItem::updateColumn(int column) -> void
{
  if (m_model) {
    QModelIndex idx = index();
...
auto TreeItem::forAllChildren(const std::function<void (TreeItem *)> &pred) const -> void
{
  for (TreeItem *item : *this) {
    pred(item);
...
auto TreeItem::forSelectedChildren(const std::function<bool (TreeItem *)> &pred) const -> void
{
  for (TreeItem *item : *this) {
    if (pred(item))
...
auto TreeItem::forChildrenAtLevel(int level, const std::function<void(TreeItem *)> &pred) const -> void
{
...
auto TreeItem::removeItemAt(int pos) -> void
{
...
auto TreeItem::expand() -> void
{
  QTC_ASSERT(m_model, return);
  emit m_model->requestExpansion(index());
}

auto TreeItem::collapse() -> void
{
  QTC_ASSERT(m_model, return);
  emit m_model->requestCollapse(index());
}

auto TreeItem::propagateModel(BaseTreeModel *m) -> void
{
  QTC_ASSERT(m, return);
...
BaseTreeModel::BaseTreeModel(QObject *parent) : QAbstractItemModel(parent), m_root(new TreeItem)
{
  m_columnCount = 1;
  m_root->m_model = this;
  #if USE_MODEL_TEST
...
BaseTreeModel::BaseTreeModel(TreeItem *root, QObject *parent) : QAbstractItemModel(parent), m_root(root)
{
  m_columnCount = 1;
...
auto BaseTreeModel::setData(const QModelIndex &idx, const QVariant &data, int role) -> bool
{
  ...
  if (res) emit dataChanged(idx, idx);
  return res;
}
...
auto BaseTreeModel::fetchMore(const QModelIndex &idx) -> void
{
  if (!idx.isValid())
    return;
  TreeItem *item = itemForIndex(idx);
...
auto BaseTreeModel::setRootItem(TreeItem *item) -> void
{
...
auto BaseTreeModel::setHeader(const QStringList &displays) -> void
{
  m_header = displays;
  m_columnCount = displays.size();
}

auto BaseTreeModel::setHeaderToolTip(const QStringList &tips) -> void
{
...
auto BaseTreeModel::destroyItem(TreeItem *item) -> void
{
  delete takeItem(item);
}

StaticTreeItem::StaticTreeItem(const QStringList &displays) : m_displays(displays) {}

StaticTreeItem::StaticTreeItem(const QString &display) : m_displays(display) {}

StaticTreeItem::StaticTreeItem(const QStringList &displays, const QStringList &toolTips) : m_displays(displays), m_toolTips(toolTips) {}
