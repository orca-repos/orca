...
public:
  JobKillOnClose()
  {
    ...
    if (!m_job) {
      fwprintf(stderr, L"orca_ctrlc_stub: CreateJobObject failed: 0x%x.\n", GetLastError());
      return;
    }
    JOBOBJECT_EXTENDED_LIMIT_INFORMATION jeli = {0};
    jeli.BasicLimitInformation.LimitFlags = JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE;
    if (!SetInformationJobObject(m_job, JobObjectExtendedLimitInformation, &jeli, sizeof(jeli))) {
      fwprintf(stderr, L"orca_ctrlc_stub: SetInformationJobObject failed: 0x%x.\n", GetLastError());
    }
  }

  auto AssignProcessToJob(HANDLE process) const -> BOOL
  {
...
constexpr wchar_t szTitle[] = L"qtcctrlcstub";
constexpr wchar_t szWindowClass[] = L"wcqtcctrlcstub";
constexpr wchar_t szNice[] = L"-nice ";
...
auto WINAPI shutdownHandler(DWORD dwCtrlType) -> BOOL;
auto WINAPI interruptHandler(DWORD dwCtrlType) -> BOOL;
auto isSpaceOrTab(wchar_t c) -> bool;
auto startProcess(wchar_t pCommandLine[], bool lowerPriority, const JobKillOnClose &job) -> bool;

auto main(int argc, char **) -> int
{
  if (argc < 2) {
    fprintf(stderr, "This is an internal helper of Orca. Do not run it manually.\n");
    return 1;
  }
  ...
  hwndMain = CreateWindow(szWindowClass, szTitle, WS_DISABLED, 0, 0, 0, 0, NULL, NULL, wcex.hInstance, NULL);
  if (!hwndMain)
    return FALSE;
...
auto isSpaceOrTab(const wchar_t c) -> bool
{
  return c == L' ' || c == L'\t';
}
...
auto WINAPI processWatcherThread(LPVOID lpParameter) -> DWORD
{
...
auto startProcess(wchar_t *pCommandLine, bool lowerPriority, const JobKillOnClose &job) -> bool
{
  ...
  if (!bSuccess) {
    fwprintf(stderr, L"orca_ctrlc_stub: Command line failed: %s\n", pCommandLine);
    return false;
  }
  CloseHandle(pi.hThread);

  if (!job.AssignProcessToJob(pi.hProcess)) {
    fwprintf(stderr, L"orca_ctrlc_stub: AssignProcessToJobObject failed: 0x%x.\n", GetLastError());
    return false;
  }

  HANDLE hThread = CreateThread(NULL, 0, processWatcherThread, reinterpret_cast<void*>(pi.hProcess), 0, NULL);
  if (!hThread) {
    fwprintf(stderr, L"orca_ctrlc_stub: The watch dog thread cannot be started.\n");
    return false;
  }
  CloseHandle(hThread);
  return true;
