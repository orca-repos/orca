...
SaveFile::SaveFile(const QString &filename) : m_finalFileName(filename) {}
...
auto SaveFile::open(OpenMode flags) -> bool
{
  ...
  if (ofi.exists()) {
    ...
    Permissions permAll = QFile::ReadOwner | QFile::ReadGroup | QFile::ReadOther | QFile::WriteOwner | QFile::WriteGroup | QFile::WriteOther;
...
auto SaveFile::commit() -> bool
{
  ...
  m_tempFile.reset();
  bool result = ReplaceFile(finalFileName.toStdWString().data(), fileName().toStdWString().data(), nullptr, REPLACEFILE_IGNORE_MERGE_ERRORS, nullptr, nullptr);
  if (!result) {
    ...
    if (!QFile::exists(finalFileName)) {
      ...
      if (replaceErrorCode == ERROR_UNABLE_TO_REMOVE_REPLACED) {
        ...
        result = MoveFileEx(fileName().toStdWString().data(), finalFileName.toStdWString().data(), MOVEFILE_COPY_ALLOWED | MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH);
        if (!result)
          replaceErrorCode = GetLastError();
      }
      if (!result) {
        wchar_t messageBuffer[256];
        FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, replaceErrorCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), messageBuffer, sizeof(messageBuffer), nullptr);
...
auto SaveFile::initializeUmask() -> void
{
  #ifdef Q_OS_WIN
