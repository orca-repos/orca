...
enum DebugSymbolsType {
  ...
  // No usable symbols.
  LinkedSymbols = 2,
  // Link to symols available.
  BuildIdSymbols = 4,
  // BuildId available.
  PlainSymbols = 8,
  // Ordinary symbols available.
...
class ORCA_UTILS_EXPORT ElfData {
public:
  ElfData() = default;
  auto indexOf(const QByteArray &name) const -> int;
...
class ORCA_UTILS_EXPORT ElfReader {
...
public:
  ...
  auto readHeaders() -> ElfData;
  auto readSection(const QByteArray &sectionName) -> QSharedPointer<ElfMapper>;
  auto errorString() const -> QString { return m_errorString; }
  auto readCoreName(bool *isCore) -> QByteArray;
...
class ORCA_UTILS_EXPORT ElfMapper {
public:
