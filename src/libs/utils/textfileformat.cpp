...
auto operator<<(QDebug d, const TextFileFormat &format) -> QDebug
{
  ...
  nsp << " hasUtf8Bom=" << format.hasUtf8Bom << (format.lineTerminationMode == TextFileFormat::LFLineTerminator ? " LF" : " CRLF");
  return d;
}
...
auto TextFileFormat::detect(const QByteArray &data) -> TextFileFormat
{
...
auto TextFileFormat::decodingErrorSample(const QByteArray &data) -> QByteArray
{
...
static auto verifyDecodingError(const QString &text, const QTextCodec *codec, const char *data, const int dataSize, const bool possibleHeader) -> bool
{
  const QByteArray verifyBuf = codec->fromUnicode(text); // slow
  // the minSize trick lets us ignore unicode headers
  const int minSize = qMin(verifyBuf.size(), dataSize);
  return (minSize < dataSize - (possibleHeader ? 4 : 0) || memcmp(verifyBuf.constData() + verifyBuf.size() - minSize, data + dataSize - minSize, minSize));
}
...
auto decodeTextFileContent(const QByteArray &dataBA, const TextFileFormat &format, Target *target, void (Target::*appendFunction)(const QString &)) -> bool
{
  ...
  for (const char *data = start; data < end;) {
    ...
    if (!hasDecodingError)
      hasDecodingError = verifyDecodingError(text, format.codec, chunkStart, data - chunkStart, chunkStart == start);
...
auto TextFileFormat::decode(const QByteArray &data, QString *target) const -> bool
{
  target->clear();
...
auto TextFileFormat::decode(const QByteArray &data, QStringList *target) const -> bool
{
  target->clear();
...
template <class Target>
auto readTextFile(const FilePath &filePath, const QTextCodec *defaultCodec, Target *target, TextFileFormat *format, QString *errorString, QByteArray *decodingErrorSampleIn = nullptr) -> TextFileFormat::ReadResult
{
...
auto TextFileFormat::readFile(const FilePath &filePath, const QTextCodec *defaultCodec, QStringList *plainTextList, TextFileFormat *format, QString *errorString, QByteArray *decodingErrorSample /* = 0 */) -> TextFileFormat::ReadResult
{
  const TextFileFormat::ReadResult result = readTextFile(filePath, defaultCodec, plainTextList, format, errorString, decodingErrorSample);
  if (debug)
    qDebug().nospace() << Q_FUNC_INFO << filePath << ' ' << *format << " returns " << result << '/' << plainTextList->size() << " chunks";
  return result;
}
...
auto TextFileFormat::readFile(const FilePath &filePath, const QTextCodec *defaultCodec, QString *plainText, TextFileFormat *format, QString *errorString, QByteArray *decodingErrorSample /* = 0 */) -> TextFileFormat::ReadResult
{
  const TextFileFormat::ReadResult result = readTextFile(filePath, defaultCodec, plainText, format, errorString, decodingErrorSample);
  if (debug)
    qDebug().nospace() << Q_FUNC_INFO << filePath << ' ' << *format << " returns " << result << '/' << plainText->size() << " characters";
  return result;
}

auto TextFileFormat::readFileUTF8(const FilePath &filePath, const QTextCodec *defaultCodec, QByteArray *plainText, QString *errorString) -> TextFileFormat::ReadResult
{
  QByteArray data;
  try {
    FileReader reader;
...
auto TextFileFormat::writeFile(const FilePath &filePath, QString plainText, QString *errorString) const -> bool
{
  ...
  if (debug)
    qDebug().nospace() << Q_FUNC_INFO << filePath << ' ' << *this << ' ' << plainText.size() << " bytes, returns " << ok;
  return ok;
}
