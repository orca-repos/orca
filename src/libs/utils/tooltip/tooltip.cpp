...
auto ToolTip::instance() -> ToolTip*
{
  static ToolTip tooltip;
  return &tooltip;
}

static auto createF1Icon() -> QWidget*
{
...
*/
auto ToolTip::show(const QPoint &pos, const QString &content, QWidget *w, const QVariant &contextHelp, const QRect &rect) -> void
{
...
/*!
    ...
    \a contextHelp of the current shown tool tip can be retrieved via ToolTip::contextHelp().
*/
auto ToolTip::show(const QPoint &pos, const QString &content, Qt::TextFormat format, QWidget *w, const QVariant &contextHelp, const QRect &rect) -> void
{
  if (content.isEmpty()) {
    instance()->hideTipWithDelay();
  } else {
    if (contextHelp.isNull()) {
      instance()->showInternal(pos, QVariant::fromValue(TextItem(content, format)), TextContent, w, contextHelp, rect);
    } else {
...
/*!
    ...
    \a contextHelp of the current shown tool tip can be retrieved via ToolTip::contextHelp().
*/
auto ToolTip::show(const QPoint &pos, const QColor &color, QWidget *w, const QVariant &contextHelp, const QRect &rect) -> void
{
  if (!color.isValid())
    instance()->hideTipWithDelay();
  else
...
/*!
    ...
    \a contextHelp of the current shown tool tip can be retrieved via ToolTip::contextHelp().
*/
auto ToolTip::show(const QPoint &pos, QWidget *content, QWidget *w, const QVariant &contextHelp, const QRect &rect) -> void
{
  if (!content)
    instance()->hideTipWithDelay();
  else
...
/*!
    ...
    \a contextHelp of the current shown tool tip can be retrieved via ToolTip::contextHelp().
*/
auto ToolTip::show(const QPoint &pos, QLayout *content, QWidget *w, const QVariant &contextHelp, const QRect &rect) -> void
{
...
auto ToolTip::move(const QPoint &pos) -> void
{
  if (isVisible())
    instance()->placeTip(pos);
}

auto ToolTip::pinToolTip(QWidget *w, QWidget *parent) -> bool
{
...
auto ToolTip::contextHelp() -> QVariant
{
  return instance()->m_tip ? instance()->m_tip->contextHelp() : QVariant();
}

auto ToolTip::acceptShow(const QVariant &content, int typeId, const QPoint &pos, QWidget *w, const QVariant &contextHelp, const QRect &rect) -> bool
{
  if (isVisible()) {
  ...
  else if (m_tip && (QApplication::isEffectEnabled(Qt::UI_FadeTooltip) || QApplication::isEffectEnabled(Qt::UI_AnimateTooltip))) {
    hideTipImmediately();
  }
  #endif
  return true;
...
auto ToolTip::setUp(const QPoint &pos, QWidget *w, const QRect &rect) -> void
{
  m_tip->configure(pos);
...
auto ToolTip::tipChanged(const QPoint &pos, const QVariant &content, int typeId, QWidget *w, const QVariant &contextHelp) const -> bool
{
...
auto ToolTip::setTipRect(QWidget *w, const QRect &rect) -> void
{
  if (!m_rect.isNull() && !w) {
...
auto ToolTip::isVisible() -> bool
{
  ToolTip *t = instance();
  return t->m_tip && t->m_tip->isVisible();
}

auto ToolTip::offsetFromPosition() -> QPoint
{
  return QPoint(2, HostOsInfo::isWindowsHost() ? 21 : 16);
}

auto ToolTip::showTip() -> void
{
  #if !defined(QT_NO_EFFECTS) && !defined(Q_OS_MAC)
...
auto ToolTip::hide() -> void
{
  instance()->hideTipWithDelay();
}

auto ToolTip::hideImmediately() -> void
{
  instance()->hideTipImmediately();
}

auto ToolTip::hideTipWithDelay() -> void
{
  if (!m_hideDelayTimer.isActive())
    m_hideDelayTimer.start(300);
}

auto ToolTip::hideTipImmediately() -> void
{
  if (m_tip) {
    m_tip->close();
    m_tip->deleteLater();
...
auto ToolTip::showInternal(const QPoint &pos, const QVariant &content, int typeId, QWidget *w, const QVariant &contextHelp, const QRect &rect) -> void
{
...
auto ToolTip::placeTip(const QPoint &pos) -> void
{
  QScreen *qscreen = QGuiApplication::screenAt(pos);
  if (!qscreen)
    qscreen = QGuiApplication::primaryScreen();
...
auto ToolTip::eventFilter(QObject *o, QEvent *event) -> bool
{
  if (m_tip && event->type() == QEvent::ApplicationStateChange && QGuiApplication::applicationState() != Qt::ApplicationActive) {
    hideTipImmediately();
  }
  ...
  case QEvent::Wheel:
    if (m_tip) {
      if (m_tip->isInteractive()) {
        // Do not close on interaction with the tooltip
  ...
  case QEvent::MouseMove:
    if (o == m_widget && !m_rect.isNull() && !m_rect.contains(static_cast<QMouseEvent*>(event)->pos())) {
      hideTipWithDelay();
    }
    break;
  default:
