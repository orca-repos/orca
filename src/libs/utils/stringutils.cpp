...
auto AbstractMacroExpander::expandNestedMacros(const QString &str, int *pos, QString *ret) -> bool
{
  QString varName;
        ...
        if (!pattern.isEmpty() && currArg == &replace) {
          ...
          if (regexp.isValid()) {
            if (replaceAll) {
              ...
              if (match.hasMatch()) {
                *ret = ret->left(match.capturedStart(0)) + match.captured(0).replace(regexp, replace) + ret->mid(match.capturedEnd(0));
              }
            }
          }
        }
...
auto AbstractMacroExpander::findMacro(const QString &str, int *pos, QString *ret) -> int
{
  forever {
...
ORCA_UTILS_EXPORT auto parseUsedPortFromNetstatOutput(const QByteArray &line) -> int
{
  ...
  if (!ok) {
    qWarning("%s: Unexpected string '%s' is not a port. Tried to read from '%s'", Q_FUNC_INFO, line.data(), portString.data());
    return -1;
  }
  return port;
}
