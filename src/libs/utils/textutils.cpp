...
auto convertPosition(const QTextDocument *document, int pos, int *line, int *column) -> bool
{
...
auto convertPosition(const QTextDocument *document, int pos) -> OptionalLineColumn
{
...
auto positionInText(const QTextDocument *textDocument, int line, int column) -> int
{
...
auto textAt(QTextCursor tc, int pos, int length) -> QString
{
  if (pos < 0)
    pos = 0;
...
auto selectAt(QTextCursor textCursor, int line, int column, uint length) -> QTextCursor
{
  if (line < 1)
    line = 1;
...
auto flippedCursor(const QTextCursor &cursor) -> QTextCursor
{
...
static auto isValidIdentifierChar(const QChar &c) -> bool
{
  return c.isLetter() || c.isNumber() || c == QLatin1Char('_') || c.isHighSurrogate() || c.isLowSurrogate();
}

static auto isAfterOperatorKeyword(QTextCursor cursor) -> bool
{
...
auto wordStartCursor(const QTextCursor &textCursor) -> QTextCursor
{
...
auto wordUnderCursor(const QTextCursor &cursor) -> QString
{
...
auto utf8NthLineOffset(const QTextDocument *textDocument, const QByteArray &buffer, int line) -> int
{
...
auto utf16LineColumn(const QByteArray &utf8Buffer, int utf8Offset) -> LineColumn
{
  Utils::LineColumn lineColumn;
  lineColumn.line = static_cast<int>(std::count(utf8Buffer.begin(), utf8Buffer.begin() + utf8Offset, '\n')) + 1;
  const int startOfLineOffset = utf8Offset ? (utf8Buffer.lastIndexOf('\n', utf8Offset - 1) + 1) : 0;
  lineColumn.column = QString::fromUtf8(utf8Buffer.mid(startOfLineOffset, utf8Offset - startOfLineOffset)).length() + 1;
  return lineColumn;
}

auto utf16LineTextInUtf8Buffer(const QByteArray &utf8Buffer, int currentUtf8Offset) -> QString
{
  const int lineStartUtf8Offset = currentUtf8Offset ? (utf8Buffer.lastIndexOf('\n', currentUtf8Offset - 1) + 1) : 0;
  const int lineEndUtf8Offset = utf8Buffer.indexOf('\n', currentUtf8Offset);
  return QString::fromUtf8(utf8Buffer.mid(lineStartUtf8Offset, lineEndUtf8Offset - lineStartUtf8Offset));
}

static auto isByteOfMultiByteCodePoint(unsigned char byte) -> bool
{
  return byte & 0x80; // Check if most significant bit is set
}

auto utf8AdvanceCodePoint(const char *&current) -> bool
{
...
auto applyReplacements(QTextDocument *doc, const Replacements &replacements) -> void
{
  ...
  for (const Utils::Text::Replacement &replacement : replacements) {
    ...
    editCursor.movePosition(QTextCursor::NextCharacter, QTextCursor::KeepAnchor, replacement.length);
