...
static auto getHalfWord(const unsigned char *&s, const ElfData &context) -> quint16
{
  quint16 res;
...
static auto getWord(const unsigned char *&s, const ElfData &context) -> quint32
{
  quint32 res;
  if (context.endian == Elf_ELFDATA2MSB)
...
static auto getAddress(const unsigned char *&s, const ElfData &context) -> quint64
{
  quint64 res;
...
static auto getOffset(const unsigned char *&s, const ElfData &context) -> quint64
{
  return getAddress(s, context);
}

static auto parseSectionHeader(const uchar *s, ElfSectionHeader *sh, const ElfData &context) -> void
{
...
static auto parseProgramHeader(const uchar *s, ElfProgramHeader *sh, const ElfData &context) -> void
{
  sh->type = getWord(s, context);
  sh->offset = getOffset(s, context);
  /* p_vaddr = */
  getAddress(s, context);
  /* p_paddr = */
  getAddress(s, context);
...
ElfMapper::ElfMapper(const ElfReader *reader) : binary(reader->m_binary) {}

auto ElfMapper::map() -> bool
{
...
ElfReader::ElfReader(const FilePath &binary) : m_binary(binary) {}

auto ElfReader::readHeaders() -> ElfData
{
  readIt();
  return m_elfData;
}

static auto msgInvalidElfObject(const FilePath &binary, const QString &why) -> QString
{
  return ElfReader::tr("\"%1\" is an invalid ELF object (%2)").arg(binary.toUserOutput(), why);
}

auto ElfReader::readIt() -> ElfReader::Result
{
  ...
  if (quint64(e_shnum) * e_shentsize > fdlen) {
    const QString reason = tr("announced %n sections, each %1 bytes, exceed file size", nullptr, e_shnum).arg(e_shentsize);
  ...
  if (e_shoff) {
    ...
    if (quint32(stringTableFileOffset + e_shentsize) >= fdlen || stringTableFileOffset == 0) {
    ...
    for (quint32 i = 0; i < e_shnum; ++i) {
      ...
      if (stringTableFileOffset + sh.index > fdlen) {
        const QString reason = tr("section name %1 of %2 behind end of file").arg(i).arg(e_shnum);
        m_errorString = msgInvalidElfObject(m_binary, reason);
        return Corrupt;
      }
      ...
      if (sh.name == ".gdb_index") {
        ...
        if (sh.size > 16)
          m_elfData.buildId = QByteArray(mapper.start + sh.offset + 16, sh.size - 16).toHex();
      }
...
auto ElfReader::readSection(const QByteArray &name) -> QSharedPointer<ElfMapper>
{
...
static auto cutout(const char *s) -> QByteArray
{
  QByteArray res(s, 80);
...
auto ElfReader::readCoreName(bool *isCore) -> QByteArray
{
  *isCore = false;
...
auto ElfData::indexOf(const QByteArray &name) const -> int
{
