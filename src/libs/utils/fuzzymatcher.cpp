 ...
 * \return the regexp
 */
auto FuzzyMatcher::createRegExp(const QString &pattern, FuzzyMatcher::CaseSensitivity caseSensitivity) -> QRegularExpression
{
  if (pattern.isEmpty())
  ...
  for (const QChar &c : pattern) {
    ...
    } else if (caseSensitivity == CaseSensitivity::CaseInsensitive || (caseSensitivity == CaseSensitivity::FirstLetterCaseSensitive && !first)) {
...
/**
    ...
    Qt::CaseSensitivity.
 */
auto FuzzyMatcher::createRegExp(const QString &pattern, Qt::CaseSensitivity caseSensitivity) -> QRegularExpression
{
  const CaseSensitivity sensitivity = (caseSensitivity == Qt::CaseSensitive) ? CaseSensitivity::CaseSensitive : CaseSensitivity::CaseInsensitive;
 ...
 */
auto FuzzyMatcher::highlightingPositions(const QRegularExpressionMatch &match) -> FuzzyMatcher::HighlightingPositions
{
  ...
  for (int i = 1, size = match.capturedTexts().size(); i < size; ++i) {
    ...
    if (!result.starts.isEmpty() && (result.starts.last() + result.lengths.last() == match.capturedStart(i))) {
