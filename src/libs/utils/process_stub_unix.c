    ...
    for (;;) {
        ...
        if (WIFSTOPPED(chldStatus)) {
            ...
            if (isDetached == 2 && isDebug) {
                /* orca was not informed and died while debugging, killing the child */
            ...
            for (;;) {
                ...
                if (nbytes <= 0) {
                    ...
                    if (!isDetached) {
                        isDetached = 2;
                        if (nbytes == 0)
                            fprintf(stderr, "Lost connection to Orca, detaching from it.\n");
                        else
                            perror("Lost connection to Orca, detaching from it");
                    }
                    break;
                } else {
                    int i;
                    ...
                    for (i = 0; i < nbytes; ++i) {
                        ...
                        default:
                            if (!hadInvalidCommand) {
                                fprintf(stderr, "Ignoring invalid commands from Orca.\n");
                                hadInvalidCommand = 1;
                            }
                        }
                    }
