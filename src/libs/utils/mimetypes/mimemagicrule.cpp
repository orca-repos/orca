...
static const char magicRuleTypes_string[] = "invalid\0" "string\0" "regexp\0" "host16\0" "host32\0" "big16\0" "big32\0" "little16\0" "little32\0" "byte\0" "\0";
static const int magicRuleTypes_indices[] = {0, 8, 15, 22, 29, 36, 42, 48, 57, 66, 71, 0};
...
namespace Utils {namespace Internal {
  ...
  public:
    auto operator==(const MimeMagicRulePrivate &other) const -> bool;
  ...
  auto MimeMagicRulePrivate::operator==(const MimeMagicRulePrivate &other) const -> bool
  {
    return type == other.type && value == other.value && startPos == other.startPos && endPos == other.endPos && mask == other.mask && pattern == other.pattern && number == other.number && numberMask == other.numberMask && matchFunction == other.matchFunction;
  }
...
auto MimeMagicRule::matchSubstring(const char *dataPtr, int dataSize, int rangeStart, int rangeLength, int valueLength, const char *valueData, const char *mask) -> bool
{
...
static auto matchString(const MimeMagicRulePrivate *d, const QByteArray &data) -> bool
{
...
static auto matchRegExp(const MimeMagicRulePrivate *d, const QByteArray &data) -> bool
{
...
template <typename T>
static auto matchNumber(const MimeMagicRulePrivate *d, const QByteArray &data) -> bool
{
...
MimeMagicRule::MimeMagicRule(MimeMagicRule::Type theType, const QByteArray &theValue, int theStartPos, int theEndPos, const QByteArray &theMask, QString *errorString) : d(new MimeMagicRulePrivate)
{
  d->type = theType;
  d->value = theValue;
  ...
  if (d->type >= Host16 && d->type <= Byte) {
    ...
    if (!ok) {
      d->type = Invalid;
      if (errorString)
        *errorString = QString::fromLatin1("Invalid magic rule value \"%1\"").arg(QString::fromLatin1(d->value));
      return;
    }
  ...
  case String:
    ...
    if (!d->mask.isEmpty()) {
      if (d->mask.size() < 4 || !d->mask.startsWith("0x")) {
        d->type = Invalid;
        if (errorString)
          *errorString = QString::fromLatin1("Invalid magic rule mask \"%1\"").arg(QString::fromLatin1(d->mask));
        return;
      }
      const QByteArray &tempMask = QByteArray::fromHex(QByteArray::fromRawData(d->mask.constData() + 2, d->mask.size() - 2));
      if (tempMask.size() != d->pattern.size()) {
        d->type = Invalid;
        if (errorString)
          *errorString = QString::fromLatin1("Invalid magic rule mask size \"%1\"").arg(QString::fromLatin1(d->mask));
        return;
      }
      d->mask = tempMask;
    } else {
    ...
    break;
  case RegExp:
    d->regexp.setPatternOptions(QRegularExpression::MultilineOption | QRegularExpression::DotMatchesEverythingOption);
    d->regexp.setPattern(QString::fromUtf8(d->value));
    if (!d->regexp.isValid()) {
      d->type = Invalid;
      if (errorString)
        *errorString = QString::fromLatin1("Invalid magic rule regexp value \"%1\"").arg(QString::fromLatin1(d->value));
      return;
    }
...
MimeMagicRule::MimeMagicRule(const MimeMagicRule &other) : m_subMatches(other.m_subMatches), d(new MimeMagicRulePrivate(*other.d)) {}

MimeMagicRule::~MimeMagicRule() = default;

auto MimeMagicRule::operator=(const MimeMagicRule &other) -> MimeMagicRule&
{
  *d = *other.d;
...
auto MimeMagicRule::type() const -> MimeMagicRule::Type
{
  return d->type;
}

auto MimeMagicRule::value() const -> QByteArray
{
  return d->value;
}

auto MimeMagicRule::startPos() const -> int
{
  return d->startPos;
}

auto MimeMagicRule::endPos() const -> int
{
  return d->endPos;
}
...
auto MimeMagicRule::isValid() const -> bool
{
...
auto MimeMagicRule::matches(const QByteArray &data) const -> bool
{
  ...
  for (QList<MimeMagicRule>::const_iterator it = m_subMatches.begin(), end = m_subMatches.end(); it != end; ++it) {
    if ((*it).matches(data)) {
