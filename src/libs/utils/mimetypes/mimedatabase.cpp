...
static struct {
  const char *source;
  const char *comment;
} ALL_FILES_FILTER = QT_TRANSLATE_NOOP3("Core", "All Files (*.*)", "On Windows");
#else
static struct {
  const char *source;
  const char *comment;
} ALL_FILES_FILTER = QT_TRANSLATE_NOOP3("Core", "All Files (*)", "On Linux/macOS");
#endif
...
auto MimeDatabasePrivate::instance() -> MimeDatabasePrivate*
{
  return staticMimeDatabase();
}

MimeDatabasePrivate::MimeDatabasePrivate() : m_provider(nullptr), m_defaultMimeType(QLatin1String("application/octet-stream")) {}
...
auto MimeDatabasePrivate::provider() -> MimeProviderBase*
{
  if (!m_provider) {
...
auto MimeDatabasePrivate::setProvider(MimeProviderBase *theProvider) -> void
{
  delete m_provider;
 ...
 */
auto MimeDatabasePrivate::mimeTypeForName(const QString &nameOrAlias) -> MimeType
{
  return provider()->mimeTypeForName(provider()->resolveAlias(nameOrAlias));
}

auto MimeDatabasePrivate::mimeTypeForFileName(const QString &fileName, QString *foundSuffix) -> QStringList
{
...
auto MimeDatabasePrivate::findByData(const QByteArray &data, int *accuracyPtr) -> MimeType
{
  //    if (data.isEmpty()) {
  //        *accuracyPtr = 100;
...
auto MimeDatabasePrivate::mimeTypeForFileNameAndData(const QString &fileName, QIODevice *device, int *accuracyPtr) -> MimeType
{
...
auto MimeDatabasePrivate::allMimeTypes() -> QList<MimeType>
{
  return provider()->allMimeTypes();
}

auto MimeDatabasePrivate::inherits(const QString &mime, const QString &parent) -> bool
{
 ...
 */
MimeDatabase::MimeDatabase() : d(staticMimeDatabase()) {}
...
auto Utils::addMimeTypes(const QString &fileName, const QByteArray &data) -> void
{
  auto d = MimeDatabasePrivate::instance();
  QMutexLocker locker(&d->mutex);

  if (d->m_startupPhase >= MimeDatabase::PluginsDelayedInitializing)
    qWarning("Adding items from %s to MimeDatabase after initialization time", qPrintable(fileName));

  auto xmlProvider = static_cast<MimeXMLProvider*>(d->provider());
  xmlProvider->addData(fileName, data);
}

auto Utils::allFiltersString(QString *allFilesFilter) -> QString
{
  MimeDatabase mdb;
...
auto Utils::allFilesFilterString() -> QString
{
  auto d = MimeDatabasePrivate::instance();
...
auto Utils::allGlobPatterns() -> QStringList
{
  auto d = MimeDatabasePrivate::instance();
  if (d->m_startupPhase <= MimeDatabase::PluginsInitializing)
 ...
 */
auto MimeDatabase::mimeTypeForName(const QString &nameOrAlias) const -> MimeType
{
  QMutexLocker locker(&d->mutex);
...
*/
auto MimeDatabase::mimeTypeForFile(const QFileInfo &fileInfo, MatchMode mode) const -> MimeType
{
  QMutexLocker locker(&d->mutex);
...
/*!
    ...
    \overload
*/
auto MimeDatabase::mimeTypeForFile(const QString &fileName, MatchMode mode) const -> MimeType
{
...
/*!
    ...
    \sa mimeTypeForFile()
*/
auto MimeDatabase::mimeTypesForFileName(const QString &fileName) const -> QList<MimeType>
{
  QMutexLocker locker(&d->mutex);
...
*/
auto MimeDatabase::suffixForFileName(const QString &fileName) const -> QString
{
  QMutexLocker locker(&d->mutex);
  QString foundSuffix;
...
/*!
    ...
    is returned.
*/
auto MimeDatabase::mimeTypeForData(const QByteArray &data) const -> MimeType
{
  QMutexLocker locker(&d->mutex);
...
/*!
    ...
    is returned.
*/
auto MimeDatabase::mimeTypeForData(QIODevice *device) const -> MimeType
{
  QMutexLocker locker(&d->mutex);
...
/*!
    ...
    is returned.
*/
auto MimeDatabase::mimeTypeForUrl(const QUrl &url) const -> MimeType
{
  if (url.isLocalFile())
...
/*!
    ...
    matches multiple MIME types.
*/
auto MimeDatabase::mimeTypeForFileNameAndData(const QString &fileName, QIODevice *device) const -> MimeType
{
  int accuracy = 0;
...
/*!
    ...
    matches multiple MIME types.
*/
auto MimeDatabase::mimeTypeForFileNameAndData(const QString &fileName, const QByteArray &data) const -> MimeType
{
...
*/
auto MimeDatabase::allMimeTypes() const -> QList<MimeType>
{
  QMutexLocker locker(&d->mutex);
...
auto Utils::magicRulesForMimeType(const MimeType &mimeType) -> QMap<int, QList<MimeMagicRule>>
{
  auto d = MimeDatabasePrivate::instance();
  QMutexLocker locker(&d->mutex);
...
auto Utils::setGlobPatternsForMimeType(const MimeType &mimeType, const QStringList &patterns) -> void
{
  auto d = MimeDatabasePrivate::instance();
  QMutexLocker locker(&d->mutex);
...
auto Utils::setMagicRulesForMimeType(const MimeType &mimeType, const QMap<int, QList<MimeMagicRule>> &rules) -> void
{
  auto d = MimeDatabasePrivate::instance();
  QMutexLocker locker(&d->mutex);
...
auto Utils::setMimeStartupPhase(MimeStartupPhase phase) -> void
{
  auto d = MimeDatabasePrivate::instance();
  QMutexLocker locker(&d->mutex);
...
auto Utils::mimeTypeForName(const QString &nameOrAlias) -> MimeType
{
  MimeDatabase mdb;
  return mdb.mimeTypeForName(nameOrAlias);
}

auto Utils::mimeTypeForFile(const QString &fileName, MimeMatchMode mode) -> MimeType
{
  MimeDatabase mdb;
  return mdb.mimeTypeForFile(fileName, MimeDatabase::MatchMode(mode));
}

auto Utils::mimeTypeForFile(const QFileInfo &fileInfo, MimeMatchMode mode) -> MimeType
{
  MimeDatabase mdb;
  return mdb.mimeTypeForFile(fileInfo, MimeDatabase::MatchMode(mode));
}

auto Utils::mimeTypeForFile(const FilePath &filePath, MimeMatchMode mode) -> MimeType
{
  MimeDatabase mdb;
...
auto Utils::mimeTypesForFileName(const QString &fileName) -> QList<MimeType>
{
  MimeDatabase mdb;
  return mdb.mimeTypesForFileName(fileName);
}

auto Utils::mimeTypeForData(const QByteArray &data) -> MimeType
{
  MimeDatabase mdb;
  return mdb.mimeTypeForData(data);
}

auto Utils::allMimeTypes() -> QList<MimeType>
{
  MimeDatabase mdb;
