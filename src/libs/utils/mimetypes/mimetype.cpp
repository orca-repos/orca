...
static auto suffixFromPattern(const QString &pattern) -> QString
{
  ...
  if (pattern.startsWith(QLatin1String("*.")) && pattern.length() > 2 && pattern.indexOf(QLatin1Char('*'), 2) < 0 && pattern.indexOf(QLatin1Char('?'), 2) < 0) {
    return pattern.mid(2);
  }
  return QString();
}

MimeTypePrivate::MimeTypePrivate() : loaded(false) {}

MimeTypePrivate::MimeTypePrivate(const MimeType &other) : name(other.d->name), localeComments(other.d->localeComments), genericIconName(other.d->genericIconName), iconName(other.d->iconName), globPatterns(other.d->globPatterns), loaded(other.d->loaded) {}

auto MimeTypePrivate::clear() -> void
{
  name.clear();
...
auto MimeTypePrivate::addGlobPattern(const QString &pattern) -> void
{
 ...
 */
MimeType::MimeType() : d(new MimeTypePrivate()) {}
...
/*!
    ...
    \internal
 */
MimeType::MimeType(const MimeTypePrivate &dd) : d(new MimeTypePrivate(dd)) {}
...
static auto collectParentMimeTypes(const QString &mime, QStringList &allParents) -> void
{
...
*/
auto MimeType::setPreferredSuffix(const QString &suffix) -> void
{
  MimeDatabasePrivate::instance()->provider()->loadMimeTypePrivate(*d);

  auto it = std::find_if(d->globPatterns.begin(), d->globPatterns.end(), [suffix](const QString &pattern) {
...
/*!
    ...
    \sa parentMimeTypes()
 */
auto MimeType::inherits(const QString &mimeTypeName) const -> bool
{
...
auto operator<<(QDebug debug, const MimeType &mime) -> QDebug
{
