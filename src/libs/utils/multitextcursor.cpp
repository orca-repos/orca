...
auto MultiTextCursor::addCursor(const QTextCursor &cursor) -> void
{
...
auto MultiTextCursor::setCursors(const QList<QTextCursor> &cursors) -> void
{
  m_cursors = cursors;
  mergeCursors();
}

auto MultiTextCursor::cursors() const -> const QList<QTextCursor>
{
  return m_cursors;
}

auto MultiTextCursor::replaceMainCursor(const QTextCursor &cursor) -> void
{
  QTC_ASSERT(!cursor.isNull(), return);
  takeMainCursor();
  addCursor(cursor);
...
auto MultiTextCursor::mainCursor() const -> QTextCursor
{
...
auto MultiTextCursor::takeMainCursor() -> QTextCursor
{
  if (m_cursors.isEmpty())
    return {};
...
auto MultiTextCursor::beginEditBlock() -> void
{
  QTC_ASSERT(!m_cursors.empty(), return);
  m_cursors.last().beginEditBlock();
}

auto MultiTextCursor::endEditBlock() -> void
{
  QTC_ASSERT(!m_cursors.empty(), return);
  m_cursors.last().endEditBlock();
}

auto MultiTextCursor::isNull() const -> bool
{
  return m_cursors.isEmpty();
}

auto MultiTextCursor::hasMultipleCursors() const -> bool
{
  return m_cursors.size() > 1;
}

auto MultiTextCursor::cursorCount() const -> int
{
  return m_cursors.size();
}

auto MultiTextCursor::movePosition(QTextCursor::MoveOperation operation, QTextCursor::MoveMode mode, int n) -> void
{
...
auto MultiTextCursor::hasSelection() const -> bool
{
  return Utils::anyOf(m_cursors, &QTextCursor::hasSelection);
}

auto MultiTextCursor::selectedText() const -> QString
{
  QString text;
...
auto MultiTextCursor::removeSelectedText() -> void
{
  beginEditBlock();
...
static auto insertAndSelect(QTextCursor &cursor, const QString &text, bool selectNewText) -> void
{
  if (selectNewText) {
...
auto MultiTextCursor::insertText(const QString &text, bool selectNewText) -> void
{
  if (m_cursors.isEmpty())
    return;
  m_cursors.last().beginEditBlock();
...
auto MultiTextCursor::operator==(const MultiTextCursor &other) const -> bool
{
...
static auto cursorsOverlap(const QTextCursor &c1, const QTextCursor &c2) -> bool
{
  if (c1.hasSelection()) {
    if (c2.hasSelection()) {
      return c2.selectionEnd() > c1.selectionStart() && c2.selectionStart() < c1.selectionEnd();
    }
...
static auto mergeCursors(QTextCursor &c1, const QTextCursor &c2) -> void
{
...
auto MultiTextCursor::mergeCursors() -> void
{
...
auto MultiTextCursor::multiCursorAddEvent(QKeyEvent *e, QKeySequence::StandardKey matchKey) -> bool
{
  uint searchkey = (e->modifiers() | e->key()) & ~(Qt::KeypadModifier | Qt::GroupSwitchModifier | Qt::AltModifier | Qt::ShiftModifier);

  const QList<QKeySequence> bindings = QKeySequence::keyBindings(matchKey);
  return bindings.contains(QKeySequence(searchkey));
}

auto MultiTextCursor::handleMoveKeyEvent(QKeyEvent *e, QPlainTextEdit *edit, bool camelCaseNavigationEnabled) -> bool
{
  ...
  for (QTextCursor &cursor : m_cursors) {
    ...
    if (e == QKeySequence::MoveToNextChar) {
      ...
      mode = QTextCursor::KeepAnchor;
      {
        ...
        if (!block.next().isValid() && line.isValid() && line.lineNumber() == block.layout()->lineCount() - 1)
          op = QTextCursor::End;
      }
