...
constexpr char ColumnKey[] = "Columns";

class BaseTreeViewPrivate : public QObject {
public:
  explicit BaseTreeViewPrivate(BaseTreeView *parent) : q(parent)
  {
    m_settingsTimer.setSingleShot(true);
    connect(&m_settingsTimer, &QTimer::timeout, this, &BaseTreeViewPrivate::doSaveState);
  ...
  auto handleSectionResized(int logicalIndex, int /*oldSize*/, int newSize) -> void
  {
  ...
  auto considerItems(int column, QModelIndex start, int *minimum, bool single) const -> void
  {
    QModelIndex a = start;
  ...
  auto suggestedColumnSize(int column) const -> int
  {
    QHeaderView *h = q->header();
    ...
    QFontMetrics fm = q->fontMetrics();
    int minimum = fm.horizontalAdvance(m->headerData(column, Qt::Horizontal).toString()) + 2 * fm.horizontalAdvance(QLatin1Char('m'));
  ...
  auto toggleColumnWidth(int logicalIndex) -> void
  {
    QHeaderView *h = q->header();
  ...
  auto rebalanceColumns() -> void
  {
    rebalanceColumns(m_spanColumn, true);
  }

  auto rebalanceColumns(int column, bool allowResizePrevious) -> void
  {
  ...
  auto tryRebalanceColumns(int column, int totalSize) -> bool
  {
    QHeaderView *h = q->header();
...
BaseTreeView::BaseTreeView(QWidget *parent) : TreeView(parent), d(new BaseTreeViewPrivate(this))
{
  ...
  connect(this, &QAbstractItemView::activated, this, &BaseTreeView::rowActivated);
  connect(this, &QAbstractItemView::clicked, this, &BaseTreeView::rowClicked);

  connect(h, &QHeaderView::sectionClicked, d, &BaseTreeViewPrivate::toggleColumnWidth);
  connect(h, &QHeaderView::sectionResized, d, &BaseTreeViewPrivate::handleSectionResized);
}
...
auto BaseTreeView::setModel(QAbstractItemModel *m) -> void
{
...
auto BaseTreeView::mousePressEvent(QMouseEvent *ev) -> void
{
...
auto BaseTreeView::mouseMoveEvent(QMouseEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  QTC_ASSERT(model(), return);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::mouseReleaseEvent(QMouseEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  QTC_ASSERT(model(), return);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::contextMenuEvent(QContextMenuEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::keyPressEvent(QKeyEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::dragEnterEvent(QDragEnterEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::dropEvent(QDropEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::dragMoveEvent(QDragMoveEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::mouseDoubleClickEvent(QMouseEvent *ev) -> void
{
  ItemViewEvent ive(ev, this);
  if (!model()->setData(ive.index(), QVariant::fromValue(ive), ItemViewEventRole))
...
auto BaseTreeView::resizeEvent(QResizeEvent *ev) -> void
{
  TreeView::resizeEvent(ev);
  d->rebalanceColumns();
}

auto BaseTreeView::showEvent(QShowEvent *ev) -> void
{
  emit aboutToShow();
 ...
 */
auto BaseTreeView::showProgressIndicator() -> void
{
 ...
 */
auto BaseTreeView::hideProgressIndicator() -> void
{
  if (d->m_progressIndicator)
    d->m_progressIndicator->hide();
}

auto BaseTreeView::rowActivated(const QModelIndex &index) -> void
{
  model()->setData(index, QVariant(), ItemActivatedRole);
}

auto BaseTreeView::rowClicked(const QModelIndex &index) -> void
{
  model()->setData(index, QVariant(), ItemClickedRole);
}

auto BaseTreeView::resizeColumns() -> void
{
  d->resizeColumns();
}

auto BaseTreeView::spanColumn() const -> int
{
  return d->m_spanColumn;
}

auto BaseTreeView::setSpanColumn(int column) -> void
{
  d->setSpanColumn(column);
}

auto BaseTreeView::enableColumnHiding() -> void
{
...
auto BaseTreeView::refreshSpanColumn() -> void
{
  d->rebalanceColumns();
}

auto BaseTreeView::setSettings(QSettings *settings, const QByteArray &key) -> void
{
...
ItemViewEvent::ItemViewEvent(QEvent *ev, QAbstractItemView *view) : m_event(ev), m_view(view)
{
...
auto ItemViewEvent::currentOrSelectedRows() const -> QModelIndexList
{
