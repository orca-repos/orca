...
#include <utils/namevaluedictionary.hpp>
...
public:
  auto updateResultNameValueDictionary() -> void
  {
  ...
  auto findInChanges(const QString &name) const -> int
  {
    for (int i = 0; i < m_items.size(); ++i)
      if (m_items.at(i).name.compare(name, m_baseNameValueDictionary.nameCaseSensitivity()) == 0) {
        return i;
      }
    return -1;
  }

  auto findInResultInsertPosition(const QString &name) const -> int
  {
    NameValueDictionary::const_iterator it;
    int i = 0;
    for (it = m_resultNameValueDictionary.constBegin(); it != m_resultNameValueDictionary.constEnd(); ++it, ++i)
      if (it.key() > DictKey(name, m_resultNameValueDictionary.nameCaseSensitivity()))
        return i;
    return m_resultNameValueDictionary.size();
  }

  auto findInResult(const QString &name) const -> int
  {
    NameValueDictionary::const_iterator it;
    int i = 0;
    for (it = m_resultNameValueDictionary.constBegin(); it != m_resultNameValueDictionary.constEnd(); ++it, ++i)
      if (m_resultNameValueDictionary.key(it).compare(name, m_resultNameValueDictionary.nameCaseSensitivity()) == 0) {
        return i;
      }
    return -1;
  }
...
NameValueModel::NameValueModel(QObject *parent) : QAbstractTableModel(parent), d(std::make_unique<Internal::NameValueModelPrivate>()) {}

NameValueModel::~NameValueModel() = default;

auto NameValueModel::indexToVariable(const QModelIndex &index) const -> QString
{
  const auto it = std::next(d->m_resultNameValueDictionary.constBegin(), index.row());
  return d->m_resultNameValueDictionary.key(it);
}

auto NameValueModel::setBaseNameValueDictionary(const NameValueDictionary &dictionary) -> void
{
...
auto NameValueModel::rowCount(const QModelIndex &parent) const -> int
{
  if (parent.isValid())
    return 0;
...
auto NameValueModel::columnCount(const QModelIndex &parent) const -> int
{
  if (parent.isValid())
    return 0;
...
auto NameValueModel::changes(const QString &name) const -> bool
{
  return d->findInChanges(name) >= 0;
}

auto NameValueModel::baseNameValueDictionary() const -> const NameValueDictionary&
{
  return d->m_baseNameValueDictionary;
}

auto NameValueModel::data(const QModelIndex &index, int role) const -> QVariant
{
  if (!index.isValid())
    return QVariant();
  ...
  case Qt::ForegroundRole: {
    ...
    return p.color(changes(d->m_resultNameValueDictionary.key(resultIterator)) ? QPalette::Link : QPalette::Text);
  }
  }
  return QVariant();
}

auto NameValueModel::flags(const QModelIndex &index) const -> Qt::ItemFlags
{
  Q_UNUSED(index)
  return Qt::ItemIsSelectable | Qt::ItemIsEditable | Qt::ItemIsEnabled;
}

auto NameValueModel::headerData(int section, Qt::Orientation orientation, int role) const -> QVariant
{
...
/// Utility functions
/// *****************
auto NameValueModel::variableToIndex(const QString &name) const -> QModelIndex
{
...
auto NameValueModel::setData(const QModelIndex &index, const QVariant &value, int role) -> bool
{
  ...
  if (index.column() == 0) {
    ...
    const QString &newName = HostOsInfo::isWindowsHost() ? value.toString().toUpper() : value.toString();
    ...
    if (changesPos != -1) {
      ...
      if (oldIt != d->m_baseNameValueDictionary.constEnd() && stringValue == d->m_baseNameValueDictionary.value(oldIt) && d->m_baseNameValueDictionary.isEnabled(oldIt) == d->m_resultNameValueDictionary.isEnabled(newIt)) {
...
auto NameValueModel::addVariable() -> QModelIndex
{
...
auto NameValueModel::addVariable(const NameValueItem &item) -> QModelIndex
{
...
auto NameValueModel::resetVariable(const QString &name) -> void
{
...
auto NameValueModel::unsetVariable(const QString &name) -> void
{
...
auto NameValueModel::toggleVariable(const QModelIndex &idx) -> void
{
  ...
  const auto op = d->m_resultNameValueDictionary.isEnabled(newIt) ? NameValueItem::SetDisabled : NameValueItem::SetEnabled;
  const int changesPos = d->findInChanges(name);
  if (changesPos != -1) {
    const auto oldIt = d->m_baseNameValueDictionary.constFind(name);
    if (oldIt == d->m_baseNameValueDictionary.constEnd() || oldIt.value().first != newIt.value().first) {
...
auto NameValueModel::isUnset(const QString &name) -> bool
{
  const int pos = d->findInChanges(name);
  return pos == -1 ? false : d->m_items.at(pos).operation == NameValueItem::Unset;
}

auto NameValueModel::isEnabled(const QString &name) const -> bool
{
  return d->m_resultNameValueDictionary.isEnabled(d->m_resultNameValueDictionary.constFind(name));
}

auto NameValueModel::canReset(const QString &name) -> bool
{
  return d->m_baseNameValueDictionary.hasKey(name);
}

auto NameValueModel::userChanges() const -> NameValueItems
{
  return d->m_items;
}

auto NameValueModel::setUserChanges(const NameValueItems &items) -> void
{
...
auto NameValueModel::currentEntryIsPathList(const QModelIndex &current) const -> bool
{
  if (!current.isValid())
    return false;
  ...
  if (varName == "PKG_CONFIG_DIR")
    return true;
  if (Utils::HostOsInfo::isWindowsHost() && QStringList{"INCLUDE", "LIB", "LIBPATH"}.contains(varName)) {
    return true;
  }
  ...
  const QStringList entries = data(valueIndex).toString().split(Utils::HostOsInfo::pathListSeparator(), Qt::SkipEmptyParts);
  if (entries.length() < 2)
    return false;
