...
static auto stringToRectangle(const QString &v) -> QRect
{
...
{
  Context() {}
  const QString orcaElement = QString("orca");
...
struct ParseValueStackEntry {
  ...
  auto addChild(const QString &key, const QVariant &v) -> void;
...
ParseValueStackEntry::ParseValueStackEntry(const QVariant &aSimpleValue, const QString &k) : type(aSimpleValue.type()), key(k), simpleValue(aSimpleValue)
{
  QTC_ASSERT(simpleValue.isValid(), return);
}

auto ParseValueStackEntry::value() const -> QVariant
{
  switch (type) {
...
auto ParseValueStackEntry::addChild(const QString &key, const QVariant &v) -> void
{
  switch (type) {
  case QVariant::Map:
  ...
  case QVariant::List:
    ...
    break;
  default: qWarning() << "ParseValueStackEntry::Internal error adding " << key << v << " to " << QVariant::typeToName(type) << value();
    break;
  }
}
...
public:
  auto parse(const FilePath &file) -> QVariantMap;
...
private:
  ...
  auto element(const StringView &r) const -> Element;
  static auto isValueElement(Element e) -> bool { return e == SimpleValueElement || e == ListValueElement || e == MapValueElement; }
  auto readSimpleValue(QXmlStreamReader &r, const QXmlStreamAttributes &attributes) const -> QVariant;

  auto handleStartElement(QXmlStreamReader &r) -> bool;
  auto handleEndElement(const StringView &name) -> bool;

  static auto formatWarning(const QXmlStreamReader &r, const QString &message) -> QString;
...
auto ParseContext::parse(const FilePath &file) -> QVariantMap
{
  ...
  while (!r.atEnd()) {
    ...
    case QXmlStreamReader::EndElement:
      if (handleEndElement(r.name()))
        return m_result;
      break;
    case QXmlStreamReader::Invalid: qWarning("Error reading %s:%d: %s", qPrintable(file.fileName()), int(r.lineNumber()), qPrintable(r.errorString()));
      return QVariantMap();
    default:
      break;
    } // switch token
...
auto ParseContext::handleStartElement(QXmlStreamReader &r) -> bool
{
  ...
  const QString key = attributes.hasAttribute(keyAttribute) ? attributes.value(keyAttribute).toString() : QString();
  switch (e) {
...
auto ParseContext::handleEndElement(const StringView &name) -> bool
{
  const Element e = element(name);
  if (ParseContext::isValueElement(e)) {
    ...
    if (m_valueStack.isEmpty()) {
      // Last element? -> Done with that variable.
  ...
  return e == OrcaElement;
}

auto ParseContext::formatWarning(const QXmlStreamReader &r, const QString &message) -> QString
{
...
auto ParseContext::element(const StringView &r) const -> ParseContext::Element
{
  if (r == valueElement)
...
auto ParseContext::readSimpleValue(QXmlStreamReader &r, const QXmlStreamAttributes &attributes) const -> QVariant
{
  // Simple value
...
auto PersistentSettingsReader::restoreValue(const QString &variable, const QVariant &defaultValue) const -> QVariant
{
...
auto PersistentSettingsReader::restoreValues() const -> QVariantMap
{
  return m_valueMap;
}

auto PersistentSettingsReader::load(const FilePath &fileName) -> bool
{
  m_valueMap.clear();
...
static auto writeVariantValue(QXmlStreamWriter &w, const Context &ctx, const QVariant &variant, const QString &key = QString()) -> void
{
...
PersistentSettingsWriter::PersistentSettingsWriter(const FilePath &fileName, const QString &docType) : m_fileName(fileName), m_docType(docType) { }

auto PersistentSettingsWriter::save(const QVariantMap &data, QString *errorString) const -> bool
{
  if (data == m_savedData)
    return true;
...
#ifdef QT_GUI_LIB
auto PersistentSettingsWriter::save(const QVariantMap &data, QWidget *parent) const -> bool
{
  QString errorString;
  const bool success = save(data, &errorString);
  if (!success)
    QMessageBox::critical(parent, QCoreApplication::translate("Utils::FileSaverBase", "File Error"), errorString);
  return success;
}
#endif // QT_GUI_LIB

auto PersistentSettingsWriter::fileName() const -> FilePath { return m_fileName; }

//** * @brief Set contents of file (e.g. from data read from it). */
auto PersistentSettingsWriter::setContents(const QVariantMap &data) -> void
{
  m_savedData = data;
}

auto PersistentSettingsWriter::write(const QVariantMap &data, QString *errorString) const -> bool
{
  ...
  if (!saver.hasError()) {
    ...
    w.writeComment(QString::fromLatin1(" Written by %1 %2, %3. ").arg(QCoreApplication::applicationName(), QCoreApplication::applicationVersion(), QDateTime::currentDateTime().toString(Qt::ISODate)));
    w.writeStartElement(ctx.orcaElement);
