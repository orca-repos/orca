...
public:
  PreprocessStackEntry(PreprocessorSection section = OtherSection, bool parentEnabled = true, bool condition = false, bool anyIfClauseMatched = false);
...
PreprocessStackEntry::PreprocessStackEntry(PreprocessorSection s, bool p, bool c, bool a) : section(s), parentEnabled(p), condition(c), anyIfClauseMatched(a) { }

// Context for preprocessing.
class PreprocessContext {
public:
  PreprocessContext();
  auto process(const QString &in, QString *out, QString *errorMessage) -> bool;

private:
  auto reset() -> void;
  auto preprocessorLine(const QString &in, QString *ifExpression) const -> PreprocessorSection;
...
PreprocessContext::PreprocessContext() :
  ...
  m_ifPattern(QLatin1String("^([\\s]*@[\\s]*if[\\s]*)(.*)$")), m_elsifPattern(QLatin1String("^([\\s]*@[\\s]*elsif[\\s]*)(.*)$")), m_elsePattern(QLatin1String("^[\\s]*@[\\s]*else.*$")), m_endifPattern(QLatin1String("^[\\s]*@[\\s]*endif.*$"))
{
  QTC_CHECK(m_ifPattern.isValid() && m_elsifPattern.isValid() && m_elsePattern.isValid() && m_endifPattern.isValid());
}

auto PreprocessContext::reset() -> void
{
...
auto PreprocessContext::preprocessorLine(const QString &in, QString *ifExpression) const -> PreprocessorSection
{
...
static inline auto msgEmptyStack(int line) -> QString
{
  return QString::fromLatin1("Unmatched '@endif' at line %1.").arg(line);
}

auto PreprocessContext::process(const QString &in, QString *out, QString *errorMessage) -> bool
{
  out->clear();
  if (in.isEmpty())
    return true;
  ...
  for (int l = 0; l < lineCount; l++) {
    ...
    case IfSection:
      // '@If': Push new section
      if (top.condition) {
        if (!TemplateEngine::evaluateBooleanJavaScriptExpression(m_scriptEngine, expression, &expressionValue, errorMessage)) {
          if (errorMessage)
            *errorMessage = QString::fromLatin1("Error in @if at %1: %2").arg(l + 1).arg(*errorMessage);
          return false;
        }
      }
      m_sectionStack.push(PreprocessStackEntry(IfSection, top.condition, expressionValue, expressionValue));
      break;
    case ElsifSection: // '@elsif': Check condition.
      if (top.section != IfSection && top.section != ElsifSection) {
        if (errorMessage)
          *errorMessage = QString::fromLatin1("No preceding @if found for @elsif at %1").arg(l + 1);
        return false;
      }
      if (top.parentEnabled) {
        if (!TemplateEngine::evaluateBooleanJavaScriptExpression(m_scriptEngine, expression, &expressionValue, errorMessage)) {
          if (errorMessage)
            *errorMessage = QString::fromLatin1("Error in @elsif at %1: %2").arg(l + 1).arg(*errorMessage);
          return false;
        }
      }
    ...
    case ElseSection: // '@else': Check condition.
      if (top.section != IfSection && top.section != ElsifSection) {
        if (errorMessage)
          *errorMessage = QString::fromLatin1("No preceding @if/@elsif found for @else at %1").arg(l + 1);
        return false;
      }
...
auto TemplateEngine::preprocessText(const QString &in, QString *out, QString *errorMessage) -> bool
{
  Internal::PreprocessContext context;
  return context.process(in, out, errorMessage);
}

auto TemplateEngine::processText(MacroExpander *expander, const QString &input, QString *errorMessage) -> QString
{
  if (errorMessage)
    errorMessage->clear();
...
auto TemplateEngine::evaluateBooleanJavaScriptExpression(QJSEngine &engine, const QString &expression, bool *result, QString *errorMessage) -> bool
{
  if (errorMessage)
    errorMessage->clear();
  if (result)
  ...
  if (value.isError()) {
    if (errorMessage)
      *errorMessage = QString::fromLatin1("Error in \"%1\": %2").arg(expression, value.toString());
    return false;
  }
  ...
  if (errorMessage)
    *errorMessage = QString::fromLatin1("Cannot convert result of \"%1\" (\"%2\"to bool.").arg(expression, value.toString());
