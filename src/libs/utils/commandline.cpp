...
QT_BEGIN_NAMESPACE auto operator<<(QDebug dbg, const Utils::CommandLine &cmd) -> QDebug
{
...
QT_BEGIN_NAMESPACE Q_DECLARE_TYPEINFO(MxState, Q_PRIMITIVE_TYPE);
...
QT_BEGIN_NAMESPACE Q_DECLARE_TYPEINFO(MxSave, Q_MOVABLE_TYPE);
...
inline static auto isMetaCharWin(ushort c) -> bool
{
...
static auto envExpandWin(QString &args, const Environment *env, const QString *pwd) -> void
{
  static const QString cdName = QLatin1String("CD");
  int off = 0;
next: for (int prev = -1, that; (that = args.indexOf(QLatin1Char('%'), off)) >= 0; prev = that, off = that + 1) {
    if (prev >= 0) {
      const QString var = args.mid(prev + 1, that - prev - 1).toUpper();
      const QString val = (var == cdName && pwd && !pwd->isEmpty()) ? QDir::toNativeSeparators(*pwd) : env->expandedValueForKey(var);
      if (!val.isEmpty()) {
        // Empty values are impossible, so this is an existence check
...
static auto prepareArgsWin(const QString &_args, ProcessArgs::SplitError *err, const Environment *env, const QString *pwd) -> ProcessArgs
{
  QString args(_args);
...
static auto isWhiteSpaceWin(ushort c) -> bool
{
  return c == ' ' || c == '\t';
}

static auto doSplitArgsWin(const QString &args, ProcessArgs::SplitError *err) -> QStringList
{
  QStringList ret;
...
static auto splitArgsWin(const QString &_args, bool abortOnMeta, ProcessArgs::SplitError *err, const Environment *env, const QString *pwd) -> QStringList
{
  if (abortOnMeta) {
...
static auto isMetaUnix(QChar cUnicode) -> bool
{
...
static auto splitArgsUnix(const QString &args, bool abortOnMeta, ProcessArgs::SplitError *err, const Environment *env, const QString *pwd) -> QStringList
{
  ...
  for (int pos = 0; ;) {
    ...
    bool hadWord = false;
    if (c == QLatin1Char('~')) {
      if (pos >= args.length() || args.unicode()[pos].isSpace() || args.unicode()[pos] == QLatin1Char('/')) {
        cret = QDir::homePath();
        hadWord = true;
        goto getc;
      } else if (abortOnMeta) {
      ...
      if (c == QLatin1Char('\'')) {
        ...
        for (;;) {
          if (pos >= args.length())
            goto quoteerr;
          c = args.unicode()[pos++];
        nextq: if (c == QLatin1Char('"'))
            break;
          if (c == QLatin1Char('\\')) {
            if (pos >= args.length())
              goto quoteerr;
            c = args.unicode()[pos++];
            if (c != QLatin1Char('"') && c != QLatin1Char('\\') && !(abortOnMeta && (c == QLatin1Char('$') || c == QLatin1Char('`'))))
              cret += QLatin1Char('\\');
            ...
            continue;
          } else if (abortOnMeta && (c == QLatin1Char('$') || c == QLatin1Char('`'))) {
            goto metaerr;
          }
          cret += c;
        }
...
inline static auto isSpecialCharUnix(ushort c) -> bool
{
...
inline static auto hasSpecialCharsUnix(const QString &arg) -> bool
{
...
auto ProcessArgs::splitArgs(const QString &args, OsType osType, bool abortOnMeta, ProcessArgs::SplitError *err, const Environment *env, const QString *pwd) -> QStringList
{
...
auto ProcessArgs::quoteArgUnix(const QString &arg) -> QString
{
  if (arg.isEmpty())
...
static auto isSpecialCharWin(ushort c) -> bool
{
  // Chars that should be quoted (TM). This includes:
  // - control chars & space
  // - the shell meta chars "&()<>^|
  // - the potential separators ,;=
  static const uchar iqm[] = {0xff, 0xff, 0xff, 0xff, 0x45, 0x13, 0x00, 0x78, 0x00, 0x00, 0x00, 0x40, 0x00, 0x00, 0x00, 0x10};

  return (c < sizeof(iqm) * 8) && (iqm[c / 8] & (1 << (c & 7)));
}

static auto hasSpecialCharsWin(const QString &arg) -> bool
{
  for (int x = arg.length() - 1; x >= 0; --x)
...
static auto quoteArgWin(const QString &arg) -> QString
{
  if (arg.isEmpty())
    return QString::fromLatin1("\"\"");
...
auto ProcessArgs::prepareArgs(const QString &args, SplitError *err, OsType osType, const Environment *env, const FilePath *pwd, bool abortOnMeta) -> ProcessArgs
{
  QString wdcopy;
  QString *wd = nullptr;
  if (pwd) {
...
auto ProcessArgs::quoteArg(const QString &arg, OsType osType) -> QString
{
  if (osType == OsTypeWindows)
...
auto ProcessArgs::addArg(QString *args, const QString &arg, OsType osType) -> void
{
  if (!args->isEmpty())
    *args += QLatin1Char(' ');
...
auto ProcessArgs::joinArgs(const QStringList &args, OsType osType) -> QString
{
  QString ret;
...
auto ProcessArgs::addArgs(QString *args, const QString &inArgs) -> void
{
  if (!inArgs.isEmpty()) {
    if (!args->isEmpty())
      *args += QLatin1Char(' ');
...
auto ProcessArgs::addArgs(QString *args, const QStringList &inArgs) -> void
{
  for (const QString &arg : inArgs)
    addArg(args, arg);
}

auto ProcessArgs::prepareCommand(const CommandLine &cmdLine, QString *outCmd, ProcessArgs *outArgs, const Environment *env, const FilePath *pwd) -> bool
{
  ...
  if (err == ProcessArgs::SplitOk) {
    *outCmd = executable.toString();
    ...
    if (executable.osType() == OsTypeWindows) {
      ...
      *outArgs = ProcessArgs::createWindowsArgs(QLatin1String("/v:off /s /c \"") + quoteArg(executable.toUserOutput()) + ' ' + arguments + '"');
    } else {
...
static auto quoteArgInternalWin(QString &ret, int bslashes) -> int
{
  // Quotes are escaped and their preceding backslashes are doubled.
  // It's impossible to escape anything inside a quoted string on cmd
  // level, so the outer quoting must be "suspended".
 ...
 */
auto ProcessArgs::expandMacros(QString *cmd, AbstractMacroExpander *mx, OsType osType) -> bool
{
  QString str = *cmd;
  if (str.isEmpty())
    return true;
  ...
  if (osType == OsTypeWindows) {
    enum {
      // cmd.exe parsing state
      ShellBasic,
      // initial state
      ShellQuoted,
      // double-quoted state => *no* other meta chars are interpreted
      ShellEscaped // circumflex-escaped state => next char is not interpreted
    } shellState = ShellBasic;
    enum {
      // CommandLineToArgv() parsing state and some more
      CrtBasic,
      // initial state
      CrtNeedWord,
      // after empty expando; insert empty argument if whitespace follows
      CrtInWord,
      // in non-whitespace
      CrtClosed,
      // previous char closed the double-quoting
      CrtHadQuote,
      // closed double-quoting after an expando
      // The remaining two need to be numerically higher
      CrtQuoted,
      // double-quoted state => spaces don't split tokens
...
auto ProcessArgs::ArgIterator::next() -> bool
{
  ...
  if (m_osType == OsTypeWindows) {
    ...
    enum {
      ...
      NewVar,
      FullVar
    } varState = NoVar; // inside a potential env variable expansion
    ...
    for (;; m_pos++) {
      ...
      if (shellState == ShellBasic && cc == '^') {
        varState = NoVar;
        shellState = ShellEscaped;
      } else if ((shellState == ShellBasic && isMetaCharWin(cc)) || !cc) {
        // A "bit" simplistic ...
        // We ignore crtQuote state here. Whatever ...
      doReturn: if (m_simple)
          while (--bslashes >= 0)
            m_value += QLatin1Char('\\');
        else
          m_value.clear();
          ...
          if (cc == '\\') {
            ...
            if (cc == '"') {
              ...
              if (cc == '%') {
                ...
                varState = (cc == '_' || cc == '-' || cc == '.' || QChar(cc).isLetterOrNumber()) ? FullVar : NoVar;
...
auto ProcessArgs::ArgIterator::deleteArg() -> void
{
  if (!m_prev)
...
auto ProcessArgs::ArgIterator::appendArg(const QString &str) -> void
{
...
auto ProcessArgs::createWindowsArgs(const QString &args) -> ProcessArgs
{
  ProcessArgs result;
...
auto ProcessArgs::createUnixArgs(const QStringList &args) -> ProcessArgs
{
  ProcessArgs result;
...
auto ProcessArgs::toWindowsArgs() const -> QString
{
  QTC_CHECK(m_isWindows);
  return m_windowsArgs;
}

auto ProcessArgs::toUnixArgs() const -> QStringList
{
  QTC_CHECK(!m_isWindows);
  return m_unixArgs;
}

auto ProcessArgs::toString() const -> QString
{
  if (m_isWindows)
    return m_windowsArgs;
  else
...
CommandLine::CommandLine(const FilePath &executable) : m_executable(executable) {}

CommandLine::CommandLine(const FilePath &exe, const QStringList &args) : m_executable(exe)
{
  addArgs(args);
}

CommandLine::CommandLine(const FilePath &exe, const QString &args, RawType) : m_executable(exe)
{
  addArgs(args, Raw);
}

auto CommandLine::fromUserInput(const QString &cmdline, MacroExpander *expander) -> CommandLine
{
  CommandLine cmd;
...
auto CommandLine::addArg(const QString &arg) -> void
{
  ProcessArgs::addArg(&m_arguments, arg, m_executable.osType());
}

auto CommandLine::addArgs(const QStringList &inArgs) -> void
{
  for (const QString &arg : inArgs)
    addArg(arg);
}
...
auto CommandLine::addCommandLineAsArgs(const CommandLine &cmd) -> void
{
  addArg(cmd.executable().path());
  addArgs(cmd.splitArguments());
}

auto CommandLine::addArgs(const QString &inArgs, RawType) -> void
{
  ProcessArgs::addArgs(&m_arguments, inArgs);
}

auto CommandLine::toUserOutput() const -> QString
{
...
auto CommandLine::splitArguments() const -> QStringList
{
