...
#include "highlightingitemdelegate.hpp"
...
HighlightingItemDelegate::HighlightingItemDelegate(int tabWidth, QObject *parent) : QItemDelegate(parent)
{
  setTabWidth(tabWidth);
}

auto HighlightingItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const -> void
{
...
auto HighlightingItemDelegate::setTabWidth(int width) -> void
{
  m_tabString = QString(width, ' ');
}

// returns the width of the line number area
auto HighlightingItemDelegate::drawLineNumber(QPainter *painter, const QStyleOptionViewItem &option, const QRect &rect, const QModelIndex &index) const -> int
{
  ...
  const int fontWidth = painter->fontMetrics().horizontalAdvance(QString(minimumLineNumberDigits, '0'));
  const int lineNumberAreaWidth = lineNumberAreaHorizontalPadding + fontWidth + lineNumberAreaHorizontalPadding;
  ...
  painter->fillRect(lineNumberAreaRect, QBrush(isSelected ? option.palette.brush(cg, QPalette::Highlight) : option.palette.color(cg, QPalette::Base).darker(111)));
...
auto HighlightingItemDelegate::drawText(QPainter *painter, const QStyleOptionViewItem &option, const QRect &rect, const QModelIndex &index) const -> void
{
  ...
  QVector<int> searchTermStarts = index.model()->data(index, int(HighlightingItemRole::StartColumn)).value<QVector<int>>();
  QVector<int> searchTermLengths = index.model()->data(index, int(HighlightingItemRole::Length)).value<QVector<int>>();
  ...
  const QColor highlightForeground = index.model()->data(index, int(HighlightingItemRole::Foreground)).value<QColor>();
  const QColor highlightBackground = index.model()->data(index, int(HighlightingItemRole::Background)).value<QColor>();
...
static auto replaceNewLine(QString text) -> QString
{
...
// copied from QItemDelegate for drawDisplay
auto doTextLayout(QTextLayout *textLayout, int lineWidth) -> QSizeF
{
  qreal height = 0;
  qreal widthUsed = 0;
...
auto HighlightingItemDelegate::drawDisplay(QPainter *painter, const QStyleOptionViewItem &option, const QRect &rect, const QString &text, const QVector<QTextLayout::FormatRange> &format) const -> void
{
