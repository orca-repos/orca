...
static auto commonPrefix(const QString &text1, const QString &text2) -> int
{
  int i = 0;
...
static auto commonSuffix(const QString &text1, const QString &text2) -> int
{
  int i = 0;
  const int text1Count = text1.count();
  const int text2Count = text2.count();
...
static auto commonOverlap(const QString &text1, const QString &text2) -> int
{
  int i = 0;
  const int text1Count = text1.count();
  const int text2Count = text2.count();
...
static auto decode(const QList<Diff> &diffList, const QStringList &lines) -> QList<Diff>
{
...
static auto squashEqualities(const QList<Diff> &diffList) -> QList<Diff>
{
  ...
  while (i < diffList.count()) {
    if (prevDiff.command == Diff::Equal && nextDiff.command == Diff::Equal) {
      if (thisDiff.text.endsWith(prevDiff.text)) {
        thisDiff.text = prevDiff.text + thisDiff.text.left(thisDiff.text.count() - prevDiff.text.count());
        nextDiff.text = prevDiff.text + nextDiff.text;
      } else if (thisDiff.text.startsWith(nextDiff.text)) {
        prevDiff.text += nextDiff.text;
        thisDiff.text = thisDiff.text.mid(nextDiff.text.count()) + nextDiff.text;
        i++;
        if (i < diffList.count())
...
static auto cleanupOverlaps(const QList<Diff> &diffList) -> QList<Diff>
{
  ...
  while (i < diffList.count()) {
    ...
    Diff nextDiff = i < diffList.count() - 1 ? diffList.at(i + 1) : Diff(Diff::Equal);
    if (thisDiff.command == Diff::Delete && nextDiff.command == Diff::Insert) {
      ...
      if (delInsOverlap >= insDelOverlap) {
        if (delInsOverlap > thisDiff.text.count() / 2 || delInsOverlap > nextDiff.text.count() / 2) {
        ...
        if (insDelOverlap > thisDiff.text.count() / 2 || insDelOverlap > nextDiff.text.count() / 2) {
...
static auto cleanupSemanticsScore(const QString &text1, const QString &text2) -> int
{
...
static auto isWhitespace(const QChar &c) -> bool
{
  return c == ' ' || c == '\t';
}

static auto isNewLine(const QChar &c) -> bool
{
  return c == '\n';
}
 ...
 */
auto Differ::splitDiffList(const QList<Diff> &diffList, QList<Diff> *leftDiffList, QList<Diff> *rightDiffList) -> void
{
 ...
 */
auto Differ::moveWhitespaceIntoEqualities(const QList<Diff> &input) -> QList<Diff>
{
  ...
  for (int i = 0; i < output.count(); i++) {
    ...
    if (diff.command != Diff::Equal) {
      if (i > 0) {
        // check previous equality
        Diff &previousDiff = output[i - 1];
        const int previousDiffCount = previousDiff.text.count();
        if (previousDiff.command == Diff::Equal && previousDiffCount && isWhitespace(previousDiff.text.at(previousDiffCount - 1))) {
      ...
      if (i < output.count() - 1) {
        ...
        if (nextDiff.command == Diff::Equal && nextDiffCount && (isWhitespace(nextDiff.text.at(0)) || isNewLine(nextDiff.text.at(0)))) {
 ...
 */
static auto encodeReducedWhitespace(const QString &input, QMap<int, QString> *codeMap) -> QString
{
  QString output;
  if (!codeMap)
    return output;
 ...
 */
static auto decodeReducedWhitespace(const QList<Diff> &input, const QMap<int, QString> &codeMap) -> QList<Diff>
{
  QList<Diff> output;
 ...
 * decode.
 */
auto Differ::diffWithWhitespaceReduced(const QString &leftInput, const QString &rightInput, QList<Diff> *leftOutput, QList<Diff> *rightOutput) -> void
{
 ...
 * decode.
 */
auto Differ::unifiedDiffWithWhitespaceReduced(const QString &leftInput, const QString &rightInput, QList<Diff> *leftOutput, QList<Diff> *rightOutput) -> void
{
  if (!leftOutput || !rightOutput)
    return;
 ...
 *
 */
static auto encodeExpandedWhitespace(const QString &leftEquality, const QString &rightEquality, QMap<int, QPair<int, QString>> *leftCodeMap, QMap<int, QPair<int, QString>> *rightCodeMap, bool *ok) -> QString
{
  if (ok)
    *ok = false;
 ...
 * the returned value contains decoded diff list.
 */
static auto decodeExpandedWhitespace(const QList<Diff> &input, const QMap<int, QPair<int, QString>> &codeMap, bool *ok) -> QList<Diff>
{
  if (ok)
    *ok = false;
 ...
 * decode.
 */
static auto diffWithWhitespaceExpandedInEqualities(const QList<Diff> &leftInput, const QList<Diff> &rightInput, QList<Diff> *leftOutput, QList<Diff> *rightOutput) -> bool
{
  if (!leftOutput || !rightOutput)
    return false;
  ...
  while (l <= leftCount && r <= rightCount) {
    ...
    if (leftDiff.command == Diff::Equal && rightDiff.command == Diff::Equal) {
      ...
      bool ok = false;
      const QString &commonEquality = encodeExpandedWhitespace(leftDiff.text, rightDiff.text, &leftCodeMap, &rightCodeMap, &ok);
      if (!ok)
        return false;
  ...
  Differ differ;
  const QList<Diff> &diffList = Differ::cleanupSemantics(differ.diff(leftText, rightText));
...
static auto appendWithEqualitiesSquashed(const QList<Diff> &leftInput, const QList<Diff> &rightInput, QList<Diff> *leftOutput, QList<Diff> *rightOutput) -> void
{
  if (!leftInput.isEmpty() && !rightInput.isEmpty() && !leftOutput->isEmpty() && !rightOutput->isEmpty() && leftInput.first().command == Diff::Equal && rightInput.first().command == Diff::Equal && leftOutput->last().command == Diff::Equal && rightOutput->last().command == Diff::Equal) {
 ...
 */
auto Differ::ignoreWhitespaceBetweenEqualities(const QList<Diff> &leftInput, const QList<Diff> &rightInput, QList<Diff> *leftOutput, QList<Diff> *rightOutput) -> void
{
  if (!leftOutput || !rightOutput)
    return;
  ...
  while (l <= leftCount && r <= rightCount) {
    ...
    if (leftDiff.command == Diff::Equal && rightDiff.command == Diff::Equal) {
      ...
      if (previousLeftDiff.command == Diff::Delete && previousRightDiff.command == Diff::Insert) {
        QList<Diff> outputLeftDiffList;
        QList<Diff> outputRightDiffList;

        QList<Diff> reducedLeftDiffList;
        QList<Diff> reducedRightDiffList;
        diffWithWhitespaceReduced(previousLeftDiff.text, previousRightDiff.text, &reducedLeftDiffList, &reducedRightDiffList);
        ...
        if (diffWithWhitespaceExpandedInEqualities(reducedLeftDiffList, reducedRightDiffList, &cleanedLeftDiffList, &cleanedRightDiffList)) {
        ...
        appendWithEqualitiesSquashed(outputLeftDiffList, outputRightDiffList, leftOutput, rightOutput);
      ...
      appendWithEqualitiesSquashed(leftEquality, rightEquality, leftOutput, rightOutput);
 ...
 */
auto Differ::diffBetweenEqualities(const QList<Diff> &leftInput, const QList<Diff> &rightInput, QList<Diff> *leftOutput, QList<Diff> *rightOutput) -> void
{
  if (!leftOutput || !rightOutput)
    return;
  ...
  while (l <= leftCount && r <= rightCount) {
    ...
    if (leftDiff.command == Diff::Equal && rightDiff.command == Diff::Equal) {
      ...
      if (previousLeftDiff.command == Diff::Delete && previousRightDiff.command == Diff::Insert) {
        ...
        const QList<Diff> commonOutput = cleanupSemantics(differ.diff(previousLeftDiff.text, previousRightDiff.text));

        QList<Diff> outputLeftDiffList;
        QList<Diff> outputRightDiffList;

        Differ::splitDiffList(commonOutput, &outputLeftDiffList, &outputRightDiffList);
...
Diff::Diff(Command com, const QString &txt) : command(com), text(txt) {}

auto Diff::operator==(const Diff &other) const -> bool
{
...
auto Diff::commandString(Command com) -> QString
{
  if (com == Delete)
...
auto Diff::toString() const -> QString
{
...
Differ::Differ(QFutureInterfaceBase *jobController) : m_jobController(jobController) {}

auto Differ::diff(const QString &text1, const QString &text2) -> QList<Diff>
{
  m_currentDiffMode = m_diffMode;
  return merge(preprocess1AndDiff(text1, text2));
}

auto Differ::unifiedDiff(const QString &text1, const QString &text2) -> QList<Diff>
{
...
auto Differ::setDiffMode(Differ::DiffMode mode) -> void
{
  m_diffMode = mode;
}

auto Differ::diffMode() const -> Differ::DiffMode
{
  return m_diffMode;
}

auto Differ::preprocess1AndDiff(const QString &text1, const QString &text2) -> QList<Diff>
{
...
auto Differ::preprocess2AndDiff(const QString &text1, const QString &text2) -> QList<Diff>
{
  QList<Diff> diffList;
...
auto Differ::diffMyers(const QString &text1, const QString &text2) -> QList<Diff>
{
  ...
  for (int d = 0; d <= D; d++) {
    ...
    // going forward
    for (int k = qMax(-d, kMinForward + qAbs(d + kMinForward) % 2); k <= qMin(d, kMaxForward - qAbs(d + kMaxForward) % 2); k = k + 2) {
      int x;
    ...
    for (int k = qMax(-d, kMinReverse + qAbs(d + kMinReverse) % 2); k <= qMin(d, kMaxReverse - qAbs(d + kMaxReverse) % 2); k = k + 2) {
      int x;
...
auto Differ::diffMyersSplit(const QString &text1, int x, const QString &text2, int y) -> QList<Diff>
{
...
auto Differ::diffNonCharMode(const QString &text1, const QString &text2) -> QList<Diff>
{
  QString encodedText1;
  QString encodedText2;
  QStringList subtexts = encode(text1, text2, &encodedText1, &encodedText2);
  ...
  for (int i = 0; i <= diffList.count(); i++) {
    ...
    const Diff diffItem = i < diffList.count() ? diffList.at(i) : Diff(Diff::Equal); // dummy, ensure we process to the end
...
auto Differ::encode(const QString &text1, const QString &text2, QString *encodedText1, QString *encodedText2) -> QStringList
{
  QStringList lines;
...
auto Differ::findSubtextEnd(const QString &text, int subtextStart) -> int
{
...
auto Differ::encode(const QString &text, QStringList *lines, QMap<QString, int> *lineToCode) -> QString
{
  int subtextStart = 0;
  int subtextEnd = -1;
  QString codes;
...
auto Differ::merge(const QList<Diff> &diffList) -> QList<Diff>
{
  QString lastDelete;
  QString lastInsert;
  QList<Diff> newDiffList;
  for (int i = 0; i <= diffList.count(); i++) {
    Diff diff = i < diffList.count() ? diffList.at(i) : Diff(Diff::Equal); // dummy, ensure we process to the end
    // even when diffList doesn't end with equality
    if (diff.command == Diff::Delete) {
      ...
      if (!(lastDelete.isEmpty() && lastInsert.isEmpty())) {
        ...
        if (prefixCount) {
          ...
          if (!newDiffList.isEmpty() && newDiffList.last().command == Diff::Equal) {
...
auto Differ::cleanupSemantics(const QList<Diff> &diffList) -> QList<Diff>
{
  int deletes = 0;
  int inserts = 0;
  ...
  for (int i = 0; i <= diffList.count(); i++) {
    const Diff diff = i < diffList.count() ? diffList.at(i) : Diff(Diff::Equal); // dummy, ensure we process to the end
    // even when diffList doesn't end with equality
    if (diff.command == Diff::Equal) {
  ...
  while (i < equalities.count()) {
    ...
    if (data.textCount <= qMax(data.deletesBefore, data.insertsBefore) && data.textCount <= qMax(data.deletesAfter, data.insertsAfter)) {
      if (i > 0) {
...
auto Differ::cleanupSemanticsLossless(const QList<Diff> &diffList) -> QList<Diff>
{
  if (diffList.count() < 3) // we need at least 3 items
    return diffList;
  ...
  while (i < diffList.count()) {
    if (prevDiff.command == Diff::Equal && nextDiff.command == Diff::Equal) {
      ...
      int bestScore = cleanupSemanticsScore(equality1, edit) + cleanupSemanticsScore(edit, equality2);

      while (!edit.isEmpty() && !equality2.isEmpty() && edit.at(0) == equality2.at(0)) {
        equality1 += edit.at(0);
        edit = edit.mid(1) + equality2.at(0);
        equality2 = equality2.mid(1);
        const int score = cleanupSemanticsScore(equality1, edit) + cleanupSemanticsScore(edit, equality2);
        if (score >= bestScore) {
