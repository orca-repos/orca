...
auto NameValueItem::sort(NameValueItems *list) -> void
{
  Utils::sort(*list, &NameValueItem::name);
}

auto NameValueItem::fromStringList(const QStringList &list) -> NameValueItems
{
  ...
  for (const QString &string : list) {
    ...
    if (hashPos != -1 && hashPos < pos) {
      result.append({string.mid(hashPos + 1, pos - hashPos - 1), string.mid(pos + 1), NameValueItem::SetDisabled});
    } else {
...
auto NameValueItem::toStringList(const NameValueItems &list) -> QStringList
{
...
auto NameValueItem::itemsFromVariantList(const QVariantList &list) -> NameValueItems
{
...
auto NameValueItem::toVariantList(const NameValueItems &list) -> QVariantList
{
...
auto NameValueItem::itemFromVariantList(const QVariantList &list) -> NameValueItem
{
...
auto NameValueItem::toVariantList(const NameValueItem &item) -> QVariantList
{
  return QVariantList() << item.name << item.operation << item.value;
}

static auto expand(const NameValueDictionary *dictionary, QString value) -> QString
{
  int replaceCount = 0;
...
auto NameValueItem::apply(NameValueDictionary *dictionary, Operation op) const -> void
{
  switch (op) {
  case SetEnabled:
...
auto operator<<(QDebug debug, const NameValueItem &i) -> QDebug
{
