...
static constexpr char AutoHideTitleBarsKey[] = "AutoHideTitleBars";
static constexpr char ShowCentralWidgetKey[] = "ShowCentralWidget";
static constexpr char StateKey[] = "State";
static constexpr int settingsVersion = 2;
static constexpr char dockWidgetActiveState[] = "DockWidgetActiveState";
...
public:
  ...
  auto handleMouseTimeout() -> void;
  auto handleToplevelChanged(bool floating) -> void;
...
public:
  DockWidgetTitleButton(QWidget *parent) : QAbstractButton(parent)
  {
  ...
  auto minimumSizeHint() const -> QSize override { return sizeHint(); }
...
auto DockWidgetTitleButton::paintEvent(QPaintEvent *) -> void
{
  QPainter p(this);
...
public:
  TitleBarWidget(DockWidget *parent, const QStyleOptionDockWidget &opt) : QWidget(parent), q(parent), m_active(true)
  {
...
DockWidget::DockWidget(QWidget *inner, FancyMainWindow *parent, bool immutable) : QDockWidget(parent), q(parent), m_immutable(immutable)
{
  setWidget(inner);
  ...
  connect(m_titleBar->m_floatButton, &QAbstractButton::clicked, origFloatButton, &QAbstractButton::clicked);

  auto origCloseButton = findChild<QAbstractButton*>(QLatin1String("qt_dockwidget_closebutton"));
  connect(m_titleBar->m_closeButton, &QAbstractButton::clicked, origCloseButton, &QAbstractButton::clicked);
}

auto DockWidget::eventFilter(QObject *, QEvent *event) -> bool
{
...
auto DockWidget::enterEvent(EnterEvent *event) -> void
{
  if (!m_immutable)
...
auto DockWidget::leaveEvent(QEvent *event) -> void
{
  if (!m_immutable) {
    if (!isFloating()) {
      m_timer.stop();
...
auto DockWidget::handleMouseTimeout() -> void
{
...
auto DockWidget::handleToplevelChanged(bool floating) -> void
{
...
FancyMainWindowPrivate::FancyMainWindowPrivate(FancyMainWindow *parent) : q(parent), m_handleDockVisibilityChanges(true), m_showCentralWidget(FancyMainWindow::tr("Central Widget"), nullptr), m_menuSeparator1(nullptr), m_menuSeparator2(nullptr), m_resetLayoutAction(FancyMainWindow::tr("Reset to Default Layout"), nullptr), m_autoHideTitleBars(FancyMainWindow::tr("Automatically Hide View Title Bars"), nullptr)
{
...
FancyMainWindow::FancyMainWindow(QWidget *parent) : QMainWindow(parent), d(new FancyMainWindowPrivate(this))
{
  connect(&d->m_resetLayoutAction, &QAction::triggered, this, &FancyMainWindow::resetLayout);
}
...
auto FancyMainWindow::addDockForWidget(QWidget *widget, bool immutable) -> QDockWidget*
{
  ...
  if (!immutable) {
    ...
    connect(dockWidget->toggleViewAction(), &QAction::triggered, this, &FancyMainWindow::onDockActionTriggered, Qt::QueuedConnection);
...
auto FancyMainWindow::onDockActionTriggered() -> void
{
...
auto FancyMainWindow::setTrackingEnabled(bool enabled) -> void
{
  if (enabled) {
...
auto FancyMainWindow::hideEvent(QHideEvent *event) -> void
{
  Q_UNUSED(event)
  handleVisibilityChanged(false);
}

auto FancyMainWindow::showEvent(QShowEvent *event) -> void
{
  Q_UNUSED(event)
  handleVisibilityChanged(true);
}

auto FancyMainWindow::contextMenuEvent(QContextMenuEvent *event) -> void
{
  QMenu menu;
...
auto FancyMainWindow::handleVisibilityChanged(bool visible) -> void
{
  d->m_handleDockVisibilityChanges = false;
  for (QDockWidget *dockWidget : dockWidgets()) {
...
auto FancyMainWindow::saveSettings(QSettings *settings) const -> void
{
...
auto FancyMainWindow::restoreSettings(const QSettings *settings) -> void
{
...
auto FancyMainWindow::saveSettings() const -> QHash<QString, QVariant>
{
  QHash<QString, QVariant> settings;
  settings.insert(QLatin1String(StateKey), saveState(settingsVersion));
  settings.insert(QLatin1String(AutoHideTitleBarsKey), d->m_autoHideTitleBars.isChecked());
...
auto FancyMainWindow::restoreSettings(const QHash<QString, QVariant> &settings) -> void
{
...
auto FancyMainWindow::dockWidgets() const -> const QList<QDockWidget*>
{
  return findChildren<QDockWidget*>();
}

auto FancyMainWindow::autoHideTitleBars() const -> bool
{
  return d->m_autoHideTitleBars.isChecked();
}

auto FancyMainWindow::setAutoHideTitleBars(bool on) -> void
{
  d->m_autoHideTitleBars.setChecked(on);
}

auto FancyMainWindow::isCentralWidgetShown() const -> bool
{
  return d->m_showCentralWidget.isChecked();
}

auto FancyMainWindow::showCentralWidget(bool on) -> void
{
  d->m_showCentralWidget.setChecked(on);
}

auto FancyMainWindow::addDockActionsToMenu(QMenu *menu) -> void
{
  QList<QAction*> actions;
  QList<QDockWidget*> dockwidgets = findChildren<QDockWidget*>();
  for (int i = 0; i < dockwidgets.size(); ++i) {
    QDockWidget *dockWidget = dockwidgets.at(i);
    if (dockWidget->property("managed_dockwidget").isNull() && dockWidget->parentWidget() == this) {
...
auto FancyMainWindow::menuSeparator1() const -> QAction*
{
  return &d->m_menuSeparator1;
}

auto FancyMainWindow::autoHideTitleBarsAction() const -> QAction*
{
  return &d->m_autoHideTitleBars;
}

auto FancyMainWindow::menuSeparator2() const -> QAction*
{
  return &d->m_menuSeparator2;
}

auto FancyMainWindow::resetLayoutAction() const -> QAction*
{
  return &d->m_resetLayoutAction;
}

auto FancyMainWindow::showCentralWidgetAction() const -> QAction*
{
  return &d->m_showCentralWidget;
}

auto FancyMainWindow::setDockActionsVisible(bool v) -> void
{
