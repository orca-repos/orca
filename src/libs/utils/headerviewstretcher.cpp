...
#include "headerviewstretcher.hpp"
#include <QHideEvent>
#include <QHeaderView>
...
HeaderViewStretcher::HeaderViewStretcher(QHeaderView *headerView, int columnToStretch) : QObject(headerView), m_columnToStretch(columnToStretch)
{
  headerView->installEventFilter(this);
  stretch();
}

auto HeaderViewStretcher::stretch() -> void
{
  QHideEvent fake;
  HeaderViewStretcher::eventFilter(parent(), &fake);
}

auto HeaderViewStretcher::softStretch() -> void
{
...
auto HeaderViewStretcher::eventFilter(QObject *obj, QEvent *ev) -> bool
{
  if (obj == parent()) {
