...
class ORCA_UTILS_EXPORT Singleton {
  Q_DISABLE_COPY_MOVE(Singleton)

public:
  static auto deleteAll() -> void;
...
private:
  ...
  Singleton() = default;
  virtual ~Singleton();
  static auto addSingleton(Singleton *singleton) -> void;
  static auto staticData(std::type_index index) -> SingletonStaticData&;
};
...
public:
  ...
  static auto instance() -> SingletonSubClass*
  {
...
private:
  ...
  static auto instantiateDependencies() -> void
  {
    static_assert ((... && std::is_base_of_v<Singleton, Dependency>), "All Dependencies must derive from SingletonWithOptionalDependencies class.");
    (..., Dependency::instance());
  }

  static auto staticData() -> SingletonStaticData&
  {
