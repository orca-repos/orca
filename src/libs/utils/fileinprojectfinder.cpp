...
static auto checkPath(const QString &candidate, int matchLength, FileInProjectFinder::FileHandler fileHandler, FileInProjectFinder::DirectoryHandler directoryHandler) -> bool
{
...
auto FileInProjectFinder::setProjectDirectory(const FilePath &absoluteProjectPath) -> void
{
  if (absoluteProjectPath == m_projectDir)
    return;

  QTC_CHECK(absoluteProjectPath.isEmpty() || (absoluteProjectPath.exists() && absoluteProjectPath.isAbsolutePath()));

  m_projectDir = absoluteProjectPath;
  m_cache.clear();
}

auto FileInProjectFinder::projectDirectory() const -> FilePath
{
  return m_projectDir;
}

auto FileInProjectFinder::setProjectFiles(const FilePaths &projectFiles) -> void
{
...
auto FileInProjectFinder::setSysroot(const FilePath &sysroot) -> void
{
...
auto FileInProjectFinder::addMappedPath(const FilePath &localFilePath, const QString &remoteFilePath) -> void
{
...
/*!
  ...
  */
auto FileInProjectFinder::findFile(const QUrl &fileUrl, bool *success) const -> FilePaths
{
...
auto FileInProjectFinder::handleSuccess(const QString &originalPath, const QStringList &found, int matchLength, const char *where) const -> bool
{
...
auto FileInProjectFinder::findFileOrDirectory(const QString &originalPath, FileHandler fileHandler, DirectoryHandler directoryHandler) const -> bool
{
  ...
  if (node) {
    if (!node->localPath.isEmpty()) {
      ...
      if (checkPath(localPath, origLength, fileHandler, directoryHandler)) {
        return handleSuccess(originalPath, QStringList(localPath), origLength, "in mapped paths");
      }
    } else if (directoryHandler) {
      directoryHandler(node->children.keys(), origLength);
      qCDebug(finderLog) << "FileInProjectFinder: found virtual directory" << originalPath << "in mapped paths";
      return true;
    }
  }
  ...
  if (!m_projectDir.isEmpty()) {
    ...
    if (originalPath.startsWith(m_projectDir.toString() + separator)) {
      ...
      if (prefixToIgnore == -1 && checkPath(originalPath, origLength, fileHandler, directoryHandler)) {
        return handleSuccess(originalPath, QStringList(originalPath), origLength, "in project directory");
      }
    }
    ...
    if (prefixToIgnore < 0) {
      if (!QFileInfo(originalPath).isAbsolute() && !originalPath.startsWith(separator)) {
        prefixToIgnore = 0;
      } else {
    ...
    while (prefixToIgnore != -1) {
      ...
      if (checkPath(candidate, matchLength, fileHandler, directoryHandler)) {
        return handleSuccess(originalPath, QStringList(candidate), matchLength, "in project directory");
      }
  ...
  if (!foundPath.paths.isEmpty()) {
    return handleSuccess(originalPath, foundPath.paths, foundPath.matchLength, "in search path");
  }
  ...
  if (!m_sysroot.isEmpty()) {
    ...
    if (checkPath(sysrootPath.toString(), origLength, fileHandler, directoryHandler)) {
      return handleSuccess(originalPath, QStringList(sysrootPath.toString()), origLength, "in sysroot");
    }
  }
...
auto FileInProjectFinder::findInSearchPaths(const QString &filePath, FileHandler fileHandler, DirectoryHandler directoryHandler) const -> FileInProjectFinder::CacheEntry
{
  for (const FilePath &dirPath : m_searchDirectories) {
    const CacheEntry found = findInSearchPath(dirPath.toString(), filePath, fileHandler, directoryHandler);
...
static auto chopFirstDir(const QString &dirPath) -> QString
{
...
auto FileInProjectFinder::findInSearchPath(const QString &searchPath, const QString &filePath, FileHandler fileHandler, DirectoryHandler directoryHandler) -> FileInProjectFinder::CacheEntry
{
...
auto FileInProjectFinder::filesWithSameFileName(const QString &fileName) const -> QStringList
{
  QStringList result;
...
auto FileInProjectFinder::pathSegmentsWithSameName(const QString &pathSegment) const -> QStringList
{
  QStringList result;
  for (const FilePath &f : m_projectFiles) {
...
auto FileInProjectFinder::commonPostFixLength(const QString &candidatePath, const QString &filePathToFind) -> int
{
  int rank = 0;
  for (int a = candidatePath.length(), b = filePathToFind.length(); --a >= 0 && --b >= 0 && candidatePath.at(a) == filePathToFind.at(b);)
    rank++;
  return rank;
}

auto FileInProjectFinder::bestMatches(const QStringList &filePaths, const QString &filePathToFind) -> QStringList
{
  if (filePaths.isEmpty())
    return {};
  if (filePaths.length() == 1) {
    qCDebug(finderLog) << "FileInProjectFinder: found" << filePaths.first() << "in project files";
    return filePaths;
  }
  int bestRank = -1;
...
auto FileInProjectFinder::searchDirectories() const -> FilePaths
{
  return m_searchDirectories;
}

auto FileInProjectFinder::setAdditionalSearchDirectories(const FilePaths &searchDirectories) -> void
{
...
auto FileInProjectFinder::QrcUrlFinder::find(const QUrl &fileUrl) const -> FilePaths
{
...
auto FileInProjectFinder::QrcUrlFinder::setProjectFiles(const FilePaths &projectFiles) -> void
{
...
auto chooseFileFromList(const FilePaths &candidates) -> FilePath
{
