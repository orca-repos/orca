...
public:
  ...
  StringHolder(const char *s, int length) : n(length), str(s)
  {
    if (!n)
  ...
  friend auto operator==(const StringHolder &sh1, const StringHolder &sh2) -> bool
  {
...
static auto theId(const char *str, int n = 0) -> quintptr
{
...
static auto theId(const QByteArray &ba) -> quintptr
{
...
*/
Id::Id(const char *name) : m_id(theId(name, 0)) {}

/*!
  Returns an internal representation of the id.
*/

auto Id::name() const -> QByteArray
{
...
auto Id::toString() const -> QString
{
...
auto Id::fromString(const QString &name) -> Id
{
  if (name.isEmpty())
    return Id();
...
auto Id::fromName(const QByteArray &name) -> Id
{
...
auto Id::toSetting() const -> QVariant
{
...
auto Id::fromSetting(const QVariant &variant) -> Id
{
...
auto Id::versionedId(const QByteArray &prefix, int major, int minor) -> Id
{
...
auto Id::fromStringList(const QStringList &list) -> QSet<Id>
{
  return Utils::transform<QSet<Id>>(list, &Id::fromString);
}

auto Id::toStringList(const QSet<Id> &ids) -> QStringList
{
...
auto Id::withSuffix(int suffix) const -> Id
{
...
auto Id::withSuffix(const char *suffix) const -> Id
{
...
auto Id::withSuffix(const QString &suffix) const -> Id
{
...
auto Id::withPrefix(const char *prefix) const -> Id
{
  const QByteArray ba = prefix + name();
  return Id(ba.constData());
}

auto Id::operator==(const char *name) const -> bool
{
...
ORCA_UTILS_EXPORT auto nameForId(quintptr id) -> const char*
{
  return stringFromId.value(id).str;
}

auto Id::alphabeticallyBefore(Id other) const -> bool
{
...
auto Id::suffixAfter(Id baseId) const -> QString
{
...
auto operator<<(QDataStream &ds, Utils::Id id) -> QDataStream&
{
  return ds << id.name();
}
...
auto operator<<(QDebug dbg, const Utils::Id &id) -> QDebug
{
  return dbg << id.name();
}
