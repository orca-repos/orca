...
OptionsParser::OptionsParser(const QStringList &args, const QMap<QString, bool> &appOptions, QMap<QString, QString> *foundAppOptions, QString *errorString, PluginManagerPrivate *pmPrivate) : m_args(args), m_appOptions(appOptions), m_foundAppOptions(foundAppOptions), m_errorString(errorString), m_pmPrivate(pmPrivate), m_it(m_args.constBegin()), m_end(m_args.constEnd()), m_isDependencyRefreshNeeded(false), m_hasError(false)
{
...
auto OptionsParser::checkForEndOfOptions() -> bool
{
...
auto OptionsParser::checkForTestOptions() -> bool
{
  if (m_currentArg == QLatin1String(TEST_OPTION)) {
    if (nextToken(RequiredToken)) {
      if (m_currentArg == QLatin1String("all")) {
        m_pmPrivate->testSpecs = Utils::transform<std::vector>(m_pmPrivate->loadQueue(), [](PluginSpec *spec) {
        ...
        if (PluginSpec *spec = m_pmPrivate->pluginByName(pluginName)) {
          if (m_pmPrivate->containsTestSpec(spec)) {
            if (m_errorString)
              *m_errorString = QCoreApplication::translate("PluginManager", "The plugin \"%1\" is specified twice for testing.").arg(pluginName);
            m_hasError = true;
          } else {
          ...
          if (m_errorString)
            *m_errorString = QCoreApplication::translate("PluginManager", "The plugin \"%1\" does not exist.").arg(pluginName);
          m_hasError = true;
        }
      }
    }
    ...
    if (nextToken(RequiredToken)) {
      if (PluginSpec *spec = m_pmPrivate->pluginByName(m_currentArg)) {
        if (!m_pmPrivate->containsTestSpec(spec)) {
          if (m_errorString)
            *m_errorString = QCoreApplication::translate("PluginManager", "The plugin \"%1\" is not tested.").arg(m_currentArg);
          m_hasError = true;
        } else {
...
auto OptionsParser::checkForScenarioOption() -> bool
{
  if (m_currentArg == QLatin1String(SCENARIO_OPTION)) {
    if (nextToken(RequiredToken)) {
      if (!m_pmPrivate->m_requestedScenario.isEmpty()) {
        if (m_errorString) {
          *m_errorString = QCoreApplication::translate("PluginManager", "Cannot request scenario \"%1\" as the scenario \"%1\" was already requested.").arg(m_currentArg, m_pmPrivate->m_requestedScenario);
        }
        m_hasError = true;
      } else {
...
auto OptionsParser::checkForLoadOption() -> bool
{
...
auto OptionsParser::checkForNoLoadOption() -> bool
{
...
auto OptionsParser::checkForAppOption() -> bool
{
...
auto OptionsParser::checkForProfilingOption() -> bool
{
...
auto OptionsParser::checkForNoCrashcheckOption() -> bool
{
...
auto OptionsParser::checkForPluginOption() -> bool
{
...
auto OptionsParser::checkForUnknownOption() -> bool
{
...
auto OptionsParser::forceDisableAllPluginsExceptTestedAndForceEnabled() -> void
{
...
auto OptionsParser::nextToken(OptionsParser::TokenType type) -> bool
{
  if (m_it == m_end) {
