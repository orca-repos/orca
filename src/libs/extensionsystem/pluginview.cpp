...
static auto icon(IconIndex icon) -> const QIcon&
{
  switch (icon) {
  case OkIcon: {
...
public:
  PluginItem(PluginSpec *spec, PluginView *view) : m_spec(spec), m_view(view) {}
  ...
  auto data(int column, int role) const -> QVariant override
  {
    ...
    case NameColumn:
      if (role == Qt::DisplayRole)
        return m_spec->isExperimental() ? PluginView::tr("%1 (experimental)").arg(m_spec->name()) : m_spec->name();
      if (role == SortRole)
        return m_spec->name();
...
public:
  CollectionItem(const QString &name, const QVector<PluginSpec*> &plugins, PluginView *view) : m_name(name), m_plugins(plugins), m_view(view)
  {
  ...
  {
    if (column == LoadedColumn && role == Qt::CheckStateRole) {
      const QVector<PluginSpec*> affectedPlugins = Utils::filtered(m_plugins, [](PluginSpec *spec) { return !spec->isRequired(); });
      if (m_view->setPluginsEnabled(Utils::transform<QSet>(affectedPlugins, [](PluginSpec *s) { return s; }), data.toBool())) {
        update();
        return true;
      }
    }
...
PluginView::PluginView(QWidget *parent) : QWidget(parent)
{
  ...
  connect(PluginManager::instance(), &PluginManager::pluginsChanged, this, &PluginView::updatePlugins);

  connect(m_categoryView, &QAbstractItemView::activated, [this](const QModelIndex &idx) { emit pluginActivated(pluginForIndex(idx)); });

  connect(m_categoryView->selectionModel(), &QItemSelectionModel::currentChanged, [this](const QModelIndex &idx) { emit currentPluginChanged(pluginForIndex(idx)); });
...
*/
auto PluginView::currentPlugin() const -> PluginSpec*
{
...
*/
auto PluginView::setFilter(const QString &filter) -> void
{
  m_sortModel->setFilterRegularExpression(QRegularExpression(QRegularExpression::escape(filter), QRegularExpression::CaseInsensitiveOption));
  m_categoryView->expandAll();
}

auto PluginView::pluginForIndex(const QModelIndex &index) const -> PluginSpec*
{
...
auto PluginView::updatePlugins() -> void
{
  // Model.
  m_model->clear();

  const QHash<QString, QVector<PluginSpec*>> pluginCollections = PluginManager::pluginCollections();
...
auto PluginView::setPluginsEnabled(const QSet<PluginSpec*> &plugins, bool enable) -> bool
{
  ...
  if (enable) {
    ...
    if (!additionalPlugins.isEmpty()) {
      if (QMessageBox::question(this, tr("Enabling Plugins"), tr("Enabling\n%1\nwill also enable the following plugins:\n\n%2").arg(pluginListString(plugins)).arg(pluginListString(additionalPlugins)), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok) != QMessageBox::Ok)
        return false;
    }
  } else {
    for (PluginSpec *spec : plugins) {
    ...
    additionalPlugins.subtract(plugins);
    if (!additionalPlugins.isEmpty()) {
      if (QMessageBox::question(this, tr("Disabling Plugins"), tr("Disabling\n%1\nwill also disable the following plugins:\n\n%2").arg(pluginListString(plugins)).arg(pluginListString(additionalPlugins)), QMessageBox::Ok | QMessageBox::Cancel, QMessageBox::Ok) != QMessageBox::Ok)
        return false;
    }
  }
