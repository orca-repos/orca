...
/*!
    ...
    Invokes \a slot on \a target by name via Orca's meta method system.
...
/*!
    ...
    Invokes \a slot on \a target with argument \a t0 by name via Orca's meta method system.
...
/*!
    ...
    Invokes \a slot on \a target with arguments \a t0 and \a t1 by name via Orca's meta method system.
...
{
  if (!success && nag)
    qWarning("Could not invoke function '%s' in object of type '%s'.", sig.constData(), target->metaObject()->className());
}
...
auto InvokerBase::setConnectionType(Qt::ConnectionType c) -> void
{
  connectionType = c;
}

auto InvokerBase::invoke(QObject *t, const char *slot) -> void
{
  target = t;
  success = false;
  ...
  if (useRet)
    success = method.invoke(target, connectionType, ret, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], arg[8], arg[9]);
  else
    success = method.invoke(target, connectionType, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6], arg[7], arg[8], arg[9]);
}
