...
public:
  ...
  // IEditor
  auto document() const -> Core::IDocument* override;
  ...
  auto setContextHelp(const Core::HelpItem &item) -> void override;
...
public:
  ...
  auto highlightScrollBarController() const -> Core::HighlightScrollBarController*;
...
public:
  ...
  auto characterAt(int pos) const -> QChar;
  auto textAt(int from, int to) const -> QString;
  auto contextHelpItem(const Core::IContext::HelpCallback &callback) -> void;
  auto setContextHelpItem(const Core::HelpItem &item) -> void;
  auto inFindScope(const QTextCursor &cursor) const -> Q_INVOKABLE bool;
  static auto currentTextEditorWidget() -> TextEditorWidget*;
  static auto fromEditor(const Core::IEditor *editor) -> TextEditorWidget*;
...
signals:
  ...
  auto activateEditor(Core::EditorManager::OpenEditorFlags flags = {}) -> void;
