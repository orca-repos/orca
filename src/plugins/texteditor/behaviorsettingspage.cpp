...
auto BehaviorSettingsPage::apply() -> void
{
  ...
  QSettings *s = Orca::Plugin::Core::ICore::settings();
  QTC_ASSERT(s, return);
  ...
  s->setValue(QLatin1String(Orca::Plugin::Core::SETTINGS_DEFAULTTEXTENCODING), d->m_page->behaviorWidget->assignedCodecName());
  s->setValue(QLatin1String(Orca::Plugin::Core::SETTINGS_DEFAULT_LINE_TERMINATOR), d->m_page->behaviorWidget->assignedLineEnding());
}
...
auto BehaviorSettingsPage::settingsToUI() -> void
{
  ...
  d->m_page->behaviorWidget->setAssignedCodec(Orca::Plugin::Core::EditorManager::defaultTextCodec());
  d->m_page->behaviorWidget->setAssignedLineEnding(Orca::Plugin::Core::EditorManager::defaultLineEnding());
}
...
auto BehaviorSettingsPage::openCodingStylePreferences(TabSettingsWidget::CodingStyleLink link) -> void
{
  ...
  case TabSettingsWidget::CppLink:
    Orca::Plugin::Core::ICore::showOptionsDialog(CppEditor::Constants::CPP_CODE_STYLE_SETTINGS_ID);
    break;
//case TabSettingsWidget::QtQuickLink:
//  Orca::Plugin::Core::ICore::showOptionsDialog(QmlJSTools::Constants::QML_JS_CODE_STYLE_SETTINGS_ID);
//  break;
  }
}
