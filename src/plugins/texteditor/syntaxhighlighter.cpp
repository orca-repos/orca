...
public:
  ...
  auto reformatBlock(const QTextBlock &block, int from, int charsRemoved, int charsAdded) -> void;

  auto rehighlight(QTextCursor &cursor, QTextCursor::MoveOperation operation) -> void
  {
    inReformatBlocks = true;
    const auto from = cursor.position();
    cursor.movePosition(operation);
    reformatBlocks(from, 0, cursor.position() - from);
    inReformatBlocks = false;
  }

  auto applyFormatChanges(int from, int charsRemoved, int charsAdded) -> void;
  auto updateFormats(const FontSettings &fontSettings) -> void;
...
static auto adjustRange(QTextLayout::FormatRange &range, int from, int charsDelta) -> bool
{
  if (range.start >= from) {
    range.start += charsDelta;
    return true;
  }
  if (range.start + range.length > from) {
...
auto SyntaxHighlighter::delayedRehighlight() -> void
{
...
auto SyntaxHighlighterPrivate::applyFormatChanges(int from, int charsRemoved, int charsAdded) -> void
{
  auto formatsChanged = false;

  const auto layout = currentBlock.layout();

  QVector<QTextLayout::FormatRange> ranges;
  QVector<QTextLayout::FormatRange> oldRanges;
  std::tie(ranges, oldRanges) = Utils::partition(layout->formats(), [](const QTextLayout::FormatRange &range) {
    return range.format.property(QTextFormat::UserProperty).toBool();
  });

  if (currentBlock.contains(from)) {
    const auto charsDelta = charsAdded - charsRemoved;
  ...
  for (auto i = 0; !formatsChanged && i < newRanges.size(); ++i) {
    const auto &o = oldRanges.at(i);
    const auto &n = newRanges.at(i);
...
auto SyntaxHighlighter::reformatBlocks(int from, int charsRemoved, int charsAdded) -> void
{
  Q_D(SyntaxHighlighter);
...
auto SyntaxHighlighterPrivate::reformatBlocks(int from, int charsRemoved, int charsAdded) -> void
{
  foldValidator.reset();
  ...
  int endPosition;
  const auto lastBlock = doc->findBlock(from + charsAdded + (charsRemoved > 0 ? 1 : 0));
...
auto SyntaxHighlighterPrivate::reformatBlock(const QTextBlock &block, int from, int charsRemoved, int charsAdded) -> void
{
...
*/
SyntaxHighlighter::SyntaxHighlighter(QObject *parent) : QObject(parent), d_ptr(new SyntaxHighlighterPrivate)
{
  d_ptr->q_ptr = this;
}
...
/*!
    ...
    SyntaxHighlighter.
*/
SyntaxHighlighter::SyntaxHighlighter(QTextDocument *parent) : QObject(parent), d_ptr(new SyntaxHighlighterPrivate)
{
  d_ptr->q_ptr = this;
  if (parent)
    setDocument(parent);
...
*/
SyntaxHighlighter::SyntaxHighlighter(QTextEdit *parent) : QObject(parent), d_ptr(new SyntaxHighlighterPrivate)
{
  d_ptr->q_ptr = this;
  if (parent)
...
*/
auto SyntaxHighlighter::setDocument(QTextDocument *doc) -> void
{
  Q_D(SyntaxHighlighter);
  if (d->doc) {
  ...
  if (d->doc) {
    if (!d->noAutomaticHighlighting) {
      ...
      QMetaObject::invokeMethod(this, &SyntaxHighlighter::delayedRehighlight, Qt::QueuedConnection);
    }
...
/*!
    ...
    installed.
*/
auto SyntaxHighlighter::document() const -> QTextDocument*
{
...
/*!
    ...
    \sa rehighlightBlock()
*/
auto SyntaxHighlighter::rehighlight() -> void
{
  Q_D(SyntaxHighlighter);
  if (!d->doc)
    return;
...
/*!
    ...
    \sa rehighlight()
*/
auto SyntaxHighlighter::rehighlightBlock(const QTextBlock &block) -> void
{
  Q_D(SyntaxHighlighter);
...
*/
auto SyntaxHighlighter::setFormat(int start, int count, const QTextCharFormat &format) -> void
{
  Q_D(SyntaxHighlighter);
...
/*!
    ...
    \sa format(), highlightBlock()
*/
auto SyntaxHighlighter::setFormat(int start, int count, const QColor &color) -> void
{
...
/*!
    ...
    \sa format(), highlightBlock()
*/
auto SyntaxHighlighter::setFormat(int start, int count, const QFont &font) -> void
{
  QTextCharFormat format;
  format.setFont(font);
  setFormat(start, count, format);
...
auto SyntaxHighlighter::formatSpaces(const QString &text, int start, int count) -> void
{
  Q_D(const SyntaxHighlighter);
  auto offset = start;
  const auto end = std::min(start + count, int(text.length()));
  while (offset < end) {
...
/*!
    ...
    \sa setFormat()
*/
auto SyntaxHighlighter::setFormatWithSpaces(const QString &text, int start, int count, const QTextCharFormat &format) -> void
{
  Q_D(const SyntaxHighlighter);
  auto visualSpaceFormat = d->whitespaceFormat;
  ...
  while (index != end) {
    const auto isSpace = text.at(index).isSpace();
    const auto start = index;

    do { ++index; } while (index != end && text.at(index).isSpace() == isSpace);

    const auto tokenLength = index - start;
    if (isSpace)
...
/*!
    ...
    current text block.
*/
auto SyntaxHighlighter::format(int pos) const -> QTextCharFormat
{
  Q_D(const SyntaxHighlighter);
...
*/
auto SyntaxHighlighter::previousBlockState() const -> int
{
  Q_D(const SyntaxHighlighter);
...
*/
auto SyntaxHighlighter::currentBlockState() const -> int
{
  Q_D(const SyntaxHighlighter);
  if (!d->currentBlock.isValid())
    return -1;
...
/*!
    ...
    \sa highlightBlock()
*/
auto SyntaxHighlighter::setCurrentBlockState(int newState) -> void
{
  Q_D(SyntaxHighlighter);
  if (!d->currentBlock.isValid())
    return;
...
*/
auto SyntaxHighlighter::setCurrentBlockUserData(QTextBlockUserData *data) -> void
{
  Q_D(SyntaxHighlighter);
  if (!d->currentBlock.isValid())
    return;
...
*/
auto SyntaxHighlighter::currentBlockUserData() const -> QTextBlockUserData*
{
  Q_D(const SyntaxHighlighter);
  if (!d->currentBlock.isValid())
    return nullptr;
...
*/
auto SyntaxHighlighter::currentBlock() const -> QTextBlock
{
  Q_D(const SyntaxHighlighter);
  return d->currentBlock;
}

static auto byStartOfRange(const QTextLayout::FormatRange &range, const QTextLayout::FormatRange &other) -> bool
{
...
auto SyntaxHighlighter::setExtraFormats(const QTextBlock &block, QVector<QTextLayout::FormatRange> &&formats) -> void
{
  Q_D(SyntaxHighlighter);

  const auto blockLength = block.length();
  if (block.layout() == nullptr || blockLength == 0)
    return;

  Utils::sort(formats, byStartOfRange);

  const auto all = block.layout()->formats();
  QVector<QTextLayout::FormatRange> previousSemanticFormats;
  QVector<QTextLayout::FormatRange> formatsToApply;
  std::tie(previousSemanticFormats, formatsToApply) = Utils::partition(all, [](const QTextLayout::FormatRange &r) {
  ...
  const auto wasInReformatBlocks = d->inReformatBlocks;
...
auto SyntaxHighlighter::clearExtraFormats(const QTextBlock &block) -> void
{
  Q_D(SyntaxHighlighter);
  ...
  const auto formatsToApply = Utils::filtered(block.layout()->formats(), [](const QTextLayout::FormatRange &r) {
    return !r.format.hasProperty(QTextFormat::UserProperty);
  });
...
auto SyntaxHighlighter::clearAllExtraFormats() -> void
{
...
auto SyntaxHighlighter::generateColors(int n, const QColor &background) -> QList<QColor>
{
  QList<QColor> result;
  ...
  const auto step = qRound(std::ceil(std::pow(double(n), oneThird)));
  result.reserve(step * step * step);
  const auto factor = 255 / step;
  const auto half = factor / 2;
  const auto bgRed = background.red();
  const auto bgGreen = background.green();
  const auto bgBlue = background.blue();
  for (auto r = step; r >= 0; --r) {
    const auto red = r * factor;
    if (bgRed - half > red || bgRed + half <= red) {
      for (auto g = step; g >= 0; --g) {
        const auto green = g * factor;
        if (bgGreen - half > green || bgGreen + half <= green) {
          for (auto b = step; b >= 0; --b) {
            const auto blue = b * factor;
...
auto SyntaxHighlighter::setFontSettings(const FontSettings &fontSettings) -> void
{
  Q_D(SyntaxHighlighter);
  d->updateFormats(fontSettings);
}

auto SyntaxHighlighter::fontSettings() const -> FontSettings
{
  Q_D(const SyntaxHighlighter);
...
/*!
    ...
    \c true.
*/
auto SyntaxHighlighter::setNoAutomaticHighlighting(bool noAutomatic) -> void
{
  Q_D(SyntaxHighlighter);
...
*/
auto SyntaxHighlighter::setDefaultTextFormatCategories() -> void
{
...
/*!
    ...
    \sa setTextFormatCategories()
*/
auto SyntaxHighlighter::setTextFormatCategories(int count, std::function<TextStyle(int)> formatMapping) -> void
{
...
/*!
    ...
    \sa setDefaultTextFormatCategories()
*/
auto SyntaxHighlighter::setTextFormatCategories(const QVector<std::pair<int, TextStyle>> &categories) -> void
{
  Q_D(SyntaxHighlighter);
  d->formatCategories = categories;
  const auto maxCategory = Utils::maxElementOr(categories, {-1, C_TEXT}).first;
  d->formats = QVector<QTextCharFormat>(maxCategory + 1);
  d->updateFormats(TextEditorSettings::fontSettings());
}

auto SyntaxHighlighter::formatForCategory(int category) const -> QTextCharFormat
{
  Q_D(const SyntaxHighlighter);
...
auto SyntaxHighlighter::highlightBlock(const QString &text) -> void
{
  formatSpaces(text);
}

auto SyntaxHighlighterPrivate::updateFormats(const FontSettings &fontSettings) -> void
{
  ...
  for (const auto &pair : qAsConst(formatCategories)) {
    formats[pair.first] = pair.second == C_TEXT ? QTextCharFormat() : fontSettings.toTextCharFormat(pair.second);
  }
