  ...
  if (modelIndex.isValid() && role == Qt::EditRole) {
    ...
    if (modelIndex.column() == 0) {
      ...
      if (!Snippet::isValidTrigger(s)) {
        QMessageBox::critical(Orca::Plugin::Core::ICore::dialogParent(), tr("Error"), tr("Not a valid trigger. A valid trigger can only contain letters, " "numbers, or underscores, where the first character is " "limited to letter or underscore."));
...
auto SnippetsTableModel::revertBuitInSnippet(const QModelIndex &modelIndex) -> void
{
  ...
  if (snippet.id().isEmpty()) {
    QMessageBox::critical(Orca::Plugin::Core::ICore::dialogParent(), tr("Error"), tr("Error reverting snippet."));
    return;
  }
  replaceSnippet(snippet, modelIndex);
}
...
auto SnippetsSettingsPagePrivate::apply() -> void
{
  ...
  if (m_snippetsCollectionChanged) {
    ...
    if (SnippetsCollection::instance()->synchronize(&errorString)) {
      ...
      QMessageBox::critical(Orca::Plugin::Core::ICore::dialogParent(), tr("Error While Saving Snippet Collection"), errorString);
    }
  }
}
...
auto SnippetsSettingsPagePrivate::loadSettings() -> void
{
  ...
  m_settings.fromSettings(m_settingsPrefix, Orca::Plugin::Core::ICore::settings());
...
auto SnippetsSettingsPagePrivate::writeSettings() -> void
{
  ...
  m_settings.toSettings(m_settingsPrefix, Orca::Plugin::Core::ICore::settings());
}
...
auto SnippetsSettingsPagePrivate::removeSnippet() -> void
{
  ...
  if (!modelIndex.isValid()) {
    QMessageBox::critical(Orca::Plugin::Core::ICore::dialogParent(), tr("Error"), tr("No snippet selected."));
    return;
  }
