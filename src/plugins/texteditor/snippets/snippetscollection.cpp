// SPDX-License-Identifier: GPL-3.0-only WITH Qt-GPL-exception-1.0

#include "snippetscollection.hpp"
#include "snippetprovider.hpp"
#include "reuse.hpp"

#include <core/core-interface.hpp>

#include <utils/algorithm.hpp>
#include <utils/fileutils.hpp>

#include <QLatin1String>
#include <QFile>
#include <QFileInfo>
#include <QDebug>
#include <QXmlStreamReader>

#include <iterator>
#include <algorithm>

using namespace Utils;

namespace TextEditor {
namespace Internal {

/*  TRANSLATOR TextEditor::Internal::Snippets

    Snippets are text fragments that can be inserted into an editor via the usual completion
    mechanics using a trigger text. The translated text (trigger variant) is used to
    disambiguate between snippets with the same trigger.
*/

static auto snippetComp(const Snippet &a, const Snippet &b) -> bool
{
  const auto comp = a.trigger().toLower().localeAwareCompare(b.trigger().toLower());
  if (comp < 0)
    return true;
  if (comp == 0 && a.complement().toLower().localeAwareCompare(b.complement().toLower()) < 0)
    return true;
  return false;
}

const QLatin1String kSnippet("snippet");
const QLatin1String kSnippets("snippets");
const QLatin1String kTrigger("trigger");
const QLatin1String kId("id");
const QLatin1String kComplement("complement");
const QLatin1String kGroup("group");
const QLatin1String kRemoved("removed");
const QLatin1String kModified("modified");

// Hint
SnippetsCollection::Hint::Hint(int index) : m_index(index) {}

SnippetsCollection::Hint::Hint(int index, QVector<Snippet>::iterator it) : m_index(index), m_it(it) {}

auto SnippetsCollection::Hint::index() const -> int
{
  return m_index;
}

auto SnippetsCollection::instance() -> SnippetsCollection*
{
  static SnippetsCollection collection;
  return &collection;
}

// SnippetsCollection
SnippetsCollection::SnippetsCollection() : m_userSnippetsFile(Orca::Plugin::Core::ICore::userResourcePath("snippets/snippets.xml")), m_builtInSnippetsFiles(Orca::Plugin::Core::ICore::resourcePath("snippets").dirEntries(FileFilter({"*.xml"})))
{
  connect(Orca::Plugin::Core::ICore::instance(), &Orca::Plugin::Core::ICore::coreOpened, this, &SnippetsCollection::identifyGroups);
}

SnippetsCollection::~SnippetsCollection() = default;

auto SnippetsCollection::insertSnippet(const Snippet &snippet) -> void
{
  insertSnippet(snippet, computeInsertionHint(snippet));
}

auto SnippetsCollection::insertSnippet(const Snippet &snippet, const Hint &hint) -> void
{
  const auto group = groupIndex(snippet.groupId());
  if (snippet.isBuiltIn() && snippet.isRemoved()) {
    m_snippets[group].append(snippet);
  } else {
    m_snippets[group].insert(hint.m_it, snippet);
    updateActiveSnippetsEnd(group);
  }
}

auto SnippetsCollection::computeInsertionHint(const Snippet &snippet) -> Hint
{
  const auto group = groupIndex(snippet.groupId());
  auto &snippets = m_snippets[group];
  const auto it = std::upper_bound(snippets.begin(), snippets.begin() + m_activeSnippetsCount.at(group), snippet, snippetComp);
  return Hint(static_cast<int>(std::distance(snippets.begin(), it)), it);
}

auto SnippetsCollection::replaceSnippet(int index, const Snippet &snippet) -> void
{
  replaceSnippet(index, snippet, computeReplacementHint(index, snippet));
}

auto SnippetsCollection::replaceSnippet(int index, const Snippet &snippet, const Hint &hint) -> void
{
  const auto group = groupIndex(snippet.groupId());
  auto replacement(snippet);
  if (replacement.isBuiltIn() && !replacement.isModified())
    replacement.setIsModified(true);

  if (index == hint.index()) {
    m_snippets[group][index] = replacement;
  } else {
    insertSnippet(replacement, hint);
    // Consider whether the row moved up towards the beginning or down towards the end.
    if (index < hint.index())
      m_snippets[group].removeAt(index);
    else
      m_snippets[group].removeAt(index + 1);
    updateActiveSnippetsEnd(group);
  }
}

auto SnippetsCollection::computeReplacementHint(int index, const Snippet &snippet) -> Hint
{
  const auto group = groupIndex(snippet.groupId());
  auto &snippets = m_snippets[group];
  const auto activeSnippetsEnd = snippets.begin() + m_activeSnippetsCount.at(group);
  auto it = std::lower_bound(snippets.begin(), activeSnippetsEnd, snippet, snippetComp);
  auto hintIndex = static_cast<int>(std::distance(snippets.begin(), it));
  if (index < hintIndex - 1)
    return Hint(hintIndex - 1, it);
  it = std::upper_bound(it, activeSnippetsEnd, snippet, snippetComp);
  hintIndex = static_cast<int>(std::distance(snippets.begin(), it));
  if (index > hintIndex)
    return Hint(hintIndex, it);
  // Even if the snipet is at a different index it is still inside a valid range.
  return Hint(index);
}

auto SnippetsCollection::removeSnippet(int index, const QString &groupId) -> void
{
  const auto group = groupIndex(groupId);
  auto snippet(m_snippets.at(group).at(index));
  m_snippets[group].removeAt(index);
  updateActiveSnippetsEnd(group);
  if (snippet.isBuiltIn()) {
    snippet.setIsRemoved(true);
    m_snippets[group].append(snippet);
  }
}

auto SnippetsCollection::snippet(int index, const QString &groupId) const -> const Snippet&
{
  return m_snippets.at(groupIndex(groupId)).at(index);
}

auto SnippetsCollection::setSnippetContent(int index, const QString &groupId, const QString &content) -> void
{
  auto &snippet = m_snippets[groupIndex(groupId)][index];
  snippet.setContent(content);
  if (snippet.isBuiltIn() && !snippet.isModified())
    snippet.setIsModified(true);
}

auto SnippetsCollection::totalActiveSnippets(const QString &groupId) const -> int
{
  const auto group = groupIndex(groupId);
  return m_activeSnippetsCount.at(group);
}

auto SnippetsCollection::totalSnippets(const QString &groupId) const -> int
{
  return m_snippets.at(groupIndex(groupId)).size();
}

auto SnippetsCollection::groupIds() const -> QList<QString>
{
  return m_groupIndexById.keys();
}

auto SnippetsCollection::clearSnippets() -> void
{
  for (auto group = 0; group < m_groupIndexById.size(); ++group)
    clearSnippets(group);
}

auto SnippetsCollection::clearSnippets(int groupIndex) -> void
{
  m_snippets[groupIndex].clear();
  m_activeSnippetsCount[groupIndex] = m_snippets[groupIndex].size();
}

auto SnippetsCollection::updateActiveSnippetsEnd(int groupIndex) -> void
{
  const auto index = indexOf(m_snippets[groupIndex], [](const Snippet &s) { return s.isRemoved(); });
  m_activeSnippetsCount[groupIndex] = index < 0 ? m_snippets[groupIndex].size() : index;
}

auto SnippetsCollection::restoreRemovedSnippets(const QString &groupId) -> void
{
  // The version restored contains the last modifications (if any) by the user.
  // Reverting the snippet can still bring it to the original version
  const auto group = groupIndex(groupId);
  if (m_activeSnippetsCount[group] == m_snippets[group].size()) // no removed snippets
    return;
  const auto toRestore = m_snippets[group].mid(m_activeSnippetsCount[group]);
  m_snippets[group].resize(m_activeSnippetsCount[group]);

  for (auto snippet : qAsConst(toRestore)) {
    snippet.setIsRemoved(false);
    insertSnippet(snippet);
  }
}

auto SnippetsCollection::revertedSnippet(int index, const QString &groupId) const -> Snippet
{
  const auto &candidate = snippet(index, groupId);
  Q_ASSERT(candidate.isBuiltIn());

  for (const auto &fileName : m_builtInSnippetsFiles) {
    const auto &builtIn = readXML(fileName, candidate.id());
    if (builtIn.size() == 1)
      return builtIn.at(0);
  }
  return Snippet(groupId);
}

auto SnippetsCollection::reset(const QString &groupId) -> void
{
  clearSnippets(groupIndex(groupId));

  const auto &builtInSnippets = allBuiltInSnippets();
  for (const auto &snippet : builtInSnippets)
    if (groupId == snippet.groupId())
      insertSnippet(snippet);
}

auto SnippetsCollection::reload() -> void
{
  clearSnippets();

  const auto &builtInSnippets = allBuiltInSnippets();
  QHash<QString, Snippet> activeBuiltInSnippets;
  for (const auto &snippet : builtInSnippets)
    activeBuiltInSnippets.insert(snippet.id(), snippet);

  const auto &userSnippets = readXML(m_userSnippetsFile);
  for (const auto &snippet : userSnippets) {
    if (snippet.isBuiltIn())
      // This user snippet overrides the corresponding built-in snippet.
      activeBuiltInSnippets.remove(snippet.id());
    insertSnippet(snippet);
  }

  for (const auto &snippet : qAsConst(activeBuiltInSnippets))
    insertSnippet(snippet);
}

auto SnippetsCollection::synchronize(QString *errorString) -> bool
{
  if (!m_userSnippetsFile.parentDir().ensureWritableDir()) {
    *errorString = tr("Cannot create user snippet directory %1").arg(m_userSnippetsFile.parentDir().toUserOutput());
    return false;
  }
  FileSaver saver(m_userSnippetsFile);
  if (!saver.hasError()) {
    using GroupIndexByIdConstIt = QHash<QString, int>::ConstIterator;

    QXmlStreamWriter writer(saver.file());
    writer.setAutoFormatting(true);
    writer.writeStartDocument();
    writer.writeStartElement(kSnippets);
    const auto cend = m_groupIndexById.constEnd();
    for (auto it = m_groupIndexById.constBegin(); it != cend; ++it) {
      const auto &groupId = it.key();
      const int size = m_snippets.at(groupIndex(groupId)).size();
      for (auto i = 0; i < size; ++i) {
        const auto &current = snippet(i, groupId);
        if (!current.isBuiltIn() || current.isRemoved() || current.isModified())
          writeSnippetXML(current, &writer);
      }
    }
    writer.writeEndElement();
    writer.writeEndDocument();

    saver.setResult(&writer);
  }
  if (!saver.finalize(errorString))
    return false;

  reload();
  return true;
}

auto SnippetsCollection::writeSnippetXML(const Snippet &snippet, QXmlStreamWriter *writer) const -> void
{
  writer->writeStartElement(kSnippet);
  writer->writeAttribute(kGroup, snippet.groupId());
  writer->writeAttribute(kTrigger, snippet.trigger());
  writer->writeAttribute(kId, snippet.id());
  writer->writeAttribute(kComplement, snippet.complement());
  writer->writeAttribute(kRemoved, fromBool(snippet.isRemoved()));
  writer->writeAttribute(kModified, fromBool(snippet.isModified()));
  writer->writeCharacters(snippet.content());
  writer->writeEndElement();
}

auto SnippetsCollection::readXML(const FilePath &fileName, const QString &snippetId) const -> QList<Snippet>
{
  QList<Snippet> snippets;
  QFile file(fileName.toString());
  if (file.exists() && file.open(QIODevice::ReadOnly)) {
    QXmlStreamReader xml(&file);
    if (xml.readNextStartElement()) {
      if (xml.name() == kSnippets) {
        while (xml.readNextStartElement()) {
          if (xml.name() == kSnippet) {
            const auto &atts = xml.attributes();
            const auto &id = atts.value(kId).toString();
            const auto &groupId = atts.value(kGroup).toString();
            const auto &trigger = atts.value(kTrigger).toString();
            if (!Snippet::isValidTrigger(trigger)) {
              qWarning() << fileName << "ignore snippet for invalid trigger" << trigger << "A valid trigger can only contain letters, " "numbers, or underscores, where the first character is " "limited to letter or underscore.";

              xml.skipCurrentElement();
            } else if (isGroupKnown(groupId) && (snippetId.isEmpty() || snippetId == id)) {
              Snippet snippet(groupId, id);
              snippet.setTrigger(trigger);
              snippet.setComplement(QCoreApplication::translate("TextEditor::Internal::Snippets", atts.value(kComplement).toString().toLatin1(), atts.value(kId).toString().toLatin1()));
              snippet.setIsRemoved(toBool(atts.value(kRemoved).toString()));
              snippet.setIsModified(toBool(atts.value(kModified).toString()));

              QString content;
              while (!xml.atEnd()) {
                xml.readNext();
                if (xml.isCharacters()) {
                  content += xml.text();
                } else if (xml.isEndElement()) {
                  snippet.setContent(content);
                  snippets.append(snippet);
                  break;
                }
              }

              if (!snippetId.isEmpty())
                break;
            } else {
              xml.skipCurrentElement();
            }
          } else {
            xml.skipCurrentElement();
          }
        }
      }
    }
    if (xml.hasError())
      qWarning() << fileName << xml.errorString() << xml.lineNumber() << xml.columnNumber();
    file.close();
  }

  return snippets;
}

auto SnippetsCollection::allBuiltInSnippets() const -> QList<Snippet>
{
  QList<Snippet> builtInSnippets;
  for (const auto &fileName : m_builtInSnippetsFiles)
    builtInSnippets.append(readXML(fileName));
  return builtInSnippets;
}

auto SnippetsCollection::groupIndex(const QString &groupId) const -> int
{
  return m_groupIndexById.value(groupId);
}

auto SnippetsCollection::identifyGroups() -> void
{
  for (const auto &provider : SnippetProvider::snippetProviders()) {
    const int groupIndex = m_groupIndexById.size();
    m_groupIndexById.insert(provider.groupId(), groupIndex);
    m_snippets.resize(groupIndex + 1);
    m_activeSnippetsCount.resize(groupIndex + 1);
    m_activeSnippetsCount[groupIndex] = m_snippets[groupIndex].size();
  }

  reload();
}

auto SnippetsCollection::isGroupKnown(const QString &groupId) const -> bool
{
  return m_groupIndexById.value(groupId, -1) != -1;
}

} // Internal
} // TextEditor
