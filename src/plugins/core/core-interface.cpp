...
#include "core-settings-dialog.hpp"
...
auto ICore::userResourcePath(const QString &rel) -> FilePath
{
  ...
  const QString urp = config_dir + '/' + QLatin1String(IDE_ID);
...
{
  return FilePath::fromString(settings(QSettings::SystemScope)->fileName()).parentDir() / IDE_ID / rel;
}
...
auto ICore::userPluginPath() -> QString
{
  ...
  plugin_path += '/' + QLatin1String(IDE_SETTINGSVARIANT_STR) + '/';
  plugin_path += QLatin1String(HostOsInfo::isMacHost() ? IDE_DISPLAY_NAME : IDE_ID);
  plugin_path += "/plugins/";
...
auto ICore::versionString() -> QString
{
  ...
  if (QLatin1String(IDE_VERSION_LONG) != QLatin1String(IDE_VERSION_DISPLAY))
    ide_version_description = tr(" (%1)").arg(QLatin1String(IDE_VERSION_LONG));

  return tr("%1 %2%3").arg(QLatin1String(IDE_DISPLAY_NAME), QLatin1String(IDE_VERSION_DISPLAY), ide_version_description);
}
...
auto ICore::systemInformation() -> QString
{
  ...
  #ifdef IDE_REVISION
  result += QString("From revision %1\n").arg(QString::fromLatin1(IDE_REVISION_STR).left(10));
  #endif
  return result;
}
