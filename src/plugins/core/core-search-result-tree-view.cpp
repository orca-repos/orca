...
auto SearchResultTreeView::keyPressEvent(QKeyEvent *event) -> void
{
  if ((event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) && event->modifiers() == 0 && currentIndex().isValid() && state() != EditingState) {
    const auto item = model()->data(currentIndex(), ResultItemRole).value<SearchResultItem>();
...
{
  if (model()->data(index, IsGeneratedRole).toBool())
    return;

  const auto item = model()->data(index, ResultItemRole).value<SearchResultItem>();
