...
CorePlugin::CorePlugin()
{
  ...
  qRegisterMetaType<Search::TextPosition>();
...
auto CorePlugin::initialize(const QStringList &arguments, QString *error_message) -> bool
{
  ...
  expander->registerVariable("HostOs:isWindows", tr("Is %1 running on Windows?").arg(Constants::IDE_DISPLAY_NAME), [] { return QVariant(HostOsInfo::isWindowsHost()).toString(); });
  expander->registerVariable("HostOs:isOSX", tr("Is %1 running on OS X?").arg(Constants::IDE_DISPLAY_NAME), [] { return QVariant(HostOsInfo::isMacHost()).toString(); });
  expander->registerVariable("HostOs:isLinux", tr("Is %1 running on Linux?").arg(Constants::IDE_DISPLAY_NAME), [] { return QVariant(HostOsInfo::isLinuxHost()).toString(); });
  expander->registerVariable("HostOs:isUnix", tr("Is %1 running on any unix-based platform?").arg(Constants::IDE_DISPLAY_NAME), [] { return QVariant(HostOsInfo::isAnyUnixHost()).toString(); });
  ...
  expander->registerVariable("IDE:ResourcePath", tr("The directory where %1 finds its pre-installed resources.").arg(Constants::IDE_DISPLAY_NAME), [] { return ICore::resourcePath().toString(); });
...
auto CorePlugin::checkSettings() -> void
{
  ...
  case QSettings::NoError: {
    if (const QFileInfo fi(user_settings->fileName()); fi.exists() && !fi.isWritable()) {
      const auto error_msg = tr("The settings file \"%1\" is not writable.\n" "You will not be able to store any %2 settings.").arg(QDir::toNativeSeparators(user_settings->fileName()), QLatin1String(Constants::IDE_DISPLAY_NAME));
  ...
  const auto error_msg = tr("Error reading settings file \"%1\": %2\n" "You will likely experience further problems using this instance of %3.").arg(QDir::toNativeSeparators(user_settings->fileName()), error_details, QLatin1String(Constants::IDE_DISPLAY_NAME));
...
auto CorePlugin::warnAboutCrashReporing() -> void
{
  ...
  if constexpr (HostOsInfo::isMacHost()) {
    warn_str = warn_str.arg(QLatin1String(Constants::IDE_DISPLAY_NAME), Constants::IDE_DISPLAY_NAME + tr(" > Preferences > Environment > System"));
  } else {
    warn_str = warn_str.arg(QLatin1String(Constants::IDE_DISPLAY_NAME), tr("Edit > Preferences > Environment > System"));
  }
