...
#include "core-search-result-tree-item-delegate.hpp"

#include "core-search-result-tree-item-roles.hpp"
...
{
  const auto line_number = index.data(ResultBeginLineNumberRole).toInt();
...
auto SearchResultTreeItemDelegate::getLayoutInfo(const QStyleOptionViewItem &option, const QModelIndex &index) const -> LayoutInfo
{
  ...
  // icon
  info.icon = index.data(ResultIconRole).value<QIcon>();
  if (!info.icon.isNull()) {
...
auto SearchResultTreeItemDelegate::drawText(QPainter *painter, const QStyleOptionViewItem &option, const QRect &rect, const QModelIndex &index) const -> void
{
  ...
  const auto search_term_start = index.model()->data(index, ResultBeginColumnNumberRole).toInt();
  auto search_term_length = index.model()->data(index, SearchTermLengthRole).toInt();
  ...
  const auto highlight_background = index.model()->data(index, ResultHighlightBackgroundColor).value<QColor>();
  ...
  const auto highlightForeground = index.model()->data(index, ResultHighlightForegroundColor).value<QColor>();
