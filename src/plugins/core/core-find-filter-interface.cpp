...
/*!
    \class Orca::Plugin::Core::IFindFilter
    ...
    \image core-search-filesystem.png
    ...
    search, you should use \l Orca::Plugin::Core::BaseTextFind, which already implements all
    ...
    \list
        ...
        \li Make this known to the Orca::Plugin::Core::ProgressManager, for a progress bar
    ...
    (see the Orca::Plugin::Core::SearchResultWindow class for details).
    Start your search implementation via the corresponding QtConcurrent
    functions. Add the returned QFuture object to the Orca::Plugin::Core::ProgressManager.
...
/*!
    \fn QString Orca::Plugin::Core::IFindFilter::id() const
...
/*!
    \fn QString Orca::Plugin::Core::IFindFilter::displayName() const
...
/*!
    \fn bool Orca::Plugin::Core::IFindFilter::isEnabled() const
...
/*!
    \fn bool Orca::Plugin::Core::IFindFilter::isValid() const
...
/*!
    \fn bool Orca::Plugin::Core::IFindFilter::isReplaceSupported() const
...
/*!
    \fn bool Orca::Plugin::Core::IFindFilter::showSearchTermInput() const
...
/*!
    \fn void Orca::Plugin::Core::IFindFilter::findAll(const QString &txt, Orca::Plugin::Core::FindFlags findFlags)
    ...
    (add it to Orca::Plugin::Core::ProgressManager for a progress bar) and presents the
    ...
    Orca::Plugin::Core::ProgressManager, and Orca::Plugin::Core::SearchResultWindow.

    \sa replaceAll()
    \sa Orca::Plugin::Core::ProgressManager
    \sa Orca::Plugin::Core::SearchResultWindow
*/

/*!
    \fn void Orca::Plugin::Core::IFindFilter::replaceAll(const QString &txt, Orca::Plugin::Core::FindFlags findFlags)
...
/*!
    \fn void Orca::Plugin::Core::IFindFilter::validChanged(bool valid)

    This signal is emitted when the \a valid state of this find filter changes.
*/

/*!
    \fn void Orca::Plugin::Core::IFindFilter::displayNameChanged()
...
static QList<IFindFilter*> g_find_filters;
...
IFindFilter::~IFindFilter()
{
  g_find_filters.removeOne(this);
}

/*!
    Returns a list of find filters.
*/
auto IFindFilter::allFindFilters() -> QList<IFindFilter*>
{
  return g_find_filters;
}
...
*/
auto IFindFilter::defaultShortcut() const -> QKeySequence
{
  return {};
}
...
/*!
    ...
    enabled or disabled.
    The default is Orca::Plugin::Core::FindCaseSensitively, Orca::Plugin::Core::FindRegularExpression
    and Orca::Plugin::Core::FindWholeWords.
*/
auto IFindFilter::supportedFindFlags() const -> FindFlags
{
...
*/
auto IFindFilter::pixmapForFindFlags(const FindFlags flags) -> QPixmap
{
  static const auto casesensitive_icon = FIND_CASE_INSENSITIVELY.pixmap();
  static const auto regexp_icon = FIND_REGEXP.pixmap();
  static const auto wholewords_icon = FIND_WHOLE_WORD.pixmap();
  static const auto preservecase_icon = FIND_PRESERVE_CASE.pixmap();
  const bool casesensitive = flags & FindCaseSensitively;
  const bool wholewords = flags & FindWholeWords;
  const bool regexp = flags & FindRegularExpression;
  const bool preservecase = flags & FindPreserveCase;
  auto width = 0;

  if (casesensitive)
    width += casesensitive_icon.width();

  if (wholewords)
    width += wholewords_icon.width();

  if (regexp)
    width += regexp_icon.width();

  if (preservecase)
    width += preservecase_icon.width();

  if (width == 0)
    return {};

  QPixmap pixmap(QSize(width, casesensitive_icon.height()));
  pixmap.fill(QColor(0xff, 0xff, 0xff, 0x28)); // Subtile contrast for dark themes
  const auto dpr = static_cast<int>(qApp->devicePixelRatio());
  pixmap.setDevicePixelRatio(dpr);
  QPainter painter(&pixmap);
  auto x = 0;

  if (casesensitive) {
    painter.drawPixmap(x, 0, casesensitive_icon);
    x += casesensitive_icon.width() / dpr;
  }

  if (wholewords) {
    painter.drawPixmap(x, 0, wholewords_icon);
    x += wholewords_icon.width() / dpr;
  }

  if (regexp) {
    painter.drawPixmap(x, 0, regexp_icon);
    x += regexp_icon.width() / dpr;
  }

  if (preservecase)
    painter.drawPixmap(x, 0, preservecase_icon);
...
*/
auto IFindFilter::descriptionForFindFlags(const FindFlags flags) -> QString
{
  QStringList flag_strings;

  if (flags & FindCaseSensitively)
    flag_strings.append(tr("Case sensitive"));

  if (flags & FindWholeWords)
    flag_strings.append(tr("Whole words"));

  if (flags & FindRegularExpression)
    flag_strings.append(tr("Regular expressions"));

  if (flags & FindPreserveCase)
    flag_strings.append(tr("Preserve case"));

  auto description = tr("Flags: %1");

  if (flag_strings.isEmpty())
    description = description.arg(tr("None"));
  else
    description = description.arg(flag_strings.join(tr(", ")));
