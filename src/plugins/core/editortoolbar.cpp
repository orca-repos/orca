...
EditorToolBar::EditorToolBar(QWidget *parent) : StyledBar(parent), d(new EditorToolBarPrivate(parent, this))
{
  ...
  connect(ActionManager::command(Constants::CLOSE), &Command::keySequenceChanged, this, &EditorToolBar::updateActionShortcuts);
  connect(ActionManager::command(Constants::GO_BACK), &Command::keySequenceChanged, this, &EditorToolBar::updateActionShortcuts);
  connect(ActionManager::command(Constants::GO_FORWARD), &Command::keySequenceChanged, this, &EditorToolBar::updateActionShortcuts);
...
{
  d->m_close_editor_button->setToolTip(ActionManager::command(Constants::CLOSE)->stringWithAppendedShortcut(EditorManager::tr("Close Document")));
  d->m_go_back_action->setToolTip(ActionManager::command(Constants::GO_BACK)->action()->toolTip());
  d->m_go_forward_action->setToolTip(ActionManager::command(Constants::GO_FORWARD)->action()->toolTip());
  d->m_close_split_button->setToolTip(ActionManager::command(Constants::REMOVE_CURRENT_SPLIT)->stringWithAppendedShortcut(tr("Remove Split")));
}
...
auto EditorToolBar::updateDocumentStatus(const IDocument *document) const -> void
{
  ...
  if (document->filePath().isEmpty())
    ...
    d->m_drag_handle->setIcon(FileIconProvider::icon(document->filePath()));
