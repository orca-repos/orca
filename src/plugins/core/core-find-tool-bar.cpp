...
#include "core-find-tool-bar.hpp"
...
Q_DECLARE_METATYPE(Orca::Plugin::Core::IFindFilter*)
...
FindToolBar::FindToolBar(CurrentDocumentFind *current_document_find) : m_current_document_find(current_document_find), m_find_completer(new QCompleter(this)), m_replace_completer(new QCompleter(this)), m_find_incremental_timer(this), m_find_step_timer(this)
{
  ...
  // register actions
  Context findcontext(C_FINDTOOLBAR);
  auto mfind = ActionManager::actionContainer(M_FIND);

  m_ui.advancedButton->setDefaultAction(ActionManager::command(ADVANCED_FIND)->action());

  m_go_to_current_find_action = new QAction(this);
  ActionManager::registerAction(m_go_to_current_find_action, S_RETURNTOEDITOR, findcontext);
  ...
  cmd = ActionManager::registerAction(m_find_in_document_action, FIND_IN_DOCUMENT);
  ...
  ActionManager::registerAction(local_find_action, FIND_IN_DOCUMENT, findcontext);
  ...
  cmd = ActionManager::registerAction(m_find_next_action, FIND_NEXT);
  ...
  cmd = ActionManager::registerAction(m_local_find_next_action, FIND_NEXT, findcontext);
  ...
  cmd = ActionManager::registerAction(m_find_previous_action, FIND_PREVIOUS);
  ...
  cmd = ActionManager::registerAction(m_local_find_previous_action, FIND_PREVIOUS, findcontext);
  ...
  cmd = ActionManager::registerAction(m_find_next_selected_action, FIND_NEXT_SELECTED);
  ...
  cmd = ActionManager::registerAction(m_find_previous_selected_action, FIND_PREV_SELECTED);
  ...
  cmd = ActionManager::registerAction(m_select_all_action, FIND_SELECT_ALL);
  ...
  cmd = ActionManager::registerAction(m_local_select_all_action, FIND_SELECT_ALL, findcontext);
  ...
  cmd = ActionManager::registerAction(m_replace_action, REPLACE);
  ...
  cmd = ActionManager::registerAction(m_local_replace_action, REPLACE, findcontext);
  ...
  cmd = ActionManager::registerAction(m_replace_next_action, REPLACE_NEXT);
  ...
  cmd = ActionManager::registerAction(m_local_replace_next_action, REPLACE_NEXT, findcontext);
  ...
  cmd = ActionManager::registerAction(m_replace_previous_action, REPLACE_PREVIOUS);
  mfind->addAction(cmd, G_FIND_ACTIONS);
  ...
  cmd = ActionManager::registerAction(m_local_replace_previous_action, REPLACE_PREVIOUS, findcontext);
  ...
  cmd = ActionManager::registerAction(m_replace_all_action, REPLACE_ALL);
  mfind->addAction(cmd, G_FIND_ACTIONS);
  ...
  cmd = ActionManager::registerAction(m_local_replace_all_action, REPLACE_ALL, findcontext);
  ...
  m_case_sensitive_action->setIcon(FIND_CASE_INSENSITIVELY.icon());
  m_case_sensitive_action->setCheckable(true);
  m_case_sensitive_action->setChecked(false);
  cmd = ActionManager::registerAction(m_case_sensitive_action, CASE_SENSITIVE);
  mfind->addAction(cmd, G_FIND_FLAGS);
  ...
  m_whole_word_action->setIcon(FIND_WHOLE_WORD.icon());
  m_whole_word_action->setCheckable(true);
  m_whole_word_action->setChecked(false);
  cmd = ActionManager::registerAction(m_whole_word_action, WHOLE_WORDS);
  mfind->addAction(cmd, G_FIND_FLAGS);
  ...
  m_regular_expression_action->setIcon(FIND_REGEXP.icon());
  m_regular_expression_action->setCheckable(true);
  m_regular_expression_action->setChecked(false);
  cmd = ActionManager::registerAction(m_regular_expression_action, REGULAR_EXPRESSION);
  mfind->addAction(cmd, G_FIND_FLAGS);
  ...
  m_preserve_case_action->setIcon(FIND_PRESERVE_CASE.icon());
  m_preserve_case_action->setCheckable(true);
  m_preserve_case_action->setChecked(false);
  cmd = ActionManager::registerAction(m_preserve_case_action, PRESERVE_CASE);
  mfind->addAction(cmd, G_FIND_FLAGS);
...
{
  const auto popup = new OptionsPopup(m_ui.findEdit, {CASE_SENSITIVE, WHOLE_WORDS, REGULAR_EXPRESSION, PRESERVE_CASE});
  popup->show();
}
