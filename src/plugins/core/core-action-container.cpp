...
#include "core-action-container-private.hpp"
...
Q_DECLARE_METATYPE(Orca::Plugin::Core::MenuActionContainer*)
...
{
  ActionContainerPrivate::appendGroup(G_DEFAULT_ONE);
  ActionContainerPrivate::appendGroup(G_DEFAULT_TWO);
  ActionContainerPrivate::appendGroup(G_DEFAULT_THREE);
  scheduleUpdate();
}
...
auto ActionContainerPrivate::addAction(Command *command, const Id group_id) -> void
{
  ...
  const auto actual_group_id = group_id.isValid() ? group_id : Id(G_DEFAULT_TWO);
...
{
  static const Context context(C_GLOBAL);
