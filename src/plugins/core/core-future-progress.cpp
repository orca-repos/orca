...
FutureProgressPrivate::FutureProgressPrivate(FutureProgress *q) : m_progress(new ProgressBar), m_widget(nullptr), m_widget_layout(new QHBoxLayout), m_status_bar_widget(nullptr), m_keep(FutureProgress::HideOnFinish), m_waiting_for_user_interaction(false), m_q(q), m_fade_starting(false), m_is_fading(false) {}
...
FutureProgress::FutureProgress(QWidget *parent) : QWidget(parent), d(new FutureProgressPrivate(this))
{
  ...
  connect(d->m_progress, &ProgressBar::clicked, this, &FutureProgress::cancel);
...
#include "core-future-progress.moc"
