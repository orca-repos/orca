...
{
  const Context navicontext(C_NAVIGATION_PANE);
...
auto NavigationWidget::updateToggleText() const -> void
{
  ...
  const auto tr_tool_tip = d->m_side == Side::Left ? (isShown() ? TR_HIDE_LEFT_SIDEBAR : TR_SHOW_LEFT_SIDEBAR) : isShown() ? TR_HIDE_RIGHT_SIDEBAR : TR_SHOW_RIGHT_SIDEBAR;
...
auto NavigationWidget::insertSubItem(const int position, const int factory_index) -> NavigationSubWidget*
{
  ...
  const auto nsw = new NavigationSubWidget(this, position, factory_index);
  connect(nsw, &NavigationSubWidget::splitMe, this, &NavigationWidget::splitSubWidget);
  connect(nsw, &NavigationSubWidget::closeMe, this, &NavigationWidget::closeSubWidget);
  connect(nsw, &NavigationSubWidget::factoryIndexChanged, this, &NavigationWidget::onSubWidgetFactoryIndexChanged);
...
{
  const auto original = qobject_cast<NavigationSubWidget*>(sender());
...
auto NavigationWidget::closeSubWidget() -> void
{
  if (d->m_subWidgets.count() != 1) {
    const auto sub_widget = qobject_cast<NavigationSubWidget*>(sender());
