...
public:
  ...
  FindToolBar *m_find_tool_bar = nullptr;
  FindToolWindow *m_find_dialog = nullptr;
...
auto Find::initialize() -> void
{
  ...
  d = new FindPrivate;
  d->setupMenu();
  d->m_current_document_find = new CurrentDocumentFind;
  d->m_find_tool_bar = new FindToolBar(d->m_current_document_find);

  auto *find_tool_bar_context = new IContext(m_instance);
  find_tool_bar_context->setWidget(d->m_find_tool_bar);
  find_tool_bar_context->setContext(Context(C_FINDTOOLBAR));
  ICore::addContextObject(find_tool_bar_context);

  d->m_find_dialog = new FindToolWindow;
...
auto FindPrivate::setupMenu() -> void
{
  ...
  const auto mfind = ActionManager::createMenu(M_FIND);
  medit->addMenu(mfind, G_EDIT_FIND);
  mfind->menu()->setTitle(tr("&Find/Replace"));
  mfind->appendGroup(G_FIND_CURRENTDOCUMENT);
  mfind->appendGroup(G_FIND_FILTERS);
  mfind->appendGroup(G_FIND_FLAGS);
  mfind->appendGroup(G_FIND_ACTIONS);
  mfind->addSeparator(G_FIND_FLAGS);
  mfind->addSeparator(G_FIND_ACTIONS);

  const auto mfindadvanced = ActionManager::createMenu(M_FIND_ADVANCED);
  mfindadvanced->menu()->setTitle(tr("Advanced Find"));
  mfind->addMenu(mfindadvanced, G_FIND_FILTERS);
  m_open_find_dialog = new QAction(tr("Open Advanced Find..."), this);
  m_open_find_dialog->setIconText(tr("Advanced..."));
  const auto cmd = ActionManager::registerAction(m_open_find_dialog, ADVANCED_FIND);
...
{
  const auto mfindadvanced = ActionManager::actionContainer(M_FIND_ADVANCED);
