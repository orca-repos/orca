...
{
  const auto title = QApplication::translate("Core", "Launching a file browser failed");
  const auto msg = QApplication::translate("Core", "Unable to start the file manager:\n\n%1\n\n").arg(app);

  QMessageBox mbox(QMessageBox::Warning, title, msg, QMessageBox::Close, parent);

  if (!error.isEmpty())
    mbox.setDetailedText(QApplication::translate("Core", "\"%1\" returned the following error:\n\n%2").arg(app, error));
  ...
  if (mbox.clickedButton() == settings_button)
    ICore::showOptionsDialog(SETTINGS_ID_INTERFACE, parent);
}
...
auto FileUtils::showInGraphicalShell(QWidget *parent, const FilePath &path) -> void
{
  ...
  if constexpr (HostOsInfo::isWindowsHost()) {
    ...
    if (explorer.isEmpty()) {
      QMessageBox::warning(parent, QApplication::translate("Core", "Launching Windows Explorer Failed"), QApplication::translate("Core", "Could not find explorer.exe in path to launch Windows Explorer."));
      return;
    }
...
{
  return QApplication::translate("Core", "Find in This Directory...");
}

auto FileUtils::msgFileSystemAction() -> QString
{
  return QApplication::translate("Core", "Show in File System View");
}

auto FileUtils::msgGraphicalShellAction() -> QString
{
  if constexpr (HostOsInfo::isWindowsHost())
    return QApplication::translate("Core", "Show in Explorer");
  else if constexpr (HostOsInfo::isMacHost())
    return QApplication::translate("Core", "Show in Finder");
  else
    return QApplication::translate("Core", "Show Containing Folder");
}

auto FileUtils::msgTerminalHereAction() -> QString
{
  if constexpr (HostOsInfo::isWindowsHost())
    return QApplication::translate("Core", "Open Command Prompt Here");
  else
    return QApplication::translate("Core", "Open Terminal Here");
}

auto FileUtils::msgTerminalWithAction() -> QString
{
  if constexpr (HostOsInfo::isWindowsHost())
    return QApplication::translate("Core", "Open Command Prompt With", "Opens a submenu for choosing an environment, such as \"Run Environment\"");
  else
    return QApplication::translate("Core", "Open Terminal With", "Opens a submenu for choosing an environment, such as \"Run Environment\"");
}
...
auto FileUtils::removeFiles(const FilePaths &file_paths, const bool delete_from_fs) -> void
{
  ...
  for (const auto &fp : file_paths) {
    ...
    if (!file.exists()) // could have been deleted by vc
      continue;
    if (!file.remove()) {
      MessageManager::writeDisrupting(QCoreApplication::translate("Core", "Failed to remove file \"%1\".").arg(fp.toUserOutput()));
    }
  }
}
