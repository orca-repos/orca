...
#include "core-side-bar.hpp"

#include "core-command.hpp"
#include "core-side-bar-widget.hpp"
...
auto SideBar::insertSideBarWidget(const int position, const QString &id) -> SideBarWidget*
{
  if (!d->m_widgets.isEmpty())
    d->m_widgets.at(0)->setCloseIcon(Utils::Icons::CLOSE_SPLIT_BOTTOM.icon());

  const auto item = new SideBarWidget(this, id);
  connect(item, &SideBarWidget::splitMe, this, &SideBar::splitSubWidget);
  connect(item, &SideBarWidget::closeMe, this, &SideBar::closeSubWidget);
  connect(item, &SideBarWidget::currentWidgetChanged, this, &SideBar::updateWidgets);
...
auto SideBar::removeSideBarWidget(SideBarWidget *widget) const -> void
{
...
{
  const auto original = qobject_cast<SideBarWidget*>(sender());
...
auto SideBar::closeSubWidget() -> void
{
  if (d->m_widgets.count() != 1) {
    const auto widget = qobject_cast<SideBarWidget*>(sender());
    if (!widget)
      return;
