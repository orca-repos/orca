...
ExternalToolManager::ExternalToolManager() : QObject(ICore::instance())
{
  ...
  const auto mexternaltools = ActionManager::createMenu(Id(Constants::M_TOOLS_EXTERNAL));
...
{
  // clear menu
  const auto mexternaltools = ActionManager::actionContainer(Id(Constants::M_TOOLS_EXTERNAL));
  ...
  for (auto it = tools.cbegin(), end = tools.cend(); it != end; ++it) {
    ...
    for(auto tool: it.value()) {
      ...
      if (d->m_actions.contains(tool_id)) {
        ...
        command->setAttribute(Command::ca_update_text);
      }
