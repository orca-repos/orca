...
static constexpr int kMenuButtonWidth = 16;
...
{
  if (m_icons_only) {
    return {MODEBAR_ICONSONLY_BUTTON_SIZE, MODEBAR_ICONSONLY_BUTTON_SIZE / (minimum ? 3 : 1)};
  }
...
static auto paintIcon(QPainter *painter, const QRect &rect, const QIcon &icon, const bool enabled, const bool selected) -> void
{
  ...
  QRect icon_rect(0, 0, MODEBAR_ICON_SIZE, MODEBAR_ICON_SIZE);
...
#include "core-fancy-tab-widget.moc"
