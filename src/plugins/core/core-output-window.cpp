...
#include "core-base-text-find.hpp"
...
public:
  ...
  int max_char_count = DEFAULT_MAX_CHAR_COUNT;
...
OutputWindow::OutputWindow(const Context& context, const QString &settings_key, QWidget *parent) : QPlainTextEdit(parent), d(new OutputWindowPrivate(document()))
{
  ...
  ActionManager::registerAction(undo_action, UNDO, context);
  ActionManager::registerAction(redo_action, REDO, context);
  ActionManager::registerAction(cut_action, CUT, context);
  ActionManager::registerAction(copy_action, COPY, context);
  ActionManager::registerAction(paste_action, PASTE, context);
  ActionManager::registerAction(select_all_action, SELECTALL, context);
