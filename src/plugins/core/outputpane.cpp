...
OutputPanePlaceHolder::~OutputPanePlaceHolder()
{
  if (OutputPanePlaceHolderPrivate::m_current == this) {
    if (const auto om = Internal::OutputPaneManager::instance()) {
...
auto OutputPanePlaceHolder::currentModeChanged(Id mode) -> void
{
  if (OutputPanePlaceHolderPrivate::m_current == this) {
    OutputPanePlaceHolderPrivate::m_current = nullptr;
    if (d->m_initialized)
      Internal::OutputPaneManager::setOutputPaneHeightSetting(d->m_non_maximized_size);
    const auto om = Internal::OutputPaneManager::instance();
    om->hide();
    om->setParent(nullptr);
  ...
  if (d->m_mode == mode) {
    if (OutputPanePlaceHolderPrivate::m_current && OutputPanePlaceHolderPrivate::m_current->d->m_initialized)
      Internal::OutputPaneManager::setOutputPaneHeightSetting(OutputPanePlaceHolderPrivate::m_current->d->m_non_maximized_size);
    OutputPanePlaceHolderPrivate::m_current = this;
    const auto om = Internal::OutputPaneManager::instance();
    layout()->addWidget(om);
    om->show();
    om->updateStatusButtons(isVisible());
    Internal::OutputPaneManager::updateMaximizeButton(d->m_is_maximized);
  }
}
