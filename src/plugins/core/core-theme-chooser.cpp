...
{
  return Theme::systemUsesDarkMode() ? QString(DEFAULT_DARK_THEME) : QString(DEFAULT_THEME);
}
...
auto ThemeChooser::apply() const -> void
{
  ...
  if (const auto current_theme_id = ThemeEntry::themeSetting().toString(); current_theme_id != theme_id) {
    ...
    settings->setValueWithDefault(SETTINGS_THEME, theme_id, defaultThemeId());
...
auto ThemeEntry::availableThemes() -> QList<ThemeEntry>
{
  ...
  if (const auto default_index = indexOf(themes, equal(&ThemeEntry::id, Id(DEFAULT_THEME))); default_index > 0) {
...
{
  const auto setting = Id::fromSetting(ICore::settings()->value(SETTINGS_THEME, defaultThemeId()));
