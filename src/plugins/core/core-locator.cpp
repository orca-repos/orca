...
#include "core-execute-filter.hpp"
#include "core-external-tools-filter.hpp"
#include "core-file-system-filter.hpp"
#include "core-interface.hpp"
#include "core-javascript-filter.hpp"
#include "core-locator-constants.hpp"
#include "core-locator-filters-filter.hpp"
...
#include "core-open-documents-filter.hpp"
...
auto Locator::initialize() -> void
{
  ...
  const auto cmd = ActionManager::registerAction(action, locate);
...
auto Locator::extensionsInitialized() -> void
{
  ...
  const auto open_command = ActionManager::command(OPEN);
  const auto locate_command = ActionManager::command(locate);
...
auto Locator::loadSettings() -> void
{
  ...
  auto count = 0;
  const Id directory_base_id(custom_directory_filter_baseid);
  const Id url_base_id(custom_url_filter_baseid);
...
auto Locator::saveSettings() const -> void
{
  ...
  for (const auto filter : m_custom_filters) {
    auto prefix = filter->id().name().startsWith(custom_directory_filter_baseid) ? k_directory_filter_prefix : k_url_filter_prefix;
    const auto state = filter->saveState();
...
auto Locator::refresh(QList<ILocatorFilter*> filters) -> void
{
  ...
  ProgressManager::addTask(m_refresh_task, tr("Updating Locator Caches"), task_index);
