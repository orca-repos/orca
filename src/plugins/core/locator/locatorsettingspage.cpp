...
public:
  LocatorSettingsWidget()
  {
    ...
    add_menu->addAction(tr("Files in Directories"), this, [this] {
      addCustomFilter(new DirectoryFilter(Id(Constants::custom_directory_filter_baseid).withSuffix(m_custom_filters.size() + 1)));
    });

    add_menu->addAction(tr("URL Template"), this, [this] {
      const auto filter = new UrlLocatorFilter(Id(Constants::custom_url_filter_baseid).withSuffix(m_custom_filters.size() + 1));
