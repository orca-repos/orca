...
auto EditorType::defaultEditorTypes(const MimeType &mime_type) -> editor_type_list
{
  ...
  mimeTypeFactoryLookup(mime_type, all_editor_factories, &result);
  mimeTypeFactoryLookup(mime_type, all_external_editors, &result);
  return result;
}
...
auto EditorType::preferredEditorTypes(const FilePath &file_path) -> editor_type_list
{
  ...
  if (const auto user_preferred = userPreferredEditorTypes().value(mime_type)) {
...
auto IEditorFactory::preferredEditorFactories(const FilePath &file_path) -> editor_factory_list
{
  ...
  // user preferred factory to front
  if (const auto user_preferred = userPreferredEditorTypes().value(mime_type); user_preferred && user_preferred->asEditorFactory())
...
/*!
    \internal
*/
auto userPreferredEditorTypes() -> QHash<MimeType, EditorType*>
{
...
/*!
    \internal
*/
auto setUserPreferredEditorTypes(const QHash<MimeType, EditorType*> &factories) -> void
{
