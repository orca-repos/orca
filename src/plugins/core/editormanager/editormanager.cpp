...
auto EditorManagerPrivate::init() -> void
{
  ...
  const Context edit_manager_context(Constants::C_EDITORMANAGER);
  const Context edit_design_context(Constants::C_EDITORMANAGER, Constants::C_DESIGN_MODE);   // combined context for edit & design modes
  ...
  // Save Action
  ActionManager::registerAction(m_save_action, Constants::SAVE, edit_manager_context);
  connect(m_save_action, &QAction::triggered, m_instance, [] { EditorManager::saveDocument(); });

  // Save As Action
  ActionManager::registerAction(m_save_as_action, Constants::SAVEAS, edit_manager_context);
  ...
  mwindow->addSeparator(edit_manager_context, Constants::G_WINDOW_SPLIT);
  mwindow->addSeparator(edit_manager_context, Constants::G_WINDOW_NAVIGATE);
  ...
  if constexpr (HostOsInfo::isWindowsHost()) {
    ...
    cmd = ActionManager::registerAction(action, Constants::CLOSE_ALTERNATIVE, edit_manager_context);
  ...
  // Close All Action
  cmd = ActionManager::registerAction(m_close_all_editors_action, Constants::CLOSEALL, edit_manager_context, true);
  ...
  cmd = ActionManager::registerAction(m_close_all_editors_except_visible_action, Constants::CLOSEALLEXCEPTVISIBLE, edit_manager_context, true);
  mfile->addAction(cmd, Constants::G_FILE_CLOSE);
  ...
  ActionManager::registerAction(m_open_graphical_shell_action, Constants::SHOWINGRAPHICALSHELL, edit_manager_context);
  ...
  ActionManager::registerAction(m_show_in_file_system_view_action, Constants::SHOWINFILESYSTEMVIEW, edit_manager_context);
  ...
  cmd = ActionManager::registerAction(m_goto_previous_doc_history_action, Constants::GOTOPREVINHISTORY, edit_design_context);
  ...
  cmd = ActionManager::registerAction(m_goto_next_doc_history_action, Constants::GOTONEXTINHISTORY, edit_design_context);
  ...
  cmd = ActionManager::registerAction(m_go_back_action, Constants::GO_BACK, edit_design_context);
  ...
  cmd = ActionManager::registerAction(m_go_forward_action, Constants::GO_FORWARD, edit_design_context);
  ...
  // Go to last edit
  cmd = ActionManager::registerAction(m_goto_last_edit_action, Constants::GOTOLASTEDIT, edit_design_context);
  mwindow->addAction(cmd, Constants::G_WINDOW_NAVIGATE);
  ...
  cmd = ActionManager::registerAction(m_split_action, Constants::SPLIT, edit_manager_context);
  if constexpr (use_mac_shortcuts)
  ...
  cmd = ActionManager::registerAction(m_split_side_by_side_action, Constants::SPLIT_SIDE_BY_SIDE, edit_manager_context);
  if constexpr (use_mac_shortcuts)
  ...
  cmd = ActionManager::registerAction(m_split_new_window_action, Constants::SPLIT_NEW_WINDOW, edit_manager_context);
  if constexpr (use_mac_shortcuts)
  ...
  cmd = ActionManager::registerAction(m_remove_current_split_action, Constants::REMOVE_CURRENT_SPLIT, edit_manager_context);
  if constexpr (use_mac_shortcuts)
  ...
  cmd = ActionManager::registerAction(m_remove_all_splits_action, Constants::REMOVE_ALL_SPLITS, edit_manager_context);
  if constexpr (use_mac_shortcuts)
  ...
  cmd = ActionManager::registerAction(m_goto_previous_split_action, Constants::GOTO_PREV_SPLIT, edit_manager_context);
  if constexpr (use_mac_shortcuts)
  ...
  cmd = ActionManager::registerAction(m_goto_next_split_action, Constants::GOTO_NEXT_SPLIT, edit_manager_context);
  if constexpr (use_mac_shortcuts)
  ...
  const auto advanced_menu = ActionManager::createMenu(Constants::M_EDIT_ADVANCED);
  medit->addMenu(advanced_menu, Constants::G_EDIT_ADVANCED);
  advanced_menu->menu()->setTitle(tr("Ad&vanced"));
  advanced_menu->appendGroup(Constants::G_EDIT_FORMAT);
  advanced_menu->appendGroup(Constants::G_EDIT_TEXT);
  advanced_menu->appendGroup(Constants::G_EDIT_COLLAPSING);
  advanced_menu->appendGroup(Constants::G_EDIT_BLOCKS);
  advanced_menu->appendGroup(Constants::G_EDIT_FONT);
  advanced_menu->appendGroup(Constants::G_EDIT_EDITOR);

  // Advanced menu separators
  advanced_menu->addSeparator(edit_manager_context, Constants::G_EDIT_TEXT);
  advanced_menu->addSeparator(edit_manager_context, Constants::G_EDIT_COLLAPSING);
  advanced_menu->addSeparator(edit_manager_context, Constants::G_EDIT_BLOCKS);
  advanced_menu->addSeparator(edit_manager_context, Constants::G_EDIT_FONT);
  advanced_menu->addSeparator(edit_manager_context, Constants::G_EDIT_EDITOR);
...
auto EditorManagerPrivate::updateWindowTitleForDocument(const IDocument *document, const QWidget *window) -> void
{
  ...
  window_title.append(Constants::IDE_DISPLAY_NAME);
...
auto EditorManager::addSaveAndCloseEditorActions(QMenu *context_menu, DocumentModel::Entry *entry, IEditor *editor) -> void
{
  ...
  assignAction(d->m_save_current_editor_context_action, ActionManager::command(Constants::SAVE)->action());
  assignAction(d->m_save_as_current_editor_context_action, ActionManager::command(Constants::SAVEAS)->action());
  assignAction(d->m_revert_to_saved_current_editor_context_action, ActionManager::command(Constants::REVERTTOSAVED)->action());
  ...
  context_menu->addAction(ActionManager::command(Constants::SAVEALL)->action());
...
auto EditorManager::defaultTextCodec() -> QTextCodec*
{
  ...
  const auto codec_name = settings->value(Constants::SETTINGS_DEFAULTTEXTENCODING).toByteArray();
...
{
  const QSettings *settings = ICore::settings();
  const auto default_line_terminator = settings->value(Constants::SETTINGS_DEFAULT_LINE_TERMINATOR, TextFileFormat::LineTerminationMode::NativeLineTerminator).toInt();
