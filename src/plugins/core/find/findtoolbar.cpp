...
FindToolBar::FindToolBar(CurrentDocumentFind *current_document_find) : m_current_document_find(current_document_find), m_find_completer(new QCompleter(this)), m_replace_completer(new QCompleter(this)), m_find_incremental_timer(this), m_find_step_timer(this)
{
  ...
  cmd = ActionManager::registerAction(m_regular_expression_action, Constants::REGULAR_EXPRESSION);
  mfind->addAction(cmd, Constants::G_FIND_FLAGS);
...
{
  const auto popup = new OptionsPopup(m_ui.findEdit, {Constants::CASE_SENSITIVE, Constants::WHOLE_WORDS, Constants::REGULAR_EXPRESSION, Constants::PRESERVE_CASE});
  popup->show();
}
