...
#include "core-plugin-install-wizard.hpp"
  ...
  auto checkContents(QFutureInterface<ArchiveIssue> &fi) const -> void
  {
    ...
    while (it.hasNext()) {
      if (fi.isCanceled())
        return;
      it.next();
      if (const auto spec = ExtensionSystem::PluginSpec::read(it.filePath())) {
        // Is a Orca plugin. Let's see if we find a Core dependency and check the
        // version
        const auto dependencies = spec->dependencies();
        if (const auto found = std::find_if(dependencies.constBegin(), dependencies.constEnd(), [coreplugin](const ExtensionSystem::PluginDependency &d) {
          return d.name == coreplugin->name();
        }); found != dependencies.constEnd()) {
          if (!coreplugin->provides(found->name, found->version)) {
            fi.reportResult({PluginInstallWizard::tr("Plugin requires an incompatible version of %1 (%2).").arg(IDE_DISPLAY_NAME).arg(found->version), InfoLabel::Error});
            return;
          }
        }
    ...
    fi.reportResult({PluginInstallWizard::tr("Did not find %1 plugin.").arg(IDE_DISPLAY_NAME), InfoLabel::Error});
  }
...
public:
  InstallLocationPage(Data *data, QWidget *parent) : WizardPage(parent), m_data(data)
  {
    ...
    const auto local_label = new QLabel(PluginInstallWizard::tr("The plugin will be available to all compatible %1 " "installations, but only for the current user.").arg(IDE_DISPLAY_NAME));
    ...
    const auto app_install = new QRadioButton(PluginInstallWizard::tr("%1 installation").arg(IDE_DISPLAY_NAME));
    app_install->setChecked(m_data->install_into_application);

    const auto app_label = new QLabel(PluginInstallWizard::tr("The plugin will be available only to this %1 " "installation, but for all users that can access it.").arg(IDE_DISPLAY_NAME));
