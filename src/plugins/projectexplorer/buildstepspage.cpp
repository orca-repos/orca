  ...
  for (auto i = 0; i < m_buildStepsData.count(); ++i) {
    ...
    connect(s->toolWidget, &ToolWidget::removeClicked, this, [this, i] {
      if (!m_buildStepList->removeStep(i)) {
        QMessageBox::warning(Orca::Plugin::Core::ICore::dialogParent(), tr("Removing Step failed"), tr("Cannot remove build step while building"), QMessageBox::Ok, QMessageBox::Ok);
      }
    });
