...
static auto createCenteredIcon(const QIcon &icon, const QIcon &overlay) -> QIcon
{
  ...
  auto pixmap = icon.pixmap(Core::Constants::MODEBAR_ICON_SIZE); // already takes app devicePixelRatio into account
  auto pixmapDevicePixelRatio = pixmap.devicePixelRatio();
  painter.drawPixmap((Core::Constants::MODEBAR_ICON_SIZE - pixmap.width() / pixmapDevicePixelRatio) / 2, (Core::Constants::MODEBAR_ICON_SIZE - pixmap.height() / pixmapDevicePixelRatio) / 2, pixmap);
  if (!overlay.isNull()) {
...
auto MiniProjectTargetSelector::doLayout(bool keepSize) -> void
{
  ...
  static auto statusBar = Core::ICore::statusBar();
