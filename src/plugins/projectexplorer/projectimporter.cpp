...
auto ProjectImporter::import(const Utils::FilePath &importPath, bool silent) -> const QList<BuildInfo>
{
  ...
  const auto handleFailure = [this, importPath, silent] {
    if (silent)
      return;
    QMessageBox::critical(Orca::Plugin::Core::ICore::dialogParent(), tr("No Build Found"), tr("No build found in %1 matching project %2.").arg(importPath.toUserOutput(), projectFilePath().toUserOutput()));
  };
  ...
  if (!warningMessage.isEmpty()) {
    ...
    if (silent)
      return result;
    QMessageBox dialog(Orca::Plugin::Core::ICore::dialogParent());
