...
ProjectTree::ProjectTree(QObject *parent) : QObject(parent)
{
  ...
  connect(Orca::Plugin::Core::EditorManager::instance(), &Orca::Plugin::Core::EditorManager::currentEditorChanged, this, &ProjectTree::update);
...
{
  if (const auto document = Orca::Plugin::Core::EditorManager::currentDocument()) {
...
auto ProjectTree::setCurrent(Node *node, Project *project) -> void
{
  ...
  if (const auto document = Orca::Plugin::Core::EditorManager::currentDocument()) {
    if (node) {
      disconnect(document, &Orca::Plugin::Core::IDocument::changed, this, &ProjectTree::updateExternalFileWarning);
      document->infoBar()->removeInfo(EXTERNAL_FILE_WARNING);
    } else {
      connect(document, &Orca::Plugin::Core::IDocument::changed, this, &ProjectTree::updateExternalFileWarning, Qt::UniqueConnection);
    }
  }
...
{
  if (m_currentProject) {
    Orca::Plugin::Core::DocumentManager::setDefaultLocationForNewFiles(m_currentProject->projectDirectory());
  } else if (const auto project = SessionManager::startupProject()) {
    Orca::Plugin::Core::DocumentManager::setDefaultLocationForNewFiles(project->projectDirectory());
    updateFromNode(nullptr); // Make startup project current if there is no other current
  } else {
    Orca::Plugin::Core::DocumentManager::setDefaultLocationForNewFiles({});
  }
  update();
}
...
{
  Orca::Plugin::Core::Context oldContext;
  oldContext.add(m_lastProjectContext);

  Orca::Plugin::Core::Context newContext;
  if (m_currentProject) {
    ...
    m_lastProjectContext = Orca::Plugin::Core::Context();
  }

  Orca::Plugin::Core::ICore::updateAdditionalContexts(oldContext, newContext);
}
...
{
  const auto document = qobject_cast<Orca::Plugin::Core::IDocument*>(sender());
  ...
  for (const auto project : projects) {
    ...
    QString topLevel;
    if (Orca::Plugin::Core::VcsManager::findVersionControlForDirectory(projectDir, &topLevel) && fileName.isChildOf(FilePath::fromString(topLevel))) {
      return;
    }
  }
...
auto ProjectTree::showContextMenu(ProjectTreeWidget *focus, const QPoint &globalPos, Node *node) -> void
{
  ...
  if (!node) {
    contextMenu = Orca::Plugin::Core::ActionManager::actionContainer(Constants::M_SESSIONCONTEXT)->menu();
  } else if (node->isProjectNodeType()) {
    if ((node->parentFolderNode() && node->parentFolderNode()->asContainerNode()) || node->asContainerNode())
      contextMenu = Orca::Plugin::Core::ActionManager::actionContainer(Constants::M_PROJECTCONTEXT)->menu();
    else
      contextMenu = Orca::Plugin::Core::ActionManager::actionContainer(Constants::M_SUBPROJECTCONTEXT)->menu();
  } else if (node->isVirtualFolderType() || node->isFolderNodeType()) {
    contextMenu = Orca::Plugin::Core::ActionManager::actionContainer(Constants::M_FOLDERCONTEXT)->menu();
  } else if (node->asFileNode()) {
    contextMenu = Orca::Plugin::Core::ActionManager::actionContainer(Constants::M_FILECONTEXT)->menu();
  }
...
{
  Orca::Plugin::Core::ModeManager::activateMode(Orca::Plugin::Core::MODE_EDIT);

  // Shows and focusses a project tree
  const auto widget = Orca::Plugin::Core::NavigationWidget::activateSubWidget(Constants::PROJECTTREE_ID, Orca::Plugin::Core::Side::Left);
