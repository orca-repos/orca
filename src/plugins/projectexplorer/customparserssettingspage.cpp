...
class CustomParsersSettingsWidget final : public Orca::Plugin::Core::IOptionsPageWidget {
...
public:
  CustomParsersSettingsWidget()
  {
    ...
    const auto hintLabel = new QLabel(tr("Custom output parsers defined here can be enabled individually " "in the project's build or run settings."));
    ...
    connect(removeButton, &QPushButton::clicked, [this] {
      const auto sel = m_parserListView.selectedItems();
    ...
    connect(editButton, &QPushButton::clicked, [this] {
      const auto sel = m_parserListView.selectedItems();
      QTC_ASSERT(sel.size() == 1, return);
      auto &s = m_customParsers[m_parserListView.row(sel.first())];
      CustomParserConfigDialog dlg(this);
      dlg.setSettings(s);
      if (dlg.exec() != QDialog::Accepted)
        return;
    ...
    const auto updateButtons = [this, removeButton, editButton] {
      const auto enable = !m_parserListView.selectedItems().isEmpty();
      removeButton->setEnabled(enable);
      editButton->setEnabled(enable);
    };
    updateButtons();
    connect(m_parserListView.selectionModel(), &QItemSelectionModel::selectionChanged, updateButtons);
  }

private:
  auto apply() -> void override { ProjectExplorerPlugin::setCustomParsers(m_customParsers); }

  auto resetListView() -> void
  {
    m_parserListView.clear();
    Utils::sort(m_customParsers, [](const CustomParserSettings &s1, const CustomParserSettings &s2) {
      return s1.displayName < s2.displayName;
    });
    for (const auto &s : qAsConst(m_customParsers)) {
