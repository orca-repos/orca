...
auto MsvcToolChain::makeCommand(const Environment &environment) const -> FilePath
{
  ...
  FilePath command;
  if (useJom) {
    tmp = environment.searchInPath(jom, {Orca::Plugin::Core::ICore::libexecPath(), Orca::Plugin::Core::ICore::libexecPath("jom")});
    if (!tmp.isEmpty())
      command = tmp;
  }
...
auto ClangClToolChainFactory::autoDetect(const ToolchainDetector &detector) const -> Toolchains
{
  ...
  auto qtCreatorsClang = Orca::Plugin::Core::ICore::clangExecutable(CLANG_BINDIR);
