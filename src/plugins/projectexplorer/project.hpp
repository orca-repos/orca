...
public:
  ...
  auto projectContext() const -> Orca::Plugin::Core::Context;
  auto projectLanguages() const -> Orca::Plugin::Core::Context;
  ...
  using DocGenerator = std::function<std::unique_ptr<Orca::Plugin::Core::IDocument>(const Utils::FilePath &)>;
  using DocUpdater = std::function<void(Orca::Plugin::Core::IDocument *)>;
  ...
  auto modifiedDocuments() const -> QList<Orca::Plugin::Core::IDocument*>;
...
protected:
  ...
  auto setProjectLanguages(Orca::Plugin::Core::Context language) -> void;
