...
auto SelectableFilesModel::data(const QModelIndex &index, int role) const -> QVariant
{
  ...
  if (role == Qt::DecorationRole) {
    if (t->icon.isNull())
      t->icon = Orca::Plugin::Core::icon(t->fullPath);
    return t->icon;
  }
  return QVariant();
}
...
{
  const auto selectFilter = Orca::Plugin::Core::ICore::settings()->value("GenericProject/ShowFileFilter", QLatin1String(SELECT_FILE_FILTER_DEFAULT)).toString();
  const auto hideFilter = Orca::Plugin::Core::ICore::settings()->value("GenericProject/FileFilter", QLatin1String(HIDE_FILE_FILTER_DEFAULT)).toString();
