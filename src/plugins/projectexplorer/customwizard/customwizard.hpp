...
protected:
  ...
  auto generateWizardFiles(QString *errorMessage) const -> Core::GeneratedFiles;
  // Create replacement map as static base fields + QWizard fields
  auto replacementMap(const QWizard *w) const -> FieldReplacementMap;
  auto writeFiles(const Core::GeneratedFiles &files, QString *errorMessage) const -> bool override;
...
public:
  ...
  static auto postGenerateOpen(const Core::GeneratedFiles &l, QString *errorMessage = nullptr) -> bool;
