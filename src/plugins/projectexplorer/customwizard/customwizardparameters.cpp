...
static constexpr char customWizardElementC[]                        = "wizard";
static constexpr char iconElementC[]                                = "icon";
static constexpr char descriptionElementC[]                         = "description";
static constexpr char displayNameElementC[]                         = "displayname";
static constexpr char wizardEnabledAttributeC[]                     = "enabled";
static constexpr char idAttributeC[]                                = "id";
static constexpr char kindAttributeC[]                              = "kind";
static constexpr char klassAttributeC[]                             = "class";
static constexpr char firstPageAttributeC[]                         = "firstpage";
static constexpr char langAttributeC[]                              = "xml:lang";
static constexpr char categoryAttributeC[]                          = "category";
static constexpr char displayCategoryElementC[]                     = "displaycategory";
static constexpr char featuresRequiredC[]                           = "featuresRequired";
static constexpr char platformIndependentC[]                        = "platformIndependent";
static constexpr char fieldPageTitleElementC[]                      = "fieldpagetitle";
static constexpr char fieldsElementC[]                              = "fields";
static constexpr char fieldElementC[]                               = "field";
static constexpr char comboEntriesElementC[]                        = "comboentries";
static constexpr char comboEntryElementC[]                          = "comboentry";
static constexpr char comboEntryTextElementC[]                      = "comboentrytext";
static constexpr char generatorScriptElementC[]                     = "generatorscript";
static constexpr char generatorScriptBinaryAttributeC[]             = "binary";
static constexpr char generatorScriptWorkingDirectoryAttributeC[]   = "workingdirectory";
static constexpr char generatorScriptArgumentElementC[]             = "argument";
static constexpr char generatorScriptArgumentValueAttributeC[]      = "value";
static constexpr char generatorScriptArgumentOmitEmptyAttributeC[]  = "omit-empty";
static constexpr char generatorScriptArgumentWriteFileAttributeC[]  = "write-file";
static constexpr char fieldDescriptionElementC[]                    = "fielddescription";
static constexpr char fieldNameAttributeC[]                         = "name";
static constexpr char fieldMandatoryAttributeC[]                    = "mandatory";
static constexpr char fieldControlElementC[]                        = "fieldcontrol";
static constexpr char filesElementC[]                               = "files";
static constexpr char fileElementC[]                                = "file";
static constexpr char fileSourceAttributeC[]                        = "source";
static constexpr char fileTargetAttributeC[]                        = "target";
static constexpr char fileBinaryAttributeC[]                        = "binary";
static constexpr char rulesElementC[]                               = "validationrules";
static constexpr char ruleElementC[]                                = "validationrule";
static constexpr char ruleConditionAttributeC[]                     = "condition";
static constexpr char ruleMessageElementC[]                         = "message";
...
constexpr char customWizardFileOpenEditorAttributeC[] = "openeditor";
constexpr char customWizardFileOpenProjectAttributeC[] = "openproject";

CustomWizardField::CustomWizardField() : mandatory(false) {}

auto CustomWizardField::clear() -> void
{
  mandatory = false;
  name.clear();
  description.clear();
...
auto CustomWizardField::comboEntryValueKey(int i) -> QString
{
  return QLatin1String("comboValue") + QString::number(i);
}

auto CustomWizardField::comboEntryTextKey(int i) -> QString
{
  return QLatin1String("comboText") + QString::number(i);
}

CustomWizardFile::CustomWizardFile() : openEditor(false), openProject(false), binary(false) {}
...
auto CustomWizardValidationRule::validateRules(const QList<CustomWizardValidationRule> &rules, const QMap<QString, QString> &replacementMap, QString *errorMessage) -> bool
{
  errorMessage->clear();
  if (rules.isEmpty())
    return true;
  QJSEngine engine;
  foreach(const CustomWizardValidationRule &rule, rules) if (!rule.validate(engine, replacementMap)) {
...
auto CustomWizardValidationRule::validate(QJSEngine &engine, const QMap<QString, QString> &replacementMap) const -> bool
{
  ...
  if (!TemplateEngine::evaluateBooleanJavaScriptExpression(engine, cond, &valid, &errorMessage)) {
    qWarning("Error in custom wizard validation expression '%s': %s", qPrintable(cond), qPrintable(errorMessage));
    return false;
  }
  return valid;
}

auto CustomWizardParameters::clear() -> void
{
  directory.clear();
  files.clear();
  fields.clear();
...
static auto wizardIcon(const QString &configFileFullPath, const QString &xmlIconFileName) -> QIcon
{
  ...
  const QFileInfo absFi(QFileInfo(configFileFullPath).absolutePath() + QLatin1Char('/') + xmlIconFileName);
  if (absFi.isFile())
...
static auto skipOverElementText(QXmlStreamReader &reader) -> bool
{
  auto next = QXmlStreamReader::EndElement;
  do {
    next = reader.readNext();
  } while (next == QXmlStreamReader::Characters || next == QXmlStreamReader::EntityReference || next == QXmlStreamReader::ProcessingInstruction || next == QXmlStreamReader::Comment);
...
static inline auto assignLanguageElementText(QXmlStreamReader &reader, const QString &desiredLanguage, QString *target) -> bool
{
...
static auto parseCustomProjectElement(QXmlStreamReader &reader, const QString &configFileFullPath, const QString &language, CustomWizardParameters *p) -> bool
{
  const auto elementName = reader.name();
  if (elementName == QLatin1String(iconElementC)) {
    const auto path = reader.readElementText();
    const auto icon = wizardIcon(configFileFullPath, path);
    if (icon.availableSizes().isEmpty()) {
      qWarning("Invalid icon path '%s' encountered in custom project template %s.", qPrintable(path), qPrintable(configFileFullPath));
    } else {
      p->icon = icon;
    }
    return true;
...
static inline auto attributesToStringMap(const QXmlStreamAttributes &attributes) -> QMap<QString, QString>
{
...
static auto nextOpeningState(ParseState in, QStringView name) -> ParseState
{
  switch (in) {
  case ParseBeginning:
...
static auto nextClosingState(ParseState in, QStringView name) -> ParseState
{
  switch (in) {
  case ParseBeginning:
    break;
...
static inline auto kindAttribute(const QXmlStreamReader &r) -> IWizardFactory::WizardKind
{
...
static inline auto readRequiredFeatures(const QXmlStreamReader &reader) -> QSet<Id>
{
  const auto value = reader.attributes().value(QLatin1String(featuresRequiredC)).toString();
  auto stringList = value.split(QLatin1Char(','), Qt::SkipEmptyParts);
  QSet<Id> features;
...
static inline auto wizardFlags(const QXmlStreamReader &reader) -> IWizardFactory::WizardFlags
{
...
static inline auto msgError(const QXmlStreamReader &reader, const QString &fileName, const QString &what) -> QString
{
  return QString::fromLatin1("Error in %1 at line %2, column %3: %4").arg(fileName).arg(reader.lineNumber()).arg(reader.columnNumber()).arg(what);
}

static inline auto booleanAttributeValue(const QXmlStreamReader &r, const char *nameC, bool defaultValue) -> bool
{
...
static inline auto integerAttributeValue(const QXmlStreamReader &r, const char *name, int defaultValue) -> int
{
  const auto sValue = r.attributes().value(QLatin1String(name));
  if (sValue.isEmpty())
    return defaultValue;
  bool ok;
  const auto value = sValue.toString().toInt(&ok);
  if (!ok) {
    qWarning("Invalid integer value specification '%s' for attribute '%s'.", qPrintable(sValue.toString()), name);
    return defaultValue;
  }
  return value;
}

static inline auto attributeValue(const QXmlStreamReader &r, const char *name) -> QString
{
  return r.attributes().value(QLatin1String(name)).toString();
}

// Return locale language attribute "de_UTF8" -> "de", empty string for "C"
static inline auto languageSetting() -> QString
{
  auto name = ICore::userInterfaceLanguage();
...
GeneratorScriptArgument::GeneratorScriptArgument(const QString &v) : value(v), flags(0) {}

// Main parsing routine
auto CustomWizardParameters::parse(QIODevice &device, const QString &configFileFullPath, QString *errorMessage) -> ParseResult
{
  auto comboEntryCount = 0;
  QXmlStreamReader reader(&device);
  auto token = QXmlStreamReader::EndDocument;
  auto state = ParseBeginning;
  clear();
  kind = IWizardFactory::ProjectWizard;
  const auto language = languageSetting();
  ...
  do {
    ...
    case QXmlStreamReader::StartElement:
      do {
        ...
        switch (state) {
        case ParseError:
          *errorMessage = msgError(reader, configFileFullPath, QString::fromLatin1("Unexpected start element %1").arg(reader.name().toString()));
          return ParseFailed;
        case ParseWithinWizard:
          if (!booleanAttributeValue(reader, wizardEnabledAttributeC, true))
            return ParseDisabled;
          {
            const auto idString = attributeValue(reader, idAttributeC);
            if (!idString.isEmpty())
        ...
        case ParseWithinComboEntry: // Combo entry with 'value' attribute
          field.controlAttributes.insert(CustomWizardField::comboEntryValueKey(comboEntryCount), attributeValue(reader, "value"));
          break;
        case ParseWithinComboEntryText: {
          ...
          if (assignLanguageElementText(reader, language, &text))
            field.controlAttributes.insert(CustomWizardField::comboEntryTextKey(comboEntryCount), text);
        ...
        break;
        case ParseWithinValidationRuleMessage: QTC_ASSERT(!rules.isEmpty(), return ParseFailed);
        // This reads away the end tag, set state here.
    ...
    case QXmlStreamReader::EndElement:
      ...
      switch (state) {
      case ParseError:
        *errorMessage = msgError(reader, configFileFullPath, QString::fromLatin1("Unexpected end element %1").arg(reader.name().toString()));
        return ParseFailed;
...
auto CustomWizardParameters::parse(const QString &configFileFullPath, QString *errorMessage) -> ParseResult
{
...
static inline auto passThrough(const QString &in) -> QString { return in; }

static inline auto headerGuard(const QString &in) -> QString
{
  QString result;
...
static inline auto structName(const QString &in) -> QString
{
  auto capNeeded = true;
  QString result;
  foreach(const QChar c, in) {
...
auto replaceFieldHelper(ValueStringTransformation transform, const CustomWizardContext::FieldReplacementMap &fm, QString *s) -> bool
{
  auto nonEmptyReplacements = false;
  if (debug) {
    qDebug().nospace() << "CustomWizardContext::replaceFields with " << fm << *s;
  }
  ...
  while (pos < s->size()) {
    ...
    if (pos < 0)
      break;
    const int nextPos = s->indexOf(delimiter, pos + 1);
    if (nextPos == -1)
      break;
    if (nextPos == pos + 1) {
    ...
    // "%field:l%"
    auto fieldSpec = s->mid(pos + 1, nextPos - pos - 1);
    ...
    const auto it = fm.constFind(fieldSpec);
    if (it == fm.constEnd()) {
      pos = nextPos; // Not found, skip
      continue;
    }
    // Assign
    auto replacement = it.value();
    switch (modifier) {
    case 'l':
...
auto CustomWizardContext::replaceFields(const FieldReplacementMap &fm, QString *s) -> bool
{
...
public:
  ...
  auto operator()(const QString &) const -> QString;
...
TemporaryFileTransform::TemporaryFileTransform(TemporaryFilePtrList *f) : m_files(f), m_pattern(TemporaryDirectory::masterDirectoryPath() + "/qtcreatorXXXXXX.txt") { }

auto TemporaryFileTransform::operator()(const QString &value) const -> QString
{
  const TemporaryFilePtr temporaryFile(new TemporaryFile(m_pattern));
  QTC_ASSERT(temporaryFile->open(), return QString());

  temporaryFile->write(value.toLocal8Bit());
  const auto name = temporaryFile->fileName();
  temporaryFile->flush();
  temporaryFile->close();
...
auto CustomWizardContext::replaceFields(const FieldReplacementMap &fm, QString *s, TemporaryFilePtrList *files) -> bool
{
  return replaceFieldHelper(TemporaryFileTransform(files), fm, s);
}

auto CustomWizardContext::reset() -> void
{
  // Basic replacement fields: Suffixes and date/time.
  const auto currentDate = QDate::currentDate();
  const auto currentTime = QTime::currentTime();
  baseReplacements.clear();
  baseReplacements.insert(QLatin1String("CppSourceSuffix"), Utils::mimeTypeForName(QLatin1String(CppEditor::Constants::CPP_SOURCE_MIMETYPE)).preferredSuffix());
  baseReplacements.insert(QLatin1String("CppHeaderSuffix"), Utils::mimeTypeForName(QLatin1String(CppEditor::Constants::CPP_HEADER_MIMETYPE)).preferredSuffix());
  baseReplacements.insert(QLatin1String("CurrentDate"), currentDate.toString(Qt::ISODate));
  baseReplacements.insert(QLatin1String("CurrentTime"), currentTime.toString(Qt::ISODate));
  baseReplacements.insert(QLatin1String("CurrentDate:ISO"), currentDate.toString(Qt::ISODate));
  baseReplacements.insert(QLatin1String("CurrentTime:ISO"), currentTime.toString(Qt::ISODate));
  baseReplacements.insert(QLatin1String("CurrentDate:RFC"), currentDate.toString(Qt::RFC2822Date));
  baseReplacements.insert(QLatin1String("CurrentTime:RFC"), currentTime.toString(Qt::RFC2822Date));
  baseReplacements.insert(QLatin1String("CurrentDate:Locale"), QLocale::system().toString(currentDate, QLocale::ShortFormat));
  baseReplacements.insert(QLatin1String("CurrentTime:Locale"), QLocale::system().toString(currentTime, QLocale::ShortFormat));
  replacements.clear();
  path.clear();
  targetPath.clear();
}

auto CustomWizardContext::processFile(const FieldReplacementMap &fm, QString in) -> QString
{
  if (in.isEmpty())
    return in;
  ...
  in = TemplateEngine::processText(expander, in, &errorMessage);
  if (!errorMessage.isEmpty()) {
    qWarning("Error processing custom widget file: %s\nFile:\n%s", qPrintable(errorMessage), qPrintable(in));
    return QString();
  }

  if (!TemplateEngine::preprocessText(in, &out, &errorMessage)) {
