...
auto fixGeneratorScript(const QString &configFile, QString binary) -> QStringList
{
  if (binary.isEmpty())
    return QStringList();
  ...
  if (!binaryInfo.isAbsolute()) {
    auto fullPath = QFileInfo(configFile).absolutePath();
  ...
  if (HostOsInfo::isWindowsHost()) {
    // Windows: Cannot run scripts by QProcess, do 'cmd /c'
    const auto extension = binaryInfo.suffix();
...
static auto runGenerationScriptHelper(const FilePath &workingDirectory, const QStringList &script, const QList<GeneratorScriptArgument> &argumentsIn, bool dryRun, const QMap<QString, QString> &fieldMap, QString *stdOut /* = 0 */, QString *errorMessage) -> bool
{
  QtcProcess process;
  const auto binary = script.front();
  QStringList arguments;
  const int binarySize = script.size();
  for (auto i = 1; i < binarySize; i++)
  ...
  foreach(const GeneratorScriptArgument &argument, argumentsIn) {
    ...
    const auto nonEmptyReplacements = argument.flags & GeneratorScriptArgument::WriteFile ? CustomWizardContext::replaceFields(fieldMap, &value, &temporaryFiles) : CustomWizardContext::replaceFields(fieldMap, &value);
    if (nonEmptyReplacements || !(argument.flags & GeneratorScriptArgument::OmitEmpty))
      arguments.push_back(value);
  }
  process.setWorkingDirectory(workingDirectory);
  process.setTimeoutS(30);
  const CommandLine cmd(FilePath::fromString(binary), arguments);
  if (CustomWizard::verbose())
    qDebug("In %s, running:\n%s\n", qPrintable(workingDirectory.toUserOutput()), qPrintable(cmd.toUserOutput()));
  ...
  if (process.result() != QtcProcess::FinishedWithSuccess) {
    *errorMessage = QString("Generator script failed: %1").arg(process.exitMessage());
    const auto stdErr = process.stdErr();
    if (!stdErr.isEmpty()) {
...
auto dryRunCustomWizardGeneratorScript(const QString &targetPath, const QStringList &script, const QList<GeneratorScriptArgument> &arguments, const QMap<QString, QString> &fieldMap, QString *errorMessage) -> Orca::Plugin::Core::GeneratedFiles
{
  // Run in temporary directory as the target path may not exist yet.
  QString stdOut;
  if (!runGenerationScriptHelper(TemporaryDirectory::masterDirectoryFilePath(), script, arguments, true, fieldMap, &stdOut, errorMessage))
    return Orca::Plugin::Core::GeneratedFiles();
  Orca::Plugin::Core::GeneratedFiles files;
  ...
  foreach(const QString &line, stdOut.split(QLatin1Char('\n'))) {
    ...
    if (!trimmed.isEmpty()) {
      Orca::Plugin::Core::GeneratedFile file;
      Orca::Plugin::Core::GeneratedFile::Attributes attributes = Orca::Plugin::Core::GeneratedFile::CustomGeneratorAttribute;
      ...
      for (auto i = 0; i < count; i++) {
        ...
        if (i) {
          ...
          const auto fullPath = fileInfo.isAbsolute() ? token : (targetPath + QLatin1Char('/') + token);
  ...
  if (CustomWizard::verbose()) {
    auto nospace = qDebug().nospace();
    nospace << script << " generated:\n";
    foreach(const Orca::Plugin::Core::GeneratedFile &f, files)
...
{
  return runGenerationScriptHelper(FilePath::fromString(targetPath), script, arguments, false, fieldMap, nullptr, errorMessage);
}
