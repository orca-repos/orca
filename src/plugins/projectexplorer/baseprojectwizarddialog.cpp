...
BaseProjectWizardDialog::BaseProjectWizardDialog(const Orca::Plugin::Core::BaseFileWizardFactory *factory, QWidget *parent, const Orca::Plugin::Core::WizardDialogParameters &parameters) : BaseFileWizard(factory, parameters.extraValues(), parent), d(std::make_unique<BaseProjectWizardDialogPrivate>(new ProjectIntroPage))
{
...
BaseProjectWizardDialog::BaseProjectWizardDialog(const Orca::Plugin::Core::BaseFileWizardFactory *factory, ProjectIntroPage *introPage, int introId, QWidget *parent, const Orca::Plugin::Core::WizardDialogParameters &parameters) : BaseFileWizard(factory, parameters.extraValues(), parent), d(std::make_unique<BaseProjectWizardDialogPrivate>(introPage, introId))
{
  setFilePath(parameters.defaultPath());
  setSelectedPlatform(parameters.selectedPlatform());
  setRequiredFeatures(parameters.requiredFeatures());
...
auto BaseProjectWizardDialog::slotAccepted() -> void
{
  if (d->introPage->useAsDefaultPath()) {
    ...
    Orca::Plugin::Core::DocumentManager::setProjectsDirectory(filePath());
    Orca::Plugin::Core::DocumentManager::setUseProjectsDirectory(true);
  }
}
