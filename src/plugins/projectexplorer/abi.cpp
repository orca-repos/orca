...
static auto moveGenericAndUnknownLast(const QList<Abi::OSFlavor> &in) -> QList<Abi::OSFlavor>
{
  auto result = in;
...
static auto insertIntoOsFlavorMap(const std::vector<Abi::OS> &oses, const Abi::OSFlavor flavor) -> void
{
...
static auto registerOsFlavor(const Abi::OSFlavor &flavor, const QByteArray &flavorName, const std::vector<Abi::OS> &oses) -> void
{
...
static auto setupPreregisteredOsFlavors() -> void
{
  ...
  registerOsFlavor(Abi::GenericFlavor, "generic", {Abi::OS::LinuxOS, Abi::OS::DarwinOS, Abi::OS::UnixOS, Abi::OS::QnxOS, Abi::OS::BareMetalOS});
  registerOsFlavor(Abi::UnknownFlavor, "unknown", {Abi::OS::BsdOS, Abi::OS::LinuxOS, Abi::OS::DarwinOS, Abi::OS::UnixOS, Abi::OS::WindowsOS, Abi::OS::VxWorks, Abi::OS::QnxOS, Abi::OS::BareMetalOS, Abi::OS::UnknownOS});
}

static auto registeredOsFlavors() -> std::vector<QByteArray>&
{
...
static auto indexOfFlavor(const QByteArray &flavor) -> int
{
  return Utils::indexOf(registeredOsFlavors(), [flavor](const QByteArray &f) { return f == flavor; });
}

static auto architectureFromQt() -> Abi::Architecture
{
...
static auto getUint8(const QByteArray &data, int pos) -> quint8
{
  return static_cast<quint8>(data.at(pos));
}

static auto getLEUint32(const QByteArray &ba, int pos) -> quint32
{
  Q_ASSERT(ba.size() >= pos + 3);
  return (static_cast<quint32>(static_cast<quint8>(ba.at(pos + 3))) << 24) + (static_cast<quint32>(static_cast<quint8>(ba.at(pos + 2)) << 16)) + (static_cast<quint32>(static_cast<quint8>(ba.at(pos + 1))) << 8) + static_cast<quint8>(ba.at(pos));
}

static auto getBEUint32(const QByteArray &ba, int pos) -> quint32
{
  Q_ASSERT(ba.size() >= pos + 3);
  return (static_cast<quint32>(static_cast<quint8>(ba.at(pos))) << 24) + (static_cast<quint32>(static_cast<quint8>(ba.at(pos + 1))) << 16) + (static_cast<quint32>(static_cast<quint8>(ba.at(pos + 2))) << 8) + static_cast<quint8>(ba.at(pos + 3));
}

static auto getLEUint16(const QByteArray &ba, int pos) -> quint32
{
  Q_ASSERT(ba.size() >= pos + 1);
  return (static_cast<quint16>(static_cast<quint8>(ba.at(pos + 1))) << 8) + static_cast<quint8>(ba.at(pos));
}

static auto getBEUint16(const QByteArray &ba, int pos) -> quint32
{
  Q_ASSERT(ba.size() >= pos + 1);
  return (static_cast<quint16>(static_cast<quint8>(ba.at(pos))) << 8) + static_cast<quint8>(ba.at(pos + 1));
}

static auto macAbiForCpu(quint32 type) -> Abi
{
  switch (type) {
...
static auto parseCoffHeader(const QByteArray &data) -> Abis
{
  Abis result;
  if (data.size() < 20)
    return result;

  auto arch = Abi::UnknownArchitecture;
  auto flavor = Abi::UnknownFlavor;
  auto width = 0;

  // Get machine field from COFF file header
  const quint16 machine = getLEUint16(data, 0);
  switch (machine) {
  case 0x01c0: // ARM LE
  ...
  if (data.size() >= 24) {
    ...
    const quint8 minorLinker = data.at(23);
    switch (data.at(22)) {
    case 2:
...
static auto abiOf(const QByteArray &data) -> Abis
{
  Abis result;
  if (data.size() <= 8)
    return result;

  if (data.size() >= 20 && getUint8(data, 0) == 0x7f && getUint8(data, 1) == 'E' && getUint8(data, 2) == 'L' && getUint8(data, 3) == 'F') {
    // ELF format:
    const auto isLE = (getUint8(data, 5) == 1);
    const quint16 machine = isLE ? getLEUint16(data, 18) : getBEUint16(data, 18);
    const auto osAbi = getUint8(data, 7);

    auto os = Abi::UnixOS;
    auto flavor = Abi::GenericFlavor;
    ...
    case 8: {
      // EM_MIPS
      const auto width = getUint8(data, 4) == 2 ? 64 : 32;
  ...
  } else if (((getUint8(data, 0) == 0xce || getUint8(data, 0) == 0xcf) && getUint8(data, 1) == 0xfa && getUint8(data, 2) == 0xed && getUint8(data, 3) == 0xfe) || (getUint8(data, 0) == 0xfe && getUint8(data, 1) == 0xed && getUint8(data, 2) == 0xfa && (getUint8(data, 3) == 0xce || getUint8(data, 3) == 0xcf))) {
    // Mach-O format (Mac non-fat binary, 32 and 64bit magic):
    const auto type = (getUint8(data, 1) == 0xfa) ? getLEUint32(data, 4) : getBEUint32(data, 4);
    result.append(macAbiForCpu(type));
  } else if ((getUint8(data, 0) == 0xbe && getUint8(data, 1) == 0xba && getUint8(data, 2) == 0xfe && getUint8(data, 3) == 0xca) || (getUint8(data, 0) == 0xca && getUint8(data, 1) == 0xfe && getUint8(data, 2) == 0xba && getUint8(data, 3) == 0xbe)) {
    // Mach-0 format Fat binary header:
    const auto isLE = (getUint8(data, 0) == 0xbe);
    const auto count = isLE ? getLEUint32(data, 4) : getBEUint32(data, 4);
    auto pos = 8;
    for (quint32 i = 0; i < count; ++i) {
      if (data.size() <= pos + 4)
        break;

      const auto type = isLE ? getLEUint32(data, pos) : getBEUint32(data, pos);
      result.append(macAbiForCpu(type));
      pos += 20;
    }
  } else if ( // Qt 5.15+ https://webassembly.github.io/spec/core/binary/modules.html#binary-module
    (getUint8(data, 0) == 0 && getUint8(data, 1) == 'a' && getUint8(data, 2) == 's' && getUint8(data, 3) == 'm')

    // Qt < 5.15 https://llvm.org/docs/BitCodeFormat.html#llvm-ir-magic-number
    || (getUint8(data, 0) == 'B' && getUint8(data, 1) == 'C' && getUint8(data, 2) == 0xc0 && getUint8(data, 3) == 0xde)) {
    result.append(Abi(Abi::AsmJsArchitecture, Abi::UnknownOS, Abi::UnknownFlavor, Abi::EmscriptenFormat, 32));
    ...
    const auto firstChar = getUint8(data, 0);
    const auto secondChar = getUint8(data, 1);
    if ((firstChar != 'M' || secondChar != 'Z') && (firstChar != 'Z' || secondChar != 'M'))
      return result;

    // Get PE/COFF header position from MZ header:
    const qint32 pePos = getLEUint32(data, 60);
    if (pePos <= 0 || data.size() < pePos + 4 + 20) // PE magic bytes plus COFF header
      return result;
    if (getUint8(data, pePos) == 'P' && getUint8(data, pePos + 1) == 'E' && getUint8(data, pePos + 2) == 0 && getUint8(data, pePos + 3) == 0)
...
Abi::Abi(const Architecture &a, const OS &o, const OSFlavor &of, const BinaryFormat &f, unsigned char w, const QString &p) : m_architecture(a), m_os(o), m_osFlavor(of), m_binaryFormat(f), m_wordWidth(w), m_param(p)
{
  QTC_ASSERT(osSupportsFlavor(o, of), m_osFlavor = UnknownFlavor);
}

auto Abi::abiFromTargetTriplet(const QString &triple) -> Abi
{
  const auto machine = triple.toLower();
  if (machine.isEmpty())
    return Abi();

  const auto parts = machine.split(QRegularExpression("[ /-]"));

  auto arch = UnknownArchitecture;
  auto os = UnknownOS;
  auto flavor = UnknownFlavor;
  auto format = UnknownFormat;
  unsigned char width = 0;
  auto unknownCount = 0;

  for (const auto &p : parts) {
    if (p == "unknown" || p == "pc" || p == "gnu" || p == "uclibc" || p == "86_64" || p == "redhat" || p == "w64") {
      continue;
      ...
      format = ElfFormat;
    } else if (p == "mingw32" || p == "win32" || p == "mingw32msvc" || p == "msys" || p == "cygwin" || p == "windows") {
      arch = X86Architecture;
      os = WindowsOS;
...
auto Abi::abiOsToOsType(const OS os) -> Utils::OsType
{
  switch (os) {
  case LinuxOS:
...
auto Abi::toString() const -> QString
{
  const QStringList dn = {toString(m_architecture), toString(m_os), toString(m_osFlavor), toString(m_binaryFormat), toString(m_wordWidth)};
  return dn.join('-');
}

auto Abi::param() const -> QString
{
  if (m_param.isEmpty())
    return toString();
  return m_param;
...
auto Abi::operator !=(const Abi &other) const -> bool
{
...
auto Abi::operator ==(const Abi &other) const -> bool
{
  return m_architecture == other.m_architecture && m_os == other.m_os && m_osFlavor == other.m_osFlavor && m_binaryFormat == other.m_binaryFormat && m_wordWidth == other.m_wordWidth;
}

static auto compatibleMSVCFlavors(const Abi::OSFlavor &left, const Abi::OSFlavor &right) -> bool
{
  // MSVC 2022, 2019, 2017 and 2015 are compatible
  return left >= Abi::WindowsMsvc2015Flavor && left <= Abi::WindowsMsvc2022Flavor && right >= Abi::WindowsMsvc2015Flavor && right <= Abi::WindowsMsvc2022Flavor;
}

auto Abi::isCompatibleWith(const Abi &other) const -> bool
{
  // Generic match: If stuff is identical or the other side is unknown, then this is a match.
  auto isCompat = (architecture() == other.architecture() || other.architecture() == UnknownArchitecture) && (os() == other.os() || other.os() == UnknownOS) && (osFlavor() == other.osFlavor() || other.osFlavor() == UnknownFlavor) && (binaryFormat() == other.binaryFormat() || other.binaryFormat() == UnknownFormat) && ((wordWidth() == other.wordWidth() && wordWidth() != 0) || other.wordWidth() == 0);
  ...
  // work for them.
  if (!isCompat && (architecture() == other.architecture() || other.architecture() == UnknownArchitecture) && ((os() == other.os()) && (os() == LinuxOS)) && (osFlavor() == GenericFlavor || other.osFlavor() == GenericFlavor) && (binaryFormat() == other.binaryFormat() || other.binaryFormat() == UnknownFormat) && ((wordWidth() == other.wordWidth() && wordWidth() != 0) || other.wordWidth() == 0)) {
    isCompat = true;
  }
  ...
  if (!isCompat && wordWidth() == other.wordWidth() && compatibleMSVCFlavors(osFlavor(), other.osFlavor())) {
    isCompat = true;
  }
...
auto Abi::isValid() const -> bool
{
  return m_architecture != UnknownArchitecture && m_os != UnknownOS && m_osFlavor != UnknownFlavor && m_binaryFormat != UnknownFormat && m_wordWidth != 0;
}

auto Abi::isNull() const -> bool
{
  return m_architecture == UnknownArchitecture && m_os == UnknownOS && m_osFlavor == UnknownFlavor && m_binaryFormat == UnknownFormat && m_wordWidth == 0;
}

auto Abi::toString(const Architecture &a) -> QString
{
  switch (a) {
  ...
  case UnknownArchitecture: Q_FALLTHROUGH();
  default:
    return QLatin1String("unknown");
  }
}

auto Abi::toString(const OS &o) -> QString
{
  switch (o) {
  case LinuxOS:
  ...
  case UnknownOS: Q_FALLTHROUGH();
  default:
    return QLatin1String("unknown");
  };
}

auto Abi::toString(const OSFlavor &of) -> QString
{
  const auto index = static_cast<size_t>(of);
  const auto &flavors = registeredOsFlavors();
  QTC_ASSERT(index < flavors.size(), return QString::fromUtf8(flavors.at(int(UnknownFlavor))));
  return QString::fromUtf8(flavors.at(index));
}

auto Abi::toString(const BinaryFormat &bf) -> QString
{
  switch (bf) {
  case ElfFormat:
  ...
  case UnknownFormat: Q_FALLTHROUGH();
  default:
    return QLatin1String("unknown");
  }
}

auto Abi::toString(int w) -> QString
{
  if (w == 0)
    return QLatin1String("unknown");
...
auto Abi::fromString(const QString &abiString) -> Abi
{
  auto architecture = UnknownArchitecture;
  const auto abiParts = abiString.split('-');
  if (!abiParts.isEmpty()) {
...
auto Abi::architectureFromString(const QString &a) -> Architecture
{
  if (a == "unknown")
...
auto Abi::osFromString(const QString &o) -> OS
{
  if (o == "unknown")
    return UnknownOS;
  if (o == "linux")
...
auto Abi::osFlavorFromString(const QString &of, const OS os) -> OSFlavor
{
  const auto index = indexOfFlavor(of.toUtf8());
...
auto Abi::binaryFormatFromString(const QString &bf) -> BinaryFormat
{
  if (bf == "unknown")
...
auto Abi::wordWidthFromString(const QString &w) -> unsigned char
{
  if (!w.endsWith("bit"))
    return 0;

  auto ok = false;
  const auto number = w.left(w.size() - 3);
  const auto bitCount = number.toInt(&ok);
  if (!ok)
    return 0;
...
auto Abi::registerOsFlavor(const std::vector<OS> &oses, const QString &flavorName) -> OSFlavor
{
  QTC_ASSERT(oses.size() > 0, return UnknownFlavor);
  const auto flavorBytes = flavorName.toUtf8();

  auto index = indexOfFlavor(flavorBytes);
  if (index < 0)
    index = int(registeredOsFlavors().size());

  const auto toRegister = OSFlavor(index);
  ProjectExplorer::registerOsFlavor(toRegister, flavorBytes, oses);
  return toRegister;
}

auto Abi::flavorsForOs(const OS &o) -> QList<OSFlavor>
{
  registeredOsFlavors(); // Make sure m_osToOsFlavorMap is populated!
  const auto it = m_osToOsFlavorMap.find(o);
  if (it == m_osToOsFlavorMap.end())
    return {};
...
auto Abi::allOsFlavors() -> QList<OSFlavor>
{
...
auto Abi::osSupportsFlavor(const OS &os, const OSFlavor &flavor) -> bool
{
  return flavorsForOs(os).contains(flavor);
}

auto Abi::flavorForMsvcVersion(int version) -> OSFlavor
{
  if (version >= 1930)
...
auto Abi::hostAbi() -> Abi
{
  const auto arch = architectureFromQt();
  auto os = UnknownOS;
  auto subos = UnknownFlavor;
  auto format = UnknownFormat;
  ...
  if (!result.isValid())
    qWarning("Unable to completely determine the host ABI (%s).", qPrintable(result.toString()));
  return result;
}

//! Extract available ABIs from a binary using heuristics.
auto Abi::abisOfBinary(const Utils::FilePath &path) -> Abis
{
  Abis tmp;
  if (path.isEmpty())
    return tmp;

  auto data = path.fileContents(1024);
  if (data.size() >= 67 && getUint8(data, 0) == '!' && getUint8(data, 1) == '<' && getUint8(data, 2) == 'a' && getUint8(data, 3) == 'r' && getUint8(data, 4) == 'c' && getUint8(data, 5) == 'h' && getUint8(data, 6) == '>' && getUint8(data, 7) == 0x0a) {
    ...
    while (!data.isEmpty()) {
      ...
      const auto fileName = QString::fromLocal8Bit(data.mid(0, 16));
      quint64 fileNameOffset = 0;
      if (fileName.startsWith("#1/"))
        fileNameOffset = fileName.mid(3).toInt();
      const auto fileLength = QString::fromLatin1(data.mid(48, 10));

      const int toSkip = 60 + fileNameOffset;
