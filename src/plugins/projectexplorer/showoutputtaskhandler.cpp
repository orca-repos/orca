...
ShowOutputTaskHandler::ShowOutputTaskHandler(Orca::Plugin::Core::IOutputPane *window, const QString &text, const QString &tooltip, const QString &shortcut) : m_window(window), m_text(text), m_tooltip(tooltip), m_shortcut(shortcut)
{
  QTC_CHECK(m_window);
...
{
  return Utils::anyOf(m_window->outputWindows(), [task](const Orca::Plugin::Core::OutputWindow *ow) {
...
auto ShowOutputTaskHandler::handle(const Task &task) -> void
{
  ...
  m_window->popup(Orca::Plugin::Core::IOutputPane::Flags(Orca::Plugin::Core::IOutputPane::ModeSwitch | Orca::Plugin::Core::IOutputPane::WithFocus));
