...
{
  auto name = Orca::Plugin::Core::ICore::userInterfaceLanguage();
...
auto JsonWizardFactory::loadDefaultValues(const QString &fileName) -> QVariantMap
{
  ...
  if (verbose()) {
    ...
    Orca::Plugin::Core::MessageManager::writeDisrupting(verboseLog);
  }
...
{
  static FilePaths m_searchPaths = {Orca::Plugin::Core::ICore::userResourcePath(WIZARD_PATH), Orca::Plugin::Core::ICore::resourcePath(WIZARD_PATH)};
...
auto JsonWizardFactory::runWizardImpl(const FilePath &path, QWidget *parent, Id platform, const QVariantMap &variables, bool showWizard) -> Wizard*
{
  ...
  QString kindStr = QLatin1String(Orca::Plugin::Core::WIZARD_KIND_UNKNOWN);
  if (kind() == FileWizard)
    kindStr = QLatin1String(Orca::Plugin::Core::WIZARD_KIND_FILE);
  else if (kind() == ProjectWizard)
    kindStr = QLatin1String(Orca::Plugin::Core::WIZARD_KIND_PROJECT);
...
auto JsonWizardFactory::isAvailable(Id platformId) const -> bool
{
  ...
  const Orca::Plugin::Core::JsExpander jsExpander;
