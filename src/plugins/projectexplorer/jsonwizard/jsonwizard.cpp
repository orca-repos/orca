...
public:
  ...
  {
    m_candidate->file.setAttributes(m_candidate->file.attributes().setFlag(Core::GeneratedFile::OpenProjectAttribute, on));
  }
  ...
  if (m_files.isEmpty() && errorMessage.isEmpty()) {
    ...
    foreach(JsonWizardGenerator *gen, m_generators) {
      ...
      if (!errorMessage.isEmpty())
        break;
      list.append(Utils::transform(tmp, [&gen](const Core::GeneratedFile &f) { return JsonWizard::GeneratorFile(f, gen); }));
    }
  }
...
auto JsonWizard::removeAttributeFromAllFiles(Core::GeneratedFile::Attribute a) -> void
{
...
auto JsonWizard::openFiles(const JsonWizard::GeneratorFiles &files) -> void
{
  ...
  foreach(const JsonWizard::GeneratorFile &f, files) {
    ...
    if (file.attributes() & Core::GeneratedFile::OpenEditorAttribute) {
      const auto editor = Core::EditorManager::openEditor(Utils::FilePath::fromString(file.path()), file.editorId());
      if (!editor) {
