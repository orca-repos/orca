...
auto JsonWizardScannerGenerator::fileList(Utils::MacroExpander *expander, const QString &wizardDir, const QString &projectDir, QString *errorMessage) -> Orca::Plugin::Core::GeneratedFiles
{
  Q_UNUSED(wizardDir)
  errorMessage->clear();

  const QDir project(projectDir);
  Orca::Plugin::Core::GeneratedFiles result;
  ...
  for (auto it = result.begin(); it != result.end(); ++it) {
    ...
    if (found) {
      it->setAttributes(it->attributes() | Orca::Plugin::Core::GeneratedFile::OpenProjectAttribute);
  ...
  for (Orca::Plugin::Core::GeneratedFile &f : result) {
    if (f.attributes().testFlag(Orca::Plugin::Core::GeneratedFile::OpenProjectAttribute) && getDepth(f.path()) > minDepth) {
      f.setAttributes(f.attributes().setFlag(Orca::Plugin::Core::GeneratedFile::OpenProjectAttribute, false));
    }
  }
...
auto JsonWizardScannerGenerator::scan(const QString &dir, const QDir &base) -> Orca::Plugin::Core::GeneratedFiles
{
  Orca::Plugin::Core::GeneratedFiles result;
  ...
  foreach(const QFileInfo &fi, entries) {
    ...
    if (fi.isDir() && matchesSubdirectoryPattern(relativePath)) {
      ...
      Orca::Plugin::Core::GeneratedFile f(fi.absoluteFilePath());
      f.setAttributes(f.attributes() | Orca::Plugin::Core::GeneratedFile::KeepExistingFileAttribute);
