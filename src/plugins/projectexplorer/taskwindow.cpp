...
auto TaskView::mouseReleaseEvent(QMouseEvent *e) -> void
{
  if (m_linksActive && m_mouseButtonPressed == Qt::LeftButton) {
    ...
    if (!loc.targetFilePath.isEmpty()) {
      Orca::Plugin::Core::EditorManager::openEditorAt(loc, {}, Orca::Plugin::Core::EditorManager::SwitchSplitIfAlreadyVisible);
    }
  }
...
public:
  ...
  TaskView *m_listview;
  Orca::Plugin::Core::IContext *m_taskWindowContext;
  QMenu *m_contextMenu;
...
TaskWindow::TaskWindow() : d(std::make_unique<TaskWindowPrivate>())
{
  ...
  agg->add(new Orca::Plugin::Core::ItemViewFind(d->m_listview, TaskModel::Description));
  ...
  d->m_taskWindowContext = new Orca::Plugin::Core::IContext(d->m_listview);
  d->m_taskWindowContext->setWidget(d->m_listview);
  d->m_taskWindowContext->setContext(Orca::Plugin::Core::Context(Orca::Plugin::Core::C_PROBLEM_PANE));
  Orca::Plugin::Core::ICore::addContextObject(d->m_taskWindowContext);
...
auto TaskWindow::delayedInitialization() -> void
{
  ...
  for (auto h : qAsConst(g_taskHandlers)) {
    ...
    if (id.isValid()) {
      const auto cmd = Orca::Plugin::Core::ActionManager::registerAction(action, id, d->m_taskWindowContext->context(), true);
      action = cmd->action();
    }
