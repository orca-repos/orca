...
BuildSystemOutputWindow::BuildSystemOutputWindow() : OutputWindow(Context(kBuildSystemOutputContext), "ProjectsMode.BuildSystemOutput.Zoom"), m_clear(new QAction)
{
  ...
  const auto clearCommand = ActionManager::command(Core::Constants::OUTPUTPANE_CLEAR);
  m_clear->setIcon(Utils::Icons::CLEAN_TOOLBAR.icon());
  m_clear->setText(clearCommand->action()->text());
  ActionManager::registerAction(m_clear, Core::Constants::OUTPUTPANE_CLEAR, Context(kBuildSystemOutputContext));
  ...
  ActionManager::registerAction(m_zoomIn, Core::Constants::ZOOM_IN, Context(kBuildSystemOutputContext));

  m_zoomOut = new QAction;
  m_zoomOut->setIcon(Utils::Icons::MINUS.icon());
  connect(m_zoomOut, &QAction::triggered, this, [this] { zoomOut(); });
  ActionManager::registerAction(m_zoomOut, Core::Constants::ZOOM_OUT, Context(kBuildSystemOutputContext));
}
...
auto BuildSystemOutputWindow::toolBar() -> QWidget*
{
  if (!m_toolBar) {
    ...
    const auto clearButton = new CommandButton(Core::Constants::OUTPUTPANE_CLEAR);
    ...
    const auto zoomInButton = new CommandButton(Core::Constants::ZOOM_IN);
    zoomInButton->setDefaultAction(m_zoomIn);
    const auto zoomOutButton = new CommandButton(Core::Constants::ZOOM_OUT);
...
public:
  ProjectWindowPrivate(ProjectWindow *parent) : q(parent)
  {
    ...
    ActionManager::registerAction(switchProjectAction, Core::Constants::GOTOPREVINHISTORY, Context(Constants::C_PROJECTEXPLORER));
