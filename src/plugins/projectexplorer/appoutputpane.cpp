...
AppOutputPane::RunControlTab::RunControlTab(RunControl *runControl, Core::OutputWindow *w) : runControl(runControl), window(w)
{
  if (runControl && w) {
    w->reset();
...
AppOutputPane::AppOutputPane() : m_mainWidget(new QWidget), m_tabWidget(new TabWidget), m_stopAction(new QAction(tr("Stop"), this)), m_closeCurrentTabAction(new QAction(tr("Close Tab"), this)), m_closeAllTabsAction(new QAction(tr("Close All Tabs"), this)), m_closeOtherTabsAction(new QAction(tr("Close Other Tabs"), this)), m_reRunButton(new QToolButton), m_stopButton(new QToolButton), m_attachButton(new QToolButton), m_settingsButton(new QToolButton), m_formatterWidget(new QWidget), m_handler(new ShowOutputTaskHandler(this, tr("Show &App Output"), tr("Show the output that generated this issue in the Application Output pane."), tr("A")))
{
  ...
  const auto cmd = Core::ActionManager::registerAction(m_stopAction, Constants::STOP);
...
{
  const auto *currentWindow = qobject_cast<Core::OutputWindow*>(m_tabWidget->currentWidget());
  if (currentWindow)
    currentWindow->clear();
}
...
auto AppOutputPane::outputWindows() const -> QList<Core::OutputWindow*>
{
...
auto AppOutputPane::ensureWindowVisible(Core::OutputWindow *ow) -> void
{
...
auto AppOutputPane::createNewOutputWindow(RunControl *rc) -> void
{
  ...
  auto ow = new Core::OutputWindow(context, SETTINGS_KEY, m_tabWidget);
...
auto AppOutputPane::handleOldOutput(Core::OutputWindow *window) const -> void
{
...
auto AppOutputPane::appendMessage(RunControl *rc, const QString &out, Utils::OutputFormat format) -> void
{
  ...
  if (index != -1) {
    const Core::OutputWindow *window = m_runControlTabs.at(index).window;
...
auto AppOutputPane::storeSettings() const -> void
{
  ...
  s->setValueWithDefault(MAX_LINES_KEY, m_settings.maxCharCount / 100, Core::Constants::DEFAULT_MAX_CHAR_COUNT);
}
...
auto AppOutputPane::loadSettings() -> void
{
  ...
  m_settings.maxCharCount = s->value(MAX_LINES_KEY, Core::Constants::DEFAULT_MAX_CHAR_COUNT).toInt() * 100;
}
...
auto AppOutputPane::closeTab(int tabIndex, CloseTabMode closeTabMode) -> void
{
  ...
  const Core::OutputWindow *window = m_runControlTabs[index].window;
