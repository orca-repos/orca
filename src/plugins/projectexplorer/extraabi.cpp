...
AbiFlavorAccessor::AbiFlavorAccessor() : UpgradingSettingsAccessor("QtCreatorExtraAbi", QCoreApplication::translate("ProjectExplorer::ToolChainManager", "ABI"), Orca::Plugin::Core::IDE_DISPLAY_NAME)
{
  setBaseFilePath(Orca::Plugin::Core::ICore::installerResourcePath("abi.xml"));
...
auto ExtraAbi::load() -> void
{
  ...
  const auto data = accessor.restoreSettings(Orca::Plugin::Core::ICore::dialogParent()).value("Flavors").toMap();
