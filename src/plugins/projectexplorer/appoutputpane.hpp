...
private:
  ...
  class RunControlTab {
  public:
    explicit RunControlTab(RunControl *runControl = nullptr, Core::OutputWindow *window = nullptr);
  ...
  auto handleOldOutput(Core::OutputWindow *window) const -> void;
  auto updateCloseActions() -> void;
  auto updateFilter() -> void override;
  auto outputWindows() const -> QList<Core::OutputWindow*> override;
  auto ensureWindowVisible(Core::OutputWindow *ow) -> void override;
