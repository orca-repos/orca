...
namespace Internal {
template<typename Result>
auto scanForFilesRecursively(QFutureInterface<Result> &future, double progressStart, double progressRange, const Utils::FilePath &directory, const std::function<FileNode *(const Utils::FilePath &)> factory, QSet<QString> &visited, const QList<Orca::Plugin::Core::IVersionControl*> &versionControls) -> QList<FileNode*>
{
  ...
  for (const auto &entry : entries) {
    ...
    if (!Utils::contains(versionControls, [&entryName](const Orca::Plugin::Core::IVersionControl *vc) {
...
auto scanForFiles(QFutureInterface<Result> &future, const Utils::FilePath &directory, const std::function<FileNode *(const Utils::FilePath &)> factory) -> QList<FileNode*>
{
  ...
  return Internal::scanForFilesRecursively(future, 0.0, 1000000.0, directory, factory, visited, Orca::Plugin::Core::VcsManager::versionControls());
}
