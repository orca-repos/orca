...
#include <core/core-document-factory-interface.hpp>
#include <core/core-mode-interface.hpp>
#include <core/core-version-control-interface.hpp>
#include <core/core-directory-filter.hpp>
#include <core/core-mini-splitter.hpp>
#include <core/core-mode-manager.hpp>
#include <core/core-navigation-widget.hpp>
#include <core/core-output-pane.hpp>
...
auto ProjectExplorerPlugin::initialize(const QStringList &arguments, QString *error) -> bool
{
  ...
  auto mfile = ActionManager::actionContainer(Orca::Plugin::Core::M_FILE);
  auto menubar = ActionManager::actionContainer(Orca::Plugin::Core::MENU_BAR);
  ...
  if (!hideBuildMenu())
    menubar->addMenu(mbuild, Orca::Plugin::Core::G_VIEW);

  // debug menu
  auto mdebug = ActionManager::createMenu(Constants::M_DEBUG);
  mdebug->menu()->setTitle(tr("&Debug"));
  if (!hideDebugMenu())
    menubar->addMenu(mdebug, Orca::Plugin::Core::G_VIEW);
  ...
  mdebug->addMenu(mstartdebugging, Orca::Plugin::Core::G_DEFAULT_ONE);
  ...
  dd->m_openTerminalMenu->setTitle(Orca::Plugin::Core::FileUtils::msgTerminalWithAction());
  ...
  mfile->addSeparator(Orca::Plugin::Core::G_FILE_PROJECT);
  ...
  cmd = ActionManager::registerAction(dd->m_newAction, Orca::Plugin::Core::NEW, projectTreeContext);
  ...
  dd->m_searchOnFileSystem = new QAction(Orca::Plugin::Core::FileUtils::msgFindInDirectory(), this);
  ...
  dd->m_showInGraphicalShell = new QAction(Orca::Plugin::Core::FileUtils::msgGraphicalShellAction(), this);
  cmd = ActionManager::registerAction(dd->m_showInGraphicalShell, Orca::Plugin::Core::SHOWINGRAPHICALSHELL, projectTreeContext);
  mfileContextMenu->addAction(cmd, Constants::G_FILE_OPEN);
  mfolderContextMenu->addAction(cmd, Constants::G_FOLDER_FILES);

  // Show in File System View
  dd->m_showFileSystemPane = new QAction(Orca::Plugin::Core::FileUtils::msgFileSystemAction(), this);
  ...
  dd->m_openTerminalHere = new QAction(Orca::Plugin::Core::FileUtils::msgTerminalHereAction(), this);
  ...
  mfile->addMenu(mrecent, Orca::Plugin::Core::G_FILE_OPEN);
  ...
  mfile->addMenu(msession, Orca::Plugin::Core::G_FILE_OPEN);
  ...
  mfile->addAction(cmd, Orca::Plugin::Core::G_FILE_PROJECT);
  ...
  mfile->addMenu(munload, Orca::Plugin::Core::G_FILE_PROJECT);
...
auto ProjectExplorerPlugin::extensionsInitialized() -> void
{
  ...
  const auto mtools = ActionManager::actionContainer(Orca::Plugin::Core::M_TOOLS);
...
auto ProjectExplorerPlugin::coreAboutToClose() -> bool
{
  ...
  if (BuildManager::isBuilding()) {
    ...
    box.setDefaultButton(cancelClose);
    box.setWindowTitle(tr("Close %1?").arg(Orca::Plugin::Core::IDE_DISPLAY_NAME));
    box.setText(tr("A project is currently being built."));
    box.setInformativeText(tr("Do you want to cancel the build process and close %1 anyway?").arg(Orca::Plugin::Core::IDE_DISPLAY_NAME));
    box.exec();
    if (box.clickedButton() != closeAnyway)
      return false;
  }
...
auto ProjectExplorerPluginPrivate::updateRecentProjectMenu() -> void
{
  ...
  if (hasRecentProjects) {
    menu->addSeparator();
    const auto action = menu->addAction(QCoreApplication::translate("Core", Orca::Plugin::Core::TR_CLEAR_MENU));
...
auto ProjectExplorerPluginPrivate::showInGraphicalShell() -> void
{
  const auto currentNode = ProjectTree::currentNode();
  QTC_ASSERT(currentNode, return);
  Orca::Plugin::Core::FileUtils::showInGraphicalShell(ICore::dialogParent(), currentNode->path());
}
...
auto ProjectExplorerPluginPrivate::showInFileSystemPane() -> void
{
  const auto currentNode = ProjectTree::currentNode();
  QTC_ASSERT(currentNode, return);
  Orca::Plugin::Core::FileUtils::showInFileSystemView(currentNode->filePath());
}
...
auto ProjectExplorerPluginPrivate::openTerminalHere(const EnvironmentGetter &env) -> void
{
  ...
  Orca::Plugin::Core::FileUtils::openTerminal(currentNode->directory(), environment.value());
}
...
auto ProjectExplorerPluginPrivate::removeFile() -> void
{
  ...
  Orca::Plugin::Core::FileUtils::removeFiles(pathList, deleteFile);
}
...
auto ProjectExplorerPluginPrivate::duplicateFile() -> void
{
  ...
  Orca::Plugin::Core::FileUtils::updateHeaderFileGuardIfApplicable(currentNode->filePath(), FilePath::fromString(newFilePath), canTryToRenameIncludeGuards(currentNode));
  ...
  if (!folderNode->canRenameFile(oldFilePath, newFilePath)) {
    QTimer::singleShot(0, [oldFilePath, newFilePath, projectFileName, handleGuards] {
      ...
      if (res == QMessageBox::Yes) {
        QTC_CHECK(Orca::Plugin::Core::FileUtils::renameFile(oldFilePath, newFilePath, handleGuards));
      }
    });
    return;
  }

  if (Orca::Plugin::Core::FileUtils::renameFile(oldFilePath, newFilePath, handleGuards)) {
