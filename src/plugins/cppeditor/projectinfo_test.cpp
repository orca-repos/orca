...
#include <projectexplorer/toolchainconfigwidget.hpp>
...
{
    ProjectInfoGeneratorTestHelper t;
    t.rawProjectPart.files = QStringList{ "foo.cpp", "foo.hpp"};
    const ProjectInfo::ConstPtr projectInfo = t.generate();
...
{
    ProjectInfoGeneratorTestHelper t;
    t.rawProjectPart.files = QStringList{ "foo.cpp", "foo.hpp", "bar.c", "bar.hpp" };
    const ProjectInfo::ConstPtr projectInfo = t.generate();
...
{
    ProjectInfoGeneratorTestHelper t;
    t.rawProjectPart.files = QStringList{ "foo.hpp" };
    const ProjectInfo::ConstPtr projectInfo = t.generate();
...
{
    const ProjectFileCategorizer categorizer({}, {"foo.c", "foo.hpp"});
    const ProjectFiles expected {
        ProjectFile("foo.c", ProjectFile::CSource),
        ProjectFile("foo.hpp", ProjectFile::CHeader),
    };
...
void ProjectFileCategorizerTest::testObjectiveC()
{
    const ProjectFiles expected {
        ...
        ProjectFile("foo.hpp", ProjectFile::ObjCHeader),
    };

    const ProjectFileCategorizer categorizer({}, {"foo.m", "foo.hpp"});
...
void ProjectFileCategorizerTest::testObjectiveCxx()
{
    const ProjectFiles expected {
        ...
        ProjectFile("foo.hpp", ProjectFile::ObjCXXHeader),
    };

    const ProjectFileCategorizer categorizer({}, {"foo.mm", "foo.hpp"});
...
void ProjectFileCategorizerTest::testMixedCAndCxx()
{
    const ProjectFiles expectedCxxSources {
        ProjectFile("foo.cpp", ProjectFile::CXXSource),
        ProjectFile("foo.hpp", ProjectFile::CXXHeader),
        ProjectFile("bar.hpp", ProjectFile::CXXHeader),
    };
    const ProjectFiles expectedCSources {
        ProjectFile("bar.c", ProjectFile::CSource),
        ProjectFile("foo.hpp", ProjectFile::CHeader),
        ProjectFile("bar.hpp", ProjectFile::CHeader),
    };

    const ProjectFileCategorizer categorizer({}, {"foo.cpp", "foo.hpp", "bar.c", "bar.hpp"});
...
{
    const ProjectFileCategorizer categorizer({}, {"foo.hpp"});

    QCOMPARE(categorizer.cSources(), {ProjectFile("foo.hpp", ProjectFile::CHeader)});
    QCOMPARE(categorizer.cxxSources(), {ProjectFile("foo.hpp", ProjectFile::CXXHeader)});
    QCOMPARE(categorizer.objcSources(), {ProjectFile("foo.hpp", ProjectFile::ObjCHeader)});
    QCOMPARE(categorizer.objcxxSources(), {ProjectFile("foo.hpp", ProjectFile::ObjCXXHeader)});
}
