...
static auto isVirtualFunction_helper(const Function *function, const LookupContext &context, VirtualType virtualType, QList<const Function*> *firstVirtuals) -> bool
{
  enum {
    Unknown,
    False,
  ...
  const Class *const klass = function->enclosingScope() ? function->enclosingScope()->asClass() : nullptr;
  if (!klass)
    return false;

  auto hierarchyDepthOfFirstVirtuals = -1;
  const auto updateFirstVirtualsList = [&hierarchyDepthOfFirstVirtuals, &context, firstVirtuals, klass](Function *candidate) {
    const Class *const candidateClass = candidate->enclosingScope() ? candidate->enclosingScope()->asClass() : nullptr;
    if (!candidateClass)
      return;
    ...
    while (!classes.isEmpty()) {
      ...
      for (auto i = 0; i < c.first->baseClassCount(); ++i) {
        const BaseClass *const baseClassSpec = c.first->baseClassAt(i);
        const ClassOrNamespace *const base = context.lookupType(baseClassSpec->name(), c.first->enclosingScope());
...
auto FunctionUtils::isVirtualFunction(const Function *function, const LookupContext &context, QList<const Function*> *firstVirtuals) -> bool
{
  return isVirtualFunction_helper(function, context, Virtual, firstVirtuals);
}

auto FunctionUtils::isPureVirtualFunction(const Function *function, const LookupContext &context, QList<const Function*> *firstVirtuals) -> bool
{
  return isVirtualFunction_helper(function, context, PureVirtual, firstVirtuals);
}

auto FunctionUtils::overrides(Function *function, Class *functionsClass, Class *staticClass, const Snapshot &snapshot) -> QList<Function*>
{
  ...
  // Find overrides
  const TypeHierarchy &staticClassHierarchy = TypeHierarchyBuilder::buildDerivedTypeHierarchy(staticClass, snapshot);
  ...
  while (!l.isEmpty()) {
    // Add derived
    const auto hierarchy = l.takeFirst();
