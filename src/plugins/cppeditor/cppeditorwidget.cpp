...
{
  const QTextCodec *defaultCodec = Orca::Plugin::Core::EditorManager::defaultTextCodec();
  QString contents;
...
static auto addSearchResults(Usages usages, SearchResult &search, const QString &text) -> void
{
  ...
  for (const auto &usage : usages) {
    ...
    if (!lineContent.isEmpty()) {
      TextRange range{TextPosition(usage.line, usage.column - 1), TextPosition(usage.line, usage.column + text.length() - 1)};
...
static auto findRenameCallback(CppEditorWidget *widget, const QTextCursor &baseCursor, const Usages &usages, bool rename = false, const QString &replacement = QString()) -> void
{
  ...
  QObject::connect(search, &SearchResult::activated, [](const Orca::Plugin::Core::SearchResultItem &item) {
