...
static auto ordering(InsertionPointLocator::AccessSpec xsSpec) -> int
{
  static auto order = QList<InsertionPointLocator::AccessSpec>() << InsertionPointLocator::Public << InsertionPointLocator::PublicSlot << InsertionPointLocator::Signals << InsertionPointLocator::Protected << InsertionPointLocator::ProtectedSlot << InsertionPointLocator::PrivateSlot << InsertionPointLocator::Private;
...
struct AccessRange {
  ...
  AccessRange(unsigned start, unsigned end, InsertionPointLocator::AccessSpec xsSpec, unsigned colonToken) : start(start), end(end), xsSpec(xsSpec), colonToken(colonToken) {}

  auto isEmpty() const -> bool
  {
    auto contentStart = 1 + (colonToken ? colonToken : start);
...
public:
  FindInClass(TranslationUnit *tu, const Class *clazz) : ASTVisitor(tu), _clazz(clazz) {}

  auto operator()() -> ClassSpecifierAST*
  {
    _result = nullptr;
...
protected:
  ...
  auto visit(ClassSpecifierAST *ast) -> bool override
  {
...
auto findMatch(const QList<AccessRange> &ranges, InsertionPointLocator::AccessSpec xsSpec, InsertionPointLocator::Position positionInAccessSpec, InsertionPointLocator::ForceAccessSpec forceAccessSpec, unsigned &beforeToken, bool &needsLeadingEmptyLine, bool &needsPrefix, bool &needsSuffix) -> void
{
  QTC_ASSERT(!ranges.isEmpty(), return);
  const int lastIndex = ranges.size() - 1;
  const auto atEnd = positionInAccessSpec == InsertionPointLocator::AccessSpecEnd;
  needsLeadingEmptyLine = false;

  // Try an exact match. Ignore the default access spec unless there is no explicit one.
  const auto firstIndex = ranges.size() == 1 && forceAccessSpec == InsertionPointLocator::ForceAccessSpec::No ? 0 : 1;
  for (auto i = lastIndex; i >= firstIndex; --i) {
  ...
  for (auto i = lastIndex; i > 0; --i) {
    const auto &current = ranges.at(i);
...
auto collectAccessRanges(const CPlusPlus::TranslationUnit *tu, DeclarationListAST *decls, InsertionPointLocator::AccessSpec initialXs, int firstRangeStart, int lastRangeEnd) -> QList<AccessRange>
{
  ...
  for (DeclarationListAST *iter = decls; iter; iter = iter->next) {
    ...
    if (AccessDeclarationAST *xsDecl = decl->asAccessDeclaration()) {
      ...
      case T_Q_SLOTS: {
        newXsSpec = (InsertionPointLocator::AccessSpec)(ranges.last().xsSpec | InsertionPointLocator::SlotBit);
        break;
      }
...
InsertionLocation::InsertionLocation(const QString &fileName, const QString &prefix, const QString &suffix, int line, int column) : m_fileName(fileName), m_prefix(prefix), m_suffix(suffix), m_line(line), m_column(column) {}

auto InsertionPointLocator::accessSpecToString(InsertionPointLocator::AccessSpec xsSpec) -> QString
{
  switch (xsSpec) {
  default: case InsertionPointLocator::Public:
...
InsertionPointLocator::InsertionPointLocator(const CppRefactoringChanges &refactoringChanges) : m_refactoringChanges(refactoringChanges) {}

auto InsertionPointLocator::methodDeclarationInClass(const QString &fileName, const Class *clazz, AccessSpec xsSpec, ForceAccessSpec forceAccessSpec) const -> InsertionLocation
{
  const Document::Ptr doc = m_refactoringChanges.file(Utils::FilePath::fromString(fileName))->cppDocument();
  if (doc) {
    FindInClass find(doc->translationUnit(), clazz);
    auto classAST = find();
    return methodDeclarationInClass(doc->translationUnit(), classAST, xsSpec, AccessSpecEnd, forceAccessSpec);
  } else {
    return InsertionLocation();
  }
}

auto InsertionPointLocator::methodDeclarationInClass(const TranslationUnit *tu, const ClassSpecifierAST *clazz, InsertionPointLocator::AccessSpec xsSpec, Position pos, ForceAccessSpec forceAccessSpec) const -> InsertionLocation
{
  if (!clazz)
    return {};
  QList<AccessRange> ranges = collectAccessRanges(tu, clazz->member_specifier_list, tu->tokenKind(clazz->classkey_token) == T_CLASS ? InsertionPointLocator::Private : InsertionPointLocator::Public, clazz->lbrace_token, clazz->rbrace_token);

  unsigned beforeToken = 0;
  auto needsLeadingEmptyLine = false;
  auto needsPrefix = false;
  auto needsSuffix = false;
  findMatch(ranges, xsSpec, pos, forceAccessSpec, beforeToken, needsLeadingEmptyLine, needsPrefix, needsSuffix);
...
auto symbolsAccessSpec(Symbol *symbol) -> InsertionPointLocator::AccessSpec
{
  if (symbol->isPrivate())
...
auto InsertionPointLocator::constructorDeclarationInClass(const CPlusPlus::TranslationUnit *tu, const ClassSpecifierAST *clazz, InsertionPointLocator::AccessSpec xsSpec, int constructorArgumentCount) const -> InsertionLocation
{
...
public:
  ...
  HighestValue(const Key &initialKey, const Value &initialValue) : _key(initialKey), _value(initialValue), _set(true) {}

  auto maybeSet(const Key &key, const Value &value) -> void
  {
    if (!_set || key > _key) {
      _value = value;
      _key = key;
...
public:
  explicit FindMethodDefinitionInsertPoint(TranslationUnit *translationUnit) : ASTVisitor(translationUnit) {}

  auto operator()(Symbol *decl, int *line, int *column) -> void
  {
...
class FindFunctionDefinition : protected ASTVisitor {
  ...
  int _column = 0;
public:
  explicit FindFunctionDefinition(TranslationUnit *translationUnit) : ASTVisitor(translationUnit) { }

  auto operator()(int line, int column) -> FunctionDefinitionAST*
  {
    _result = nullptr;
    _line = line;
    _column = column;
...
static auto isNonVirtualFunctionDeclaration(Symbol *s) -> Declaration*
{
  if (!s)
    return nullptr;
...
static auto nextToSurroundingDefinitions(Symbol *declaration, const CppRefactoringChanges &changes, const QString &destinationFile) -> InsertionLocation
{
  ...
  for (auto i = declIndex - 1; i >= 0; --i) {
    Symbol *s = klass->memberAt(i);
    if (s->isGenerated() || !(surroundingFunctionDecl = isNonVirtualFunctionDeclaration(s)))
      continue;
    if ((definitionFunction = symbolFinder.findMatchingDefinition(surroundingFunctionDecl, changes.snapshot()))) {
      if (destinationFile.isEmpty() || destinationFile == QString::fromUtf8(definitionFunction->fileName(), definitionFunction->fileNameLength())) {
        prefix = QLatin1String("\n\n");
        break;
      }
  ...
  if (!definitionFunction) {
    ...
    for (auto i = declIndex + 1; i < klass->memberCount(); ++i) {
      Symbol *s = klass->memberAt(i);
  ...
  if (suffix.isEmpty()) {
    ...
    CppRefactoringFilePtr targetFile = changes.file(Utils::FilePath::fromString(QString::fromUtf8(definitionFunction->fileName())));
    ...
    for (auto it = path.rbegin(); it != path.rend(); ++it) {
      if (const auto templateDecl = (*it)->asTemplateDeclaration()) {
        if (templateDecl->declaration == functionDefinition) {
          targetFile->cppDocument()->translationUnit()->getTokenStartPosition(templateDecl->firstToken(), &line, &column);
        }
        break;
      }
    }
...
auto InsertionPointLocator::methodDefinition(Symbol *declaration, bool useSymbolFinder, const QString &destinationFile) const -> const QList<InsertionLocation>
{
  ...
  const auto location = nextToSurroundingDefinitions(declaration, m_refactoringChanges, destinationFile);
  if (location.isValid()) {
    result += location;
    return result;
  }

  const auto declFileName = QString::fromUtf8(declaration->fileName(), declaration->fileNameLength());
  auto target = declFileName;
  if (!ProjectFile::isSource(ProjectFile::classify(declFileName))) {
    auto candidate = correspondingHeaderOrSource(declFileName);
    if (!candidate.isEmpty())
      target = candidate;
  }

  CppRefactoringFilePtr targetFile = m_refactoringChanges.file(Utils::FilePath::fromString(target));
 ...
 */
static auto getListOfMissingNamespacesForLocation(const CppRefactoringFile *file, const QStringList &wantedNamespaces, InsertionLocation loc) -> QStringList
{
 ...
 * at the last index
 */
auto getNamespaceNames(const Namespace *firstNamespace) -> QStringList
{
  QStringList namespaces;
  for (auto scope = firstNamespace; scope; scope = scope->enclosingNamespace()) {
    if (scope->name() && scope->name()->identifier()) {
      namespaces.prepend(QString::fromUtf8(scope->name()->identifier()->chars(), scope->name()->identifier()->size()));
    } else {
 ...
 * at the last index
 */
auto getNamespaceNames(const Symbol *symbol) -> QStringList
{
  return getNamespaceNames(symbol->enclosingNamespace());
}

auto insertLocationForMethodDefinition(Symbol *symbol, const bool useSymbolFinder, NamespaceHandling namespaceHandling, const CppRefactoringChanges &refactoring, const QString &fileName, QStringList *insertedNamespaces) -> InsertionLocation
{
  ...
  const auto list = locator.methodDefinition(symbol, useSymbolFinder, fileName);
  const auto isHeader = ProjectFile::isHeader(ProjectFile::classify(fileName));
  const bool hasIncludeGuard = isHeader && !file->cppDocument()->includeGuardMacroName().isEmpty();
  int lastLine;
  if (hasIncludeGuard) {
  ...
  auto i = 0;
  for (; i < list.count(); ++i) {
    auto location = list.at(i);
    if (!location.isValid() || location.fileName() != fileName)
      continue;
    if (hasIncludeGuard && location.line() == lastLine)
      continue;
    if (!requiredNamespaces.isEmpty()) {
      QStringList missing = getListOfMissingNamespacesForLocation(file.get(), requiredNamespaces, location);
      if (!missing.isEmpty())
        continue;
    }
    return location;
