...
auto CppLocatorFilter::filterEntryFromIndexItem(IndexItem::Ptr info) -> Orca::Plugin::Core::LocatorFilterEntry
{
  const auto id = QVariant::fromValue(info);
  Orca::Plugin::Core::LocatorFilterEntry filterEntry(this, info->scopedSymbolName(), id, info->icon());
...
auto CppLocatorFilter::matchesFor(QFutureInterface<Orca::Plugin::Core::LocatorFilterEntry> &future, const QString &entry) -> QList<Orca::Plugin::Core::LocatorFilterEntry>
{
  QList<Orca::Plugin::Core::LocatorFilterEntry> entries[int(MatchLevel::Count)];
    ...
    if (type & wanted) {
      ...
      if (match.hasMatch()) {
        ...
        if (matchInParameterList && filterEntry.highlight_info.starts.isEmpty()) {
          ...
          filterEntry.highlight_info = highlightInfo(match);
          filterEntry.highlight_info.dataType = Orca::Plugin::Core::LocatorFilterEntry::HighlightInfo::ExtraInfo;
  ...
  for (auto &entry : entries) {
    if (entry.size() < 1000)
      Utils::sort(entry, Orca::Plugin::Core::LocatorFilterEntry::compareLexigraphically);
  }

  return std::accumulate(std::begin(entries), std::end(entries), QList<Orca::Plugin::Core::LocatorFilterEntry>());
}

auto CppLocatorFilter::accept(const Orca::Plugin::Core::LocatorFilterEntry &selection, QString *newText, int *selectionStart, int *selectionLength) const -> void
{
  Q_UNUSED(newText)
  Q_UNUSED(selectionStart)
  Q_UNUSED(selectionLength)
  auto info = qvariant_cast<IndexItem::Ptr>(selection.internal_data);
  Orca::Plugin::Core::EditorManager::openEditorAt({Utils::FilePath::fromString(info->fileName()), info->line(), info->column()}, {}, Orca::Plugin::Core::EditorManager::AllowExternalEditor);
}
...
auto CppClassesFilter::filterEntryFromIndexItem(IndexItem::Ptr info) -> Orca::Plugin::Core::LocatorFilterEntry
{
  const auto id = QVariant::fromValue(info);
  Orca::Plugin::Core::LocatorFilterEntry filterEntry(this, info->symbolName(), id, info->icon());
...
auto CppFunctionsFilter::filterEntryFromIndexItem(IndexItem::Ptr info) -> Orca::Plugin::Core::LocatorFilterEntry
{
  const auto id = QVariant::fromValue(info);
  ...
  Orca::Plugin::Core::LocatorFilterEntry filterEntry(this, name + info->symbolType(), id, info->icon());
