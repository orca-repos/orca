...
static QTextCursor openEditor(const QString &text)
{
  ...
  Orca::Plugin::Core::IEditor *editor = Orca::Plugin::Core::EditorManager::openEditorWithContents(Constants::CPPEDITOR_ID, &name, text.toLocal8Bit());
...
void AutoCompleterTest::testAutoComplete()
{
  ...
  Utils::ExecuteOnDestruction guard([]() {
    Orca::Plugin::Core::EditorManager::closeAllEditors(false);
  });
