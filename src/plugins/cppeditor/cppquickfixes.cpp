...
#include <core/core-message-box.hpp>
...
auto CppQuickFixFactory::cppQuickFixFactories() -> const QList<CppQuickFixFactory*>&
{
...
auto inlinePrefix(const QString &targetFile, const std::function<bool()> &extraCondition = {}) -> QString
{
  if (ProjectFile::isHeader(ProjectFile::classify(targetFile)) && (!extraCondition || extraCondition())) {
    return "inline ";
  }
  return {};
}
...
inline auto isQtStringLiteral(const QByteArray &id) -> bool
{
  return id == "QLatin1String" || id == "QLatin1Literal" || id == "QStringLiteral";
}

inline auto isQtStringTranslation(const QByteArray &id) -> bool
{
  return id == "tr" || id == "trUtf8" || id == "translate" || id == "QT_TRANSLATE_NOOP";
}

auto isMemberFunction(const LookupContext &context, Function *function) -> Class*
{
...
auto isNamespaceFunction(const LookupContext &context, Function *function) -> Namespace*
{
  QTC_ASSERT(function, return nullptr);
...
// Given include is e.g. "afile.hpp" or <afile.hpp> (quotes/angle brackets included!).
auto insertNewIncludeDirective(const QString &include, CppRefactoringFilePtr file, const Document::Ptr &cppDocument) -> void
{
  // Find optimal position
  using namespace IncludeUtils;
  LineForNewIncludeDirective finder(file->document(), cppDocument, LineForNewIncludeDirective::IgnoreMocIncludes, LineForNewIncludeDirective::AutoDetect);
  unsigned newLinesToPrepend = 0;
  unsigned newLinesToAppend = 0;
  const auto insertLine = finder(include, &newLinesToPrepend, &newLinesToAppend);
  QTC_ASSERT(insertLine >= 1, return);
  const auto insertPosition = file->position(insertLine, 1);
...
auto nameIncludesOperatorName(const Name *name) -> bool
{
  return name->isOperatorNameId() || (name->isQualifiedNameId() && name->asQualifiedNameId()->name()->isOperatorNameId());
}

auto memberBaseName(const QString &name) -> QString
{
  ...
  auto settings = CppQuickFixProjectsSettings::getQuickFixSettings(ProjectExplorer::ProjectTree::currentProject());
  const auto &nameTemplate = settings->memberVariableNameTemplate;
  const auto prefix = nameTemplate.left(nameTemplate.indexOf('<'));
  const auto postfix = nameTemplate.mid(nameTemplate.lastIndexOf('>') + 1);
  if (name.startsWith(prefix) && name.endsWith(postfix)) {
    const auto base = name.mid(prefix.length(), name.length() - postfix.length());
    if (validName(base))
      return base;
  }
  ...
  if (baseName.startsWith(QLatin1String("m_"))) {
    baseName.remove(0, 2);
  } else if (baseName.startsWith(QLatin1Char('m')) && baseName.length() > 1 && baseName.at(1).isUpper()) {
    baseName.remove(0, 1);
...
auto astForClassOperations(const CppQuickFixInterface &interface) -> ClassSpecifierAST*
{
...
public:
  InverseLogicalComparisonOp(const CppQuickFixInterface &interface, int priority, BinaryExpressionAST *binary, Kind invertToken) : CppQuickFixOperation(interface, priority), binary(binary)
  {
    Token tok;
...
auto InverseLogicalComparison::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
...
public:
  FlipLogicalOperandsOp(const CppQuickFixInterface &interface, int priority, BinaryExpressionAST *binary, QString replacement) : CppQuickFixOperation(interface), binary(binary), replacement(replacement)
  {
...
auto FlipLogicalOperands::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  if (path.isEmpty())
    return;
...
public:
  ...
  RewriteLogicalAndOp(const CppQuickFixInterface &interface) : CppQuickFixOperation(interface), mk(new ASTPatternBuilder)
  {
...
auto RewriteLogicalAnd::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  ...
  if (expression->match(op->pattern, &matcher) && file->tokenAt(op->pattern->binary_op_token).is(T_AMPER_AMPER) && file->tokenAt(op->left->unary_op_token).is(T_EXCLAIM) && file->tokenAt(op->right->unary_op_token).is(T_EXCLAIM)) {
    op->setDescription(QApplication::translate("CppEditor::QuickFix", "Rewrite Condition Using ||"));
    op->setPriority(index);
    result.append(op);
  }
}

static auto checkDeclarationForSplit(SimpleDeclarationAST *declaration) -> bool
{
...
public:
  SplitSimpleDeclarationOp(const CppQuickFixInterface &interface, int priority, SimpleDeclarationAST *decl) : CppQuickFixOperation(interface, priority), declaration(decl)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Split Declaration"));
  }
...
auto SplitSimpleDeclaration::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  CoreDeclaratorAST *core_declarator = nullptr;
  const QList<AST*> &path = interface.path();
  auto file = interface.currentFile();
  const auto cursorPosition = file->cursor().selectionStart();
...
public:
  AddBracesToIfOp(const CppQuickFixInterface &interface, int priority, const IfStatementAST *statement) : CppQuickFixOperation(interface, priority), _statement(statement)
  {
...
auto AddBracesToIf::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  if (path.isEmpty())
    return;
  ...
  if (ifStatement && interface.isCursorOn(ifStatement->if_token) && ifStatement->statement && !ifStatement->statement->asCompoundStatement()) {
  ...
  for (; index != -1; --index) {
    IfStatementAST *ifStatement = path.at(index)->asIfStatement();
    if (ifStatement && ifStatement->statement && interface.isCursorOn(ifStatement->statement) && !ifStatement->statement->asCompoundStatement()) {
      result << new AddBracesToIfOp(interface, index, ifStatement);
      return;
    }
  }
...
public:
  MoveDeclarationOutOfIfOp(const CppQuickFixInterface &interface) : CppQuickFixOperation(interface)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Move Declaration out of Condition"));
...
auto MoveDeclarationOutOfIf::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
...
public:
  MoveDeclarationOutOfWhileOp(const CppQuickFixInterface &interface) : CppQuickFixOperation(interface)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Move Declaration out of Condition"));
    reset();
  }
...
auto MoveDeclarationOutOfWhile::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
...
public:
  SplitIfStatementOp(const CppQuickFixInterface &interface, int priority, IfStatementAST *pattern, BinaryExpressionAST *condition) : CppQuickFixOperation(interface, priority), pattern(pattern), condition(condition)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Split if Statement"));
  }
  ...
  auto splitAndCondition(CppRefactoringFilePtr currentFile) const -> void
  {
    ChangeSet changes;
  ...
  auto splitOrCondition(CppRefactoringFilePtr currentFile) const -> void
  {
    ChangeSet changes;
...
auto SplitIfStatement::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  IfStatementAST *pattern = nullptr;
  const QList<AST*> &path = interface.path();
 ...
 * special cases "a" --> 'a', "'" --> '\'', "\n" --> '\n', "\"" --> '"'. */
static auto stringToCharEscapeSequences(const QByteArray &content) -> QByteArray
{
  if (content.size() == 1)
 ...
 * 'a' -> "a", '\n' -> "\n", '\'' --> "'", '"' --> "\"". */
static auto charToStringEscapeSequences(const QByteArray &content) -> QByteArray
{
  if (content.size() == 1)
    return content.at(0) == '"' ? QByteArray("\\\"") : content;
  if (content.size() == 2)
...
static auto msgQtStringLiteralDescription(const QString &replacement) -> QString
{
  return QApplication::translate("CppEditor::QuickFix", "Enclose in %1(...)").arg(replacement);
}

static auto stringLiteralReplacement(unsigned actions) -> QString
{
...
static auto analyzeStringLiteral(const QList<AST*> &path, const CppRefactoringFilePtr &file, StringLiteralType *type, QByteArray *enclosingFunction = nullptr, CallAST **enclosingFunctionCall = nullptr) -> ExpressionAST*
{
  *type = TypeNone;
...
public:
  WrapStringLiteralOp(const CppQuickFixInterface &interface, int priority, unsigned actions, const QString &description, ExpressionAST *literal, const QString &translationContext = QString()) : CppQuickFixOperation(interface, priority), m_actions(actions), m_literal(literal), m_translationContext(translationContext)
  {
  ...
  auto perform() -> void override
  {
    ...
    if (m_actions & (EncloseActionMask | TranslationMask)) {
      ...
      auto leading = stringLiteralReplacement(m_actions);
      leading += QLatin1Char('(');
...
auto WrapStringLiteral::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  auto type = TypeNone;
  ...
  if (!literal || type == TypeNone)
    return;
  if ((type == TypeChar && enclosingFunction == "QLatin1Char") || isQtStringLiteral(enclosingFunction) || isQtStringTranslation(enclosingFunction))
    return;

  const int priority = path.size() - 1; // very high priority
  if (type == TypeChar) {
    unsigned actions = EncloseInQLatin1CharAction;
    auto description = msgQtStringLiteralDescription(stringLiteralReplacement(actions));
    ...
    if (NumericLiteralAST *charLiteral = literal->asNumericLiteral()) {
      ...
      if (!charToStringEscapeSequences(contents).isEmpty()) {
        ...
        description = QApplication::translate("CppEditor::QuickFix", "Convert to String Literal");
        result << new WrapStringLiteralOp(interface, priority, actions, description, literal);
      }
    }
  } else {
    const auto objectiveCActions = type == TypeObjCString ? unsigned(RemoveObjectiveCAction) : 0u;
    unsigned actions = 0;
    if (StringLiteralAST *stringLiteral = literal->asStringLiteral()) {
      ...
      if (!stringToCharEscapeSequences(contents).isEmpty()) {
        actions = EncloseInQLatin1CharAction | SingleQuoteAction | ConvertEscapeSequencesToCharAction | objectiveCActions;
        auto description = QApplication::translate("CppEditor::QuickFix", "Convert to Character Literal and Enclose in QLatin1Char(...)");
        result << new WrapStringLiteralOp(interface, priority, actions, description, literal);
        actions &= ~EncloseInQLatin1CharAction;
        description = QApplication::translate("CppEditor::QuickFix", "Convert to Character Literal");
        result << new WrapStringLiteralOp(interface, priority, actions, description, literal);
      }
    }
    actions = EncloseInQLatin1StringAction | objectiveCActions;
    result << new WrapStringLiteralOp(interface, priority, actions, msgQtStringLiteralDescription(stringLiteralReplacement(actions)), literal);
    actions = EncloseInQStringLiteralAction | objectiveCActions;
    result << new WrapStringLiteralOp(interface, priority, actions, msgQtStringLiteralDescription(stringLiteralReplacement(actions)), literal);
  }
}

auto TranslateStringLiteral::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  // Initialize
  auto type = TypeNone;
  QByteArray enclosingFunction;
  ...
  auto file = interface.currentFile();
  ExpressionAST *literal = analyzeStringLiteral(path, file, &type, &enclosingFunction);
  if (!literal || type != TypeString || isQtStringLiteral(enclosingFunction) || isQtStringTranslation(enclosingFunction))
    return;
  ...
  const auto description = QApplication::translate("CppEditor::QuickFix", "Mark as Translatable");
  for (int i = path.size() - 1; i >= 0; --i) {
    if (FunctionDefinitionAST *definition = path.at(i)->asFunctionDefinition()) {
      ...
      if (b) {
        ...
        foreach(const LookupItem &r, b->find(trName)) {
          ...
          if (s->type()->isFunctionType()) {
            ...
            result << new WrapStringLiteralOp(interface, path.size() - 1, TranslateTrAction, description, literal);
            return;
          }
        }
      }
      ...
      result << new WrapStringLiteralOp(interface, path.size() - 1, TranslateQCoreApplicationAction, description, literal, trContext);
      return;
    }
  }

  // We need to use Q_TRANSLATE_NOOP
  result << new WrapStringLiteralOp(interface, path.size() - 1, TranslateNoopAction, description, literal, trContext);
}
...
public:
  ConvertCStringToNSStringOp(const CppQuickFixInterface &interface, int priority, StringLiteralAST *stringLiteral, CallAST *qlatin1Call) : CppQuickFixOperation(interface, priority), stringLiteral(stringLiteral), qlatin1Call(qlatin1Call)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Convert to Objective-C String Literal"));
  }
...
auto ConvertCStringToNSString::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  auto file = interface.currentFile();
  ...
  CallAST *qlatin1Call;
  const QList<AST*> &path = interface.path();
  ExpressionAST *literal = analyzeStringLiteral(path, file, &type, &enclosingFunction, &qlatin1Call);
  if (!literal || type != TypeString)
    return;
  if (!isQtStringLiteral(enclosingFunction))
    qlatin1Call = nullptr;

  result << new ConvertCStringToNSStringOp(interface, path.size() - 1, literal->asStringLiteral(), qlatin1Call);
}
...
public:
  ConvertNumericLiteralOp(const CppQuickFixInterface &interface, int start, int end, const QString &replacement) : CppQuickFixOperation(interface), start(start), end(end), replacement(replacement) {}
...
auto ConvertNumericLiteral::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  auto file = interface.currentFile();
  ...
  if (!numeric->isHex()) {
    ...
    */
    const auto replacement = QString::asprintf("0x%lX", value);
  ...
  if (!isOctal) {
    /*
      ...
      With
        040
    */
    const auto replacement = QString::asprintf("0%lo", value);
    auto op = new ConvertNumericLiteralOp(interface, start, start + numberLength, replacement);
    ...
    /*
      ...
      Replace
       ...
       With
        32
    */
    const auto replacement = QString::asprintf("%lu", value);
    auto op = new ConvertNumericLiteralOp(interface, start, start + numberLength, replacement);
...
public:
  AddLocalDeclarationOp(const CppQuickFixInterface &interface, int priority, const BinaryExpressionAST *binaryAST, const SimpleNameAST *simpleNameAST) : CppQuickFixOperation(interface, priority), binaryAST(binaryAST), simpleNameAST(simpleNameAST)
  {
  ...
  auto perform() -> void override
  {
    ...
    const QList<LookupItem> result = typeOfExpression(currentFile->textOf(binaryAST->right_expression).toUtf8(), scope, TypeOfExpression::Preprocess);
    ...
    if (!result.isEmpty()) {
      ...
      auto oo = CppCodeStyleSettings::currentProjectCodeStyleOverview();
      QString ty = oo.prettyType(tn, simpleNameAST->name);
      if (!ty.isEmpty()) {
        ChangeSet changes;
        changes.replace(currentFile->startOf(binaryAST), currentFile->endOf(simpleNameAST), ty);
        currentFile->setChangeSet(changes);
        currentFile->apply();
      }
    }
...
auto AddLocalDeclaration::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  auto file = interface.currentFile();
  ...
  for (int index = path.size() - 1; index != -1; --index) {
    if (BinaryExpressionAST *binary = path.at(index)->asBinaryExpression()) {
      if (binary->left_expression && binary->right_expression && file->tokenAt(binary->binary_op_token).is(T_EQUAL)) {
        ...
        if (interface.isCursorOn(binary->left_expression) && idExpr && idExpr->name->asSimpleName() != nullptr) {
...
public:
  ConvertToCamelCaseOp(const CppQuickFixInterface &interface, const QString &name, const AST *nameAst, bool test) : CppQuickFixOperation(interface, -1), m_name(name), m_nameAst(nameAst), m_isAllUpper(name.isUpper()), m_test(test)
  {
  ...
  auto perform() -> void override
  {
    ...
    auto newName = m_isAllUpper ? m_name.toLower() : m_name;
    for (auto i = 1; i < newName.length(); ++i) {
      const auto c = newName.at(i);
  ...
  static auto isConvertibleUnderscore(const QString &name, int pos) -> bool
  {
    return name.at(pos) == QLatin1Char('_') && name.at(pos + 1).isLetter() && !(pos == 1 && name.at(0) == QLatin1Char('m'));
  }
...
auto ConvertToCamelCase::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  ...
  if (nameString.length() < 3)
    return;
  for (auto i = 1; i < nameString.length() - 1; ++i) {
...
AddIncludeForUndefinedIdentifierOp::AddIncludeForUndefinedIdentifierOp(const CppQuickFixInterface &interface, int priority, const QString &include) : CppQuickFixOperation(interface, priority), m_include(include)
{
  setDescription(QApplication::translate("CppEditor::QuickFix", "Add #include %1").arg(m_include));
}

auto AddIncludeForUndefinedIdentifierOp::perform() -> void
{
  CppRefactoringChanges refactoring(snapshot());
...
AddForwardDeclForUndefinedIdentifierOp::AddForwardDeclForUndefinedIdentifierOp(const CppQuickFixInterface &interface, int priority, const QString &fqClassName, int symbolPos) : CppQuickFixOperation(interface, priority), m_className(fqClassName), m_symbolPos(symbolPos)
{
  setDescription(QApplication::translate("CppEditor::QuickFix", "Add forward declaration for %1").arg(m_className));
}

auto AddForwardDeclForUndefinedIdentifierOp::perform() -> void
{
  const auto parts = m_className.split("::");
  QTC_ASSERT(!parts.isEmpty(), return);
  const auto namespaces = parts.mid(0, parts.length() - 1);
  ...
  const auto stringToInsert = [&visitor, symbol = parts.last()] {
    QString s = "\n";
    for (const auto &ns : visitor.remainingNamespaces())
      s += "namespace " + ns + " { ";
    s += "class " + symbol + ';';
    for (auto i = 0; i < visitor.remainingNamespaces().size(); ++i)
      s += " }";
    return s;
  };
  ...
  if (visitor.enclosingNamespace()) {
    ...
    const QTextCursor tc = file->document()->find(QRegularExpression("^\\s*#include .*$"), m_symbolPos, QTextDocument::FindBackward | QTextDocument::FindCaseSensitively);
    if (!tc.isNull())
...
auto findShortestInclude(const QString currentDocumentFilePath, const QString candidateFilePath, const ProjectExplorer::HeaderPaths &headerPaths) -> QString
{
  QString result;
  ...
  if (fileInfo.path() == QFileInfo(currentDocumentFilePath).path()) {
    ...
    foreach(const ProjectExplorer::HeaderPath &headerPath, headerPaths) {
      if (!candidateFilePath.startsWith(headerPath.path))
        continue;
      auto relativePath = candidateFilePath.mid(headerPath.path.size());
...
auto findMatchingInclude(const QString &className, const ProjectExplorer::HeaderPaths &headerPaths) -> QString
{
  const QStringList candidateFileNames{className, className + ".hpp", className + ".hpp", className.toLower(), className.toLower() + ".hpp", className.toLower() + ".hpp"};
...
auto relevantHeaderPaths(const QString &filePath) -> ProjectExplorer::HeaderPaths
{
  ProjectExplorer::HeaderPaths headerPaths;

  auto modelManager = CppModelManager::instance();
  const auto projectParts = modelManager->projectPart(filePath);
  if (projectParts.isEmpty()) {
    // Not part of any project, better use all include paths than none
...
auto nameUnderCursor(const QList<AST*> &path) -> NameAST*
{
  if (path.isEmpty())
    return nullptr;
...
auto lookUpDefinition(const CppQuickFixInterface &interface, const NameAST *nameAst) -> LookupResult
{
...
auto templateNameAsString(const TemplateNameId *templateName) -> QString
{
  const Identifier *id = templateName->identifier();
  return QString::fromUtf8(id->chars(), id->size());
}

auto forwardingHeaders(const CppQuickFixInterface &interface) -> Snapshot
{
  Snapshot result;
...
auto matchName(const Name *name, QList<Orca::Plugin::Core::LocatorFilterEntry> *matches, QString *className) -> bool
{
  if (!name)
    return false;

  QString simpleName;
  if (auto classesFilter = CppModelManager::instance()->classesFilter()) {
    QFutureInterface<Orca::Plugin::Core::LocatorFilterEntry> dummy;
...
auto AddIncludeForUndefinedIdentifier::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  ...
  QString className;
  QList<Orca::Plugin::Core::LocatorFilterEntry> matches;
  ...
  if (matchName(nameAst->name, &matches, &className)) {
    ...
    const auto forwardHeaders = forwardingHeaders(interface);
    foreach(const Orca::Plugin::Core::LocatorFilterEntry &entry, matches) {
      IndexItem::Ptr info = entry.internal_data.value<IndexItem::Ptr>();
      if (!info || info->symbolName() != className)
        continue;
      indexItems << info;

      auto localForwardHeaders = forwardHeaders;
      ...
      foreach(const Utils::FilePath &header, headerAndItsForwardingHeaders) {
        const QString include = findShortestInclude(currentDocumentFilePath, header.toString(), headerPaths);
        if (include.size() > 2) {
          const auto headerFileName = Utils::FilePath::fromString(info->fileName()).fileName();
          ...
          result << new AddIncludeForUndefinedIdentifierOp(interface, priority, include);
          headers << header;
        }
      }
    }
    ...
    if (lookupResult == LookupResult::NotDeclared && indexItems.size() == 1) {
      ...
      if (indexItems.first()->scopedSymbolName().endsWith(qualifiedName)) {
        const ProjectExplorer::Node *const node = ProjectExplorer::ProjectTree::nodeForFile(interface.filePath());
        auto fileType = node && node->asFileNode() ? node->asFileNode()->fileType() : ProjectExplorer::FileType::Unknown;
        if (fileType == ProjectExplorer::FileType::Unknown && ProjectFile::isHeader(ProjectFile::classify(interface.filePath().toString()))) {
          fileType = ProjectExplorer::FileType::Header;
        }
        if (fileType == ProjectExplorer::FileType::Header) {
          result << new AddForwardDeclForUndefinedIdentifierOp(interface, 0, indexItems.first()->scopedSymbolName(), interface.currentFile()->startOf(nameAst));
        }
      }
    }
  }
  ...
  //           for the given name.
  if (!Utils::contains(headers, [&className](const Utils::FilePath &fp) { return fp.fileName() == className; })) {
...
public:
  ...
  RearrangeParamDeclarationListOp(const CppQuickFixInterface &interface, AST *currentParam, AST *targetParam, Target target) : CppQuickFixOperation(interface), m_currentParam(currentParam), m_targetParam(targetParam)
  {
    QString targetString;
    if (target == TargetPrevious)
      targetString = QApplication::translate("CppEditor::QuickFix", "Switch with Previous Parameter");
    else
      targetString = QApplication::translate("CppEditor::QuickFix", "Switch with Next Parameter");
  ...
  auto perform() -> void override
  {
    ...
    ChangeSet changes;
    changes.flip(currentFile->startOf(m_currentParam), currentFile->endOf(m_currentParam), currentFile->startOf(m_targetParam), targetEndPos);
    currentFile->setChangeSet(changes);
...
auto RearrangeParamDeclarationList::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> path = interface.path();
  ...
  if (prevParamListNode)
    result << new RearrangeParamDeclarationListOp(interface, paramListNode->value, prevParamListNode->value, RearrangeParamDeclarationListOp::TargetPrevious);
  if (paramListNode->next)
    result << new RearrangeParamDeclarationListOp(interface, paramListNode->value, paramListNode->next->value, RearrangeParamDeclarationListOp::TargetNext);
}
...
public:
  ReformatPointerDeclarationOp(const CppQuickFixInterface &interface, const ChangeSet change) : CppQuickFixOperation(interface), m_change(change)
  {
    QString description;
    if (m_change.operationList().size() == 1) {
      description = QApplication::translate("CppEditor::QuickFix", "Reformat to \"%1\"").arg(m_change.operationList().constFirst().text);
    } else {
      // > 1
      description = QApplication::translate("CppEditor::QuickFix", "Reformat Pointers or References");
    }
    setDescription(description);
  }
...
public:
  auto filter(const QList<AST*> &astPathList) -> QList<AST*>
  {
    QList<AST*> filtered;
...
auto ReformatPointerDeclaration::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  auto file = interface.currentFile();

  auto overview = CppCodeStyleSettings::currentProjectCodeStyleOverview();
  overview.showArgumentNames = true;
  overview.showReturnTypes = true;

  const auto cursor = file->cursor();
  ChangeSet change;
  PointerDeclarationFormatter formatter(file, overview, PointerDeclarationFormatter::RespectCursor);
  ...
  if (cursor.hasSelection()) {
    ...
    const QList<AST*> suitableASTs = ReformatPointerDeclarationASTPathResultsFilter().filter(path);
...
public:
  CaseStatementCollector(Document::Ptr document, const Snapshot &snapshot, Scope *scope) : ASTVisitor(document->translationUnit()), document(document), scope(scope)
  {
    typeOfExpression.init(document, snapshot);
  }

  auto operator ()(AST *ast) -> QStringList
  {
    values.clear();
...
public:
  CompleteSwitchCaseStatementOp(const CppQuickFixInterface &interface, int priority, CompoundStatementAST *compoundStatement, const QStringList &values) : CppQuickFixOperation(interface, priority), compoundStatement(compoundStatement), values(values)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Complete Switch Statement"));
  }
...
static auto findEnum(const QList<LookupItem> &results, const LookupContext &ctxt) -> Enum*
{
  foreach(const LookupItem &result, results) {
    const FullySpecifiedType fst = result.type();

    Type *type = result.declaration() ? result.declaration()->type().type() : fst.type();
...
auto conditionEnum(const CppQuickFixInterface &interface, SwitchStatementAST *statement) -> Enum*
{
  ...
  const QList<LookupItem> results = typeOfExpression(statement->condition, interface.semanticInfo().doc, scope);
...
auto CompleteSwitchCaseStatement::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  ...
  for (int depth = path.size() - 1; depth >= 0; --depth) {
    ...
    if (switchStatement) {
      ...
      if (Enum *e = conditionEnum(interface, switchStatement)) {
        ...
        QStringList values;
        Overview prettyPrint;
        for (auto i = 0; i < e->memberCount(); ++i) {
          if (Declaration *decl = e->memberAt(i)->asDeclaration())
            values << prettyPrint.prettyName(LookupContext::fullyQualifiedName(decl));
        }
        // Get the used values
        Block *block = switchStatement->symbol;
        CaseStatementCollector caseValues(interface.semanticInfo().doc, interface.snapshot(), interface.semanticInfo().doc->scopeAt(block->line(), block->column()));
        ...
        if (!values.isEmpty())
          result << new CompleteSwitchCaseStatementOp(interface, depth, compoundStatement, values);
        return;
      }
...
public:
  InsertDeclOperation(const CppQuickFixInterface &interface, const QString &targetFileName, const Class *targetSymbol, InsertionPointLocator::AccessSpec xsSpec, const QString &decl, int priority) : CppQuickFixOperation(interface, priority), m_targetFileName(targetFileName), m_targetSymbol(targetSymbol), m_xsSpec(xsSpec), m_decl(decl)
  {
    setDescription(QCoreApplication::translate("CppEditor::InsertDeclOperation", "Add %1 Declaration").arg(InsertionPointLocator::accessSpecToString(xsSpec)));
  }
  ...
  auto perform() -> void override
  {
    ...
    const InsertionLocation loc = locator.methodDeclarationInClass(m_targetFileName, m_targetSymbol, m_xsSpec);
    QTC_ASSERT(loc.isValid(), return);

    CppRefactoringFilePtr targetFile = refactoring.file(Utils::FilePath::fromString(m_targetFileName));
  ...
  static auto generateDeclaration(const Function *function) -> QString;
...
public:
  DeclOperationFactory(const CppQuickFixInterface &interface, const QString &fileName, const Class *matchingClass, const QString &decl) : m_interface(interface), m_fileName(fileName), m_matchingClass(matchingClass), m_decl(decl) {}

  auto operator()(InsertionPointLocator::AccessSpec xsSpec, int priority) -> QuickFixOperation*
  {
...
auto InsertDeclFromDef::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  auto file = interface.currentFile();
  ...
  if (Class *matchingClass = isMemberFunction(interface.context(), fun)) {
    ...
    for (Symbol *symbol = matchingClass->find(qName->identifier()); symbol; symbol = symbol->next()) {
      Symbol *s = symbol;
      ...
      if (!s->name() || !qName->identifier()->match(s->identifier()) || !s->type()->isFunctionType())
        continue;
    ...
    QString fileName = QString::fromUtf8(matchingClass->fileName(), matchingClass->fileNameLength());
    ...
    result << operation(InsertionPointLocator::Public, 5) << operation(InsertionPointLocator::PublicSlot, 4) << operation(InsertionPointLocator::Protected, 3) << operation(InsertionPointLocator::ProtectedSlot, 2) << operation(InsertionPointLocator::Private, 1) << operation(InsertionPointLocator::PrivateSlot, 0);
  }
}

auto InsertDeclOperation::generateDeclaration(const Function *function) -> QString
{
  auto oo = CppCodeStyleSettings::currentProjectCodeStyleOverview();
...
public:
  ...
  InsertDefOperation(const CppQuickFixInterface &interface, Declaration *decl, DeclaratorAST *declAST, const InsertionLocation &loc, const DefPos defpos, const QString &targetFileName = QString(), bool freeFunction = false) : CppQuickFixOperation(interface, 0), m_decl(decl), m_declAST(declAST), m_loc(loc), m_defpos(defpos), m_targetFileName(targetFileName)
  {
    if (m_defpos == DefPosImplementationFile) {
      const auto declFile = QString::fromUtf8(decl->fileName(), decl->fileNameLength());
      const auto dir = QFileInfo(declFile).dir();
      setPriority(2);
      setDescription(QCoreApplication::translate("CppEditor::InsertDefOperation", "Add Definition in %1").arg(dir.relativeFilePath(m_loc.isValid() ? m_loc.fileName() : m_targetFileName)));
    } else if (freeFunction) {
      setDescription(QCoreApplication::translate("CppEditor::InsertDefOperation", "Add Definition Here"));
    } else if (m_defpos == DefPosInsideClass) {
      setDescription(QCoreApplication::translate("CppEditor::InsertDefOperation", "Add Definition Inside Class"));
    } else if (m_defpos == DefPosOutsideClass) {
      setPriority(1);
      setDescription(QCoreApplication::translate("CppEditor::InsertDefOperation", "Add Definition Outside Class"));
    }
  }

  static auto insertDefinition(const CppQuickFixOperation *op, InsertionLocation loc, DefPos defPos, DeclaratorAST *declAST, Declaration *decl, const QString &targetFilePath, ChangeSet *changeSet = nullptr, QList<ChangeSet::Range> *indentRanges = nullptr) -> void
  {
    CppRefactoringChanges refactoring(op->snapshot());
    if (!loc.isValid())
      loc = insertLocationForMethodDefinition(decl, true, NamespaceHandling::Ignore, refactoring, targetFilePath);
    QTC_ASSERT(loc.isValid(), return);

    CppRefactoringFilePtr targetFile = refactoring.file(Utils::FilePath::fromString(loc.fileName()));
    auto oo = CppCodeStyleSettings::currentProjectCodeStyleOverview();
    oo.showFunctionSignatures = true;
    oo.showReturnTypes = true;
    oo.showArgumentNames = true;
    ...
    oo.showTemplateParameters = decl->enclosingClass() && decl->enclosingClass()->enclosingTemplate();

    if (defPos == DefPosInsideClass) {
      const int targetPos = targetFile->position(loc.line(), loc.column());
      ChangeSet localChangeSet;
      const auto target = changeSet ? changeSet : &localChangeSet;
      ...
      const QString defText = inlinePrefix(targetFilePath, [defPos] { return defPos == DefPosOutsideClass; }) + oo.prettyType(tn, name) + QLatin1String("\n{\n\n}");
      ...
      if (!changeSet) {
        ...
        QTextCursor c = targetFile->cursor();
        c.setPosition(targetPos);
        c.movePosition(QTextCursor::Down, QTextCursor::MoveAnchor, loc.prefix().count(QLatin1String("\n")) + 2);
        c.movePosition(QTextCursor::EndOfLine);
...
auto InsertDefFromDecl::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
        ...
        if (Symbol *symbol = simpleDecl->symbols->value) {
          if (Declaration *decl = symbol->asDeclaration()) {
            if (Function *func = decl->type()->asFunctionType()) {
              ...
              if (symbolFinder.findMatchingDefinition(decl, interface.snapshot(), true)) {
                return;
              }
              ...
              auto kind = ProjectFile::classify(interface.filePath().toString());
              const auto isHeaderFile = ProjectFile::isHeader(kind);
              if (isHeaderFile) {
                ...
                foreach(const InsertionLocation &location, locator.methodDefinition(decl, false, QString())) {
                  if (!location.isValid())
                    continue;
                  ...
                  if (ProjectFile::isHeader(ProjectFile::classify(fileName))) {
                    ...
                    if (!source.isEmpty()) {
                      op = new InsertDefOperation(interface, decl, declAST, InsertionLocation(), DefPosImplementationFile, source);
                    }
                  } else {
                    op = new InsertDefOperation(interface, decl, declAST, InsertionLocation(), DefPosImplementationFile, fileName);
                  }
              ...
              if (!isFreeFunction) {
                result << new InsertDefOperation(interface, decl, declAST, InsertionLocation(), DefPosOutsideClass, interface.filePath().toString());
              }

              // Insert Position: Inside Class
              // Determine insert location direct after the declaration.
              int line, column;
              const auto file = interface.currentFile();
              file->lineAndColumn(file->endOf(simpleDecl), &line, &column);
              const auto loc = InsertionLocation(interface.filePath().toString(), QString(), QString(), line, column);
              result << new InsertDefOperation(interface, decl, declAST, loc, DefPosInsideClass, QString(), isFreeFunction);
...
public:
  InsertMemberFromInitializationOp(const CppQuickFixInterface &interface, const Class *theClass, const QString &member, const QString &type) : CppQuickFixOperation(interface), m_class(theClass), m_member(member), m_type(type)
  {
    setDescription(QCoreApplication::translate("CppEditor::Quickfix", "Add Class Member \"%1\"").arg(m_member));
  }
...
private:
  auto perform() -> void override
  {
    auto type = m_type;
    if (type.isEmpty()) {
      type = QInputDialog::getText(Orca::Plugin::Core::ICore::dialogParent(), QCoreApplication::translate("CppEditor::Quickfix", "Provide the type"), QCoreApplication::translate("CppEditor::Quickfix", "Data type:"), QLineEdit::Normal);
    }
    if (type.isEmpty())
      return;
    ...
    const auto filePath = QString::fromUtf8(m_class->fileName());
    const InsertionLocation loc = locator.methodDeclarationInClass(filePath, m_class, InsertionPointLocator::Private);
    QTC_ASSERT(loc.isValid(), return);
...
auto InsertMemberFromInitialization::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  ...
  if (!theClass) {
    ...
    SymbolFinder finder;
    const QList<Declaration*> matches = finder.findMatchingDeclaration(LookupContext(interface.currentFile()->cppDocument(), interface.snapshot()), ctor->symbol);
    if (!matches.isEmpty())
  ...
  if (theClass->find(interface.currentFile()->cppDocument()->translationUnit()->identifier(name->identifier_token))) {
    return;
  }

  const QString type = getType(interface, memInitializer, ctor);
  const Identifier *const memberId = interface.currentFile()->cppDocument()->translationUnit()->identifier(name->identifier_token);
...
auto InsertMemberFromInitialization::getType(const CppQuickFixInterface &interface, const MemInitializerAST *memInitializer, const FunctionDefinitionAST *ctor) const -> QString
{
  ...
  if (!memInitializer->expression)
    return {};
  const ExpressionListParenAST *const lParenAst = memInitializer->expression->asExpressionListParen();
  ...
  if (!idExpr) {
    // Not a variable, so check for function call.
  ...
  if (matches.isEmpty())
    return {};
  auto o = CppCodeStyleSettings::currentProjectCodeStyleOverview();
...
public:
  AddImplementationsDialog(const QList<Symbol*> &candidates, const Utils::FilePath &implFile) : QDialog(Orca::Plugin::Core::ICore::dialogParent()), m_candidates(candidates)
  {
    ...
    connect(defaultImplTargetComboBox, qOverload<int>(&QComboBox::currentIndexChanged), this, [this](int index) {
      for (auto i = 0; i < m_implTargetBoxes.size(); ++i) {
    ...
    for (auto i = 0; i < m_candidates.size(); ++i) {
      ...
      candidatesLayout->addWidget(new QLabel(oo.prettyType(func->type(), func->name())), i, 0);
  ...
  auto settings() const -> MemberFunctionImplSettings
  {
    ...
    for (auto i = 0; i < m_candidates.size(); ++i) {
      ...
      const auto index = m_implTargetBoxes.at(i)->currentIndex();
      const auto addImplementation = index != 0;
...
public:
  ...
  auto isApplicable() const -> bool { return !m_declarations.isEmpty(); }
  auto setMode(InsertDefsFromDecls::Mode mode) -> void { m_mode = mode; }
...
private:
  auto perform() -> void override
  {
    ...
    SymbolFinder symbolFinder;
    for (const auto s : qAsConst(m_declarations)) {
    ...
    CppRefactoringChanges refactoring(snapshot());
    const auto isHeaderFile = ProjectFile::isHeader(ProjectFile::classify(filePath().toString()));
    QString cppFile; // Only set if the class is defined in a header file.
    if (isHeaderFile) {
      InsertionPointLocator locator(refactoring);
      for (const auto &location : locator.methodDefinition(unimplemented.first(), false, {})) {
        if (!location.isValid())
          continue;
        const auto fileName = location.fileName();
        if (ProjectFile::isHeader(ProjectFile::classify(fileName))) {
          const auto source = correspondingHeaderOrSource(fileName);
          if (!source.isEmpty())
            cppFile = source;
        } else {
          cppFile = fileName;
    ...
    case InsertDefsFromDecls::Mode::Alternating: {
      ...
      for (const auto func : qAsConst(unimplemented)) {
        incDefPos();
        if (defPos > DefPosImplementationFile)
          continue;
        MemberFunctionImplSetting setting;
    ...
    public:
      DeclFinder(const CppRefactoringFile *file, const Symbol *func) : ASTVisitor(file->cppDocument()->translationUnit()), m_func(func) {}

      auto decl() const -> SimpleDeclarationAST* { return m_decl; }

    private:
      auto visit(SimpleDeclarationAST *decl) -> bool override
      {
        if (m_decl)
          return false;
    ...
    for (const auto &setting : qAsConst(settings)) {
      ...
      const auto targetFilePath = setting.defPos == DefPosImplementationFile ? cppFile : filePath().toString();
      ...
      InsertDefOperation::insertDefinition(this, loc, setting.defPos, finder.decl()->declarator_list->value, setting.func->asDeclaration(), targetFilePath, &changeSet.first, &changeSet.second);
    }
    for (auto it = changeSets.cbegin(); it != changeSets.cend(); ++it) {
      const CppRefactoringFilePtr file = refactoring.file(Utils::FilePath::fromString(it.key()));
      for (const auto &r : it.value().second)
...
auto InsertDefsFromDecls::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
...
auto getFirstTemplateParameter(const Name *name) -> Utils::optional<FullySpecifiedType>
{
  if (const QualifiedNameId *qualifiedName = name->asQualifiedNameId())
...
auto getFirstTemplateParameter(Type *type) -> Utils::optional<FullySpecifiedType>
{
...
auto getFirstTemplateParameter(FullySpecifiedType type) -> Utils::optional<FullySpecifiedType>
{
  return getFirstTemplateParameter(type.type());
}

auto symbolAtDifferentLocation(const CppQuickFixInterface &interface, Symbol *symbol, const CppRefactoringFilePtr &targetFile, InsertionLocation targetLocation) -> QString
{
  QTC_ASSERT(symbol, return QString());
  Scope *scopeAtInsertPos = targetFile->cppDocument()->scopeAt(targetLocation.line(), targetLocation.column());
...
auto typeAtDifferentLocation(const CppQuickFixInterface &interface, FullySpecifiedType type, Scope *originalScope, const CppRefactoringFilePtr &targetFile, InsertionLocation targetLocation, const QStringList &newNamespaceNamesAtLoc = {}) -> FullySpecifiedType
{
  Scope *scopeAtInsertPos = targetFile->cppDocument()->scopeAt(targetLocation.line(), targetLocation.column());
  for (const auto &nsName : newNamespaceNamesAtLoc) {
    const auto utf8Name = nsName.toUtf8();
...
struct ExistingGetterSetterData {
  ...
  auto computePossibleFlags() const -> int;
};

class GetterSetterRefactoringHelper {
public:
  GetterSetterRefactoringHelper(CppQuickFixOperation *operation, const QString &fileName, Class *clazz) : m_operation(operation), m_changes(m_operation->snapshot()), m_locator(m_changes), m_headerFile(m_changes.file(Utils::FilePath::fromString(fileName))), m_sourceFile([&] {
    auto cppFileName = correspondingHeaderOrSource(fileName, &m_isHeaderHeaderFile);
  ...
  auto performGeneration(ExistingGetterSetterData data, int generationFlags) -> void;

  auto applyChanges() -> void
  {
    const auto classLayout = {InsertionPointLocator::Public, InsertionPointLocator::PublicSlot, InsertionPointLocator::Signals, InsertionPointLocator::Protected, InsertionPointLocator::ProtectedSlot, InsertionPointLocator::PrivateSlot, InsertionPointLocator::Private,};
    for (auto spec : classLayout) {
      const auto iter = m_headerFileCode.find(spec);
      if (iter != m_headerFileCode.end()) {
        const auto loc = headerLocationFor(spec);
    ...
    if (!m_sourceFileCode.isEmpty() && m_sourceFileInsertionPoint.isValid()) {
      m_sourceFile->setOpenEditor(true, m_sourceFile->position(m_sourceFileInsertionPoint.line(), m_sourceFileInsertionPoint.column()));
  ...
  auto hasSourceFile() const -> bool { return m_headerFile != m_sourceFile; }
  auto isHeaderHeaderFile() const -> bool { return m_isHeaderHeaderFile; }

protected:
  auto insertAndIndent(const RefactoringFilePtr &file, const InsertionLocation &loc, const QString &text) -> void
  {
    auto targetPosition1 = file->position(loc.line(), loc.column());
    auto targetPosition2 = qMax(0, file->position(loc.line(), 1) - 1);
    auto &changeSet = file == m_headerFile ? m_headerFileChangeSet : m_sourceFileChangeSet;
  ...
  auto makeConstRef(FullySpecifiedType type) -> FullySpecifiedType
  {
    type.setConst(true);
    return m_operation->currentFile()->cppDocument()->control()->referenceType(type, false);
  }

  auto addConstToReference(FullySpecifiedType type) -> FullySpecifiedType
  {
  ...
  auto symbolAt(Symbol *symbol, const CppRefactoringFilePtr &targetFile, InsertionLocation targetLocation) -> QString
  {
    return symbolAtDifferentLocation(*m_operation, symbol, targetFile, targetLocation);
  }

  auto typeAt(FullySpecifiedType type, Scope *originalScope, const CppRefactoringFilePtr &targetFile, InsertionLocation targetLocation, const QStringList &newNamespaceNamesAtLoc = {}) -> FullySpecifiedType
  {
    return typeAtDifferentLocation(*m_operation, type, originalScope, targetFile, targetLocation, newNamespaceNamesAtLoc);
  }
   ...
   */
  auto isValueType(FullySpecifiedType type, Scope *enclosingScope, bool *customValueType = nullptr) -> bool
  {
    if (customValueType)
    ...
    const auto isTypeValueType = [](const FullySpecifiedType &t) {
      return t->isPointerType() || t->isEnumType() || t->isIntegerType() || t->isFloatType() || t->isReferenceType();
    };
    if (type->isNamedType()) {
      // we need a recursive search and a lookup context
      LookupContext context(m_headerFile->cppDocument(), m_changes.snapshot());
      auto isValueType = [settings = m_settings, &customValueType, &context, &isTypeValueType](const Name *name, Scope *scope, auto &isValueType) mutable -> bool {
        ...
        for (auto &&i : localLookup) {
          if (isTypeValueType(i.type()))
            return true;
          if (i.type()->isNamedType()) {
            // check if we have to search recursively
            const Name *newName = i.type()->asNamedType()->name();
            Scope *newScope = i.declaration()->enclosingScope();
            if (Matcher::match(newName, name) && Matcher::match(newScope->name(), scope->name())) {
  ...
  auto isValueType(Symbol *symbol, bool *customValueType = nullptr) -> bool
  {
    return isValueType(symbol->type(), symbol->enclosingScope(), customValueType);
  }

  auto addHeaderCode(InsertionPointLocator::AccessSpec spec, QString code) -> void
  {
    auto &existing = m_headerFileCode[spec];
    existing += code;
    if (!existing.endsWith('\n'))
      existing += '\n';
  }

  auto headerLocationFor(InsertionPointLocator::AccessSpec spec) -> InsertionLocation
  {
    const auto insertionPoint = m_headerInsertionPoints.find(spec);
    if (insertionPoint != m_headerInsertionPoints.end())
      return *insertionPoint;
    const InsertionLocation loc = m_locator.methodDeclarationInClass(m_headerFile->filePath().toString(), m_class, spec, InsertionPointLocator::ForceAccessSpec::Yes);
    m_headerInsertionPoints.insert(spec, loc);
    return loc;
  }

  auto sourceLocationFor(Symbol *symbol, QStringList *insertedNamespaces = nullptr) -> InsertionLocation
  {
    if (m_sourceFileInsertionPoint.isValid())
      return m_sourceFileInsertionPoint;
    m_sourceFileInsertionPoint = insertLocationForMethodDefinition(symbol, false, m_settings->createMissingNamespacesinCppFile() ? NamespaceHandling::CreateMissing : NamespaceHandling::Ignore, m_changes, m_sourceFile->filePath().toString(), insertedNamespaces);
    if (m_settings->addUsingNamespaceinCppFile()) {
      // check if we have to insert a using namespace ...
      auto requiredNamespaces = getNamespaceNames(symbol->isClass() ? symbol : symbol->enclosingClass());
      NSCheckerVisitor visitor(m_sourceFile.get(), requiredNamespaces, m_sourceFile->position(m_sourceFileInsertionPoint.line(), m_sourceFileInsertionPoint.column()));
      ...
      if (auto rns = visitor.remainingNamespaces(); !rns.empty()) {
        ...
        for (auto &n : rns) {
          if (!n.isEmpty()) {
            // we have to ignore unnamed namespaces
            ns += n;
            ns += "::";
            if (insertedNamespaces)
        ...
        m_sourceFileInsertionPoint = InsertionLocation(loc.fileName(), loc.prefix() + ns, loc.suffix(), loc.line(), loc.column());
      }
    }
    return m_sourceFileInsertionPoint;
  }

  auto addSourceFileCode(QString code) -> void
  {
...
protected:
  ...
  CppQuickFixSettings *const m_settings = CppQuickFixProjectsSettings::getQuickFixSettings(ProjectExplorer::ProjectTree::currentProject());
  Class *const m_class;
...
public:
  ...
  GenerateGetterSetterOp(const CppQuickFixInterface &interface, ExistingGetterSetterData data, int generateFlags, int priority, const QString &description) : CppQuickFixOperation(interface), m_generateFlags(generateFlags), m_data(data)
  {
    setDescription(description);
    setPriority(priority);
  }

  static auto generateQuickFixes(QuickFixOperations &results, const CppQuickFixInterface &interface, const ExistingGetterSetterData &data, const int possibleFlags) -> void
  {
    ...
    if (possibleFlags & HaveExistingQProperty) {
      ...
      if (possibleFlags & GenerateConstantProperty) {
        const auto desc = CppQuickFixFactory::tr("Generate Constant Q_PROPERTY and Missing Members");
      ...
      if (possibleFlags & GenerateProperty) {
        if (possibleFlags & GenerateReset) {
          const auto desc = CppQuickFixFactory::tr("Generate Q_PROPERTY and Missing Members with Reset Function");
  ...
  auto perform() -> void override
  {
    GetterSetterRefactoringHelper helper(this, currentFile()->filePath().toString(), m_data.clazz);
...
auto ExistingGetterSetterData::computePossibleFlags() const -> int
{
  ...
  if (!isStatic) {
    const auto hasSignal = !signalName.isEmpty() || generateFlags & Flag::GenerateSignal;
...
auto GetterSetterRefactoringHelper::performGeneration(ExistingGetterSetterData data, int generateFlags) -> void
{
  using Flag = GenerateGetterSetterOp::GenerateFlag;
  ...
  if (generateFlags & Flag::GenerateGetter && data.getterName.isEmpty()) {
    ...
    if (data.getterName == data.memberVariableName) {
      data.getterName = "get" + data.memberVariableName.left(1).toUpper() + data.memberVariableName.mid(1);
    }
  }
  ...
  const bool isValueType = this->isValueType(memberVariableType, data.declarationSymbol->enclosingScope());
  const FullySpecifiedType parameterType = isValueType ? memberVariableType : makeConstRef(memberVariableType);

  auto baseName = memberBaseName(data.memberVariableName);
  if (baseName.isEmpty())
    baseName = data.memberVariableName;

  const auto parameterName = m_settings->getSetterParameterName(baseName);
  ...
  const auto returnTypeHeader = [&] {
    if (!getSetTemplate.returnTypeTemplate.has_value())
      return parameterType;
    QString typeTemplate = getSetTemplate.returnTypeTemplate.value();
    if (returnTypeTemplateParameter.has_value())
      typeTemplate.replace(Pattern::TEMPLATE_PARAMETER_PATTERN, overview.prettyType(returnTypeTemplateParameter.value()));
    if (typeTemplate.contains(Pattern::TYPE_PATTERN))
      typeTemplate.replace(Pattern::TYPE_PATTERN, overview.prettyType(data.declarationSymbol->type()));
  ...
  if (generateFlags & Flag::GenerateGetter) {
    ...
    QString getterInClassDeclaration = overview.prettyType(returnTypeHeader, data.getterName) + QLatin1String("()");
    ...
    if (getterLocation != CppQuickFixSettings::FunctionLocation::InsideClass) {
      const auto getReturnTypeAt = [&](CppRefactoringFilePtr targetFile, InsertionLocation targetLoc) {
        if (getSetTemplate.returnTypeTemplate.has_value()) {
          QString returnType = getSetTemplate.returnTypeTemplate.value();
          if (returnTypeTemplateParameter.has_value()) {
            const QString templateTypeName = overview.prettyType(typeAt(returnTypeTemplateParameter.value(), data.clazz, targetFile, targetLoc));
            returnType.replace(Pattern::TEMPLATE_PARAMETER_PATTERN, templateTypeName);
          }
          if (returnType.contains(Pattern::TYPE_PATTERN)) {
            const QString declarationType = overview.prettyType(typeAt(memberVariableType, data.clazz, targetFile, targetLoc));
            returnType.replace(Pattern::TYPE_PATTERN, declarationType);
          }
          Control *control = m_operation->currentFile()->cppDocument()->control();
          std::string utf8String = returnType.toUtf8().toStdString();
          return FullySpecifiedType(control->namedType(control->identifier(utf8String.c_str())));
        } else {
          FullySpecifiedType returnType = typeAt(memberVariableType, data.clazz, targetFile, targetLoc);
          if (!isValueType)
            return makeConstRef(returnType);
          return returnType;
        }
      };
      const QString constSpec = isMemberVariableStatic ? QLatin1String("") : QLatin1String(" const");
      if (getterLocation == CppQuickFixSettings::FunctionLocation::CppFile) {
        auto loc = sourceLocationFor(data.declarationSymbol);
        ...
        const QString code = overview.prettyType(returnType, clazz + "::" + data.getterName) + "()" + constSpec + "\n{\nreturn " + returnExpression + ";\n}";
        addSourceFileCode(code);
      } else if (getterLocation == CppQuickFixSettings::FunctionLocation::OutsideClass) {
        auto loc = insertLocationForMethodDefinition(data.declarationSymbol, false, NamespaceHandling::Ignore, m_changes, m_headerFile->filePath().toString());
        const FullySpecifiedType returnType = getReturnTypeAt(m_headerFile, loc);
        const auto clazz = symbolAt(data.clazz, m_headerFile, loc);
        QString code = overview.prettyType(returnType, clazz + "::" + data.getterName) + "()" + constSpec + "\n{\nreturn " + returnExpression + ";\n}";
  ...
  // setter declaration
  auto setterAccessSpec = InsertionPointLocator::Public;
  if (m_settings->setterAsSlot) {
    ...
    for (const LookupItem &item : items) {
      if (item.declaration() && item.declaration()->enclosingClass() && overview.prettyName(item.declaration()->enclosingClass()->name()) == "QObject") {
  ...
  if (generateFlags & Flag::GenerateSetter) {
    QString headerDeclaration = "void " + data.setterName + '(' + overview.prettyType(addConstToReference(parameterType), parameterName) + ")";
    if (isMemberVariableStatic)
    ...
    if (setterLocation == CppQuickFixSettings::FunctionLocation::InsideClass) {
      ...
      if (setterLocation == CppQuickFixSettings::FunctionLocation::CppFile) {
        ...
        const QString code = "void " + clazz + "::" + data.setterName + '(' + overview.prettyType(newParameterType, parameterName) + ')' + body;
        addSourceFileCode(code);
      } else if (setterLocation == CppQuickFixSettings::FunctionLocation::OutsideClass) {
        auto loc = insertLocationForMethodDefinition(data.declarationSymbol, false, NamespaceHandling::Ignore, m_changes, m_headerFile->filePath().toString());

        FullySpecifiedType newParameterType = typeAt(data.declarationSymbol->type(), data.clazz, m_headerFile, loc);
        if (!isValueType)
          newParameterType = makeConstRef(newParameterType);
        newParameterType = addConstToReference(newParameterType);
        auto clazz = symbolAt(data.clazz, m_headerFile, loc);

        QString code = "void " + clazz + "::" + data.setterName + '(' + overview.prettyType(newParameterType, parameterName) + ')' + body;
        if (m_isHeaderHeaderFile)
          code.prepend("inline ");
        insertAndIndent(m_headerFile, loc, code);
      }
  ...
  if (generateFlags & Flag::GenerateReset) {
    ...
    if (!data.setterName.isEmpty()) {
      ...
      body += "static $TYPE defaultValue{}; " "// TODO: Adapt to use your actual default value\n";
      if (data.signalName.isEmpty())
        body += getSetTemplate.assignment + ";\n";
      else
        body += createSetterBodyWithSignal();
    ...
    if (resetLocation == CppQuickFixSettings::FunctionLocation::InsideClass) {
      ...
      if (resetLocation == CppQuickFixSettings::FunctionLocation::CppFile) {
        const auto loc = sourceLocationFor(data.declarationSymbol);
        QString clazz;
        ...
        const QString code = "void " + clazz + "::" + data.resetName + "()" + body.replace("$TYPE", overview.prettyType(type));
        addSourceFileCode(code);
      } else if (resetLocation == CppQuickFixSettings::FunctionLocation::OutsideClass) {
        const auto loc = insertLocationForMethodDefinition(data.declarationSymbol, false, NamespaceHandling::Ignore, m_changes, m_headerFile->filePath().toString());
        const FullySpecifiedType type = typeAt(data.declarationSymbol->type(), data.clazz, m_headerFile, loc);
        const auto clazz = symbolAt(data.clazz, m_headerFile, loc);
        QString code = "void " + clazz + "::" + data.resetName + "()" + body.replace("$TYPE", overview.prettyType(type));
        if (m_isHeaderHeaderFile)
          code.prepend("inline ");
        insertAndIndent(m_headerFile, loc, code);
      }
  ...
  if (generateFlags & Flag::GenerateMemberVariable) {
    const QString storageDeclaration = overview.prettyType(memberVariableType, data.memberVariableName) + QLatin1String(";\n");
  ...
  if (generateFlags & Flag::GenerateProperty || generateFlags & Flag::GenerateConstantProperty) {
    ...
    QString propertyDeclaration = QLatin1String("Q_PROPERTY(") + overview.prettyType(type, memberBaseName(data.memberVariableName));
    auto needMember = false;
...
auto toStringList(const QList<Symbol*> names) -> QStringList
{
  QStringList list;
...
auto findExistingFunctions(ExistingGetterSetterData &existing, QStringList memberFunctionNames) -> void
{
  const CppQuickFixSettings *settings = CppQuickFixProjectsSettings::getQuickFixSettings(ProjectExplorer::ProjectTree::currentProject());
  const auto lowerBaseName = memberBaseName(existing.memberVariableName).toLower();
  const QStringList getterNames{lowerBaseName, "get_" + lowerBaseName, "get" + lowerBaseName, "is_" + lowerBaseName, "is" + lowerBaseName, settings->getGetterName(lowerBaseName)};
  const QStringList setterNames{"set_" + lowerBaseName, "set" + lowerBaseName, settings->getSetterName(lowerBaseName)};
  const QStringList resetNames{"reset_" + lowerBaseName, "reset" + lowerBaseName, settings->getResetName(lowerBaseName)};
  const QStringList signalNames{lowerBaseName + "_changed", lowerBaseName + "changed", settings->getSignalName(lowerBaseName)};
  for (const auto &memberFunctionName : memberFunctionNames) {
    const auto lowerName = memberFunctionName.toLower();
...
auto getMemberFunctions(const Class *clazz) -> QList<Symbol*>
{
...
auto GenerateGetterSetter::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  ...
  const auto baseName = memberBaseName(existing.memberVariableName);
  ...
  const auto possibleFlags = existing.computePossibleFlags();
  GenerateGetterSetterOp::generateQuickFixes(result, interface, existing, possibleFlags);
}

class MemberInfo {
public:
  MemberInfo(ExistingGetterSetterData data, int possibleFlags) : data(data), possibleFlags(possibleFlags) {}
...
public:
  ...
  using Flag = GenerateGetterSetterOp::GenerateFlag;
  constexpr static Flag ColumnFlag[] = {static_cast<Flag>(-1), Flag::GenerateGetter, Flag::GenerateSetter, Flag::GenerateSignal, Flag::GenerateReset, Flag::GenerateProperty, Flag::GenerateConstantProperty,};

  CandidateTreeItem(MemberInfo *memberInfo) : m_memberInfo(memberInfo) {}
...
private:
  auto data(int column, int role) const -> QVariant override
  {
    ...
    if (role == Qt::CheckStateRole && column > 0 && column <= static_cast<int>(std::size(ColumnFlag))) {
  ...
  auto setData(int column, const QVariant &data, int role) -> bool override
  {
    ...
    if (!(m_memberInfo->possibleFlags & ColumnFlag[column]))
      return false;
    const auto nowChecked = data.toInt() == Qt::Checked;
    if (nowChecked)
...
public:
  GenerateGettersSettersDialog(const GetterSetterCandidates &candidates) : QDialog(), m_candidates(candidates)
  {
    ...
    model->setHeader(QStringList({tr("Member"), tr("Getter"), tr("Setter"), tr("Signal"), tr("Reset"), tr("QProperty"), tr("Constant QProperty"),}));
    for (auto &candidate : m_candidates)
      model->rootItem()->appendChild(new CandidateTreeItem(&candidate));
    const auto view = new Utils::BaseTreeView(this);
    view->setModel(model);
    auto optimalWidth = 0;
    for (auto i = 0; i < model->columnCount(QModelIndex{}); ++i) {
    ...
    constexpr Column CheckBoxColumn[4] = {Column::GetterColumn, Column::SetterColumn, Column::SignalColumn, Column::QPropertyColumn};
    static_assert(std::size(CheckBoxColumn) == checkBoxes.size(), "Must contain the same number of elements");
    for (std::size_t i = 0; i < checkBoxes.size(); ++i) {
      if (Utils::anyOf(candidates, [i, CheckBoxColumn](const MemberInfo &mi) {
        ...
        const auto column = CheckBoxColumn[i];
    ...
    connect(model, &QAbstractItemModel::dataChanged, this, [this, checkBoxes, CheckBoxColumn] {
      ...
      for (std::size_t i = 0; i < checkBoxes.size(); ++i) {
        if (checkBoxes[i]) {
          const auto flag = CandidateTreeItem::ColumnFlag[CheckBoxColumn[i]];
          checkBoxes[i]->setCheckState(countToState(countRequested(flag), countExisting(flag)));
        }
      }
    });

    const auto mainLayout = new QVBoxLayout(this);
    mainLayout->addWidget(new QLabel(tr("Select the getters and setters " "to be created.")));
  ...
  auto candidates() const -> GetterSetterCandidates { return m_candidates; }
...
public:
  GenerateGettersSettersOperation(const CppQuickFixInterface &interface) : CppQuickFixOperation(interface)
  {
    ...
    const auto memberFunctionsAsStrings = toStringList(memberFunctions);

    for (const auto member : qAsConst(dataMembers)) {
      ExistingGetterSetterData existing;
      existing.memberVariableName = QString::fromUtf8(member->identifier()->chars(), member->identifier()->size());
      existing.declarationSymbol = member->asDeclaration();
      existing.clazz = theClass;

      // check if a Q_PROPERTY exist
      const auto baseName = memberBaseName(existing.memberVariableName);
      if (qPropertyNames.contains(baseName) || qPropertyNames.contains(existing.memberVariableName))
        continue;

      findExistingFunctions(existing, memberFunctionsAsStrings);
      existing.qPropertyName = baseName;

      auto possibleFlags = existing.computePossibleFlags();
      if (possibleFlags == 0)
        continue;
  ...
  auto candidates() const -> GetterSetterCandidates { return m_candidates; }
  auto isApplicable() const -> bool { return !m_candidates.empty(); }

  auto setGetterSetterData(const GetterSetterCandidates &data) -> void
  {
    m_candidates = data;
    m_hasData = true;
  }
...
private:
  auto perform() -> void override
  {
    ...
    if (m_candidates.empty())
      return;
    GetterSetterRefactoringHelper helper(this, currentFile()->filePath().toString(), m_candidates.front().data.clazz);
    for (auto &mi : m_candidates) {
...
auto GenerateGettersSettersForClass::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const auto op = QSharedPointer<GenerateGettersSettersOperation>::create(interface);
  if (!op->isApplicable())
    return;
  if (m_test) {
    auto candidates = op->candidates();
...
public:
  static auto isValidFunctionName(const QString &name) -> bool
  {
    return !name.isEmpty() && isValidIdentifier(name);
  }

  auto hasValidFunctionName() const -> bool
  {
...
public:
  ExtractFunctionOperation(const CppQuickFixInterface &interface, int extractionStart, int extractionEnd, FunctionDefinitionAST *refFuncDef, Symbol *funcReturn, QList<QPair<QString, QString>> relevantDecls, ExtractFunction::FunctionNameGetter functionNameGetter = ExtractFunction::FunctionNameGetter()) : CppQuickFixOperation(interface), m_extractionStart(extractionStart), m_extractionEnd(extractionEnd), m_refFuncDef(refFuncDef), m_funcReturn(funcReturn), m_relevantDecls(relevantDecls), m_functionNameGetter(functionNameGetter)
  {
  ...
  auto perform() -> void override
  {
    ...
    if (!options.hasValidFunctionName())
      return;
    const auto &funcName = options.funcName;
    ...
    for (auto i = m_funcReturn ? 1 : 0; i < m_relevantDecls.length(); ++i) {
      auto p = m_relevantDecls.at(i);
    ...
    if (matchingClass)
      funcDecl.append(QLatin1String(";\n"));
    if (m_funcReturn) {
      funcDef.append(QLatin1String("\nreturn ") + m_relevantDecls.at(0).first + QLatin1Char(';'));
    ...
    auto indentedExtractionStart = m_extractionStart;
    ...
    if (matchingClass) {
      ...
      const InsertionLocation &location = locator.methodDeclarationInClass(fileName, matchingClass, options.access);
  ...
  auto getOptions() const -> ExtractFunctionOptions
  {
    QDialog dlg(Orca::Plugin::Core::ICore::dialogParent());
    dlg.setWindowTitle(QCoreApplication::translate("QuickFix::ExtractFunction", "Extract Function Refactoring"));
    ...
    layout->addRow(QCoreApplication::translate("QuickFix::ExtractFunction", "Function name"), funcNameEdit);

    auto accessCombo = new QComboBox;
    accessCombo->addItem(InsertionPointLocator::accessSpecToString(InsertionPointLocator::Public), InsertionPointLocator::Public);
    accessCombo->addItem(InsertionPointLocator::accessSpecToString(InsertionPointLocator::PublicSlot), InsertionPointLocator::PublicSlot);
    accessCombo->addItem(InsertionPointLocator::accessSpecToString(InsertionPointLocator::Protected), InsertionPointLocator::Protected);
    accessCombo->addItem(InsertionPointLocator::accessSpecToString(InsertionPointLocator::ProtectedSlot), InsertionPointLocator::ProtectedSlot);
    accessCombo->addItem(InsertionPointLocator::accessSpecToString(InsertionPointLocator::Private), InsertionPointLocator::Private);
    accessCombo->addItem(InsertionPointLocator::accessSpecToString(InsertionPointLocator::PrivateSlot), InsertionPointLocator::PrivateSlot);
    layout->addRow(QCoreApplication::translate("QuickFix::ExtractFunction", "Access"), accessCombo);
    ...
    ok->setEnabled(false);
    QObject::connect(funcNameEdit, &Utils::FancyLineEdit::validChanged, ok, &QPushButton::setEnabled);
    layout->addWidget(buttonBox);

    if (dlg.exec() == QDialog::Accepted) {
      ExtractFunctionOptions options;
      options.funcName = funcNameEdit->text();
      options.access = static_cast<InsertionPointLocator::AccessSpec>(accessCombo->currentData().toInt());
      return options;
    }
...
auto assembleDeclarationData(const QString &specifiers, DeclaratorAST *decltr, const CppRefactoringFilePtr &file, const Overview &printer) -> QPair<QString, QString>
{
  QTC_ASSERT(decltr, return (QPair<QString, QString>()));
  if (decltr->core_declarator && decltr->core_declarator->asDeclaratorId() && decltr->core_declarator->asDeclaratorId()->name) {
    QString decltrText = file->textOf(file->startOf(decltr), file->endOf(decltr->core_declarator));
    if (!decltrText.isEmpty()) {
      const QString &name = printer.prettyName(decltr->core_declarator->asDeclaratorId()->name->name);
      auto completeDecl = specifiers;
...
public:
  FunctionExtractionAnalyser(TranslationUnit *unit, const int selStart, const int selEnd, const CppRefactoringFilePtr &file, const Overview &printer) : ASTVisitor(unit), m_done(false), m_failed(false), m_selStart(selStart), m_selEnd(selEnd), m_extractionStart(0), m_extractionEnd(0), m_file(file), m_printer(printer) {}

  auto operator()(FunctionDefinitionAST *refFunDef) -> bool
  {
    accept(refFunDef);
  ...
  auto preVisit(AST *) -> bool override
  {
    return !m_done;
  }

  auto statement(StatementAST *stmt) -> void
  {
    if (!stmt)
      return;
  ...
  auto visit(CaseStatementAST *stmt) -> bool override
  {
    statement(stmt->statement);
    return false;
  }

  auto visit(CompoundStatementAST *stmt) -> bool override
  {
  ...
  auto visit(DoStatementAST *stmt) -> bool override
  {
    statement(stmt->statement);
    return false;
  }

  auto visit(ForeachStatementAST *stmt) -> bool override
  {
    statement(stmt->statement);
    return false;
  }

  auto visit(RangeBasedForStatementAST *stmt) -> bool override
  {
    statement(stmt->statement);
    return false;
  }

  auto visit(ForStatementAST *stmt) -> bool override
  {
  ...
  auto visit(IfStatementAST *stmt) -> bool override
  {
    statement(stmt->statement);
    if (!m_done)
  ...
  auto visit(TryBlockStatementAST *stmt) -> bool override
  {
    statement(stmt->statement);
  ...
  auto visit(WhileStatementAST *stmt) -> bool override
  {
    statement(stmt->statement);
    return false;
  }

  auto visit(DeclarationStatementAST *declStmt) -> bool override
  {
    ...
    // typing style.
    if (declStmt && declStmt->declaration && declStmt->declaration->asSimpleDeclaration()) {
      SimpleDeclarationAST *simpleDecl = declStmt->declaration->asSimpleDeclaration();
      if (simpleDecl->decl_specifier_list && simpleDecl->declarator_list) {
        const QString &specifiers = m_file->textOf(m_file->startOf(simpleDecl), m_file->endOf(simpleDecl->decl_specifier_list->lastValue()));
        for (DeclaratorListAST *decltrList = simpleDecl->declarator_list; decltrList; decltrList = decltrList->next) {
          const QPair<QString, QString> p = assembleDeclarationData(specifiers, decltrList->value, m_file, m_printer);
          if (!p.first.isEmpty())
  ...
  auto visit(ReturnStatementAST *) -> bool override
  {
...
ExtractFunction::ExtractFunction(FunctionNameGetter functionNameGetter) : m_functionNameGetter(functionNameGetter) {}

auto ExtractFunction::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  auto file = interface.currentFile();
  ...
  if (!refFuncDef || !refFuncDef->function_body || !refFuncDef->function_body->asCompoundStatement() || !refFuncDef->function_body->asCompoundStatement()->statement_list || !refFuncDef->symbol || !refFuncDef->symbol->name() || refFuncDef->symbol->enclosingScope()->isTemplate() /* TODO: Templates... */) {
    return;
  }
  ...
  FunctionExtractionAnalyser analyser(interface.semanticInfo().doc->translationUnit(), selStart, selEnd, file, printer);
  ...
  if (refFuncDef->declarator->postfix_declarator_list && refFuncDef->declarator->postfix_declarator_list->value && refFuncDef->declarator->postfix_declarator_list->value->asFunctionDeclarator()) {
    FunctionDeclaratorAST *funcDecltr = refFuncDef->declarator->postfix_declarator_list->value->asFunctionDeclarator();
    if (funcDecltr->parameter_declaration_clause && funcDecltr->parameter_declaration_clause->parameter_declaration_list) {
      for (ParameterDeclarationListAST *it = funcDecltr->parameter_declaration_clause->parameter_declaration_list; it; it = it->next) {
        ...
        if (paramDecl->declarator) {
          const QString &specifiers = file->textOf(file->startOf(paramDecl), file->endOf(paramDecl->type_specifier_list->lastValue()));
          const QPair<QString, QString> &p = assembleDeclarationData(specifiers, paramDecl->declarator, file, printer);
          if (!p.first.isEmpty()) {
  ...
  for (auto it = localUses.cbegin(), end = localUses.cend(); it != end; ++it) {
    auto usedBeforeExtraction = false;
    auto usedAfterExtraction = false;
    auto usedInsideExtraction = false;
    ...
    if ((usedBeforeExtraction && usedInsideExtraction) || (usedInsideExtraction && refFuncParams.contains(name))) {
  ...
  result << new ExtractFunctionOperation(interface, analyser.m_extractionStart, analyser.m_extractionEnd, refFuncDef, funcReturn, relevantDecls, m_functionNameGetter);
}
...
public:
  ReplaceLiterals(const CppRefactoringFilePtr &file, ChangeSet *changes, T *literal) : ASTVisitor(file->cppDocument()->translationUnit()), m_file(file), m_changes(changes), m_literal(literal)
  {
  ...
  auto apply(AST *ast) -> ReplaceLiteralsResult
  {
    ast->accept(this);
    return m_result;
  }
...
private:
  auto visit(T *ast) -> bool override
  {
    if (ast != m_literal && strcmp(m_file->tokenAt(ast->firstToken()).spell(), m_literalTokenText) != 0) {
      return true;
    }
    int start, end;
...
public:
  ExtractLiteralAsParameterOp(const CppQuickFixInterface &interface, int priority, ExpressionAST *literal, FunctionDefinitionAST *function) : CppQuickFixOperation(interface, priority), m_literal(literal), m_functionDefinition(function)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Extract Constant as Function Parameter"));
  }
  ...
  auto findDeclaration(const CppRefactoringChanges &refactoring, FunctionDefinitionAST *ast) -> FoundDeclaration
  {
    ...
    if (Class *matchingClass = isMemberFunction(context(), func)) {
      ...
      for (Symbol *s = matchingClass->find(qName->identifier()); s; s = s->next()) {
        if (!s->name() || !qName->identifier()->match(s->identifier()) || !s->type()->isFunctionType() || !s->type().match(func->type()) || s->isFunction()) {
          continue;
        }

        declFileName = QString::fromUtf8(matchingClass->fileName(), matchingClass->fileNameLength());
  ...
  auto perform() -> void override
  {
    FunctionDeclaratorAST *functionDeclaratorOfDefinition = functionDeclarator(m_functionDefinition);
    const CppRefactoringChanges refactoring(snapshot());
    ...
    if (NumericLiteralAST *concreteLiteral = m_literal->asNumericLiteral()) {
      m_literalInfo = ReplaceLiterals<NumericLiteralAST>(currentFile, &changes, concreteLiteral).apply(m_functionDefinition->function_body);
    } else if (StringLiteralAST *concreteLiteral = m_literal->asStringLiteral()) {
      m_literalInfo = ReplaceLiterals<StringLiteralAST>(currentFile, &changes, concreteLiteral).apply(m_functionDefinition->function_body);
    } else if (BoolLiteralAST *concreteLiteral = m_literal->asBoolLiteral()) {
      m_literalInfo = ReplaceLiterals<BoolLiteralAST>(currentFile, &changes, concreteLiteral).apply(m_functionDefinition->function_body);
    }
    const FoundDeclaration functionDeclaration = findDeclaration(refactoring, m_functionDefinition);
    appendFunctionParameter(functionDeclaratorOfDefinition, currentFile, &changes, !functionDeclaration.ast);
    if (functionDeclaration.ast) {
      if (currentFile->filePath() != functionDeclaration.file->filePath()) {
        ChangeSet declChanges;
        appendFunctionParameter(functionDeclaration.ast, functionDeclaration.file, &declChanges, true);
        functionDeclaration.file->setChangeSet(declChanges);
        functionDeclaration.file->apply();
      } else {
        appendFunctionParameter(functionDeclaration.ast, currentFile, &changes, true);
      }
    }
    currentFile->setChangeSet(changes);
    currentFile->apply();
...
private:
  auto hasParameters(FunctionDeclaratorAST *ast) const -> bool
  {
    return ast->parameter_declaration_clause && ast->parameter_declaration_clause->parameter_declaration_list && ast->parameter_declaration_clause->parameter_declaration_list->value;
  }

  auto deduceTypeNameOfLiteral(const Document::Ptr &document) -> void
  {
    TypeOfExpression typeOfExpression;
    typeOfExpression.init(document, snapshot());
    Overview overview;
  ...
  static auto parameterName() -> QString { return QLatin1String("newParameter"); }

  auto parameterDeclarationTextToInsert(FunctionDeclaratorAST *ast) const -> QString
  {
    QString str;
  ...
  auto functionDeclarator(SimpleDeclarationAST *ast) const -> FunctionDeclaratorAST*
  {
  ...
  auto functionDeclarator(DeclaratorAST *ast) const -> FunctionDeclaratorAST*
  {
  ...
  auto functionDeclarator(FunctionDefinitionAST *ast) const -> FunctionDeclaratorAST*
  {
    return functionDeclarator(ast->declarator);
  }

  auto appendFunctionParameter(FunctionDeclaratorAST *ast, const CppRefactoringFileConstPtr &file, ChangeSet *changes, bool addDefaultValue) -> void
  {
    if (!ast)
      return;
    if (m_declarationInsertionString.isEmpty())
      m_declarationInsertionString = parameterDeclarationTextToInsert(ast);
    auto insertion = m_declarationInsertionString;
    if (addDefaultValue)
...
auto ExtractLiteralAsParameter::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  if (path.count() < 2)
    return;
  ...
  if (!((literal = lastAst->asNumericLiteral()) || (literal = lastAst->asStringLiteral()) || (literal = lastAst->asBoolLiteral()))) {
    return;
  }
  ...
  if (FunctionDeclaratorAST *declarator = declaratorList->value->asFunctionDeclarator()) {
    if (declarator->parameter_declaration_clause && declarator->parameter_declaration_clause->dot_dot_dot_token) {
...
public:
  ...
  ConvertFromAndToPointerOp(const CppQuickFixInterface &interface, int priority, Mode mode, bool isAutoDeclaration, const SimpleDeclarationAST *simpleDeclaration, const DeclaratorAST *declaratorAST, const SimpleNameAST *identifierAST, Symbol *symbol) : CppQuickFixOperation(interface, priority), m_mode(mode), m_isAutoDeclaration(isAutoDeclaration), m_simpleDeclaration(simpleDeclaration), m_declaratorAST(declaratorAST), m_identifierAST(identifierAST), m_symbol(symbol), m_refactoring(snapshot()), m_file(m_refactoring.file(filePath())), m_document(interface.semanticInfo().doc)
  {
    setDescription(mode == FromPointer ? CppQuickFixFactory::tr("Convert to Stack Variable") : CppQuickFixFactory::tr("Convert to Pointer"));
  }
...
private:
  auto removePointerOperator(ChangeSet &changes) const -> void
  {
  ...
  auto removeReferenceOperator(ChangeSet &changes) const -> void
  {
  ...
  auto removeNewExpression(ChangeSet &changes, NewExpressionAST *newExprAST) const -> void
  {
    ...
    if (exprlist) {
      ...
      changes.remove(m_file->startOf(newExprAST->new_token), m_file->startOf(newExprAST->new_initializer));

      changes.remove(m_file->endOf(m_declaratorAST->equal_token - 1), m_file->startOf(m_declaratorAST->equal_token + 1));
    } else {
      // remove the whole new expression
      changes.remove(m_file->endOf(m_identifierAST->firstToken()), m_file->startOf(newExprAST->lastToken()));
    }
  }

  auto removeNewKeyword(ChangeSet &changes, NewExpressionAST *newExprAST) const -> void
  {
    // remove 'new' keyword before initializer
    changes.remove(m_file->startOf(newExprAST->new_token), m_file->startOf(newExprAST->new_type_id));
  }

  auto convertToStackVariable(ChangeSet &changes) const -> void
  {
    ...
    foreach(const SemanticInfo::Use &use, semanticInfo().localUses.value(m_symbol)) {
      ...
      AST *idAST = path.last();
      auto declarationFound = false;
      auto starFound = false;
      auto ampersandPos = 0;
      auto memberAccess = false;
      auto deleteCall = false;
  ...
  auto typeNameOfDeclaration() const -> QString
  {
    if (!m_simpleDeclaration || !m_simpleDeclaration->decl_specifier_list || !m_simpleDeclaration->decl_specifier_list->value) {
      return QString();
    }
    NamedTypeSpecifierAST *namedType = m_simpleDeclaration->decl_specifier_list->value->asNamedTypeSpecifier();
    if (!namedType)
      return QString();
  ...
  auto insertNewExpression(ChangeSet &changes, ExpressionAST *ast) const -> void
  {
    const auto typeName = typeNameOfDeclaration();
    if (CallAST *callAST = ast->asCall()) {
      if (typeName.isEmpty()) {
        changes.insert(m_file->startOf(callAST), QLatin1String("new "));
      } else {
        changes.insert(m_file->startOf(callAST), QLatin1String("new ") + typeName + QLatin1Char('('));
  ...
  auto insertNewExpression(ChangeSet &changes) const -> void
  {
    const auto typeName = typeNameOfDeclaration();
    if (typeName.isEmpty())
      return;
    changes.insert(m_file->endOf(m_identifierAST->firstToken()), QLatin1String(" = new ") + typeName);
  }

  auto convertToPointer(ChangeSet &changes) const -> void
  {
    // Handle initializer.
    if (m_declaratorAST->initializer) {
      ...
      } else if (ExpressionListParenAST *exprListAST = m_declaratorAST->initializer->asExpressionListParen()) {
        insertNewExpression(changes, exprListAST);
      } else if (BracedInitializerAST *bracedInitializerAST = m_declaratorAST->initializer->asBracedInitializer()) {
...
auto ConvertFromAndToPointer::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  if (path.count() < 2)
    return;
  ...
  auto isFunctionLocal = false;
  auto isClassLocal = false;
  auto mode = ConvertFromAndToPointerOp::FromVariable;
  for (int i = path.count() - 2; i >= 0; --i) {
    AST *ast = path.at(i);
  ...
  if (symbol->storage() == Symbol::Auto) {
    ...
    QList<LookupItem> result = typeOfExpression(file->textOf(declarator->initializer).toUtf8(), scope, TypeOfExpression::Preprocess);
  ...
  const int priority = path.size() - 1;
  result << new ConvertFromAndToPointerOp(interface, priority, mode, isAutoDeclaration, simpleDeclaration, declarator, identifier, symbol);
}

namespace {

auto extractNames(const CppRefactoringFilePtr &file, QtPropertyDeclarationAST *qtPropertyDeclaration, ExistingGetterSetterData &data) -> void
{
...
auto InsertQtPropertyMembers::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  using Flag = GenerateGetterSetterOp::GenerateFlag;
  ExistingGetterSetterData existing;
  ...
  existing.declarationSymbol = control->newDeclaration(ast->firstToken(), qtPropertyDeclaration->property_name->name);
...

  {
    ...
    QByteArray newSource = file->document()->toPlainText().insert(file->startOf(qtPropertyDeclaration), QString::fromUtf8(type + " __dummy;\n")).toUtf8();

    Document::Ptr doc = interface.snapshot().preprocessedDocument(newSource, "___quickfix.hpp");
    ...
    public:
      ...
      TypeFinder(TranslationUnit *u) : ASTVisitor(u) {}
  ...
  const auto haveFixMemberVariableName = !existing.memberVariableName.isEmpty();
  ...
  if (!existing.signalName.isEmpty())
    generateFlags |= Flag::GenerateSignal;
  Overview overview;
  for (auto i = 0; i < existing.clazz->memberCount(); ++i) {
    ...
    if (member->asFunction() || (type.isValid() && type->asFunctionType())) {
      ...
      if (haveFixMemberVariableName) {
        ...
        const auto baseName = memberBaseName(name);
...
public:
  explicit ApplyDeclDefLinkOperation(const CppQuickFixInterface &interface, const QSharedPointer<FunctionDeclDefLink> &link) : CppQuickFixOperation(interface, 100), m_link(link) {}
...
protected:
  virtual auto performChanges(const CppRefactoringFilePtr &, const CppRefactoringChanges &) -> void
  {
...
auto ApplyDeclDefLinkChanges::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  auto link = interface.editor()->declDefLink();
...
auto definitionSignature(const CppQuickFixInterface *assist, FunctionDefinitionAST *functionDefinitionAST, CppRefactoringFilePtr &baseFile, CppRefactoringFilePtr &targetFile, Scope *scope) -> QString
{
  ...
  oo.trailingReturnType = functionDefinitionAST->declarator && functionDefinitionAST->declarator->postfix_declarator_list && functionDefinitionAST->declarator->postfix_declarator_list->value && functionDefinitionAST->declarator->postfix_declarator_list->value->asFunctionDeclarator() && functionDefinitionAST->declarator->postfix_declarator_list->value->asFunctionDeclarator()->trailing_return_type;
...
public:
  ...
  MoveFuncDefRefactoringHelper(CppQuickFixOperation *operation, MoveType type, const QString &fromFile, const QString &toFile) : m_operation(operation), m_type(type), m_changes(m_operation->snapshot())
  {
    m_fromFile = m_changes.file(Utils::FilePath::fromString(fromFile));
    m_toFile = (m_type == MoveOutside) ? m_fromFile : m_changes.file(Utils::FilePath::fromString(toFile));
  }

  auto performMove(FunctionDefinitionAST *funcAST) -> void
  {
    // Determine file, insert position and scope
    InsertionLocation l = insertLocationForMethodDefinition(funcAST->symbol, false, NamespaceHandling::Ignore, m_changes, m_toFile->filePath().toString());
    ...
    const QString funcDec = inlinePrefix(m_toFile->filePath().toString(), [this] { return m_type == MoveOutside; }) + definitionSignature(m_operation, funcAST, m_fromFile, m_toFile, scopeAtInsertPos);
...
public:
  MoveFuncDefOutsideOp(const CppQuickFixInterface &interface, MoveFuncDefRefactoringHelper::MoveType type, FunctionDefinitionAST *funcDef, const QString &cppFileName) : CppQuickFixOperation(interface, 0), m_funcDef(funcDef), m_type(type), m_cppFileName(cppFileName), m_headerFileName(QString::fromUtf8(funcDef->symbol->fileName(), funcDef->symbol->fileNameLength()))
  {
    if (m_type == MoveFuncDefRefactoringHelper::MoveOutside) {
      setDescription(QCoreApplication::translate("CppEditor::QuickFix", "Move Definition Outside Class"));
    } else {
      const auto dir = QFileInfo(m_headerFileName).dir();
      setDescription(QCoreApplication::translate("CppEditor::QuickFix", "Move Definition to %1").arg(dir.relativeFilePath(m_cppFileName)));
    }
  }
...
auto MoveFuncDefOutside::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  SimpleDeclarationAST *classAST = nullptr;
  FunctionDefinitionAST *funcAST = nullptr;
  auto moveOutsideMemberDefinition = false;
  ...
  for (auto idx = 1; idx < pathSize; ++idx) {
    if ((funcAST = path.at(idx)->asFunctionDefinition())) {
      ...
      if (idx != pathSize - 1 // Do not allow "void a() @ {..."
        ...
        if (path.at(idx - 1)->asTranslationUnit()) {
          // normal function
  ...
  const auto cppFileName = correspondingHeaderOrSource(interface.filePath().toString(), &isHeaderFile);

  if (isHeaderFile && !cppFileName.isEmpty()) {
    const auto type = moveOutsideMemberDefinition ? MoveFuncDefRefactoringHelper::MoveOutsideMemberToCppFile : MoveFuncDefRefactoringHelper::MoveToCppFile;
    result << new MoveFuncDefOutsideOp(interface, type, funcAST, cppFileName);
  }

  if (classAST)
    result << new MoveFuncDefOutsideOp(interface, MoveFuncDefRefactoringHelper::MoveOutside, funcAST, QLatin1String(""));
...
public:
  MoveAllFuncDefOutsideOp(const CppQuickFixInterface &interface, MoveFuncDefRefactoringHelper::MoveType type, ClassSpecifierAST *classDef, const QString &cppFileName) : CppQuickFixOperation(interface, 0), m_type(type), m_classDef(classDef), m_cppFileName(cppFileName), m_headerFileName(QString::fromUtf8(classDef->symbol->fileName(), classDef->symbol->fileNameLength()))
  {
    if (m_type == MoveFuncDefRefactoringHelper::MoveOutside) {
      setDescription(QCoreApplication::translate("CppEditor::QuickFix", "Move All Function " "Definitions Outside Class"));
    } else {
      const auto dir = QFileInfo(m_headerFileName).dir();
      setDescription(QCoreApplication::translate("CppEditor::QuickFix", "Move All Function Definitions to %1").arg(dir.relativeFilePath(m_cppFileName)));
    }
  }
...
auto MoveAllFuncDefOutside::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  ...
  auto classContainsFunctions = false;
  ...
  const auto cppFileName = correspondingHeaderOrSource(interface.filePath().toString(), &isHeaderFile);
  if (isHeaderFile && !cppFileName.isEmpty()) {
    result << new MoveAllFuncDefOutsideOp(interface, MoveFuncDefRefactoringHelper::MoveToCppFile, classAST, cppFileName);
  }
  result << new MoveAllFuncDefOutsideOp(interface, MoveFuncDefRefactoringHelper::MoveOutside, classAST, QLatin1String(""));
}
...
public:
  MoveFuncDefToDeclOp(const CppQuickFixInterface &interface, const QString &fromFileName, const QString &toFileName, FunctionDefinitionAST *funcDef, const QString &declText, const ChangeSet::Range &fromRange, const ChangeSet::Range &toRange) : CppQuickFixOperation(interface, 0), m_fromFileName(fromFileName), m_toFileName(toFileName), m_funcAST(funcDef), m_declarationText(declText), m_fromRange(fromRange), m_toRange(toRange)
  {
    if (m_toFileName == m_fromFileName) {
      setDescription(QCoreApplication::translate("CppEditor::QuickFix", "Move Definition to Class"));
    } else {
      const auto dir = QFileInfo(m_fromFileName).dir();
      setDescription(QCoreApplication::translate("CppEditor::QuickFix", "Move Definition to %1").arg(dir.relativeFilePath(m_toFileName)));
    }
  }
  ...
  auto perform() -> void override
  {
    ...
    const QString wholeFunctionText = m_declarationText + fromFile->textOf(fromFile->endOf(m_funcAST->declarator), fromFile->endOf(m_funcAST->function_body));
...
auto MoveFuncDefToDecl::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  ...
  if (!declFileName.isEmpty() && !declText.isEmpty())
    result << new MoveFuncDefToDeclOp(interface, interface.filePath().toString(), declFileName, funcAST, declText, defRange, declRange);
}
...
public:
  explicit AssignToLocalVariableOperation(const CppQuickFixInterface &interface, const int insertPos, const AST *ast, const Name *name) : CppQuickFixOperation(interface), m_insertPos(insertPos), m_ast(ast), m_name(name)
  {
  ...
  auto perform() -> void override
  {
    ...
    const QList<LookupItem> result = typeOfExpression(file->textOf(m_ast).toUtf8(), scope, TypeOfExpression::Preprocess);
    ...
    if (!result.isEmpty()) {
      ...
      if (newName.startsWith(QLatin1String("get"), Qt::CaseInsensitive) && newName.length() > 3 && newName.at(3).isUpper()) {
        newName.remove(0, 3);
        newName.replace(0, 1, newName.at(0).toLower());
      } else if (newName.startsWith(QLatin1String("to"), Qt::CaseInsensitive) && newName.length() > 2 && newName.at(2).isUpper()) {
        newName.remove(0, 2);
        newName.replace(0, 1, newName.at(0).toLower());
      } else {
      ...
      const QString insertString = tempType.replace(tempType.length() - nameLength, nameLength, newName + QLatin1String(" = "));
      if (!tempType.isEmpty()) {
        ChangeSet changes;
...
auto AssignToLocalVariable::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
    ...
    if (CallAST *callAST = path.at(i)->asCall()) {
      ...
      if (i - 2 >= 0) {
        ...
        if (path.at(idx)->asMemInitializer())
          return;
        if (path.at(idx)->asCall()) {
          // Fallback if we have a->b()->c()...
          --i;
          continue;
        }
      }
      ...
      if (MemberAccessAST *member = path.at(i + 1)->asMemberAccess()) {
        // member
        if (NameAST *name = member->member_name)
          nameAST = name->asSimpleName();
      } else if (QualifiedNameAST *qname = path.at(i + 2)->asQualifiedName()) {
        // static or
  ...
  if (outerAST && nameAST) {
    ...
    TypeOfExpression typeOfExpression;
    typeOfExpression.init(interface.semanticInfo().doc, interface.snapshot(), interface.context().bindings());
    typeOfExpression.setExpandTemplates(true);

    // If items are empty, AssignToLocalVariableOperation will fail.
    items = typeOfExpression(file->textOf(outerAST).toUtf8(), file->scopeAt(outerAST->firstToken()), TypeOfExpression::Preprocess);
    if (items.isEmpty())
      return;

    if (CallAST *callAST = outerAST->asCall()) {
      items = typeOfExpression(file->textOf(callAST->base_expression).toUtf8(), file->scopeAt(callAST->base_expression->firstToken()), TypeOfExpression::Preprocess);
    } else {
      items = typeOfExpression(file->textOf(nameAST).toUtf8(), file->scopeAt(nameAST->firstToken()), TypeOfExpression::Preprocess);
    }
...
public:
  OptimizeForLoopOperation(const CppQuickFixInterface &interface, const ForStatementAST *forAst, const bool optimizePostcrement, const ExpressionAST *expression, const FullySpecifiedType &type) : CppQuickFixOperation(interface), m_forAst(forAst), m_optimizePostcrement(optimizePostcrement), m_expression(expression), m_type(type)
  {
  ...
  auto perform() -> void override
  {
    ...
    if (m_optimizePostcrement && m_forAst->expression) {
      ...
      if (incrdecr && incrdecr->base_expression && incrdecr->incr_decr_token) {
        change.flip(file->range(incrdecr->base_expression), file->range(incrdecr->incr_decr_token));
      }
    }
    ...
    if (m_expression) {
      ...
      if (file->textOf(m_forAst->initializer).length() == 1) {
        ...
        if (DeclarationStatementAST *ds = m_forAst->initializer->asDeclarationStatement()) {
          if (DeclarationAST *decl = ds->declaration) {
            if (SimpleDeclarationAST *sdecl = decl->asSimpleDeclaration()) {
              for (;;) {
                auto match = false;
                for (DeclaratorListAST *it = sdecl->declarator_list; it; it = it->next) {
        ...
        change.insert(file->endOf(m_forAst->initializer) - 1, // "-1" because of ";"
                      QLatin1String(", ") + varName + QLatin1String(" = ") + file->textOf(m_expression));
      }
...
auto OptimizeForLoop::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> path = interface.path();
  ...
  const auto file = interface.currentFile();
  auto optimizePostcrement = false;
  ...
  if (forAst->initializer && forAst->condition) {
    if (BinaryExpressionAST *binary = forAst->condition->asBinaryExpression()) {
      ...
      if (conditionId && conditionExpression && !(conditionExpression->asNumericLiteral() || conditionExpression->asStringLiteral() || conditionExpression->asIdExpression() || conditionExpression->asUnaryExpression())) {
        ...
        const QList<LookupItem> conditionItems = typeOfExpression(conditionId, interface.semanticInfo().doc, scope);
        if (!conditionItems.isEmpty())
          conditionType = conditionItems.first().type();

        if (conditionType.isValid() && (file->textOf(forAst->initializer) == QLatin1String(";") || initializerType == conditionType)) {
  ...
  if (optimizePostcrement || optimizeCondition) {
    result << new OptimizeForLoopOperation(interface, forAst, optimizePostcrement, optimizeCondition ? conditionExpression : nullptr, conditionType);
  }
}
...
public:
  EscapeStringLiteralOperation(const CppQuickFixInterface &interface, ExpressionAST *literal, bool escape) : CppQuickFixOperation(interface), m_literal(literal), m_escape(escape)
  {
    if (m_escape) {
      setDescription(QApplication::translate("CppEditor::QuickFix", "Escape String Literal as UTF-8"));
    } else {
      setDescription(QApplication::translate("CppEditor::QuickFix", "Unescape String Literal as UTF-8"));
    }
  }

private:
  static inline auto isDigit(quint8 ch, int base) -> bool
  {
    if (base == 8)
      return ch >= '0' && ch < '8';
    if (base == 16)
  ...
  static auto escapeString(const QByteArray &contents) -> QByteArrayList
  {
    QByteArrayList newContents;
    QByteArray chunk;
    auto wasEscaped = false;
    for (const quint8 c : contents) {
      const auto needsEscape = !isascii(c) || !isprint(c);
  ...
  static auto unescapeString(const QByteArray &contents) -> QByteArray
  {
...
public:
  auto perform() -> void override
  {
    ...
    if (newContents.isEmpty() || (newContents.size() == 1 && newContents.first() == oldContents)) {
      return;
    }
    ...
    auto replace = true;
    for (const auto &chunk : qAsConst(newContents)) {
      const auto str = decoder->toUnicode(chunk);
      const auto utf8buf = str.toUtf8();
...
auto EscapeStringLiteral::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  if (path.isEmpty())
    return;
...
public:
  ConvertQt4ConnectOperation(const CppQuickFixInterface &interface, const ChangeSet &changes) : CppQuickFixOperation(interface, 1), m_changes(changes)
  {
    setDescription(QApplication::translate("CppEditor::QuickFix", "Convert connect() to Qt 5 Style"));
  }
...
auto skipForwardDeclarations(const QList<Symbol*> &symbols) -> Symbol*
{
...
auto findRawAccessFunction(Class *klass, PointerType *pointerType, QString *objAccessFunction) -> bool
{
  ...
  for (auto it = klass->memberBegin(), end = klass->memberEnd(); it != end; ++it) {
    if (Function *func = (*it)->asFunction()) {
      ...
      if (!funcName->isOperatorNameId() && !funcName->isConversionNameId() && func->returnType().type() == pointerType && func->isConst() && func->argumentCount() == 0) {
        candidates << func;
      }
    }
  }
...
auto determineConvertedType(NamedType *namedType, const LookupContext &context, Scope *scope, QString *objAccessFunction) -> PointerType*
{
  if (!namedType)
    return nullptr;
  if (ClassOrNamespace *binding = context.lookupType(namedType->name(), scope)) {
    if (Symbol *objectClassSymbol = skipForwardDeclarations(binding->symbols())) {
      if (Class *klass = objectClassSymbol->asClass()) {
        for (auto it = klass->memberBegin(), end = klass->memberEnd(); it != end; ++it) {
          if (Function *func = (*it)->asFunction()) {
            if (const ConversionNameId *conversionName = func->name()->asConversionNameId()) {
...
auto senderOrReceiverClass(const CppQuickFixInterface &interface, const CppRefactoringFilePtr &file, const ExpressionAST *objectPointerAST, Scope *objectPointerScope, QString *objAccessFunction) -> Class*
{
  ...
  const QList<LookupItem> objectPointerExpressions = toe(objectPointerExpression, objectPointerScope, TypeOfExpression::Preprocess);
  ...
  if (!objectPointerType) {
    objectPointerType = determineConvertedType(objectPointerTypeBase->asNamedType(), context, objectPointerScope, objAccessFunction);
  }
...
auto findConnectReplacement(const CppQuickFixInterface &interface, const ExpressionAST *objectPointerAST, const QtMethodAST *methodAST, const CppRefactoringFilePtr &file, QString *replacement, QString *objAccessFunction) -> bool
{
  // Get name of method
  ...
  Class *objectClass = senderOrReceiverClass(interface, file, objectPointerAST, scope, objAccessFunction);
  ...
  const auto oo = CppCodeStyleSettings::currentProjectCodeStyleOverview();
  *replacement = QLatin1Char('&') + oo.prettyName(shortName);
  return true;
}

auto onConnectOrDisconnectCall(AST *ast, const ExpressionListAST **arguments) -> bool
{
  if (!ast)
    return false;
...
auto collectConnectArguments(const ExpressionListAST *arguments, const ExpressionAST **arg1, const QtMethodAST **arg2, const ExpressionAST **arg3, const QtMethodAST **arg4) -> bool
{
...
auto ConvertQt4Connect::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
...
auto ExtraRefactoringOperations::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const auto processor = CppModelManager::cppEditorDocumentProcessor(interface.filePath().toString());
  if (processor) {
...
public:
  auto count(const Name *name) -> int
  {
    counter = 0;
    accept(name);
    return counter;
...
private:
  auto visit(const Identifier *) -> void override { ++counter; }
  auto visit(const DestructorNameId *) -> void override { ++counter; }
  auto visit(const TemplateNameId *) -> void override { ++counter; }
 ...
 */
auto getBaseName(const Name *name) -> const Identifier*
{
  class GetBaseName : public NameVisitor {
    auto visit(const Identifier *name) -> void override { baseName = name; }
 ...
 */
auto countNames(const Name *name) -> int
{
 ...
 */
auto removeLine(const CppRefactoringFile *file, AST *ast, ChangeSet &changeSet) -> void
{
...
public:
   ...
   */
  auto isGlobalUsingNamespace() const -> bool { return m_parentNode == nullptr; }
   ...
   */
  auto foundGlobalUsingNamespace() const -> bool { return m_foundNamespace; }
...
private:
  ...
  auto visit(NamespaceAST *ast) -> bool override
  {
  ...
  auto visit(LinkageBodyAST *ast) -> bool override { return visitNamespaceScope(ast); }
  auto visit(CompoundStatementAST *ast) -> bool override { return visitNamespaceScope(ast); }

  auto visitNamespaceScope(AST *ast) -> bool
  {
  ...
  auto endVisit(LinkageBodyAST *ast) -> void override { endVisitNamespaceScope(ast); }
  auto endVisit(CompoundStatementAST *ast) -> void override { endVisitNamespaceScope(ast); }

  auto endVisitNamespaceScope(AST *ast) -> void
  {
    --m_namespaceScopeCounter;
  ...
  auto visit(UsingDirectiveAST *ast) -> bool override
  {
  ...
  auto visit(DeclaratorIdAST *ast) -> bool override
  {
  ...
  auto handleAstWithLongestName(AST *ast) -> bool
  {
    if (m_start) {
      ...
      for (const LookupItem &item : localLookup) {
        QList<const Name*> names = m_context.fullyQualifiedName(item.declaration(), LookupContext::HideInlineNamespaces);
  ...
  auto visit(NamedTypeSpecifierAST *ast) -> bool override { return handleAstWithName(ast); }

  auto visit(IdExpressionAST *ast) -> bool override { return handleAstWithName(ast); }

  template <typename AST>
  auto handleAstWithName(AST *ast) -> bool
  {
    if (m_start) {
      Scope *scope = m_file->scopeAt(ast->firstToken());
      ...
      if (!lookups.empty()) {
        QList<const Name*> fullName = m_context.fullyQualifiedName(lookups.first().declaration(), LookupContext::HideInlineNamespaces);
        const int currentNameCount = countNames(wantToLookup);
        const bool needNamespace = needMissingNamespaces(std::move(fullName), currentNameCount);
        if (needNamespace)
          insertMissingNamespace(ast);
      }
    }
  ...
  template <typename AST>
  auto insertMissingNamespace(AST *ast) -> void
  {
  ...
  auto needMissingNamespaces(QList<const Name*> &&fullName, int currentNameCount) -> bool
  {
  ...
  static auto nameEqual(const Name *name1, const Name *name2) -> bool
  {
    return Matcher::match(name1, name2);
  }

  auto toString(const Name *id) -> QString
  {
...
public:
  RemoveUsingNamespaceOperation(const CppQuickFixInterface &interface, UsingDirectiveAST *usingDirective, bool removeAllAtGlobalScope) : CppQuickFixOperation(interface, 1), m_usingDirective(usingDirective), m_removeAllAtGlobalScope(removeAllAtGlobalScope)
  {
    const QString name = Overview{}.prettyName(usingDirective->name->name);
    if (m_removeAllAtGlobalScope) {
      setDescription(QApplication::translate("CppEditor::QuickFix", "Remove All Occurrences of \"using namespace %1\" in Global Scope " "and Adjust Type Names Accordingly").arg(name));
    } else {
      setDescription(QApplication::translate("CppEditor::QuickFix", "Remove \"using namespace %1\" and " "Adjust Type Names Accordingly").arg(name));
    }
  }

private:
  auto buildIncludeGraph(CppRefactoringChanges &refactoring) -> std::map<Utils::FilePath, Node>
  {
    ...
    auto handleFile = [&](const FilePath &filePath, Document::Ptr doc, auto shouldHandle) {
      auto &node = includeGraph[filePath];
      node.document = doc;
    ...
    if (const Project *project = SessionManager::projectForFile(filePath())) {
      const auto files = project->files(ProjectExplorer::Project::SourceFiles);
  ...
  auto removeAllUsingsAtGlobalScope(CppRefactoringChanges &refactoring) -> void
  {
    ...
    while (!nodesWithProcessedParents.empty()) {
      ...
      CppRefactoringFilePtr file = refactoring.file(Utils::FilePath::fromString(node.document->fileName()));
      const auto parentHasUsing = Utils::anyOf(node.includes, &Node::hasGlobalUsingDirective);
      const auto startPos = parentHasUsing ? 0 : RemoveNamespaceVisitor::SearchGlobalUsingDirectivePos;
  ...
  auto perform() -> void override
  {
    ...
    } else if (refactorFile(currentFile, refactoring.snapshot(), currentFile->endOf(m_usingDirective), true)) {
   ...
   */
  auto refactorFile(CppRefactoringFilePtr &file, const Snapshot &snapshot, int startSymbol, bool removeUsing = false) -> bool
  {
    RemoveNamespaceVisitor visitor(file.get(), snapshot, m_usingDirective->name->name, startSymbol, m_removeAllAtGlobalScope);
    visitor.accept(file->cppDocument()->translationUnit()->ast());
    auto changes = visitor.getChanges();
    if (removeUsing)
  ...
  auto processIncludes(CppRefactoringChanges &refactoring, const QString &fileName) -> void
  {
    QList<Snapshot::IncludeLocation> includeLocationsOfDocument = refactoring.snapshot().includeLocationsOfDocument(fileName);
    for (Snapshot::IncludeLocation &loc : includeLocationsOfDocument) {
      if (m_processed.contains(loc.first))
        continue;

      CppRefactoringFilePtr file = refactoring.file(Utils::FilePath::fromString(loc.first->fileName()));
      const bool noGlobalUsing = refactorFile(file, refactoring.snapshot(), file->position(loc.second, 1));
...
auto RemoveUsingNamespace::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
  const QList<AST*> &path = interface.path();
  // We expect something like
  // [0] TranslationUnitAST
  ...
  if (usingDirective && usingDirective->name->name->isNameId()) {
    ...
    const auto isHeader = ProjectFile::isHeader(ProjectFile::classify(interface.filePath().toString()));
...
public:
  ConstructorMemberInfo(const QString &name, Symbol *symbol, int numberOfMember) : memberVariableName(name), parameterName(memberBaseName(name)), symbol(symbol), type(symbol->type()), numberOfMember(numberOfMember) {}
  ConstructorMemberInfo(const QString &memberName, const QString &paramName, const QString &defaultValue, Symbol *symbol, const ParentClassConstructorInfo *parentClassConstructor) : parentClassConstructor(parentClassConstructor), memberVariableName(memberName), parameterName(paramName), defaultValue(defaultValue), init(defaultValue.isEmpty()), symbol(symbol), type(symbol->type()) {}
...
class ConstructorParams : public QAbstractTableModel {
  Q_OBJECT std::list<ConstructorMemberInfo> candidates;
  std::vector<ConstructorMemberInfo*> infos;

  auto validateOrder() -> void
  {
    // parameters with default values must be at the end
    auto foundWithDefault = false;
    for (auto info : infos) {
      if (info->init) {
...
public:
  ...
  auto emplaceBackParameter(_Args &&...__args) -> void
  {
    candidates.emplace_back(std::forward<_Args>(__args)...);
    infos.push_back(&candidates.back());
  }

  auto getInfos() const -> const std::vector<ConstructorMemberInfo*>& { return infos; }

  auto addRow(ConstructorMemberInfo *info) -> void
  {
  ...
  auto removeRow(ConstructorMemberInfo *info) -> void
  {
  ...
  auto rowCount(const QModelIndex & /*parent*/ = {}) const -> int override { return int(infos.size()); }
  auto columnCount(const QModelIndex & /*parent*/ = {}) const -> int override { return 4; }

  auto data(const QModelIndex &index, int role) const -> QVariant override
  {
    if (index.row() < 0 || index.row() >= rowCount())
      return {};
    if (role == Qt::CheckStateRole && index.column() == ShouldInitColumn && !infos[index.row()]->parentClassConstructor)
    ...
    if ((role == Qt::DisplayRole || role == Qt::EditRole) && index.column() == ParameterNameColumn)
      return infos[index.row()]->parameterName;
    if ((role == Qt::DisplayRole || role == Qt::EditRole) && index.column() == DefaultValueColumn)
  ...
  auto dropMimeData(const QMimeData *data, Qt::DropAction /*action*/, int row, int /*column*/, const QModelIndex & /*parent*/) -> bool override
  {
    if (row == -1)
      row = rowCount();
    bool ok;
    auto sourceRow = data->data("application/x-qabstractitemmodeldatalist").toInt(&ok);
    if (ok) {
  ...
  auto mimeData(const QModelIndexList &indexes) const -> QMimeData* override
  {
    for (const auto &i : indexes) {
      ...
      data->setData("application/x-qabstractitemmodeldatalist", QString::number(i.row()).toLatin1());
      return data;
    }
    return nullptr;
  }
  ...
  public:
    TableViewStyle(QStyle *style) : QProxyStyle(style) {}

    auto drawPrimitive(PrimitiveElement element, const QStyleOption *option, QPainter *painter, const QWidget *widget) const -> void override
    {
...
signals:
  auto validOrder(bool valid) -> void;
};

class TopMarginDelegate : public QStyledItemDelegate {
public:
  TopMarginDelegate(QObject *parent = nullptr) : QStyledItemDelegate(parent) {}
...
struct ParentClassConstructorParameter : public ConstructorMemberInfo {
  ...
  ParentClassConstructorParameter(const QString &name, const QString &defaultValue, Symbol *symbol, const ParentClassConstructorInfo *parentClassConstructor);
...
struct ParentClassConstructorInfo {
  ParentClassConstructorInfo(const QString &name, ConstructorParams &model) : className(name), model(model) {}
  ...
  auto addParameter(ParentClassConstructorParameter &param) -> void { model.addRow(&param); }
  auto removeParameter(ParentClassConstructorParameter &param) -> void { model.removeRow(&param); }

  auto removeAllParameters() -> void
  {
...
public:
  ParentClassesModel(QObject *parent, ParentClassConstructors &constructors) : QAbstractItemModel(parent), constructors(constructors) {}
  ...
  auto data(const QModelIndex &index, int role) const -> QVariant override
  {
    ...
    if (role == Qt::SizeHintRole && index.row() > 0 && constructor.className != constructors.at(index.row() - 1).className) {
      return QSize(-1, 25);
    }
    return {};
  }
  ...
  auto setData(const QModelIndex &index, const QVariant &value, int /*role*/) -> bool override
  {
    if (index.isValid() && index.column() == 0) {
      auto info = static_cast<ParentClassConstructorInfo*>(index.internalPointer());
      if (info) {
        const auto nowUse = value.toBool();
      ...
      emit dataChanged(this->index(0, 0, parentIndex), this->index(rowCount(parentIndex), columnCount()));
...
public:
  GenerateConstructorDialog(ConstructorParams *constructorParamsModel, ParentClassConstructors &constructors)
  {
    ...
    auto optimalWidth = 0;
    for (auto i = 0; i < constructorParamsModel->columnCount(QModelIndex{}); ++i) {
      view->resizeColumnToContents(i);
      optimalWidth += view->columnWidth(i);
    }
    ...
    const auto errorLabel = new QLabel(tr("Parameters without default value must come before parameters with default value."));
    errorLabel->setStyleSheet("color: #ff0000");
    errorLabel->setVisible(false);
    auto labelSizePolicy = errorLabel->sizePolicy();
    labelSizePolicy.setRetainSizeWhenHidden(true);
    errorLabel->setSizePolicy(labelSizePolicy);
    connect(constructorParamsModel, &ConstructorParams::validOrder, [=, button = buttonBox->button(QDialogButtonBox::Ok)](bool valid) {
      button->setEnabled(valid);
      errorLabel->setVisible(!valid);
    });

    // setup select all/none checkbox
    const auto checkBox = new QCheckBox(tr("Initialize all members"));
    checkBox->setChecked(true);
    connect(checkBox, &QCheckBox::stateChanged, [model = constructorParamsModel](int state) {
      if (state != Qt::PartiallyChecked) {
        for (auto i = 0; i < model->rowCount(); ++i)
          model->setData(model->index(i, ConstructorParams::ShouldInitColumn), state, Qt::CheckStateRole);
      }
    });
    ...
    connect(constructorParamsModel, &QAbstractItemModel::dataChanged, this, [model = constructorParamsModel, checkBox] {
  ...
  auto accessSpec() const -> InsertionPointLocator::AccessSpec { return m_accessSpec; }
...
public:
  GenerateConstructorOperation(const CppQuickFixInterface &interface) : CppQuickFixOperation(interface)
  {
    ...
    for (auto it = theClass->memberBegin(); it != theClass->memberEnd(); ++it) {
      ...
      if (s->isDeclaration() && (s->isPrivate() || s->isProtected()) && !s->isStatic()) {
        const auto name = QString::fromUtf8(s->identifier()->chars(), s->identifier()->size());
        ...
        for (auto i = d->asClass()->memberBegin(); i != d->asClass()->memberEnd(); ++i) {
          ...
          if (s->isProtected() || s->isPublic()) {
            if (s->name()->match(d->name())) {
              ...
              if (!func)
                continue;
              const bool isFirst = parentClassConstructors.empty() || parentClassConstructors.back().className != className;
              parentClassConstructors.emplace_back(className, parameterModel);
              auto &constructor = parentClassConstructors.back();
              constructor.declaration = className + o.prettyType(func->type());
              constructor.declaration.replace("std::__1::__get_nullptr_t()", "nullptr");
              ...
              for (auto arg = func->memberBegin(); arg != func->memberEnd(); ++arg) {
                ...
                QString defaultValue;
                if (ini)
                  defaultValue = QString::fromUtf8(ini->chars(), ini->size()).replace("std::__1::__get_nullptr_t()", "nullptr");
                constructor.parameters.emplace_back(name, defaultValue, param, &constructor);
  ...
  auto isApplicable() const -> bool
  {
    return parameterModel.rowCount() > 0 || Utils::anyOf(parentClassConstructors, [](const auto &parent) { return !parent.parameters.empty(); });
  }

  auto setTest(bool isTest = true) -> void { m_test = isTest; }
...
private:
  auto perform() -> void override
  {
    ...
    auto accessSpec = InsertionPointLocator::Public;
    if (!m_test) {
    ...
    struct GenerateConstructorRefactoringHelper : public GetterSetterRefactoringHelper {
      ...
      InsertionPointLocator::AccessSpec m_accessSpec;
      GenerateConstructorRefactoringHelper(CppQuickFixOperation *operation, const QString &fileName, Class *clazz, const ClassSpecifierAST *classAST, InsertionPointLocator::AccessSpec accessSpec) : GetterSetterRefactoringHelper(operation, fileName, clazz), m_classAST(classAST), m_accessSpec(accessSpec) {}

      auto generateConstructor(std::vector<ConstructorMemberInfo*> members, const ParentClassConstructors &parentClassConstructors) -> void
      {
        auto constructorLocation = m_settings->determineSetterLocation(int(members.size()));
        if (constructorLocation == CppQuickFixSettings::FunctionLocation::CppFile && !hasSourceFile())
        ...
        if (constructorLocation == CppQuickFixSettings::FunctionLocation::CppFile) {
          ...
          if (m_settings->rewriteTypesinCppFile())
            ...
            implCode = className;
          implCode += "::" + className + "(";
        } else if (constructorLocation == CppQuickFixSettings::FunctionLocation::OutsideClass) {
          implLoc = insertLocationForMethodDefinition(m_class, false, NamespaceHandling::Ignore, m_changes, m_headerFile->filePath().toString(), &insertedNamespaces);
        ...
        auto constructorBody = members.empty() ? QString(") {}") : QString(") : ");
        for (auto &member : members) {
          ...
          inClassDeclaration += ", ";
          auto param = member->parameterName;
          if (implFile) {
            FullySpecifiedType type = typeAt(member->type, m_class, implFile, implLoc, insertedNamespaces);
        ...
        for (const auto &parent : parentClassConstructors) {
          ...
          if (Utils::anyOf(parent.parameters, [](const auto &param) {
            ...
            auto defaultAtEndCount = 0;
            for (auto i = parent.parameters.crbegin(); i != parent.parameters.crend(); ++i) {
              if (i->init || i->originalDefaultValue.isEmpty())
                break;
              ++defaultAtEndCount;
            }
            const auto numberOfParameters = static_cast<int>(parent.parameters.size()) - defaultAtEndCount;
        ...
        insertAndIndent(m_headerFile, m_locator.constructorDeclarationInClass(tu, m_classAST, m_accessSpec, int(members.size())), inClassDeclaration);
    ...
    GenerateConstructorRefactoringHelper helper(this, currentFile()->filePath().toString(), m_classAST->symbol, m_classAST, accessSpec);
...
auto GenerateConstructor::match(const CppQuickFixInterface &interface, QuickFixOperations &result) -> void
{
