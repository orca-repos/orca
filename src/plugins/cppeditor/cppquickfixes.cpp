...
auto matchName(const Name *name, QList<Core::LocatorFilterEntry> *matches, QString *className) -> bool
{
  if (!name)
    return false;
...
private:
  auto perform() -> void override
  {
    auto type = m_type;
    if (type.isEmpty()) {
      type = QInputDialog::getText(Core::ICore::dialogParent(), QCoreApplication::translate("CppEditor::Quickfix", "Provide the type"), QCoreApplication::translate("CppEditor::Quickfix", "Data type:"), QLineEdit::Normal);
    }
    if (type.isEmpty())
      return;
...
public:
  AddImplementationsDialog(const QList<Symbol*> &candidates, const Utils::FilePath &implFile) : QDialog(Core::ICore::dialogParent()), m_candidates(candidates)
  {
