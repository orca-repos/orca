...
#include <texteditor/codeassist/iassistproposalmodel.hpp>
...
static bool closeEditorsWithoutGarbageCollectorInvocation(const QList<Orca::Plugin::Core::IEditor *> &editors)
{
    CppModelManager::instance()->enableGarbageCollector(false);
    const bool closeEditorsSucceeded = Orca::Plugin::Core::EditorManager::closeEditors(editors, false);
...
void TestCase::closeEditorAtEndOfTestCase(Orca::Plugin::Core::IEditor *editor)
{
    if (editor && !m_editorsToClose.contains(editor))
        m_editorsToClose.append(editor);
}

bool TestCase::closeEditorWithoutGarbageCollectorInvocation(Orca::Plugin::Core::IEditor *editor)
{
