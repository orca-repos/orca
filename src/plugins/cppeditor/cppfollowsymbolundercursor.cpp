...
public:
  VirtualFunctionHelper(TypeOfExpression &typeOfExpression, Scope *scope, const Document::Ptr &document, const Snapshot &snapshot, SymbolFinder *symbolFinder);
  VirtualFunctionHelper() = delete;

  auto canLookupVirtualFunctionOverrides(Function *function) -> bool;

  /// Returns != 0 if canLookupVirtualFunctionOverrides() succeeded.
  auto staticClassOfFunctionCallExpression() const -> Class* { return m_staticClassOfFunctionCallExpression; }

private:
  Q_DISABLE_COPY(VirtualFunctionHelper)

  auto staticClassOfFunctionCallExpression_internal() const -> Class*;
...
auto VirtualFunctionHelper::canLookupVirtualFunctionOverrides(Function *function) -> bool
{
  m_function = function;
  ...
  if (!m_baseExpressionAST || !m_expressionDocument || m_scope->isClass() || m_scope->isFunction()) {
    return false;
  }
  ...
  if (IdExpressionAST *idExpressionAST = m_baseExpressionAST->asIdExpression()) {
    ...
    result = !nameIsQualified && Internal::FunctionUtils::isVirtualFunction(function, LookupContext(m_document, m_snapshot));
  } else if (MemberAccessAST *memberAccessAST = m_baseExpressionAST->asMemberAccess()) {
    NameAST *name = memberAccessAST->member_name;
    const bool nameIsQualified = name && name->asQualifiedName();
    if (!nameIsQualified && Internal::FunctionUtils::isVirtualFunction(function, LookupContext(m_document, m_snapshot))) {
      ...
      if (m_accessTokenKind == T_ARROW) {
        ...
        const QList<LookupItem> items = m_typeOfExpression.reference(memberAccessAST->base_expression, m_document, m_scope);
        if (!items.isEmpty()) {
...
auto VirtualFunctionHelper::staticClassOfFunctionCallExpression_internal() const -> Class*
{
  if (!m_finder)
    return nullptr;
  ...
  if (m_baseExpressionAST->asIdExpression()) {
    ...
    const QList<LookupItem> items = m_typeOfExpression(memberAccessAST->base_expression, m_expressionDocument, m_scope);
...
auto findMacroLink_helper(const QByteArray &name, Document::Ptr doc, const Snapshot &snapshot, QSet<QString> *processed) -> Link
{
  if (doc && !name.startsWith('<') && !processed->contains(doc->fileName())) {
    ...
    for (int index = includes.size() - 1; index != -1; --index) {
      ...
      Link link = findMacroLink_helper(name, snapshot.document(i.resolvedFileName()), snapshot, processed);
...
auto findMacroLink(const QByteArray &name, const Document::Ptr &doc) -> Link
{
  if (!name.isEmpty()) {
    if (doc) {
...
static auto isForwardClassDeclaration(Type *type) -> bool
{
  if (!type)
    return false;
...
inline auto skipForwardDeclarations(const QList<LookupItem> &resolvedSymbols) -> LookupItem
{
...
auto attemptDeclDef(const QTextCursor &cursor, Snapshot snapshot, const Document::Ptr &document, SymbolFinder *symbolFinder) -> Link
{
  Link result;
  ...
  if (FunctionDefinitionAST *funDef = declParent->asFunctionDefinition()) {
    QList<Declaration*> candidates = symbolFinder->findMatchingDeclaration(LookupContext(document, snapshot), funDef->symbol);
  ...
  if (target) {
    ...
    document->translationUnit()->getTokenStartPosition(name->firstToken(), &startLine, &startColumn);
    document->translationUnit()->getTokenEndPosition(name->lastToken() - 1, &endLine, &endColumn);

    auto textDocument = cursor.document();
    result.linkTextStart = textDocument->findBlockByNumber(startLine - 1).position() + startColumn - 1;
    result.linkTextEnd = textDocument->findBlockByNumber(endLine - 1).position() + endColumn - 1;
  }

  return result;
}

auto findDefinition(Symbol *symbol, const Snapshot &snapshot, SymbolFinder *symbolFinder) -> Symbol*
{
...
auto maybeAppendArgumentOrParameterList(QString *expression, const QTextCursor &textCursor) -> bool
{
  QTC_ASSERT(expression, return false);
  auto textDocument = textCursor.document();
...
auto isCursorOnTrailingReturnType(const QList<AST*> &astPath) -> bool
{
  if (astPath.size() < 3)
    return false;
  for (auto it = astPath.cend() - 3, begin = astPath.cbegin(); it >= begin; --it) {
    ...
    return (*nextIt)->asNamedTypeSpecifier() && ((*nextNextIt)->asSimpleName() || (*nextNextIt)->asQualifiedName() || (*nextNextIt)->asTemplateId());
  }
  return false;
}

auto maybeFixExpressionInTrailingReturnType(QString *expression, const QTextCursor &textCursor, const Document::Ptr documentFromSemanticInfo) -> void
{
...
auto expressionUnderCursorAsString(const QTextCursor &textCursor, const Document::Ptr documentFromSemanticInfo, const LanguageFeatures &features) -> QString
{
...
FollowSymbolUnderCursor::FollowSymbolUnderCursor() : m_virtualFunctionAssistProvider(new VirtualFunctionAssistProvider) {}

static auto skipMatchingParentheses(const Tokens &tokens, int idx, int initialDepth) -> int
{
  auto j = idx;
...
auto FollowSymbolUnderCursor::findLink(const CursorInEditor &data, Utils::ProcessLinkCallback &&processLinkCallback, bool resolveTarget, const Snapshot &theSnapshot, const Document::Ptr &documentFromSemanticInfo, SymbolFinder *symbolFinder, bool inNextSplit) -> void
{
  Link link;
  ...
  auto snapshot = theSnapshot;

  // Move to end of identifier
  auto tc = cursor;
  auto ch = document->characterAt(tc.position());
  ...
  if (documentFromSemanticInfo && documentFromSemanticInfo->translationUnit() && documentFromSemanticInfo->translationUnit()->ast()) {
    auto pos = tc.position();
    while (document->characterAt(pos).isSpace())
      ++pos;
    const auto ch = document->characterAt(pos);
    if (ch == '(' || ch == ';') {
  ...
  const LanguageFeatures features = documentFromSemanticInfo ? documentFromSemanticInfo->languageFeatures() : LanguageFeatures::defaultFeatures();
  ...
  for (auto i = 0; i < tokens.size(); ++i) {
    ...
    if (positionInBlock >= tk.utf16charsBegin() && positionInBlock < tk.utf16charsEnd()) {
      ...
      if (i >= 2 && tokens.at(i).is(T_IDENTIFIER) && tokens.at(i - 1).is(T_LPAREN) && (tokens.at(i - 2).is(T_SIGNAL) || tokens.at(i - 2).is(T_SLOT))) {
      ...
      } else if ((i > 3 && tk.is(T_LPAREN) && tokens.at(i - 1).is(T_IDENTIFIER) && tokens.at(i - 2).is(T_LPAREN) && (tokens.at(i - 3).is(T_SIGNAL) || tokens.at(i - 3).is(T_SLOT)))) {
  ...
  if (!recognizedQtMethod && documentFromSemanticInfo) {
    auto cursorRegionReached = false;
    for (auto i = 0; i < tokens.size(); ++i) {
      const Token &tk = tokens.at(i);
      ...
      if (positionInBlock >= tk.utf16charsBegin() && positionInBlock <= tk.utf16charsEnd()) {
        ...
        if (tk.is(T_OPERATOR)) {
          link = attemptDeclDef(cursor, theSnapshot, documentFromSemanticInfo, symbolFinder);
          if (link.hasValidLinkText())
            return processLinkCallback(link);
        } else if (tk.isPunctuationOrOperator() && i > 0 && tokens.at(i - 1).is(T_OPERATOR)) {
          auto c = cursor;
          c.movePosition(QTextCursor::Left, QTextCursor::MoveAnchor, positionInBlock - tokens.at(i - 1).utf16charsBegin());
  ...
  if (!recognizedQtMethod) {
    ...
    auto ch = document->characterAt(cursor.position());
    if (pos > 0 && !isValidIdentifierChar(ch))
      --pos; // positionInBlock points to a delimiter character.
    const Token tk = SimpleLexer::tokenAt(block.text(), pos, BackwardsScanner::previousBlockState(block), features);
    ...
    if (tk.is(T_STRING_LITERAL) || tk.is(T_ANGLE_STRING_LITERAL)) {
      const auto lineno = cursor.blockNumber() + 1;
  ...
  const QString expression = expressionUnderCursorAsString(tc, documentFromSemanticInfo, features);
  const QSharedPointer<TypeOfExpression> typeOfExpression(new TypeOfExpression);
  typeOfExpression->init(doc, snapshot);
  // make possible to instantiate templates
  typeOfExpression->setExpandTemplates(true);
  const QList<LookupItem> resolvedSymbols = typeOfExpression->reference(expression.toUtf8(), scope, TypeOfExpression::Preprocess);
    ...
    foreach(const LookupItem &r, resolvedSymbols) {
      if (Symbol *d = r.declaration()) {
        if (d->isDeclaration() || d->isFunction()) {
          ...
          auto tokenBeginLineNumber = 0;
          auto tokenBeginColumnNumber = 0;
          Utils::Text::convertPosition(document, beginOfToken, &tokenBeginLineNumber, &tokenBeginColumnNumber);
          if (tokenBeginLineNumber > d->line() || (tokenBeginLineNumber == d->line() && tokenBeginColumnNumber >= d->column())) {
            result = r; // take the symbol under cursor.
            break;
          }
        }
      ...
      if (resolveTarget) {
        ...
        if (helper.canLookupVirtualFunctionOverrides(function)) {
          ...
          if (m_virtualFunctionAssistProvider->configure(params)) {
            editorWidget->invokeTextEditorWidgetAssist(FollowSymbol, m_virtualFunctionAssistProvider.data());
...
auto FollowSymbolUnderCursor::switchDeclDef(const CursorInEditor &data, Utils::ProcessLinkCallback &&processLinkCallback, const CPlusPlus::Snapshot &snapshot, const CPlusPlus::Document::Ptr &documentFromSemanticInfo, SymbolFinder *symbolFinder) -> void
{
  if (!documentFromSemanticInfo) {
    processLinkCallback({});
    return;
  ...
  if (functionDeclarationSymbol) {
    ...
    auto symbol = symbolFinder->findMatchingVarDefinition(declarationSymbol, snapshot);
    if (symbol)
      symbolLink = symbol->toLink();
    ...
    const QList<LookupItem> declarations = context.lookup(functionDefinitionSymbol->name(), functionDefinitionSymbol->enclosingScope());
...
auto FollowSymbolUnderCursor::virtualFunctionAssistProvider() -> QSharedPointer<VirtualFunctionAssistProvider>
{
  return m_virtualFunctionAssistProvider;
}

auto FollowSymbolUnderCursor::setVirtualFunctionAssistProvider(const QSharedPointer<VirtualFunctionAssistProvider> &provider) -> void
{
