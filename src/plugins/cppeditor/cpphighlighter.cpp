...
CppHighlighter::CppHighlighter(QTextDocument *document) : SyntaxHighlighter(document)
{
  setDefaultTextFormatCategories();
}

auto CppHighlighter::highlightBlock(const QString &text) -> void
{
  const auto previousBlockState_ = previousBlockState();
  auto lexerState = 0, initialBraceDepth = 0;
  if (previousBlockState_ != -1) {
    lexerState = previousBlockState_ & 0xff;
    initialBraceDepth = previousBlockState_ >> 8;
  }

  auto braceDepth = initialBraceDepth;
  ...
  auto initialLexerState = lexerState;
  ...
  auto foldingIndent = initialBraceDepth;
  if (auto userData = TextDocumentLayout::textUserData(currentBlock())) {
  ...
  auto parentheses = Utils::filtered(TextDocumentLayout::parentheses(currentBlock()), [](const Parenthesis &p) { return p.source.isValid(); });
  const auto insertParen = [&parentheses](const Parenthesis &p) { insertSorted(parentheses, p); };
  parentheses.reserve(5);

  auto expectPreprocessorKeyword = false;
  auto onlyHighlightComments = false;

  for (auto i = 0; i < tokens.size(); ++i) {
    const Token &tk = tokens.at(i);

    auto previousTokenEnd = 0;
    if (i != 0) {
      // mark the whitespaces
      previousTokenEnd = tokens.at(i - 1).utf16charsBegin() + tokens.at(i - 1).utf16chars();
    }

    if (previousTokenEnd != tk.utf16charsBegin()) {
      setFormat(previousTokenEnd, tk.utf16charsBegin() - previousTokenEnd, formatForCategory(C_VISUAL_WHITESPACE));
    }
    ...
    auto highlightCurrentWordAsPreprocessor = expectPreprocessorKeyword;
    ...
    if (i == 0 && tk.is(T_POUND)) {
      setFormatWithSpaces(text, tk.utf16charsBegin(), tk.utf16chars(), formatForCategory(C_PREPROCESSOR));
      expectPreprocessorKeyword = true;
    } else if (highlightCurrentWordAsPreprocessor && (tk.isKeyword() || tk.is(T_IDENTIFIER)) && isPPKeyword(Utils::midView(text, tk.utf16charsBegin(), tk.utf16chars()))) {
      ...
      if (ppKeyword == QLatin1String("error") || ppKeyword == QLatin1String("warning") || ppKeyword == QLatin1String("pragma")) {
      ...
      if (tk.is(T_COMMENT) || tk.is(T_CPP_COMMENT)) {
        setFormatWithSpaces(text, startPosition, tk.utf16charsEnd() - startPosition, formatForCategory(C_COMMENT));
      ...
      if (initialLexerState && i == 0 && (tk.is(T_COMMENT) || tk.is(T_DOXY_COMMENT)) && (tokens.size() > 1 || !lexerState)) {
        --braceDepth;
        // unless we are at the end of the block, we reduce the folding indent
        if (i == tokens.size() - 1)
          TextDocumentLayout::userData(currentBlock())->setFoldingEndIncluded(true);
    ...
    } else if (tk.isKeyword() || (m_languageFeatures.qtKeywordsEnabled && isQtKeyword(QStringView{text}.mid(tk.utf16charsBegin(), tk.utf16chars()))) || (m_languageFeatures.objCEnabled && tk.isObjCAtKeyword())) {
      setFormat(tk.utf16charsBegin(), tk.utf16chars(), formatForCategory(C_KEYWORD));
    } else if (tk.isPrimitiveType()) {
      setFormat(tk.utf16charsBegin(), tk.utf16chars(), formatForCategory(C_PRIMITIVE_TYPE));
      ...
      highlightWord(Utils::midView(text, tk.utf16charsBegin(), tk.utf16chars()), tk.utf16charsBegin(), tk.utf16chars());
    }
  }
  ...
  auto currentState = currentBlockState();
  if (currentState != -1) {
    auto oldState = currentState & 0xff;
    auto oldBraceDepth = currentState >> 8;
    if (oldState == tokenize.state() && oldBraceDepth != braceDepth) {
      ...
      auto delta = braceDepth - oldBraceDepth;
  ...
  TextDocumentLayout::setExpectedRawStringSuffix(currentBlock(), tokenize.expectedRawStringSuffix());
}

auto CppHighlighter::setLanguageFeatures(const LanguageFeatures &languageFeatures) -> void
{
...
auto CppHighlighter::isPPKeyword(QStringView text) const -> bool
{
  switch (text.length()) {
  case 2:
  ...
  case 4:
    if (text.at(0) == QLatin1Char('e') && (text == QLatin1String("elif") || text == QLatin1String("else")))
      return true;
    break;
...
auto CppHighlighter::highlightWord(QStringView word, int position, int length) -> void
{
  ...
  if (word.length() > 2 && word.at(0) == QLatin1Char('Q')) {
    if (word.at(1) == QLatin1Char('_') // Q_
      || (word.at(1) == QLatin1Char('T') && word.at(2) == QLatin1Char('_'))) {
      // QT_
...
auto CppHighlighter::highlightRawStringLiteral(QStringView _text, const Token &tk) -> bool
{
  ...
  const auto delimiterFormat = formatForCategory(C_KEYWORD);
  const int stringOffset = delimiterOffset + delimiter.length() + 1;
  setFormat(tk.utf16charsBegin(), stringOffset, delimiterFormat);
  setFormatWithSpaces(text, stringOffset, endDelimiterOffset - stringOffset - 1, formatForCategory(C_STRING));
  setFormat(endDelimiterOffset - 1, delimiter.length() + 2, delimiterFormat);
  return true;
}

auto CppHighlighter::highlightDoxygenComment(const QString &text, int position, int) -> void
{
  auto initial = position;

  auto uc = text.unicode();
  auto it = uc + position;

  const auto &format = formatForCategory(C_DOXYGEN_COMMENT);
  const auto &kwFormat = formatForCategory(C_DOXYGEN_TAG);

  while (!it->isNull()) {
    if (it->unicode() == QLatin1Char('\\') || it->unicode() == QLatin1Char('@')) {
      ++it;
      ...
      auto k = classifyDoxygenTag(start, it - start);
