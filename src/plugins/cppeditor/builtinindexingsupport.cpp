...
public:
  ...
  auto runSearch(QFutureInterface<Orca::Plugin::Core::SearchResultItem> &future) -> void override
  {
    ...
    auto findString = (m_parameters.flags & Orca::Plugin::Core::FindRegularExpression ? m_parameters.text : QRegularExpression::escape(m_parameters.text));
    if (m_parameters.flags & Orca::Plugin::Core::FindWholeWords)
      findString = QString::fromLatin1("\\b%1\\b").arg(findString);
    QRegularExpression matcher(findString, (m_parameters.flags & Orca::Plugin::Core::FindCaseSensitively ? QRegularExpression::NoPatternOption : QRegularExpression::CaseInsensitiveOption));
    matcher.optimize();
    while (it != m_snapshot.end()) {
      ...
      if (m_fileNames.isEmpty() || m_fileNames.contains(it.value()->fileName())) {
        QVector<Orca::Plugin::Core::SearchResultItem> resultItems;
...
auto BuiltinIndexingSupport::refreshSourceFiles(const QSet<QString> &sourceFiles, CppModelManager::ProgressNotificationMode mode) -> QFuture<void>
{
  ...
  if (mode == CppModelManager::ForcedProgressNotification || sourceFiles.count() > 1) {
    Orca::Plugin::Core::ProgressManager::addTask(result, QCoreApplication::translate("CppEditor::Internal::BuiltinIndexingSupport", "Parsing C/C++ Files"), CppEditor::Constants::TASK_INDEX);
  }
