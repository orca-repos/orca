...
FunctionDeclDefLinkFinder::FunctionDeclDefLinkFinder(QObject *parent) : QObject(parent) {}

auto FunctionDeclDefLinkFinder::onFutureDone() -> void
{
  auto link = m_watcher->result();
  m_watcher.reset();
  if (link) {
  ...
  if (link) emit foundLink(link);
}

auto FunctionDeclDefLinkFinder::scannedSelection() const -> QTextCursor
{
...
static auto findDeclOrDef(const Document::Ptr &doc, int line, int column, DeclarationAST **parent, DeclaratorAST **decl, FunctionDeclaratorAST **funcDecl) -> bool
{
...
static auto declDefLinkStartEnd(const CppRefactoringFileConstPtr &file, DeclarationAST *parent, FunctionDeclaratorAST *funcDecl, int *start, int *end) -> void
{
...
static auto getDeclaratorId(DeclaratorAST *declarator) -> DeclaratorIdAST*
{
...
static auto findLinkHelper(QSharedPointer<FunctionDeclDefLink> link, CppRefactoringChanges changes) -> QSharedPointer<FunctionDeclDefLink>
{
  ...
  if (FunctionDefinitionAST *funcDef = link->sourceDeclaration->asFunctionDefinition()) {
    ...
    finder.findMatchingDeclaration(LookupContext(link->sourceDocument, snapshot), funcDef->symbol, &typeMatch, &argumentCountMatch, &nameMatch);
  ...
  const auto targetFileName = QString::fromUtf8(target->fileName(), target->fileNameLength());
  auto targetFile = changes.fileNoEditor(Utils::FilePath::fromString(targetFileName));
  ...
  if (!findDeclOrDef(targetFile->cppDocument(), target->line(), target->column(), &targetParent, &targetDeclarator, &targetFuncDecl))
    return noResult;
  ...
  QString targetInitial = targetFile->textOf(targetFile->startOf(targetParent), targetEnd);
...
auto FunctionDeclDefLinkFinder::startFindLinkAt(QTextCursor cursor, const Document::Ptr &doc, const Snapshot &snapshot) -> void
{
  // check if cursor is on function decl/def
  DeclarationAST *parent = nullptr;
  FunctionDeclaratorAST *funcDecl = nullptr;
  DeclaratorAST *declarator = nullptr;
  if (!findDeclOrDef(doc, cursor.blockNumber() + 1, cursor.columnNumber() + 1, &parent, &declarator, &funcDecl))
    return;

  // find the start/end offsets
  CppRefactoringChanges refactoringChanges(snapshot);
  CppRefactoringFilePtr sourceFile = refactoringChanges.file(Utils::FilePath::fromString(doc->fileName()));
  ...
  if (!m_scannedSelection.isNull() && m_scannedSelection.selectionStart() == start && m_scannedSelection.selectionEnd() == end) {
    return;
  }
...
auto FunctionDeclDefLink::isValid() const -> bool
{
  return !linkSelection.isNull();
}

auto FunctionDeclDefLink::isMarkerVisible() const -> bool
{
  return hasMarker;
}

static auto namesEqual(const Name *n1, const Name *n2) -> bool
{
  return n1 == n2 || (n1 && n2 && n1->match(n2));
}

auto FunctionDeclDefLink::apply(CppEditorWidget *editor, bool jumpToMatch) -> void
{
  ...
  if (targetInitial == newTargetFile->textOf(targetStart, targetEnd)) {
    ...
    ToolTip::show(editor->toolTipPosition(linkSelection), tr("Target file was changed, could not apply changes"));
  }
}

auto FunctionDeclDefLink::hideMarker(CppEditorWidget *editor) -> void
{
  if (!hasMarker)
    return;
  editor->setRefactorMarkers(RefactorMarker::filterOutType(editor->refactorMarkers(), Constants::CPP_FUNCTION_DECL_DEF_LINK_MARKER_ID));
  hasMarker = false;
}

auto FunctionDeclDefLink::showMarker(CppEditorWidget *editor) -> void
{
  if (hasMarker)
    return;

  auto markers = RefactorMarker::filterOutType(editor->refactorMarkers(), Constants::CPP_FUNCTION_DECL_DEF_LINK_MARKER_ID);
  ...
  const auto endBlockNr = marker.cursor.blockNumber();
  marker.cursor.setPosition(linkSelection.selectionEnd() + 1, QTextCursor::KeepAnchor);
  if (marker.cursor.blockNumber() != endBlockNr || marker.cursor.selectedText() != QLatin1String(";")) {
    marker.cursor.setPosition(linkSelection.selectionEnd());
  }
  ...
  auto quickfixCommand = Orca::Plugin::Core::ActionManager::command(TextEditor::Constants::QUICKFIX_THIS);
  if (quickfixCommand)
...
static auto declaredParameterCount(Function *function) -> int
{
...
static auto hasCommentedName(TranslationUnit *unit, const QString &source, FunctionDeclaratorAST *declarator, int i) -> bool
{
  if (!declarator || !declarator->parameter_declaration_clause || !declarator->parameter_declaration_clause->parameter_declaration_list)
    return false;
  ...
  auto text = source.mid(nameStart, nameEnd - nameStart);
...
static auto canReplaceSpecifier(TranslationUnit *translationUnit, SpecifierAST *specifier) -> bool
{
...
static auto findFirstReplaceableSpecifier(TranslationUnit *translationUnit, SpecifierListAST *list) -> SpecifierAST*
{
...
static auto findUniqueTypeMatch(int sourceParamIndex, Function *sourceFunction, Function *newFunction, const IndicesListType &sourceParams, const IndicesListType &newParams) -> int
{
  ...
  for (auto i = 0; i < sourceParams.size(); ++i) {
  ...
  auto newParamWithSameTypeIndex = -1;
  for (auto i = 0; i < newParams.size(); ++i) {
...
static auto unmatchedIndices(const IndicesList &indices) -> IndicesList
{
  IndicesList ret;
...
static auto ensureCorrectParameterSpacing(const QString &text, bool isFirstParam) -> QString
{
  if (isFirstParam) {
    // drop leading spaces
...
static auto findCommaTokenBetween(const CppRefactoringFileConstPtr &file, ParameterDeclarationAST *left, ParameterDeclarationAST *right) -> unsigned
{
  unsigned last = left->lastToken() - 1;
  for (unsigned tokenIndex = right->firstToken(); tokenIndex > last; --tokenIndex) {
...
auto FunctionDeclDefLink::changes(const Snapshot &snapshot, int targetOffset) -> ChangeSet
{
  ChangeSet changes;
  ...
  auto newDeclText = linkSelection.selectedText();
  for (auto i = 0; i < newDeclText.size(); ++i) {
  ...
  const auto overviewFromCurrentProjectStyle = CppCodeStyleSettings::currentProjectCodeStyleOverview();

  auto overview = overviewFromCurrentProjectStyle;
  ...
  if (!newDeclId || !newDeclId->name || !newDeclId->name->name || overview.prettyName(newDeclId->name->name) != normalizedInitialName()) {
    return changes;
  }
  ...
  do {
    ...
    if (SimpleDeclarationAST *simple = targetDeclaration->asSimpleDeclaration()) {
      ...
      firstReplaceableSpecifier = findFirstReplaceableSpecifier(targetTranslationUnit, simple->decl_specifier_list);
    } else if (FunctionDefinitionAST *def = targetDeclaration->asFunctionDefinition()) {
      declarator = def->declarator;
      firstReplaceableSpecifier = findFirstReplaceableSpecifier(targetTranslationUnit, def->decl_specifier_list);
    } else {
      // no proper AST to synchronize the return type
      break;
    }

    auto returnTypeStart = 0;
    ...
    if (!newFunction->returnType().match(sourceFunction->returnType()) && !newFunction->returnType().match(targetFunction->returnType())) {
      ...
      changes.replace(returnTypeStart, targetFile->startOf(targetFunctionDeclarator->lparen_token), replacement);
    }
  } while (false);
  ...
  // sync parameters
  {
    ...
    if (targetFunctionDeclarator->parameter_declaration_clause) {
      for (ParameterDeclarationListAST *it = targetFunctionDeclarator->parameter_declaration_clause->parameter_declaration_list; it; it = it->next) {
    ...
    // fill newParamToSourceParam
    {
      ...
      for (auto sourceParamIndex = 0; sourceParamIndex < existingParamCount; ++sourceParamIndex) {
        Symbol *sourceParam = sourceFunction->argumentAt(sourceParamIndex);
        ...
        if (newParams.size() == 1 && sourceParams.size() == 1) {
          ...
          const int newParamWithSameTypeIndex = findUniqueTypeMatch(sourceParamIndex, sourceFunction, newFunction, sourceParams, newParams);
          if (newParamWithSameTypeIndex != -1) {
      ...
      for (auto i = 0; i < freeSourceParams.size(); ++i) {
        int sourceParamIndex = freeSourceParams.at(i);
        const int newParamWithSameTypeIndex = findUniqueTypeMatch(sourceParamIndex, sourceFunction, newFunction, freeSourceParams, freeNewParams);
        if (newParamWithSameTypeIndex != -1) {
          sourceParamToNewParam[sourceParamIndex] = newParamWithSameTypeIndex;
          newParamToSourceParam[newParamWithSameTypeIndex] = sourceParamIndex;
        }
      ...
      for (auto i = 0; i < existingParamCount && i < newParamCount; ++i) {
    ...
    for (auto newParamIndex = 0; newParamIndex < newParamCount; ++newParamIndex) {
      ...
      if (existingParamIndex == -1) {
        ...
        if (hasCommentedName(targetFile->cppDocument()->translationUnit(), QString::fromUtf8(targetFile->cppDocument()->utf8Source()), targetFunctionDeclarator, existingParamIndex))
        ...
        if (!newParam->type().match(sourceParam->type()) && !replacementType.match(targetParam->type())) {
          const int parameterTypeStart = targetFile->startOf(targetParamAst);
          auto parameterTypeEnd = 0;
          ...
          if (id) {
            ...
            if (replacementNameStr.isEmpty()) {
              ...
              if (nextToken == T_COMMA || nextToken == T_EQUAL || nextToken == T_RPAREN) {
    ...
    if (hadChanges) {
      changes.replace(targetFile->endOf(targetFunctionDeclarator->lparen_token), targetFile->startOf(targetFunctionDeclarator->rparen_token), newTargetParameters);
    }
    ...
    if (targetDefinition && !renamedTargetParameters.isEmpty()) {
      ...
      for (auto it = renamedTargetParameters.cbegin(), end = renamedTargetParameters.cend(); it != end; ++it) {
  ...
  if (targetFunction->isConst() != newFunction->isConst() || targetFunction->isVolatile() != newFunction->isVolatile()) {
    QString cvString;
  ...
  // sync noexcept/throw()
  const QString exceptionSpecTarget = targetFunction->exceptionSpecification() ? QString::fromUtf8(targetFunction->exceptionSpecification()->chars()) : QString();
  const QString exceptionSpecNew = newFunction->exceptionSpecification() ? QString::fromUtf8(newFunction->exceptionSpecification()->chars()) : QString();
  if (exceptionSpecTarget != exceptionSpecNew) {
    if (!exceptionSpecTarget.isEmpty() && !exceptionSpecNew.isEmpty()) {
      changes.replace(targetFile->range(targetFunctionDeclarator->exception_specification), exceptionSpecNew);
  ...
  if (targetOffset != -1) {
    ...
    auto ops = changes.operationList();
...
auto FunctionDeclDefLink::normalizedInitialName() const -> QString
{
  auto n = nameInitial;
