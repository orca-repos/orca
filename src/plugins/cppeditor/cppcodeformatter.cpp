...
auto CodeFormatter::setTabSize(int tabSize) -> void
{
  m_tabSize = tabSize;
}

auto CodeFormatter::recalculateStateAfter(const QTextBlock &block) -> void
{
  restoreCurrentState(block.previous());

  auto endedJoined = false;
  // Discard newline expected bit from state
  const auto lexerState = tokenizeBlock(block, &endedJoined) & ~0x80;
  m_tokenIndex = 0;
  m_newStates.clear();
  ...
  while (m_tokenIndex < m_tokens.size()) {
    ...
    case namespace_open:
      ...
      case T_RBRACE:
        leave();
        continue; // always nested in namespace_start
      }
      break;
    ...
    case extern_start:
      ...
      case T_STRING_LITERAL:
        break; // continue looking for the lbrace
      case T_LBRACE:
        enter(extern_open);
        break;
      default:
    ...
    case extern_open:
      ...
      case T_RBRACE:
        leave();
        leave();
        break; // always nested in extern_start
      }
      break;
    ...
    case class_start:
      ...
      case T_SEMICOLON:
        ...
        break;
      case T_LPAREN:
        turnInto(declaration_start);
        continue; // "struct Foo bar() {"
      case T_LBRACE:
        enter(class_open);
        break;
      }
    ...
    case class_open:
      ...
      case T_RBRACE:
        leave();
        continue; // always nested in class_start
      }
      break;
    ...
    case enum_open:
      ...
      case T_RBRACE:
        leave();
        continue; // always nested in enum_start
      case T_LBRACE:
        enter(brace_list_open);
        break;
      }
      break;
    ...
    case template_param:
      ...
      case T_GREATER_GREATER:
        leave();
        leave();
        break; // call leave twice to pop both template_param states
      }
      break;
    ...
    case declaration_start:
      ...
      case T_SEMICOLON:
        ...
        break;
      case T_EQUAL:
        enter(assign_open_or_initializer);
        break;
      case T_LBRACE:
        enter(defun_open);
        break;
      case T_COLON:
        enter(member_init_open);
        enter(member_init_expected);
        break;
      case T_OPERATOR:
        enter(operator_declaration);
        break;
      case T_GREATER_GREATER:
        break;
      case T_LBRACKET:
    ...
    case lambda_instroducer_or_subscribtion:
      switch (kind) {
      case T_RBRACKET:
        turnInto(lambda_declarator_expected);
        break; // we can't determine exact kind of expression. Try again
      case T_COMMA:
      case T_EQUAL:
        turnInto(lambda_instroducer);
        break;           // ',' or '=' inside brackets can be only within lambda capture list
      ...
      default:
        leave();
        leave();
        tryExpression(m_currentState.at(m_currentState.size() - 1).type == declaration_start);
        break;
    ...
    case lambda_declarator_expected:
      switch (kind) {
      case T_LPAREN:
        turnInto(lambda_declarator_or_expression);
        break; // '(' just after ']'. We can't make decisioin here
      case T_LBRACE:
        turnInto(substatement_open);
        break; // '{' just after ']' opens a lambda-compound statement
      default:
    ...
    case lambda_declarator_or_expression:
      switch (kind) {
      case T_LBRACE:
        turnInto(substatement_open); /*tryStatement();*/
        break;
      case T_RPAREN:
        turnInto(lambda_statement_expected);
        break;
      case T_IDENTIFIER:
      case T_SEMICOLON:
        leave();
      ...
      default:
        if (tryDeclaration()) {
          // We found the declaration within '()' so it is lambda declarator
          leave();
          turnInto(lambda_declarator);
          break;
        } else {
    ...
    case ternary_op:
      ...
      case T_SEMICOLON:
        leave();
        continue; // always nested, propagate
      default:
        tryExpression();
        break;
      }
    ...
    case defun_open:
      ...
      case T_RBRACE:
        leave();
        leave();
        break; // always nested in declaration_start
      }
      break;
    ...
    case for_statement:
      switch (kind) {
      case T_LPAREN:
        enter(for_statement_paren_open);
        break;
      default:
        leave(true);
        continue;
    ...
    case for_statement_paren_open:
      enter(for_statement_init);
      continue;

    case for_statement_init:
      switch (kind) {
      case T_SEMICOLON:
        turnInto(for_statement_condition);
        break;
      case T_LPAREN:
        enter(condition_paren_open);
        break;
      case T_RPAREN:
        turnInto(for_statement_expression);
        continue;
      }
      break;
    ...
    case multiline_comment_cont:
      if (kind != T_COMMENT && kind != T_DOXY_COMMENT) {
        leave();
        continue;
      } else if (m_tokenIndex == m_tokens.size() - 1 && lexerState == 0) {
        leave();
    ...
    case cpp_macro_start: {
      ...
      auto previousMarker = -1;
      auto previousPreviousMarker = -1;
      ...
      auto tokenText = currentTokenText();
      if (tokenText == QLatin1String("ifdef") || tokenText == QLatin1String("if") || tokenText == QLatin1String("ifndef")) {
        enter(cpp_macro_conditional);
        // copy everything right of previousMarker, excluding cpp_macro_conditional
        for (auto i = previousMarker + 1; i < size; ++i)
          m_currentState += m_currentState.at(i);
      }
      if (previousMarker != -1) {
        if (tokenText == QLatin1String("endif")) {
          auto begin = m_currentState.begin() + previousPreviousMarker + 1;
          auto end = m_currentState.begin() + previousMarker + 1;
          m_currentState.erase(begin, end);
        } else if (tokenText == QLatin1String("else") || tokenText == QLatin1String("elif")) {
          m_currentState.resize(previousMarker + 1);
          for (auto i = previousPreviousMarker + 1; i < previousMarker; ++i)
            m_currentState += m_currentState.at(i);
        }
      }
    ...
    default: qWarning() << "Unhandled state" << m_currentState.top().type;
      break;
  ...
  if (topState != multiline_comment_start && topState != multiline_comment_cont && (lexerState == T_COMMENT || lexerState == T_DOXY_COMMENT)) {
  ...
  if ((topState == cpp_macro_cont || topState == cpp_macro) && !endedJoined)
    leave();
...
auto CodeFormatter::indentFor(const QTextBlock &block, int *indent, int *padding) -> void
{
...
auto CodeFormatter::indentForNewLineAfter(const QTextBlock &block, int *indent, int *padding) -> void
{
  restoreCurrentState(block);
  *indent = m_indentDepth;
  *padding = m_paddingDepth;

  auto lexerState = loadLexerState(block);
  m_tokens.clear();
  m_currentLine.clear();
  adjustIndent(m_tokens, lexerState, indent, padding);
}

auto CodeFormatter::updateStateUntil(const QTextBlock &endBlock) -> void
{
  auto previousState = initialState();
  auto it = endBlock.document()->firstBlock();
  ...
  for (; it.isValid() && it != endBlock; it = it.next()) {
    ...
    if (blockData.m_blockRevision != it.revision())
      break;
    if (previousState.isEmpty() || blockData.m_beginState.isEmpty() || previousState != blockData.m_beginState)
      break;
...
auto CodeFormatter::updateLineStateChange(const QTextBlock &block) -> void
{
  if (!block.isValid())
    return;
...
auto CodeFormatter::isInRawStringLiteral(const QTextBlock &block) const -> bool
{
...
auto CodeFormatter::state(int belowTop) const -> CodeFormatter::State
{
...
auto CodeFormatter::tokenIndex() const -> int
{
  return m_tokenIndex;
}

auto CodeFormatter::tokenCount() const -> int
{
  return m_tokens.size();
}

auto CodeFormatter::currentToken() const -> const Token&
{
  return m_currentToken;
}

auto CodeFormatter::invalidateCache(QTextDocument *document) -> void
{
  if (!document)
    return;
...
auto CodeFormatter::enter(int newState) -> void
{
  auto savedIndentDepth = m_indentDepth;
  auto savedPaddingDepth = m_paddingDepth;
...
auto CodeFormatter::leave(bool statementDone) -> void
{
  ...
  auto poppedState = m_currentState.pop();
  ...
  if (statementDone) {
    if (topState == substatement || topState == statement_with_condition || topState == for_statement || topState == switch_statement || topState == do_statement) {
      leave(true);
...
auto CodeFormatter::correctIndentation(const QTextBlock &block) -> void
{
  const auto lexerState = tokenizeBlock(block);
  QTC_ASSERT(m_currentState.size() >= 1, return);

  adjustIndent(m_tokens, lexerState, &m_indentDepth, &m_paddingDepth);
}

auto CodeFormatter::tryExpression(bool alsoExpression) -> bool
{
  auto newState = -1;
  ...
  switch (kind) {
  case T_LPAREN:
    newState = arglist_open;
    break;
  case T_QUESTION:
    newState = ternary_op;
    break;
  case T_LBRACE:
    newState = braceinit_open;
    break;
  ...
  case T_GREATER_GREATER:
    ...
    for (int i = m_currentState.size() - 1; i >= 0; --i) {
      ...
      if (type == arglist_open || type == braceinit_open) {
        // likely a left-shift instead
        newState = -1;
        break;
      }
      if (type == topmost_intro || type == substatement_open || type == defun_open || type == namespace_open || type == extern_open || type == class_open || type == brace_list_open) {
        break;
      }
    }
    break;
  ...
  if (m_currentToken.isStringLiteral()) {
    newState = m_currentToken.kind() >= T_FIRST_RAW_STRING_LITERAL && m_currentToken.kind() <= T_LAST_RAW_STRING_LITERAL ? raw_string_open : string_open;
  }
...
auto CodeFormatter::tryDeclaration() -> bool
{
  const int kind = m_currentToken.kind();
  switch (kind) {
  case T_Q_ENUMS:
  ...
  case T_IDENTIFIER:
    if (m_tokenIndex == 0) {
      const auto tokenText = currentTokenText();
      if (tokenText.startsWith(QLatin1String("Q_")) || tokenText.startsWith(QLatin1String("QT_")) || tokenText.startsWith(QLatin1String("QML_")) || tokenText.startsWith(QLatin1String("QDOC_"))) {
        enter(qt_like_macro);
        return true;
      }
...
auto CodeFormatter::tryStatement() -> bool
{
  const int kind = m_currentToken.kind();
  if (tryDeclaration())
    return true;
...
auto CodeFormatter::isBracelessState(int type) const -> bool
{
  return type == substatement || type == if_statement || type == else_clause || type == statement_with_condition || type == for_statement || type == do_statement;
}

auto CodeFormatter::tokenAt(int idx) const -> const Token&
{
...
auto CodeFormatter::column(int index) const -> int
{
  auto col = 0;
...
auto CodeFormatter::currentTokenText() const -> QStringView
{
...
auto CodeFormatter::turnInto(int newState) -> void
{
  leave(false);
  enter(newState);
}

auto CodeFormatter::saveCurrentState(const QTextBlock &block) -> void
{
  if (!block.isValid())
    return;
  ...
  auto saveableBlock(block);
  saveBlockData(&saveableBlock, blockData);
}

auto CodeFormatter::restoreCurrentState(const QTextBlock &block) -> void
{
  if (block.isValid()) {
    BlockData blockData;
...
auto CodeFormatter::initialState() -> QStack<CodeFormatter::State>
{
...
auto CodeFormatter::tokenizeBlock(const QTextBlock &block, bool *endedJoined) -> int
{
  auto startState = loadLexerState(block.previous());
  ...
  tokenize.setExpectedRawStringSuffix(TextDocumentLayout::expectedRawStringSuffix(block.previous()));
...
auto CodeFormatter::dump() const -> void
{
  auto metaEnum = staticMetaObject.enumerator(staticMetaObject.indexOfEnumerator("StateType"));
...
QtStyleCodeFormatter::QtStyleCodeFormatter(const TabSettings &tabSettings, const CppCodeStyleSettings &settings) : m_tabSettings(tabSettings), m_styleSettings(settings)
{
  setTabSize(tabSettings.m_tabSize);
}

auto QtStyleCodeFormatter::setTabSettings(const TabSettings &tabSettings) -> void
{
  m_tabSettings = tabSettings;
  setTabSize(tabSettings.m_tabSize);
}

auto QtStyleCodeFormatter::setCodeStyleSettings(const CppCodeStyleSettings &settings) -> void
{
  m_styleSettings = settings;
}

auto QtStyleCodeFormatter::saveBlockData(QTextBlock *block, const BlockData &data) const -> void
{
...
auto QtStyleCodeFormatter::loadBlockData(const QTextBlock &block, BlockData *data) const -> bool
{
  auto userData = TextDocumentLayout::textUserData(block);
  if (!userData)
    return false;
...
auto QtStyleCodeFormatter::saveLexerState(QTextBlock *block, int state) const -> void
{
  TextDocumentLayout::setLexerState(*block, state);
}

auto QtStyleCodeFormatter::loadLexerState(const QTextBlock &block) const -> int
{
  return TextDocumentLayout::lexerState(block);
}

auto QtStyleCodeFormatter::addContinuationIndent(int *paddingDepth) const -> void
{
  if (*paddingDepth == 0)
...
auto QtStyleCodeFormatter::onEnter(int newState, int *indentDepth, int *savedIndentDepth, int *paddingDepth, int *savedPaddingDepth) const -> void
{
  const auto &parentState = state();
  const Token &tk = currentToken();
  const auto firstToken = (tokenIndex() == 0);
  const auto lastToken = (tokenIndex() == tokenCount() - 1);
  ...
  // after the return type in "void\nfoo() {}"
    for (auto i = 0; state(i).type != topmost_intro; ++i) {
  ...
  // fixed extra indent when continuing 'if (', but not for 'else if ('
    if (m_styleSettings.extraPaddingForConditionsIfConfusingAlign && nextTokenPosition - *indentDepth <= m_tabSettings.m_indentSize)
      *paddingDepth = 2 * m_tabSettings.m_indentSize;
    else
      *paddingDepth = nextTokenPosition - *indentDepth;
    break;
...
auto QtStyleCodeFormatter::adjustIndent(const Tokens &tokens, int lexerState, int *indentDepth, int *paddingDepth) const -> void
{
  auto topState = state();
  auto previousState = state(1);

  const auto topWasMaybeElse = (topState.type == maybe_else);
  if (topWasMaybeElse) {
    auto outermostBraceless = 1;
  ...
  case T_LBRACE: {
    if (topState.type == case_cont) {
      ...
      *paddingDepth = 0;
      ...
      // or constructor
    } else if ((topState.type == expression && previousState.type == declaration_start) || topState.type == member_init || topState.type == member_init_open) {
      ...
      *indentDepth = topState.savedIndentDepth;
      if (m_styleSettings.indentBlockBraces)
        *indentDepth += m_tabSettings.m_indentSize;
      *paddingDepth = 0;
    } else if (topState.type != defun_open && topState.type != block_open && topState.type != substatement_open && topState.type != brace_list_open && topState.type != arglist_open && !topWasMaybeElse) {
      *indentDepth = topState.savedIndentDepth;
      *paddingDepth = 0;
    }
  ...
  case T_RBRACE: {
    ...
    for (auto i = 0; state(i).type != topmost_intro; ++i) {
      ...
      if (type == class_open || type == namespace_open || type == extern_open || type == enum_open || type == defun_open || type == substatement_open || type == brace_list_open || type == block_open) {
        *indentDepth = state(i).savedIndentDepth;
        *paddingDepth = state(i).savedPaddingDepth;
        if ((type == defun_open && m_styleSettings.indentFunctionBraces) || (type == class_open && m_styleSettings.indentClassBraces) || (type == namespace_open && m_styleSettings.indentNamespaceBraces) || (type == enum_open && m_styleSettings.indentEnumBraces) || (type == substatement_open && m_styleSettings.indentBlockBraces))
          *indentDepth += m_tabSettings.m_indentSize;
        break;
      }
    }
  ...
  case T_PROTECTED:
  case T_Q_SIGNALS:
    if (m_styleSettings.indentDeclarationsRelativeToAccessSpecifiers && topState.type == class_open) {
      if (tokenAt(1).is(T_COLON) || tokenAt(2).is(T_COLON) || (tokenAt(tokenCount() - 1).is(T_COLON) && (tokenAt(1).is(T___ATTRIBUTE__) || tokenAt(1).is(T___DECLSPEC)))) {
        *indentDepth = topState.savedIndentDepth;
  ...
  case T_CPP_DOXY_COMMENT:
    ...
    if ((topState.type == multiline_comment_cont || topState.type == multiline_comment_start) && (kind == T_COMMENT || kind == T_DOXY_COMMENT) && (lexerState == T_EOF_SYMBOL || tokens.size() != 1)) {
    ...
    break;
  case T_IDENTIFIER:
    if (topState.type == substatement || topState.type == substatement_open || topState.type == case_cont || topState.type == block_open || topState.type == defun_open) {
...
auto QtStyleCodeFormatter::shouldClearPaddingOnEnter(int state) -> bool
{
  switch (state) {
  case defun_open:
  case class_start:
