...
void DoxygenTest::cleanup()
{
    ...
    QVERIFY(Orca::Plugin::Core::EditorManager::closeAllEditors(false));
...
void DoxygenTest::testWithMacroFromHeaderBeforeFunction()
{
    ...
    const CppTestDocument headerDocumentDefiningMacro("header.hpp", "#define API\n");
...
void DoxygenTest::verifyCleanState() const
{
    ...
    QVERIFY(Orca::Plugin::Core::DocumentModel::openedDocuments().isEmpty());
    QVERIFY(Orca::Plugin::Core::EditorManager::visibleEditors().isEmpty());
}
