...
{
    TestHelper t;
    t.projectConfigFile = "dummy_file.hpp";
    t.finalize();
    t.compilerOptionsBuilder->addProjectConfigFileInclude();

    QCOMPARE(t.compilerOptionsBuilder->options(), (QStringList{"-include", "dummy_file.hpp"}));
}
...
void CompilerOptionsBuilderTest::testAddProjectConfigFileIncludeMsvc()
{
    ...
    QCOMPARE(compilerOptionsBuilder.options(), (QStringList{"/FI", "dummy_file.hpp"}));
}
