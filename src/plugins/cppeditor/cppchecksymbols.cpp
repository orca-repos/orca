...
static constexpr bool highlightCtorDtorAsType = true;
...
public:
  CollectSymbols(Document::Ptr doc, const Snapshot &snapshot) : _doc(doc), _snapshot(snapshot), _mainDocument(false)
  {
    QSet<Namespace*> processed;
    process(doc, &processed);
  }

  auto types() const -> const QSet<QByteArray>&
  {
    return _types;
  }

  auto fields() const -> const QSet<QByteArray>&
  {
    return _fields;
  }

  auto functions() const -> const QSet<QByteArray>&
  {
    return _functions;
  }

  auto statics() const -> const QSet<QByteArray>&
  {
    return _statics;
  }

protected:
  auto process(Document::Ptr doc, QSet<Namespace*> *processed) -> void
  {
    if (!doc)
      return;
  ...
  auto addType(const Identifier *id) -> void
  {
    if (id)
      _types.insert(QByteArray::fromRawData(id->chars(), id->size()));
  }

  auto addType(const Name *name) -> void
  {
    if (!name) {
      return;
  ...
  auto addField(const Name *name) -> void
  {
    if (!name) {
      return;
  ...
  auto addFunction(const Name *name) -> void
  {
    if (!name) {
      return;
  ...
  auto addStatic(const Name *name) -> void
  {
    if (!name) {
      return;
  ...
  // nothing to do
  auto visit(UsingNamespaceDirective *) -> bool override { return true; }
  auto visit(UsingDeclaration *) -> bool override { return true; }
  auto visit(Argument *) -> bool override { return true; }
  auto visit(BaseClass *) -> bool override { return true; }

  auto visit(Function *symbol) -> bool override
  {
  ...
  auto visit(NamespaceAlias *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(Declaration *symbol) -> bool override
  {
  ...
  auto visit(TypenameArgument *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(Enum *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(Namespace *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(Template *) -> bool override
  {
    return true;
  }

  auto visit(Class *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(ForwardClassDeclaration *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }
  ...
  // Objective-C
  auto visit(ObjCBaseClass *) -> bool override { return true; }
  auto visit(ObjCBaseProtocol *) -> bool override { return true; }
  auto visit(ObjCPropertyDeclaration *) -> bool override { return true; }
  auto visit(ObjCMethod *) -> bool override { return true; }

  auto visit(ObjCClass *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(ObjCForwardClassDeclaration *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(ObjCProtocol *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }

  auto visit(ObjCForwardProtocolDeclaration *symbol) -> bool override
  {
    addType(symbol->name());
    return true;
  }
...
static auto sortByLinePredicate(const CheckSymbols::Result &lhs, const CheckSymbols::Result &rhs) -> bool
{
...
static auto acceptName(NameAST *ast, unsigned *referenceToken) -> bool
{
  *referenceToken = ast->firstToken();
  ...
  return !dtor && !ast->asConversionFunctionId() && !ast->asOperatorFunctionId();
}

auto CheckSymbols::go(Document::Ptr doc, const LookupContext &context, const QList<CheckSymbols::Result> &macroUses) -> CheckSymbols::Future
{
...
auto CheckSymbols::create(Document::Ptr doc, const LookupContext &context, const QList<CheckSymbols::Result> &macroUses) -> CheckSymbols*
{
...
CheckSymbols::CheckSymbols(Document::Ptr doc, const LookupContext &context, const QList<CheckSymbols::Result> &macroUses) : ASTVisitor(doc->translationUnit()), _doc(doc), _context(context), _lineOfLastUsage(0), _macroUses(macroUses)
{
  auto line = 0;
...
auto CheckSymbols::warning(unsigned line, unsigned column, const QString &text, unsigned length) -> bool
{
...
auto CheckSymbols::warning(AST *ast, const QString &text) -> bool
{
...
auto CheckSymbols::enclosingFunctionDefinition(bool skipTopOfStack) const -> FunctionDefinitionAST*
{
...
auto CheckSymbols::enclosingTemplateDeclaration() const -> TemplateDeclarationAST*
{
...
auto CheckSymbols::enclosingScope() const -> Scope*
{
  for (int index = _astStack.size() - 1; index != -1; --index) {
    AST *ast = _astStack.at(index);
...
auto CheckSymbols::preVisit(AST *ast) -> bool
{
  _astStack.append(ast);
  return !isCanceled();
}

auto CheckSymbols::postVisit(AST *) -> void
{
  _astStack.takeLast();
}

auto CheckSymbols::visit(NamespaceAST *ast) -> bool
{
...
auto CheckSymbols::visit(UsingDirectiveAST *) -> bool
{
  return true;
}

auto CheckSymbols::visit(EnumeratorAST *ast) -> bool
{
  addUse(ast->identifier_token, SemanticHighlighter::EnumerationUse);
  return true;
}

auto CheckSymbols::visit(DotDesignatorAST *ast) -> bool
{
  addUse(ast->identifier_token, SemanticHighlighter::FieldUse);
  return true;
}

auto CheckSymbols::visit(SimpleDeclarationAST *ast) -> bool
{
    ...
    if (ast->symbols && !ast->symbols->next && !ast->symbols->value->isGenerated()) {
      ...
      if (NameAST *nameAST = declaratorId(ast->declarator_list->value)) {
        if (Function *funTy = decl->type()->asFunctionType()) {
          if (funTy->isVirtual() || (nameAST->asDestructorName() && hasVirtualDestructor(_context.lookupType(funTy->enclosingScope())))) {
            addUse(nameAST, SemanticHighlighter::VirtualFunctionDeclarationUse);
            declrIdNameAST = nameAST;
          } else if (maybeAddFunction(_context.lookup(decl->name(), decl->enclosingScope()), nameAST, funTy->argumentCount(), FunctionDeclaration)) {
            declrIdNameAST = nameAST;
            ...
            if ((_usages.back().kind != SemanticHighlighter::VirtualFunctionDeclarationUse)) {
              if (funTy->isOverride())
                warning(declrIdNameAST, QCoreApplication::translate("CPlusplus::CheckSymbols", "Only virtual functions can be marked 'override'"));
              else if (funTy->isFinal())
                warning(declrIdNameAST, QCoreApplication::translate("CPlusPlus::CheckSymbols", "Only virtual functions can be marked 'final'"));
            }
          }
        }
      }
  ...
  for (DeclaratorListAST *it = ast->declarator_list; it; it = it->next) {
    ...
    if (declrIdNameAST && declr->core_declarator && declr->core_declarator->asDeclaratorId() && declr->core_declarator->asDeclaratorId()->name == declrIdNameAST) {
...
auto CheckSymbols::visit(ElaboratedTypeSpecifierAST *ast) -> bool
{
...
auto CheckSymbols::visit(ObjCProtocolDeclarationAST *ast) -> bool
{
  accept(ast->attribute_list);
  accept(ast->name);
...
auto CheckSymbols::visit(ObjCProtocolForwardDeclarationAST *ast) -> bool
{
  accept(ast->attribute_list);
...
auto CheckSymbols::visit(ObjCClassDeclarationAST *ast) -> bool
{
  accept(ast->attribute_list);
...
auto CheckSymbols::visit(ObjCClassForwardDeclarationAST *ast) -> bool
{
  accept(ast->attribute_list);
  accept(ast->identifier_list);
  for (NameListAST *i = ast->identifier_list; i; i = i->next)
...
auto CheckSymbols::visit(ObjCProtocolRefsAST *ast) -> bool
{
  accept(ast->identifier_list);
  for (NameListAST *i = ast->identifier_list; i; i = i->next)
...
auto CheckSymbols::visit(MemberAccessAST *ast) -> bool
{
  ...
  if (const Name *name = ast->member_name->name) {
    if (const Identifier *ident = name->identifier()) {
      ...
      if (_potentialFields.contains(id)) {
        ...
        const QByteArray expression = _doc->utf8Source().mid(start.bytesBegin(), end.bytesEnd() - start.bytesBegin());
...
auto CheckSymbols::visit(CallAST *ast) -> bool
{
    ...
    if (MemberAccessAST *access = ast->base_expression->asMemberAccess()) {
      if (access->member_name && access->member_name->name) {
        if (maybeFunction(access->member_name->name)) {
          ...
          if (!maybeAddFunction(candidates, memberName, argumentCount, FunctionCall) && highlightCtorDtorAsType) {
      ...
      if (const Name *name = idExpr->name->name) {
        if (maybeFunction(name)) {
          ...
          const QList<LookupItem> candidates = typeOfExpression(textOf(idExpr), enclosingScope(), TypeOfExpression::Preprocess);

          if (!maybeAddFunction(candidates, exprName, argumentCount, FunctionCall) && highlightCtorDtorAsType) {
            expr = ast->base_expression;
          }
        }
      }
...
auto CheckSymbols::visit(ObjCSelectorArgumentAST *ast) -> bool
{
  addUse(ast->firstToken(), SemanticHighlighter::FunctionUse);
  return true;
}

auto CheckSymbols::visit(NewExpressionAST *ast) -> bool
{
  ...
  if (highlightCtorDtorAsType) {
    ...
    if (binding && nameAST) {
      ...
      maybeAddFunction(_context.lookup(nameAST->name, scope), nameAST, arguments, FunctionCall);
    }
  }
...
auto CheckSymbols::textOf(AST *ast) const -> QByteArray
{
  const Token start = tokenAt(ast->firstToken());
  const Token end = tokenAt(ast->lastToken() - 1);
  const QByteArray text = _doc->utf8Source().mid(start.bytesBegin(), end.bytesEnd() - start.bytesBegin());
  return text;
}

auto CheckSymbols::checkNamespace(NameAST *name) -> void
{
  if (!name)
    return;
  ...
  const unsigned length = tokenAt(name->lastToken() - 1).utf16charsEnd() - tokenAt(name->firstToken()).utf16charsBegin();
  warning(line, column, QCoreApplication::translate("CPlusPlus::CheckSymbols", "Expected a namespace-name"), length);
}

auto CheckSymbols::hasVirtualDestructor(Class *klass) const -> bool
{
  if (!klass)
    return false;
...
auto CheckSymbols::hasVirtualDestructor(ClassOrNamespace *binding) const -> bool
{
...
auto CheckSymbols::checkName(NameAST *ast, Scope *scope) -> void
{
  if (ast && ast->name) {
    if (!scope)
...
auto CheckSymbols::visit(SimpleNameAST *ast) -> bool
{
  checkName(ast);
  return true;
}

auto CheckSymbols::visit(TemplateIdAST *ast) -> bool
{
  checkName(ast);
  return true;
}

auto CheckSymbols::visit(DestructorNameAST *ast) -> bool
{
  checkName(ast);
  return true;
}

auto CheckSymbols::visit(ParameterDeclarationAST *ast) -> bool
{
...
auto CheckSymbols::visit(QualifiedNameAST *ast) -> bool
{
  if (ast->name) {
    ...
    if (binding && ast->unqualified_name) {
      if (ast->unqualified_name->asDestructorName() != nullptr) {
        if (hasVirtualDestructor(binding)) {
          ...
          bool added = false;
          if (highlightCtorDtorAsType && maybeType(ast->name))
            added = maybeAddTypeOrStatic(binding->find(ast->unqualified_name->name), ast->unqualified_name);
          if (!added)
...
auto CheckSymbols::checkNestedName(QualifiedNameAST *ast) -> ClassOrNamespace*
{
  ClassOrNamespace *binding = nullptr;
...
auto CheckSymbols::visit(TypenameTypeParameterAST *ast) -> bool
{
  addUse(ast->name, SemanticHighlighter::TypeUse);
  accept(ast->type_id);
  return false;
...
auto CheckSymbols::visit(TemplateTypeParameterAST *ast) -> bool
{
...
auto CheckSymbols::visit(MemInitializerAST *ast) -> bool
{
        ...
        foreach(Symbol *s, binding->symbols()) {
          if (Class *klass = s->asClass()) {
            ...
            if (highlightCtorDtorAsType && maybeType(nameAST->name)) {
              ...
              maybeAddFunction(_context.lookup(nameAST->name, klass), nameAST, arguments, FunctionCall);
            }
...
auto CheckSymbols::visit(GotoStatementAST *ast) -> bool
{
  if (ast->identifier_token)
...
auto CheckSymbols::visit(LabeledStatementAST *ast) -> bool
{
 ...
 */
auto CheckSymbols::visit(SimpleSpecifierAST *ast) -> bool
{
  if (ast->specifier_token) {
    const Token &tk = tokenAt(ast->specifier_token);
    if (tk.is(T_IDENTIFIER)) {
      const Identifier &id = *(tk.identifier);
      if (id.equalTo(_doc->control()->cpp11Override()) || id.equalTo(_doc->control()->cpp11Final())) {
...
auto CheckSymbols::visit(ClassSpecifierAST *ast) -> bool
{
  if (ast->final_token)
...
auto CheckSymbols::visit(FunctionDefinitionAST *ast) -> bool
{
  ...
  if (ast->declarator && ast->symbol && !ast->symbol->isGenerated()) {
    ...
    if (NameAST *declId = declaratorId(ast->declarator)) {
      ...
      if (fun->isVirtual() || (declId->asDestructorName() && hasVirtualDestructor(_context.lookupType(fun->enclosingScope())))) {
        addUse(declId, SemanticHighlighter::VirtualFunctionDeclarationUse);
      } else if (!maybeAddFunction(_context.lookup(fun->name(), fun->enclosingScope()), declId, fun->argumentCount(), FunctionDeclaration)) {
...
auto CheckSymbols::addUse(NameAST *ast, Kind kind) -> void
{
  if (!ast)
    return;
...
auto CheckSymbols::addUse(unsigned tokenIndex, Kind kind) -> void
{
  if (!tokenIndex)
    return;
...
auto CheckSymbols::addUse(const Result &use) -> void
{
  if (use.isInvalid())
    return;
...
auto CheckSymbols::addType(ClassOrNamespace *b, NameAST *ast) -> void
{
  unsigned startToken;
...
auto CheckSymbols::isTemplateClass(Symbol *symbol) const -> bool
{
  if (symbol) {
    if (Template *templ = symbol->asTemplate()) {
      if (Symbol *declaration = templ->declaration()) {
        return declaration->isClass() || declaration->isForwardClassDeclaration() || declaration->isTypedef();
      }
    }
  }
  return false;
...
auto CheckSymbols::maybeAddTypeOrStatic(const QList<LookupItem> &candidates, NameAST *ast) -> bool
{
  unsigned startToken;
  ...
  foreach(const LookupItem &r, candidates) {
    ...
    if (c->isUsingNamespaceDirective()) // ... and using namespace directives.
      continue;
    if (c->isTypedef() || c->isNamespace() || c->isStatic() || //consider also static variable
      c->isClass() || c->isEnum() || isTemplateClass(c) || c->isForwardClassDeclaration() || c->isTypenameArgument() || c->enclosingEnum()) {
      int line, column;
      getTokenStartPosition(startToken, &line, &column);
      const unsigned length = tok.utf16chars();

      auto kind = SemanticHighlighter::TypeUse;
...
auto CheckSymbols::maybeAddField(const QList<LookupItem> &candidates, NameAST *ast) -> bool
{
  unsigned startToken;
  if (!acceptName(ast, &startToken))
    return false;
  ...
  foreach(const LookupItem &r, candidates) {
    ...
    const Result use(line, column, length, c->isStatic() ? SemanticHighlighter::StaticFieldUse : SemanticHighlighter::FieldUse);
    addUse(use);
...
auto CheckSymbols::maybeAddFunction(const QList<LookupItem> &candidates, NameAST *ast, int argumentCount, FunctionKind functionKind) -> bool
{
  int startToken = ast->firstToken();
  auto isDestructor = false;
  auto isConstructor = false;
  if (DestructorNameAST *dtor = ast->asDestructorName()) {
    isDestructor = true;
  ...
  auto kind = functionKind == FunctionDeclaration ? SemanticHighlighter::FunctionDeclarationUse : SemanticHighlighter::FunctionUse;
  foreach(const LookupItem &r, candidates) {
    Symbol *c = r.declaration();
    ...
    if (functionKind == FunctionDeclaration) {
      matchingKind = isVirtual ? SemanticHighlighter::VirtualFunctionDeclarationUse : isStaticMember ? SemanticHighlighter::StaticMethodDeclarationUse : SemanticHighlighter::FunctionDeclarationUse;
    } else {
      matchingKind = isVirtual ? SemanticHighlighter::VirtualMethodUse : isStaticMember ? SemanticHighlighter::StaticMethodUse : SemanticHighlighter::FunctionUse;
    }
  ...
  if (matchType != Match_None) {
    ...
    if (highlightCtorDtorAsType && (isConstructor || isDestructor) && maybeType(ast->name) && kind == SemanticHighlighter::FunctionDeclarationUse) {
      return false;
    }
...
auto CheckSymbols::declaratorId(DeclaratorAST *ast) const -> NameAST*
{
...
auto CheckSymbols::maybeType(const Name *name) const -> bool
{
  if (name) {
    if (const Identifier *ident = name->identifier()) {
      const QByteArray id = QByteArray::fromRawData(ident->chars(), ident->size());
...
auto CheckSymbols::maybeField(const Name *name) const -> bool
{
  if (name) {
    if (const Identifier *ident = name->identifier()) {
      const QByteArray id = QByteArray::fromRawData(ident->chars(), ident->size());
...
auto CheckSymbols::maybeStatic(const Name *name) const -> bool
{
  if (name) {
    if (const Identifier *ident = name->identifier()) {
      const QByteArray id = QByteArray::fromRawData(ident->chars(), ident->size());
...
auto CheckSymbols::maybeFunction(const Name *name) const -> bool
{
  if (name) {
    if (const Identifier *ident = name->identifier()) {
      const QByteArray id = QByteArray::fromRawData(ident->chars(), ident->size());
...
auto CheckSymbols::isConstructorDeclaration(Symbol *declaration) -> bool
{
