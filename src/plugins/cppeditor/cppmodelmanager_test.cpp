...
#include "modelmanagertesthelper.hpp"
...
#include <cplusplus/LookupContext.hpp>
...
void ModelManagerTest::testRefreshAlsoIncludesOfProjectFiles()
{
    ...
    const QString testHeader(testDataDir.fileFromSourcesDir( _("test_modelmanager_refresh.hpp")));
...
void ModelManagerTest::testRefreshSeveralTimes()
{
    ...
    const QString testHeader1(testDataDir.file(_("defines.hpp")));
    const QString testHeader2(testDataDir.file(_("header.hpp")));
...
void ModelManagerTest::testRefreshAddedAndPurgeRemoved()
{
    ...
    const QString testHeader1(testDataDir.file(_("header.hpp")));
    const QString testHeader2(testDataDir.file(_("defines.hpp")));
    const QString testCpp(testDataDir.file(_("source.cpp")));
...
void ModelManagerTest::testExtraeditorsupportUiFiles()
{
    ...
    QCOMPARE(workingCopy.size(), 2); // mm->configurationFileName() and "ui_*.hpp"
    ...
    QCOMPARE(Utils::FilePath::fromString(includedFiles.at(0)).fileName(), _("mainwindow.hpp"));
...
void ModelManagerTest::testGcIfLastCppeditorClosed()
{
    ...
    QCOMPARE(Orca::Plugin::Core::DocumentModel::openedDocuments().size(), 0);
    Orca::Plugin::Core::IEditor *editor = Orca::Plugin::Core::EditorManager::openEditor(Utils::FilePath::fromString(file));
    QVERIFY(editor);
    QCOMPARE(Orca::Plugin::Core::DocumentModel::openedDocuments().size(), 1);
    ...
    // Close file/editor
    Orca::Plugin::Core::EditorManager::closeDocuments({editor->document()}, /*askAboutModifiedEditors=*/false);
...
void ModelManagerTest::testDontGcOpenedFiles()
{
    ...
    // Close editor
    Orca::Plugin::Core::EditorManager::closeDocuments({editor->document()});
    helper.waitForFinishedGc();
...
namespace {
struct EditorCloser {
    Orca::Plugin::Core::IEditor *editor;
    explicit EditorCloser(Orca::Plugin::Core::IEditor *editor): editor(editor) {}
    ~EditorCloser()
    {
        if (editor)
...
void ModelManagerTest::testDefinesPerProject()
{
    ...
    const QString header = testDataDirectory.file(_("header.hpp"));
    ...
    for (auto &i : d) {
        ...
        Orca::Plugin::Core::IEditor *editor = Orca::Plugin::Core::EditorManager::openEditor(
...
void ModelManagerTest::testPrecompiledHeaders()
{
    ...
    const QString main2File = testDataDirectory.file(_("main2.cpp"));
    const QString header = testDataDirectory.file(_("header.hpp"));
    const QString pch1File = testDataDirectory.file(_("pch1.hpp"));
    const QString pch2File = testDataDirectory.file(_("pch2.hpp"));
...
void ModelManagerTest::testUpdateEditorsAfterProjectUpdate()
{
    ...
    Orca::Plugin::Core::IEditor *editorA = Orca::Plugin::Core::EditorManager::openEditor(Utils::FilePath::fromString(fileA));
    QVERIFY(editorA);
    ...
    Orca::Plugin::Core::IEditor *editorB = Orca::Plugin::Core::EditorManager::openEditor(Utils::FilePath::fromString(fileB));
    QVERIFY(editorB);
    EditorCloser closerB(editorB);
    QCOMPARE(Orca::Plugin::Core::DocumentModel::openedDocuments().size(), 2);
    ...
    Orca::Plugin::Core::EditorManager::activateEditor(editorA);
    ...
    Orca::Plugin::Core::EditorManager::activateEditor(editorB);
    QVERIFY(TestCase::waitForProcessedEditorDocument(fileB));
...
void ModelManagerTest::testRenameIncludes()
{
    ...
    const QStringList fileNames = {"foo.hpp", "foo.cpp", "main.cpp"};
    const QString oldHeader(workingDir.filePath(_("foo.hpp")));
    const QString newHeader(workingDir.filePath(_("bar.hpp")));
    ...
    // Renaming the header
    QVERIFY(Orca::Plugin::Core::FileUtils::renameFile(Utils::FilePath::fromString(oldHeader),
                                        Utils::FilePath::fromString(newHeader),
                                        Orca::Plugin::Core::HandleIncludeGuards::Yes));
...
void ModelManagerTest::testRenameIncludesInEditor()
{
    ...
    const QDir workingDir(tmpDir.path());
    const QStringList fileNames = {"baz.hpp", "baz2.hpp", "baz3.hpp", "foo.hpp", "foo.cpp", "main.cpp"};
    const QString headerWithPragmaOnce(workingDir.filePath(_("foo.hpp")));
    const QString renamedHeaderWithPragmaOnce(workingDir.filePath(_("bar.hpp")));
    const QString headerWithNormalGuard(workingDir.filePath(_("baz.hpp")));
    const QString renamedHeaderWithNormalGuard(workingDir.filePath(_("foobar2000.hpp")));
    const QString headerWithUnderscoredGuard(workingDir.filePath(_("baz2.hpp")));
    const QString renamedHeaderWithUnderscoredGuard(workingDir.filePath(_("foobar4000.hpp")));
    const QString headerWithMalformedGuard(workingDir.filePath(_("baz3.hpp")));
    const QString renamedHeaderWithMalformedGuard(workingDir.filePath(_("foobar5000.hpp")));
    ...
    // Open a file in the editor
    QCOMPARE(Orca::Plugin::Core::DocumentModel::openedDocuments().size(), 0);
    Orca::Plugin::Core::IEditor *editor = Orca::Plugin::Core::EditorManager::openEditor(Utils::FilePath::fromString(mainFile));
    QVERIFY(editor);
    EditorCloser editorCloser(editor);
    Utils::ExecuteOnDestruction saveAllFiles([](){
        Orca::Plugin::Core::DocumentManager::saveAllModifiedDocumentsSilently();
    });
    QCOMPARE(Orca::Plugin::Core::DocumentModel::openedDocuments().size(), 1);
    ...
    QVERIFY(Orca::Plugin::Core::FileUtils::renameFile(Utils::FilePath::fromString(headerWithPragmaOnce),
    ...
    QVERIFY(Orca::Plugin::Core::FileUtils::renameFile(Utils::FilePath::fromString(headerWithNormalGuard),
                                        Utils::FilePath::fromString(renamedHeaderWithNormalGuard),
                                        Orca::Plugin::Core::HandleIncludeGuards::Yes));

    const MyTestDataDir testDir2(_("testdata_project2"));
    QFile foobar2000Header(testDir2.file("foobar2000.hpp"));
    ...
    // The contents should match the foobar2000.h in the testdata_project2 project
    QVERIFY(
        Orca::Plugin::Core::FileUtils::renameFile(Utils::FilePath::fromString(headerWithUnderscoredGuard),
                                    Utils::FilePath::fromString(renamedHeaderWithUnderscoredGuard),
                                    Orca::Plugin::Core::HandleIncludeGuards::Yes));

    QFile foobar4000Header(testDir2.file("foobar4000.hpp"));
    ...
    QVERIFY(Orca::Plugin::Core::FileUtils::renameFile(Utils::FilePath::fromString(headerWithMalformedGuard),
...
void ModelManagerTest::testDocumentsAndRevisions()
{
    ...
    // Index two files
    const MyTestDataDir testDir(_("testdata_project1"));
    const QString filePath1 = testDir.file(QLatin1String("foo.hpp"));
