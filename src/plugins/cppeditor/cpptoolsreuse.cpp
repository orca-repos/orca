...
{
  const Orca::Plugin::Core::IDocument *currentDocument = Orca::Plugin::Core::EditorManager::currentDocument();
  ...
  if (!otherFile.isEmpty())
    Orca::Plugin::Core::EditorManager::openEditor(otherFile);
}
...
auto fileSizeExceedsLimit(const QFileInfo &fileInfo, int sizeLimitInMb) -> bool
{
  ...
  if (fileSizeInMB > sizeLimitInMb) {
    ...
    QMetaObject::invokeMethod(Orca::Plugin::Core::MessageManager::instance(), [msg]() { Orca::Plugin::Core::MessageManager::writeSilently(msg); });
