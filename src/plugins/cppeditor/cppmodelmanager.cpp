...
auto CppModelManager::setLocatorFilter(std::unique_ptr<Core::ILocatorFilter> &&filter) -> void
{
  setFilter(d->m_locatorFilter, std::move(filter));
}

auto CppModelManager::setClassesFilter(std::unique_ptr<Core::ILocatorFilter> &&filter) -> void
{
  setFilter(d->m_classesFilter, std::move(filter));
}

auto CppModelManager::setIncludesFilter(std::unique_ptr<Core::ILocatorFilter> &&filter) -> void
{
  setFilter(d->m_includesFilter, std::move(filter));
}

auto CppModelManager::setFunctionsFilter(std::unique_ptr<Core::ILocatorFilter> &&filter) -> void
{
  setFilter(d->m_functionsFilter, std::move(filter));
}

auto CppModelManager::setSymbolsFindFilter(std::unique_ptr<Core::IFindFilter> &&filter) -> void
{
  setFilter(d->m_symbolsFindFilter, std::move(filter));
}

auto CppModelManager::setCurrentDocumentFilter(std::unique_ptr<Core::ILocatorFilter> &&filter) -> void
{
  setFilter(d->m_currentDocumentFilter, std::move(filter));
}

auto CppModelManager::locatorFilter() const -> Core::ILocatorFilter*
{
  return d->m_locatorFilter.get();
}

auto CppModelManager::classesFilter() const -> Core::ILocatorFilter*
{
  return d->m_classesFilter.get();
}

auto CppModelManager::includesFilter() const -> Core::ILocatorFilter*
{
  return d->m_includesFilter.get();
}

auto CppModelManager::functionsFilter() const -> Core::ILocatorFilter*
{
  return d->m_functionsFilter.get();
}

auto CppModelManager::symbolsFindFilter() const -> Core::IFindFilter*
{
  return d->m_symbolsFindFilter.get();
}

auto CppModelManager::currentDocumentFilter() const -> Core::ILocatorFilter*
{
...
{
  // Objects
  connect(Core::VcsManager::instance(), &Core::VcsManager::repositoryChanged, this, &CppModelManager::updateModifiedSourceFiles);
  connect(Core::DocumentManager::instance(), &Core::DocumentManager::filesChangedInternally, [this](const Utils::FilePaths &filePaths) {
...
CppModelManager::CppModelManager() : CppModelManagerBase(nullptr), d(new CppModelManagerPrivate)
{
  ...
  connect(Core::EditorManager::instance(), &Core::EditorManager::currentEditorChanged, this, &CppModelManager::onCurrentEditorChanged);

  connect(Core::DocumentManager::instance(), &Core::DocumentManager::allDocumentsRenamed, this, &CppModelManager::renameIncludes);

  connect(Core::ICore::instance(), &Core::ICore::coreAboutToClose, this, &CppModelManager::onCoreAboutToClose);
...
auto CppModelManager::updateCppEditorDocuments(bool projectsUpdated) const -> void
{
  ...
  auto invisibleCppEditorDocuments = Utils::toSet(Core::DocumentModel::openedDocuments());
...
auto CppModelManager::isCppEditor(Core::IEditor *editor) -> bool
{
...
auto CppModelManager::onCurrentEditorChanged(Core::IEditor *editor) -> void
{
