...
auto CppEditorPlugin::initialize(const QStringList & /*arguments*/, QString *errorMessage) -> bool
{
  ...
  // Menus
  auto mtools = ActionManager::actionContainer(Orca::Plugin::Core::M_TOOLS);
  ...
  contextMenu->insertGroup(Orca::Plugin::Core::G_DEFAULT_ONE, Constants::G_CONTEXT_FIRST);

  Command *cmd;
  auto cppToolsMenu = ActionManager::actionContainer(Constants::M_TOOLS_CPP);
  auto touchBar = ActionManager::actionContainer(Orca::Plugin::Core::TOUCH_BAR);
  ...
  touchBar->addAction(cmd, Orca::Plugin::Core::G_TOUCHBAR_NAVIGATION);
  ...
  cppToolsMenu->addSeparator(Orca::Plugin::Core::G_DEFAULT_THREE);
  ...
  cppToolsMenu->addAction(cmd, Orca::Plugin::Core::G_DEFAULT_THREE);

  auto toolsDebug = ActionManager::actionContainer(Orca::Plugin::Core::M_TOOLS_DEBUG);
...
auto CppEditorPlugin::extensionsInitialized() -> void
{
  ...
  registerIconOverlayForMimeType(orcaTheme()->imageFile(Theme::IconOverlayCppSource, ProjectExplorer::Constants::FILEOVERLAY_CPP), Constants::CPP_SOURCE_MIMETYPE);
  registerIconOverlayForMimeType(orcaTheme()->imageFile(Theme::IconOverlayCSource, ProjectExplorer::Constants::FILEOVERLAY_C), Constants::C_SOURCE_MIMETYPE);
  registerIconOverlayForMimeType(orcaTheme()->imageFile(Theme::IconOverlayCppHeader, ProjectExplorer::Constants::FILEOVERLAY_H), Constants::CPP_HEADER_MIMETYPE);
}
