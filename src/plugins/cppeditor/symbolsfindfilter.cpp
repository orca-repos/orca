...
auto SymbolsFindFilter::startSearch(SearchResult *search) -> void
{
  ...
  auto progress = ProgressManager::addTask(watcher->future(), tr("Searching for Symbol"), Orca::Plugin::Core::TASK_SEARCH);
...
auto SymbolsFindFilter::openEditor(const SearchResultItem &item) -> void
{
  ...
  EditorManager::openEditorAt({FilePath::fromString(info->fileName()), info->line(), info->column()}, {}, Orca::Plugin::Core::EditorManager::AllowExternalEditor);
}
