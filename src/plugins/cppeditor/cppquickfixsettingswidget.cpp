...
CppQuickFixSettingsWidget::CppQuickFixSettingsWidget(QWidget *parent) : QWidget(parent), ui(new ::Ui::CppQuickFixSettingsWidget), typeSplitter("\\s*,\\s*")
{
  ui->setupUi(this);
  QObject::connect(ui->listWidget_customTemplates, &QListWidget::currentItemChanged, this, &CppQuickFixSettingsWidget::currentCustomItemChanged);
  ...
  const auto toolTip = QCoreApplication::translate("CppQuickFixSettingsWidget", R"==(Use <name> for the variable
...
"set<Camel> => "setTestFoo")==");
  ...
  auto then = [this] {
    if (!isLoadingSettings) emit settingsChanged();
  };
...
auto CppQuickFixSettingsWidget::loadSettings(CppQuickFixSettings *settings) -> void
{
  ...
  for (const auto &customTemplate : settings->customTemplates) {
    auto item = new QListWidgetItem(customTemplate.types.join(", "), ui->listWidget_customTemplates);
    item->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled | Qt::ItemNeverHasChildren);
...
auto CppQuickFixSettingsWidget::saveSettings(CppQuickFixSettings *settings) -> void
{
  ...
  if (ui->listWidget_customTemplates->currentItem() != nullptr) {
    ...
    item->setData(CustomDataRoles::ReturnExpression, ui->lineEdit_customTemplateReturnExpression->text());
  }
  ...
  for (auto i = 0; i < ui->valueTypes->count(); ++i) {
    settings->valueTypes << ui->valueTypes->item(i)->text();
  }
  settings->customTemplates.clear();
  for (auto i = 0; i < ui->listWidget_customTemplates->count(); ++i) {
...
auto CppQuickFixSettingsWidget::currentCustomItemChanged(QListWidgetItem *newItem, QListWidgetItem *oldItem) -> void
{
  if (oldItem) {
    auto list = ui->lineEdit_customTemplateTypes->text().split(typeSplitter, Qt::SkipEmptyParts);
    ...
    oldItem->setData(CustomDataRoles::ReturnExpression, ui->lineEdit_customTemplateReturnExpression->text());
  }
  ...
  if (newItem) {
    this->ui->lineEdit_customTemplateTypes->setText(newItem->data(CustomDataRoles::Types).toStringList().join(", "));
    this->ui->lineEdit_customTemplateComparison->setText(newItem->data(CustomDataRoles::Comparison).toString());
    this->ui->lineEdit_customTemplateAssignment->setText(newItem->data(CustomDataRoles::Assignment).toString());
    this->ui->lineEdit_customTemplateReturnType->setText(newItem->data(CustomDataRoles::ReturnType).toString());
    this->ui->lineEdit_customTemplateReturnExpression->setText(newItem->data(CustomDataRoles::ReturnExpression).toString());
  } else {
