...
auto QmakePriFile::recursiveEnumerate(const QString &folder) -> QSet<FilePath>
{
  ...
  foreach(const QFileInfo &file, dir.entryInfoList()) {
    ...
    else if (!Orca::Plugin::Core::EditorManager::isAutoSaveFile(file.fileName()))
...
{
  auto document = Orca::Plugin::Core::DocumentModel::documentForFilePath(filePath());
  if (!document || !document->isModified())
    return true;

  if (!Orca::Plugin::Core::DocumentManager::saveDocument(document))
    return false;
...
auto QmakePriFile::ensureWriteableProFile(const QString &file) -> bool
{
  ...
  if (!fi.isWritable()) {
    // Try via vcs manager
    auto versionControl = Orca::Plugin::Core::VcsManager::findVersionControlForDirectory(FilePath::fromString(fi.absolutePath()));
    if (!versionControl || !versionControl->vcsOpen(FilePath::fromString(file))) {
      ...
      if (!makeWritable) {
        QMessageBox::warning(Orca::Plugin::Core::ICore::dialogParent(), QCoreApplication::translate("QmakePriFile", "Failed"), QCoreApplication::translate("QmakePriFile", "Could not write project file %1.").arg(file));
        return false;
      }
    }
  }
...
auto QmakePriFile::readProFile() -> QPair<ProFile*, QStringList>
{
  ...
  ProFile *includeFile = nullptr;
  {
    ...
    {
      QString errorMsg;
      if (TextFileFormat::readFile(filePath(), Orca::Plugin::Core::EditorManager::defaultTextCodec(), &contents, &m_textFormat, &errorMsg) != TextFileFormat::ReadSuccess) {
...
{
  {
    ...
    if (!m_textFormat.writeFile(filePath(), lines.join('\n'), &errorMsg)) {
      QMessageBox::critical(Orca::Plugin::Core::ICore::dialogParent(), QCoreApplication::translate("QmakePriFile", "File Error"), errorMsg);
    }
  }
  ...
  if (document) {
    QString errorString;
    if (!document->reload(&errorString, Orca::Plugin::Core::IDocument::FlagReload, Orca::Plugin::Core::IDocument::TypeContents))
      errorStrings << errorString;
  }
  if (!errorStrings.isEmpty())
    QMessageBox::warning(Orca::Plugin::Core::ICore::dialogParent(), QCoreApplication::translate("QmakePriFile", "File Error"), errorStrings.join(QLatin1Char('\n')));
}
