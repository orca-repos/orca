...
class QmakePriFileDocument : public Orca::Plugin::Core::IDocument {
public:
  QmakePriFileDocument(QmakePriFile *qmakePriFile, const Utils::FilePath &filePath) : IDocument(nullptr), m_priFile(qmakePriFile)
  {
    ...
    Orca::Plugin::Core::DocumentManager::addDocument(this);
  }
...
QmakeProject::QmakeProject(const FilePath &fileName) : Project(QmakeProjectManager::Constants::PROFILE_MIMETYPE, fileName)
{
  ...
  setProjectLanguages(Orca::Plugin::Core::Context(ProjectExplorer::Constants::CXX_LANGUAGE_ID));
...
auto QmakeBuildSystem::updateDocuments() -> void
{
  ...
  const auto docGenerator = [&](const FilePath &fp) -> std::unique_ptr<Orca::Plugin::Core::IDocument> {
    const auto priFile = priFileForPath(fp);
    QTC_ASSERT(priFile, return std::make_unique<Orca::Plugin::Core::IDocument>());
    return std::make_unique<QmakePriFileDocument>(priFile, fp);
  };
  const auto docUpdater = [&](Orca::Plugin::Core::IDocument *doc) {
...
auto QmakeBuildSystem::asyncUpdate() -> void
{
  ...
  Orca::Plugin::Core::ProgressManager::addTask(m_asyncUpdateFutureInterface->future(), tr("Reading Project \"%1\"").arg(project()->displayName()), Constants::PROFILE_EVALUATE);
