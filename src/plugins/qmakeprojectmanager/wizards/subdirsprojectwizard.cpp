...
auto SubdirsProjectWizard::create(QWidget *parent, const Orca::Plugin::Core::WizardDialogParameters &parameters) const -> Orca::Plugin::Core::BaseFileWizard*
{
...
auto SubdirsProjectWizard::generateFiles(const QWizard *w, QString * /*errorMessage*/) const -> Orca::Plugin::Core::GeneratedFiles
{
  ...
  const auto profileName = Orca::Plugin::Core::BaseFileWizardFactory::buildFileName(projectPath, params.fileName, profileSuffix());

  Orca::Plugin::Core::GeneratedFile profile(profileName);
  profile.setAttributes(Orca::Plugin::Core::GeneratedFile::OpenProjectAttribute | Orca::Plugin::Core::GeneratedFile::OpenEditorAttribute);
  profile.setContents(QLatin1String("TEMPLATE = subdirs\n"));
  return Orca::Plugin::Core::GeneratedFiles() << profile;
}

auto SubdirsProjectWizard::postGenerateFiles(const QWizard *w, const Orca::Plugin::Core::GeneratedFiles &files, QString *errorMessage) const -> bool
{
  const auto *wizard = qobject_cast<const SubdirsProjectWizardDialog*>(w);
  if (QtWizard::qt4ProjectPostGenerateFiles(wizard, files, errorMessage)) {
    ...
    IWizardFactory::requestNewItemDialog(tr("New Subproject", "Title of dialog"), Utils::filtered(Orca::Plugin::Core::IWizardFactory::allWizardFactories(), [](Orca::Plugin::Core::IWizardFactory *f) {
