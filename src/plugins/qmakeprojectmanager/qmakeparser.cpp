...
auto QMakeParser::handleLine(const QString &line, OutputFormat type) -> OutputLineParser::Result
{
  if (type != Utils::StdErrFormat)
    return Status::NotHandled;
  auto lne = rightTrimmed(line);
  auto match = m_error.match(lne);
  if (match.hasMatch()) {
    auto fileName = match.captured(1);
    auto type = Task::Error;
    const auto description = match.captured(3);
    ...
    BuildSystemTask t(type, description, absoluteFilePath(FilePath::fromUserInput(fileName)), match.captured(2).toInt() /* line */);
    LinkSpecs linkSpecs;
    addLinkSpecForAbsoluteFilePath(linkSpecs, t.file, t.line, fileNameOffset, fileName.length());
    scheduleTask(t, 1);
    return {Status::Done, linkSpecs};
  }
  if (lne.startsWith(QLatin1String("Project ERROR: ")) || lne.startsWith(QLatin1String("ERROR: "))) {
    const auto description = lne.mid(lne.indexOf(QLatin1Char(':')) + 2);
    scheduleTask(BuildSystemTask(Task::Error, description), 1);
    return Status::Done;
  }
  if (lne.startsWith(QLatin1String("Project WARNING: ")) || lne.startsWith(QLatin1String("WARNING: "))) {
    const auto description = lne.mid(lne.indexOf(QLatin1Char(':')) + 2);
    ...
    QTest::newRow("moc note")
            ...
            << (Tasks()
                << BuildSystemTask(Task::Unknown,
                        ...
                        FilePath::fromUserInput("/home/qtwebkithelpviewer.hpp"), -1))
            << QString();
}
