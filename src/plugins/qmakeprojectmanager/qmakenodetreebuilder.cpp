...
QmakeStaticData::QmakeStaticData()
{
  ...
  for (const auto &fileType : fileTypeDataStorage) {
    ...
    fileTypeData.push_back(QmakeStaticData::FileTypeData(fileType.type, desc, filter, Orca::Plugin::Core::directoryIcon(QLatin1String(fileType.icon))));
  }
  // Project icon
  projectIcon = Orca::Plugin::Core::directoryIcon(ProjectExplorer::Constants::FILEOVERLAY_QT);
  productIcon = Orca::Plugin::Core::directoryIcon(ProjectExplorer::Constants::FILEOVERLAY_PRODUCT);
  groupIcon = Orca::Plugin::Core::directoryIcon(ProjectExplorer::Constants::FILEOVERLAY_GROUP);
