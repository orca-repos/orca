...
{
  auto mbuild = Orca::Plugin::Core::ActionManager::actionContainer(ProjectExplorer::Constants::M_BUILDPROJECT);
  auto mproject = Orca::Plugin::Core::ActionManager::actionContainer(ProjectExplorer::Constants::M_PROJECTCONTEXT);
  auto msubproject = Orca::Plugin::Core::ActionManager::actionContainer(ProjectExplorer::Constants::M_SUBPROJECTCONTEXT);
  auto mfile = Orca::Plugin::Core::ActionManager::actionContainer(ProjectExplorer::Constants::M_FILECONTEXT);

  const Orca::Plugin::Core::Context projectContext(CMakeProjectManager::Constants::CMAKE_PROJECT_ID);
  const Orca::Plugin::Core::Context globalContext(Orca::Plugin::Core::C_GLOBAL);

  auto command = Orca::Plugin::Core::ActionManager::registerAction(m_runCMakeAction, Constants::RUN_CMAKE, globalContext);
  command->setAttribute(Orca::Plugin::Core::Command::CA_Hide);
  mbuild->addAction(command, ProjectExplorer::Constants::G_BUILD_BUILD);
  connect(m_runCMakeAction, &QAction::triggered, [this]() {
    runCMake(SessionManager::startupBuildSystem());
  });

  command = Orca::Plugin::Core::ActionManager::registerAction(m_clearCMakeCacheAction, Constants::CLEAR_CMAKE_CACHE, globalContext);
  command->setAttribute(Orca::Plugin::Core::Command::CA_Hide);
  mbuild->addAction(command, ProjectExplorer::Constants::G_BUILD_BUILD);
  ...
  command = Orca::Plugin::Core::ActionManager::registerAction(m_runCMakeActionContextMenu, Constants::RUN_CMAKE_CONTEXT_MENU, projectContext);
  command->setAttribute(Orca::Plugin::Core::Command::CA_Hide);
  ...
  command = Orca::Plugin::Core::ActionManager::registerAction(m_buildFileContextMenu, Constants::BUILD_FILE_CONTEXT_MENU, projectContext);
  command->setAttribute(Orca::Plugin::Core::Command::CA_Hide);
  mfile->addAction(command, ProjectExplorer::Constants::G_FILE_OTHER);
  connect(m_buildFileContextMenu, &QAction::triggered, this, &CMakeManager::buildFileContextMenu);

  command = Orca::Plugin::Core::ActionManager::registerAction(m_rescanProjectAction, Constants::RESCAN_PROJECT, globalContext);
  command->setAttribute(Orca::Plugin::Core::Command::CA_Hide);
  mbuild->addAction(command, ProjectExplorer::Constants::G_BUILD_BUILD);
  ...
  command = Orca::Plugin::Core::ActionManager::registerAction(m_buildFileAction, Constants::BUILD_FILE);
  command->setAttribute(Orca::Plugin::Core::Command::CA_Hide);
  ...
  connect(Orca::Plugin::Core::EditorManager::instance(), &Orca::Plugin::Core::EditorManager::currentEditorChanged, this, &CMakeManager::updateBuildFileAction);
...
{
  Node *node = nullptr;
  if (auto currentDocument = Orca::Plugin::Core::EditorManager::currentDocument())
...
auto CMakeManager::buildFile(Node *node) -> void
{
  ...
  if (generator == "Ninja") {
    ...
    Orca::Plugin::Core::MessageManager::writeFlashing(tr("Build File is not supported for generator \"%1\"").arg(generator));
    return;
  }
