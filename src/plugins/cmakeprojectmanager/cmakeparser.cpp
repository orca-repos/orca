...
constexpr char COMMON_ERROR_PATTERN[] = "^CMake Error at (.*?):([0-9]*?)( \\((.*?)\\))?:";
constexpr char NEXT_SUBERROR_PATTERN[] = "^CMake Error in (.*?):";
constexpr char COMMON_WARNING_PATTERN[] = "^CMake Warning (\\(dev\\) )?at (.*?):([0-9]*?)( \\((.*?)\\))?:";
constexpr char LOCATION_LINE_PATTERN[] = ":(\\d+?):(?:(\\d+?))?$";
...
auto CMakeParser::setSourceDirectory(const QString &sourceDir) -> void
{
  if (m_sourceDirectory) emit searchDirExpired(FilePath::fromString(m_sourceDirectory.value().path()));
  m_sourceDirectory = QDir(sourceDir);
  emit newSearchDirFound(FilePath::fromString(sourceDir));
}

auto CMakeParser::handleLine(const QString &line, OutputFormat type) -> OutputLineParser::Result
{
  if (type != StdErrFormat)
    return Status::NotHandled;

  QRegularExpressionMatch match;
  auto trimmedLine = rightTrimmed(line);
  ...
  case NONE:
    ...
    if (match.hasMatch()) {
      auto path = m_sourceDirectory ? m_sourceDirectory->absoluteFilePath(QDir::fromNativeSeparators(match.captured(1))) : QDir::fromNativeSeparators(match.captured(1));
      m_lastTask = BuildSystemTask(Task::Error, QString(), absoluteFilePath(FilePath::fromUserInput(path)), match.captured(2).toInt());
      m_lines = 1;
      LinkSpecs linkSpecs;
    ...
    if (match.hasMatch()) {
      m_lastTask = BuildSystemTask(Task::Error, QString(), absoluteFilePath(FilePath::fromUserInput(match.captured(1))));
      LinkSpecs linkSpecs;
      addLinkSpecForAbsoluteFilePath(linkSpecs, m_lastTask.file, m_lastTask.line, match, 1);
      m_lines = 1;
      return {Status::InProgress, linkSpecs};
    ...
    if (match.hasMatch()) {
      auto path = m_sourceDirectory ? m_sourceDirectory->absoluteFilePath(QDir::fromNativeSeparators(match.captured(2))) : QDir::fromNativeSeparators(match.captured(2));
      m_lastTask = BuildSystemTask(Task::Warning, QString(), absoluteFilePath(FilePath::fromUserInput(path)), match.captured(3).toInt());
      m_lines = 1;
      LinkSpecs linkSpecs;
      addLinkSpecForAbsoluteFilePath(linkSpecs, m_lastTask.file, m_lastTask.line, match, 1);
      return {Status::InProgress, linkSpecs};
      ...
      flush();
      const auto type = trimmedLine.contains(QLatin1String("Error")) ? Task::Error : Task::Warning;
  ...
  case LINE_LOCATION: {
    ...
    m_lastTask.file = absoluteFilePath(FilePath::fromUserInput(trimmedLine.mid(0, match.capturedStart())));
    m_lastTask.line = match.captured(1).toInt();
    m_expectTripleLineErrorData = LINE_DESCRIPTION;
    LinkSpecs linkSpecs;
    addLinkSpecForAbsoluteFilePath(linkSpecs, m_lastTask.file, m_lastTask.line, 0, match.capturedStart());
    return {Status::InProgress, linkSpecs};
  }
