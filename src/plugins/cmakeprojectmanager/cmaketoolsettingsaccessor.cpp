...
constexpr char CMAKE_TOOL_COUNT_KEY[] = "CMakeTools.Count";
constexpr char CMAKE_TOOL_DATA_KEY[] = "CMakeTools.";
constexpr char CMAKE_TOOL_DEFAULT_KEY[] = "CMakeTools.Default";
constexpr char CMAKE_TOOL_FILENAME[] = "cmaketools.xml";

static auto autoDetectCMakeTools() -> std::vector<std::unique_ptr<CMakeTool>>
{
  ...
  if (HostOsInfo::isWindowsHost()) {
    for (auto envVar : {"ProgramFiles", "ProgramFiles(x86)", "ProgramW6432"}) {
      if (qEnvironmentVariableIsSet(envVar)) {
        const auto progFiles = qEnvironmentVariable(envVar);
  ...
  const auto execs = env.appendExeExtensions(QLatin1String("cmake"));
...
static auto mergeTools(std::vector<std::unique_ptr<CMakeTool>> &sdkTools, std::vector<std::unique_ptr<CMakeTool>> &userTools, std::vector<std::unique_ptr<CMakeTool>> &autoDetectedTools) -> std::vector<std::unique_ptr<CMakeTool>>
{
  auto result = std::move(sdkTools);
  while (userTools.size() > 0) {
    auto userTool = std::move(userTools[0]);
    userTools.erase(std::begin(userTools));

    auto userToolIndex = Utils::indexOf(result, [&userTool](const std::unique_ptr<CMakeTool> &tool) {
    ...
    if (userToolIndex >= 0) {
      ...
      if (userTool->isAutoDetected() && !Utils::contains(autoDetectedTools, Utils::equal(&CMakeTool::cmakeExecutable, userTool->cmakeExecutable()))) {

        qWarning() << QString::fromLatin1("Previously SDK provided CMakeTool \"%1\" (%2) dropped.").arg(userTool->cmakeExecutable().toUserOutput(), userTool->id().toString());
        continue;
      }
  ...
  while (autoDetectedTools.size() > 0) {
    auto autoDetectedTool = std::move(autoDetectedTools[0]);
    autoDetectedTools.erase(std::begin(autoDetectedTools));

    if (!Utils::contains(result, Utils::equal(&CMakeTool::cmakeExecutable, autoDetectedTool->cmakeExecutable())))
...
CMakeToolSettingsAccessor::CMakeToolSettingsAccessor() : UpgradingSettingsAccessor("QtCreatorCMakeTools", QCoreApplication::translate("CMakeProjectManager::CMakeToolManager", "CMake"), Orca::Plugin::Core::IDE_DISPLAY_NAME)
{
  setBaseFilePath(Orca::Plugin::Core::ICore::userResourcePath(CMAKE_TOOL_FILENAME));

  addVersionUpgrader(std::make_unique<CMakeToolSettingsUpgraderV0>());
}

auto CMakeToolSettingsAccessor::restoreCMakeTools(QWidget *parent) const -> CMakeToolSettingsAccessor::CMakeTools
{
  CMakeTools result;

  const auto sdkSettingsFile = Orca::Plugin::Core::ICore::installerResourcePath(CMAKE_TOOL_FILENAME);

  auto sdkTools = cmakeTools(restoreSettings(sdkSettingsFile, parent), true);

  //read the tools from the user settings file
  auto userTools = cmakeTools(restoreSettings(parent), false);

  //autodetect tools
  auto autoDetectedTools = autoDetectCMakeTools();

  //filter out the tools that were stored in SDK
  auto toRegister = mergeTools(sdkTools.cmakeTools, userTools.cmakeTools, autoDetectedTools);
...
auto CMakeToolSettingsAccessor::saveCMakeTools(const QList<CMakeTool*> &cmakeTools, const Id &defaultId, QWidget *parent) -> void
{
  QVariantMap data;
  ...
  for (const CMakeTool *item : cmakeTools) {
    auto fi = item->cmakeExecutable();

    if (fi.needsDevice() || fi.isExecutableFile()) {
      // be graceful for device related stuff
      auto tmp = item->toMap();
      if (tmp.isEmpty())
        continue;
...
auto CMakeToolSettingsAccessor::cmakeTools(const QVariantMap &data, bool fromSdk) const -> CMakeToolSettingsAccessor::CMakeTools
{
  CMakeTools result;

  auto count = data.value(QLatin1String(CMAKE_TOOL_COUNT_KEY), 0).toInt();
  for (auto i = 0; i < count; ++i) {
    const QString key = QString::fromLatin1(CMAKE_TOOL_DATA_KEY) + QString::number(i);
    if (!data.contains(key))
      continue;

    const auto dbMap = data.value(key).toMap();
    auto item = std::make_unique<CMakeTool>(dbMap, fromSdk);
    const auto cmakeExecutable = item->cmakeExecutable();
    if (item->isAutoDetected() && !cmakeExecutable.needsDevice() && !cmakeExecutable.isExecutableFile()) {
      qWarning() << QString("CMakeTool \"%1\" (%2) dropped since the command is not executable.").arg(cmakeExecutable.toUserOutput(), item->id().toString());
      continue;
    }
