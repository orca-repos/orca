...
static auto noAutoAdditionNotify(const FilePaths &filePaths, const ProjectNode *node) -> void
{
  ...
  if (!srcPaths.empty()) {
    ...
    case AskUser: {
      ...
      auto reply = CheckableMessageBox::question(Orca::Plugin::Core::ICore::dialogParent(), QMessageBox::tr("Copy to Clipboard?"), QMessageBox::tr("Files are not automatically added to the " "CMakeLists.txt file of the CMake project." "\nCopy the path to the source files to the clipboard?"), "Remember My Choice", &checkValue, QDialogButtonBox::Yes | QDialogButtonBox::No, QDialogButtonBox::Yes);
      if (checkValue) {
...
auto CMakeBuildSystem::mustApplyConfigurationChangesArguments(const BuildDirParameters &parameters) const -> bool
{
  ...
  auto answer = QMessageBox::question(Orca::Plugin::Core::ICore::mainWindow(), tr("Apply configuration changes?"), "<p>" + tr("Run CMake with configuration changes?") + "</p><pre>" + parameters.configurationChangesArguments.join("\n") + "</pre>", QMessageBox::Apply | QMessageBox::Discard, QMessageBox::Apply);
...
auto CMakeBuildSystem::updateFallbackProjectData() -> void
{
  ...
  if (m_treeScanner.asyncScanForFiles(projectDirectory()))
    Orca::Plugin::Core::ProgressManager::addTask(m_treeScanner.future(), tr("Scan \"%1\" project tree").arg(project()->displayName()), "CMake.Scan.Tree");
}
