...
public:
  ...
  auto cancelScheduledUpdate() -> void;
  auto resetParser() -> void;
  auto findItemByRoot(const QStandardItem *item, bool skipRoot = false) const -> ParserTreeItem::ConstPtr;
};

auto ManagerPrivate::cancelScheduledUpdate() -> void
{
  m_timer.stop();
  m_awaitingDocuments.clear();
}

auto ManagerPrivate::resetParser() -> void
{
  cancelScheduledUpdate();

  QHash<FilePath, QPair<QString, FilePaths>> projectData;
  for (const Project *project : SessionManager::projects()) {
    projectData.insert(project->projectFilePath(), qMakePair(project->displayName(), project->files(Project::SourceFiles)));
  }
...
/*!
    ...
    item.
*/
auto ManagerPrivate::findItemByRoot(const QStandardItem *item, bool skipRoot) const -> ParserTreeItem::ConstPtr
{
  if (!item)
  ...
  auto internal = m_root;
  while (uiList.count() > 0) {
    cur = uiList.last();
    uiList.removeLast();
    const auto &inf = Internal::symbolInformationFromItem(cur);
...
Manager::Manager(QObject *parent) : QObject(parent), d(new ManagerPrivate())
{
...
*/
auto Manager::canFetchMore(QStandardItem *item, bool skipRoot) const -> bool
{
  const auto ptr = d->findItemByRoot(item, skipRoot);
  if (ptr.isNull())
    return false;
...
*/
auto Manager::fetchMore(QStandardItem *item, bool skipRoot) -> void
{
  const auto ptr = d->findItemByRoot(item, skipRoot);
  if (ptr.isNull())
    return;
...
auto Manager::hasChildren(QStandardItem *item) const -> bool
{
  const auto ptr = d->findItemByRoot(item);
  if (ptr.isNull())
    return false;
...
auto Manager::initialize() -> void
{
  ...
  connect(sessionManager, &SessionManager::projectAdded, this, [this](Project *project) {
    const auto projectPath = project->projectFilePath();
    const auto projectName = project->displayName();
    const auto projectFiles = project->files(Project::SourceFiles);
  ...
  connect(sessionManager, &SessionManager::projectRemoved, this, [this](Project *project) {
    const auto projectPath = project->projectFilePath();
  ...
  connect(d->m_parser, &Parser::treeRegenerated, this, [this](const ParserTreeItem::ConstPtr &root) {
    d->m_root = root;

    if (!state())
      return;

    const QSharedPointer<QStandardItem> rootItem(new QStandardItem());
  ...
  const auto codeModelManager = CppEditor::CppModelManager::instance();

  // when code manager signals that document is updated - handle it by ourselves
  connect(codeModelManager, &CppEditor::CppModelManager::documentUpdated, this, [this](CPlusPlus::Document::Ptr doc) {
    // do nothing if Manager is disabled
    if (!state())
      return;
  ...
  connect(&d->m_timer, &QTimer::timeout, this, [this]() {
    const auto docsToBeUpdated = d->m_awaitingDocuments;
    d->cancelScheduledUpdate();
  ...
  connect(codeModelManager, &CppEditor::CppModelManager::aboutToRemoveFiles, d->m_parser, &Parser::removeFiles, Qt::QueuedConnection);
}
...
auto Manager::setState(bool state) -> void
{
  if (state == d->state)
    return;
...
auto Manager::onWidgetVisibilityIsChanged(bool visibility) -> void
{
...
auto Manager::gotoLocation(const QString &fileName, int line, int column) -> void
{
...
auto Manager::gotoLocations(const QList<QVariant> &list) -> void
{
  const auto locations = Internal::roleToLocations(list);
  if (locations.size() == 0)
    return;
  ...
  if (locations.size() > 1) {
    ...
    const auto textEditor = qobject_cast<TextEditor::BaseTextEditor*>(EditorManager::currentEditor());
    if (textEditor) {
      // check if current cursor position is a known location of the symbol
      const auto fileName = textEditor->document()->filePath().toString();
      int line;
      int column;
...
auto Manager::setFlatMode(bool flat) -> void
{
