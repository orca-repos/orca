...
{
  return Orca::Plugin::Core::ICore::installerResourcePath(QTVERSION_FILENAME);
}

static auto settingsFileName(const QString &path) -> FilePath
{
  return Orca::Plugin::Core::ICore::userResourcePath(path);
}
...
static auto saveQtVersions() -> void
{
  ...
  m_writer->save(data, Orca::Plugin::Core::ICore::dialogParent());
}
...
auto QtVersionManager::updateDocumentation(const QtVersions &added, const QtVersions &removed, const QtVersions &allNew) -> void
{
  ...
  Orca::Plugin::Core::unregisterDocumentation(docsToRemove);
  Orca::Plugin::Core::registerDocumentation(docsToAdd);
}
...
auto QtVersionManager::setDocumentationSetting(const DocumentationSetting &setting) -> void
{
  if (setting == documentationSetting())
    return;
  Orca::Plugin::Core::ICore::settings()->setValueWithDefault(DOCUMENTATION_SETTING_KEY, int(setting), 0);
...
{
  return DocumentationSetting(Orca::Plugin::Core::ICore::settings()->value(DOCUMENTATION_SETTING_KEY, 0).toInt());
}
