...
{
  const auto id = Orca::Plugin::Core::ICore::settings()->value(QLatin1String(kSelectedExampleSetKey));
...
ExampleSetModel::ExampleSetModel()
{
  ...
  connect(Orca::Plugin::Core::Signals::instance(), &Orca::Plugin::Core::Signals::setupFinished, this, &ExampleSetModel::helpManagerInitialized);
}
...
ExamplesListModel::ExamplesListModel(QObject *parent) : ListModel(parent)
{
  ...
  connect(Orca::Plugin::Core::Signals::instance(), &Orca::Plugin::Core::Signals::documentationChanged, this, &ExamplesListModel::updateExamples);
}
...
static auto resourcePath() -> QString
{
  ...
  return Orca::Plugin::Core::ICore::resourcePath().normalizedPathName().toString();
}
...
auto ExamplesListModel::fetchPixmapAndUpdatePixmapCache(const QString &url) const -> QPixmap
{
  ...
  if (pixmap.isNull()) {
    auto fetchedData = Orca::Plugin::Core::fileData(url);
...
auto ExamplesListModelFilter::leaveFilterAcceptsRowBeforeFiltering(const Orca::Plugin::Core::ListItem *item, bool *earlyExitResult) const ->  bool
{
