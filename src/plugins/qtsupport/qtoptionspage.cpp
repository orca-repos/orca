...
#include <core/core-restart-dialog.hpp>
...
class QtOptionsPageWidget : public Orca::Plugin::Core::IOptionsPageWidget {
...
{
  return baseDir + (baseDir.isEmpty() ? "" : "/") + Orca::Plugin::Core::IDE_SETTINGSVARIANT_STR + '/' + Orca::Plugin::Core::IDE_CASED_ID + ".ini";
}

static auto qtVersionsFile(const QString &baseDir) -> QString
{
  return baseDir + (baseDir.isEmpty() ? "" : "/") + Orca::Plugin::Core::IDE_SETTINGSVARIANT_STR + '/' + Orca::Plugin::Core::IDE_ID + '/' + "qtversion.xml";
}

static auto currentlyLinkedQtDir(bool *hasInstallSettings) -> optional<FilePath>
{
  const auto installSettingsFilePath = settingsFile(Orca::Plugin::Core::ICore::resourcePath().toString());
...
static auto canLinkWithQt(QString *toolTip) -> bool
{
  ...
  if (!Orca::Plugin::Core::ICore::resourcePath().isWritableDir()) {
    canLink = false;
    tip << QtOptionsPageWidget::tr("%1's resource directory is not writable.").arg(Orca::Plugin::Core::IDE_DISPLAY_NAME);
  }
  ...
  if (installSettingsExist && !installSettingsValue) {
    canLink = false;
    tip << QtOptionsPageWidget::tr("%1 is part of a Qt installation.").arg(Orca::Plugin::Core::IDE_DISPLAY_NAME);
  }
  const auto link = installSettingsValue ? *installSettingsValue : FilePath();
  if (!link.isEmpty())
    tip << QtOptionsPageWidget::tr("%1 is currently linked to \"%2\".").arg(QString(Orca::Plugin::Core::IDE_DISPLAY_NAME), link.toUserOutput());
  if (toolTip)
    *toolTip = tip.join("\n\n");
  return canLink;
}
...
auto QtOptionsPageWidget::linkWithQt() -> void
{
  ...
  QDialog dialog(Orca::Plugin::Core::ICore::dialogParent());
  ...
  connect(unlinkButton, &QPushButton::clicked, &dialog, [&dialog, &askForRestart] {
    ...
    const auto filePath = settingsFile(Orca::Plugin::Core::ICore::resourcePath().toString());
    {
  ...
  if (dialog.result() == QDialog::Accepted) {
    ...
    if (QTC_GUARD(settingsDir)) {
      QSettings(settingsFile(Orca::Plugin::Core::ICore::resourcePath().toString()), QSettings::IniFormat).setValue(kInstallSettingsKey, *settingsDir);
      askForRestart = true;
    }
  }
  if (askForRestart) {
