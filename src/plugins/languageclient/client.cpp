...
Client::Client(BaseClientInterface *clientInterface) : m_id(Utils::Id::fromString(QUuid::createUuid().toString())), m_clientInterface(clientInterface), m_diagnosticManager(this), m_documentSymbolCache(this), m_hoverHandler(this), m_symbolSupport(this), m_tokenSupport(this)
{
  ...
  connect(Orca::Plugin::Core::EditorManager::instance(), &Orca::Plugin::Core::EditorManager::documentClosed, this, &Client::documentClosed);
...
static auto updateEditorToolBar(QList<TextEditor::TextDocument*> documents) -> void
{
  for (const auto document : documents) {
    for (const auto editor : Orca::Plugin::Core::DocumentModel::editorsForDocument(document))
...
Client::~Client()
{
  ...
  const auto &editors = Orca::Plugin::Core::DocumentModel::editorsForOpenedDocuments();
...
auto Client::sendContent(const IContent &content, SendDocUpdates sendUpdates) -> void
{
  ...
  if (!QTC_GUARD(content.isValid(&error)))
    Orca::Plugin::Core::MessageManager::writeFlashing(error);
...
auto Client::documentClosed(Orca::Plugin::Core::IDocument *document) -> void
{
...
auto Client::documentWillSave(Orca::Plugin::Core::IDocument *document) -> void
{
  const auto &filePath = document->filePath();
...
auto Client::log(const QString &message) const -> void
{
  switch (m_logTarget) {
  case LogTarget::Ui:
    Orca::Plugin::Core::MessageManager::writeFlashing(QString("LanguageClient %1: %2").arg(name(), message));
    break;
...
auto Client::initializeCallback(const InitializeRequest::Response &initResponse) -> void
{
  ...
  if (const auto error = initResponse.error()) {
    if (error.value().data().has_value() && error.value().data().value().retry()) {
      ...
      const auto result = QMessageBox::warning(Orca::Plugin::Core::ICore::dialogParent(), title, error.value().message(), QMessageBox::Retry | QMessageBox::Cancel, QMessageBox::Retry);
