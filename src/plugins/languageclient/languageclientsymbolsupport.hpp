...
public:
  ...
  static auto convertRange(const LanguageServerProtocol::Range &range) -> Core::Search::TextRange;
...
private:
  ...
  auto requestRename(const LanguageServerProtocol::TextDocumentPositionParams &positionParams, const QString &newName, Core::SearchResult *search) -> void;
  auto startRenameSymbol(const LanguageServerProtocol::TextDocumentPositionParams &params, const QString &placeholder) -> void;
  auto handleRenameResponse(Core::SearchResult *search, const LanguageServerProtocol::RenameRequest::Response &response) -> void;
  auto applyRename(const QList<Core::SearchResultItem> &checkedItems) -> void;
