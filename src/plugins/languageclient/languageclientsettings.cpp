...
class LanguageClientSettingsPage : public Orca::Plugin::Core::IOptionsPage {
...
{
  m_model.reset(LanguageClientSettings::fromSettings(Orca::Plugin::Core::ICore::settings()));
  apply();
  finish();
}
...
{
  const auto settings = Orca::Plugin::Core::ICore::settings();
  settings->beginGroup(settingsGroupKey);
...
{
  MimeTypeDialog dialog(m_mimeTypes->text().split(filterSeparator, Qt::SkipEmptyParts), Orca::Plugin::Core::ICore::dialogParent());
...
auto LanguageFilter::isSupported(const Orca::Plugin::Core::IDocument *document) const -> bool
{
