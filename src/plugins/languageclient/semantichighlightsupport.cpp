...
SemanticTokenSupport::SemanticTokenSupport(Client *client) : m_client(client)
{
  ...
  QObject::connect(Orca::Plugin::Core::EditorManager::instance(), &Orca::Plugin::Core::EditorManager::currentEditorChanged, this, &SemanticTokenSupport::onCurrentEditorChanged);
}
...
auto SemanticTokenSupport::refresh() -> void
{
  ...
  m_tokens.clear();
  for (const auto editor : Orca::Plugin::Core::EditorManager::visibleEditors())
...
auto SemanticTokenSupport::onCurrentEditorChanged(Orca::Plugin::Core::IEditor *editor) -> void
{
