...
auto generateSearchResultItems(const QMap<Utils::FilePath, QList<ItemData>> &rangesInDocument) -> QList<Core::SearchResultItem>
{
...
auto generateSearchResultItems(const LanguageClientArray<Location> &locations) -> QList<Core::SearchResultItem>
{
  if (locations.isNull())
    return {};
...
auto SymbolSupport::handleFindReferencesResponse(const FindReferencesRequest::Response &response, const QString &wordUnderCursor, const ResultHandler &handler) -> void
{
  ...
  if (result) {
    const auto search = Core::SearchResultWindow::instance()->startNewSearch(tr("Find References with %1 for:").arg(m_client->name()), "", wordUnderCursor);
...
auto SymbolSupport::requestRename(const TextDocumentPositionParams &positionParams, const QString &newName, Core::SearchResult *search) -> void
{
...
auto generateReplaceItems(const WorkspaceEdit &edits) -> QList<Core::SearchResultItem>
{
...
{
  auto search = Core::SearchResultWindow::instance()->startNewSearch(tr("Find References with %1 for:").arg(m_client->name()), "", placeholder, Core::SearchResultWindow::SearchAndReplace);
  ...
  QObject::connect(search, &Core::SearchResult::searchAgainRequested, [this, positionParams, search]() {
    search->restart();
    requestRename(positionParams, search->textToReplace(), search);
  });
  QObject::connect(search, &Core::SearchResult::replaceButtonClicked, [this, positionParams](const QString & /*replaceText*/, const QList<Core::SearchResultItem> &checkedItems) {
...
auto SymbolSupport::handleRenameResponse(Core::SearchResult *search, const RenameRequest::Response &response) -> void
{
  const auto &error = response.error();
  if (error.has_value())
    m_client->log(*error);
...
auto SymbolSupport::applyRename(const QList<Core::SearchResultItem> &checkedItems) -> void
{
...
auto SymbolSupport::convertRange(const Range &range) -> Core::Search::TextRange
{
