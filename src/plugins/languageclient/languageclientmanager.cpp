...
auto LanguageClientManager::applySettings() -> void
{
  ...
  LanguageClientSettings::toSettings(Orca::Plugin::Core::ICore::settings(), managerInstance->m_currentSettings);
  ...
  for (const auto setting : restarts) {
    ...
    case BaseSettings::RequiresFile: {
      const auto &openedDocuments = Orca::Plugin::Core::DocumentModel::openedDocuments();
...
auto LanguageClientManager::showInspector() -> void
{
  ...
  Orca::Plugin::Core::ICore::registerWindow(inspectorWidget, Orca::Plugin::Core::Context("LanguageClient.Inspector"));
...
auto LanguageClientManager::editorOpened(Orca::Plugin::Core::IEditor *editor) -> void
{
...
auto LanguageClientManager::documentOpened(Orca::Plugin::Core::IDocument *document) -> void
{
  const auto textDocument = qobject_cast<TextEditor::TextDocument*>(document);
  if (!textDocument)
    return;
...
auto LanguageClientManager::documentClosed(Orca::Plugin::Core::IDocument *document) -> void
{
  if (const auto textDocument = qobject_cast<TextEditor::TextDocument*>(document))
    m_clientForDocument.remove(textDocument);
}

auto LanguageClientManager::documentContentsSaved(Orca::Plugin::Core::IDocument *document) -> void
{
  if (const auto textDocument = qobject_cast<TextEditor::TextDocument*>(document)) {
...
auto LanguageClientManager::documentWillSave(Orca::Plugin::Core::IDocument *document) -> void
{
  if (const auto textDocument = qobject_cast<TextEditor::TextDocument*>(document)) {
    const auto &clients = reachableClients();
    for (const auto client : clients)
