...
auto ResourceFile::load() -> Orca::Plugin::Core::IDocument::OpenResult
{
  m_error_message.clear();

  if (m_filePath.isEmpty()) {
    m_error_message = tr("The file name is empty.");
    return Orca::Plugin::Core::IDocument::OpenResult::ReadError;
  }
  ...
  if (m_contents.isEmpty()) {
    ...
    if (!doc.setContent(data, &error_msg, &error_line, &error_col)) {
      ...
      return Orca::Plugin::Core::IDocument::OpenResult::CannotHandle;
    }
  ...
  return Orca::Plugin::Core::IDocument::OpenResult::Success;
}
...
auto ResourceFile::renameFile(const QString &fileName, const QString &newFileName) -> bool
{
  ...
  if (entries.at(0)->exists()) {
    foreach(File *file, entries)
      file->setExists(true);
    success = Orca::Plugin::Core::FileUtils::renameFile(Utils::FilePath::fromString(entries.at(0)->name), Utils::FilePath::fromString(newFileName));
  }
...
{
  static auto resourceFolderIcon = Orca::Plugin::Core::directoryIcon(QLatin1String(ProjectExplorer::Constants::FILEOVERLAY_QRC));
    ...
    if (isFileNode) {
      ...
      if (file->icon.isNull()) {
        ...
        if (iconFileExtension(path))
          ...
          file->icon = Orca::Plugin::Core::icon(Utils::FilePath::fromString(path));
      }
      if (!file->icon.isNull())
        result = file->icon;
...
auto ResourceModel::addFiles(int prefixIndex, const QStringList &fileNames, int cursorFile, int &firstFile, int &lastFile) -> void
{
  ...
  Orca::Plugin::Core::VcsManager::promptToAdd(m_resource_file.filePath().absolutePath(), Utils::transform(fileNames, &FilePath::fromString));
}
...
auto ResourceModel::reload() -> Orca::Plugin::Core::IDocument::OpenResult
{
  beginResetModel();
  auto result = m_resource_file.load();
  if (result == Orca::Plugin::Core::IDocument::OpenResult::Success)
    setDirty(false);
  endResetModel();
  return result;
}
...
auto RelativeResourceModel::removeEntry(const QModelIndex &index) -> EntryBackup*
{
  ...
  if (isPrefixNode) {
    ...
    Utils::RemoveFileDialog removeFileDialog(Utils::FilePath::fromString(fileNameBackup), Orca::Plugin::Core::ICore::dialogParent());
    if (removeFileDialog.exec() == QDialog::Accepted) {
      deleteItem(index);
      Orca::Plugin::Core::FileUtils::removeFiles({Utils::FilePath::fromString(fileNameBackup)}, removeFileDialog.isDeleteFileChecked());
      return new FileEntryBackup(*this, prefixIndex.row(), index.row(), fileNameBackup, aliasBackup);
    }
    return nullptr;
  }
