...
ResourceEditorPluginPrivate::ResourceEditorPluginPrivate(ResourceEditorPlugin *q) : m_editorFactory(q)
{
  ...
  const auto folderContextMenu = Core::ActionManager::actionContainer(ProjectExplorer::Constants::M_FOLDERCONTEXT);
  const auto fileContextMenu = Core::ActionManager::actionContainer(ProjectExplorer::Constants::M_FILECONTEXT);
...
auto ResourceEditorPluginPrivate::removePrefixContextMenu() -> void
{
  ...
  if (QMessageBox::question(Core::ICore::dialogParent(), tr("Remove Prefix"), tr("Remove prefix %1 and all its files?").arg(rfn->displayName())) == QMessageBox::Yes) {
...
auto ResourceEditorPluginPrivate::removeFileContextMenu() -> void
{
  ...
  if (parent->removeFiles({path}) != RemovedFilesFromProject::Ok)
    QMessageBox::warning(Core::ICore::dialogParent(), tr("File Removal Failed"), tr("Removing file %1 from the project failed.").arg(path.toUserOutput()));
}
...
auto ResourceEditorPluginPrivate::renamePrefixContextMenu() -> void
{
  ...
  PrefixLangDialog dialog(tr("Rename Prefix"), node->prefix(), node->lang(), Core::ICore::dialogParent());
  if (dialog.exec() != QDialog::Accepted)
    return;
  const auto prefix = dialog.prefix();
  if (prefix.isEmpty())
