...
ResourceEditorW::ResourceEditorW(const Orca::Plugin::Core::Context &context, ResourceEditorPlugin *plugin, QWidget *parent) : m_resourceDocument(new ResourceEditorDocument(this)), m_plugin(plugin), m_contextMenu(new QMenu), m_toolBar(new QToolBar)
{
  ...
  Orca::Plugin::Core::CommandButton *refreshButton = new Orca::Plugin::Core::CommandButton(Constants::REFRESH, m_toolBar);
...
auto ResourceEditorDocument::open(QString *errorString, const FilePath &filePath, const FilePath &realFilePath) -> Orca::Plugin::Core::IDocument::OpenResult
{
  if (debugResourceEditorW)
...
auto ResourceEditorDocument::setContents(const QByteArray &contents) -> bool
{
  ...
  if (!saver.finalize(Orca::Plugin::Core::ICore::dialogParent()))
    return false;
...
{
  Orca::Plugin::Core::EditorManager::populateOpenWithMenu(m_openWithMenu, FilePath::fromString(fileName));
...
{
  Orca::Plugin::Core::EditorManager::openEditor(FilePath::fromString(fileName));
}
