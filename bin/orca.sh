...
# libraries that Orca depends on.
...
# Add Orca library path
_ORIGINAL_LD_LIBRARY_PATH=$LD_LIBRARY_PATH
LD_LIBRARY_PATH=$libdir:$libdir/orca$qtlibpath${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}
export LD_LIBRARY_PATH
exec "$bindir/orca" -user-library-path "$_ORIGINAL_LD_LIBRARY_PATH" ${1+"$@"}
