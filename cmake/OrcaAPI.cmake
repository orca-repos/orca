...
include(${CMAKE_CURRENT_LIST_DIR}/OrcaAPIInternal.cmake)
include(${CMAKE_CURRENT_LIST_DIR}/OrcaSeparateDebugInfo.cmake)
...
set(_THIS_MODULE_BASE_DIR "${CMAKE_CURRENT_FUNCTION_LIST_DIR}")
...
# e.g. ORCA_BUILD_PLUGINS=Core;TextEditor will result in ORCA_BUILD_PLUGINS_BY_DEFAULT=OFF and ORCA_BUILD_PLUGIN_CORE=ON and ORCA_BUILD_PLUGIN_TEXTEDITOR ON
...
function(qtc_output_binary_dir varName)
  if (QTC_MERGE_BINARY_DIR)
    set(${varName} ${Orca_BINARY_DIR} PARENT_SCOPE)
  else()
...
function(qtc_source_dir varName)
  if (QTC_MERGE_BINARY_DIR)
    set(${varName} ${Orca_SOURCE_DIR} PARENT_SCOPE)
  else()
...
function(add_qtc_library name)
  ...
  string(TOUPPER "ORCA_BUILD_LIBRARY_${name}" _orca_build_library_var)
  if (DEFINED _arg_BUILD_DEFAULT)
    set(_orca_build_library_default ${_arg_BUILD_DEFAULT})
  else()
    set(_orca_build_library_default ${ORCA_BUILD_LIBRARIES_BY_DEFAULT})
  endif()
  if (DEFINED ENV{QTC_${_orca_build_library_var}})
    set(_orca_build_library_default "$ENV{QTC_${_orca_build_library_var}}")
  endif()
  set(${_orca_build_library_var} "${_orca_build_library_default}" CACHE BOOL "Build library ${name}.")

  if ((${_arg_CONDITION}) AND ${_orca_build_library_var})
  ...
  add_library(Orca::${name} ALIAS ${name})
  ...
  if (ORCA_BUILD_WITH_PLUGINS_TESTS)
    set(TEST_DEFINES ORCA_BUILD_WITH_PLUGINS_TESTS SRCDIR="${CMAKE_CURRENT_SOURCE_DIR}")
  endif()
  ...
  orca_separate_debug_info(${name} "${IDE_LIBRARY_PATH}")
...
function(add_qtc_plugin target_name)
  ...
  string(TOUPPER "ORCA_BUILD_PLUGIN_${target_name}" _orca_build_plugin_var)
  if (DEFINED _arg_BUILD_DEFAULT)
    set(_orca_build_plugin_default ${_arg_BUILD_DEFAULT})
  else()
    set(_orca_build_plugin_default ${ORCA_BUILD_PLUGINS_BY_DEFAULT})
  endif()
  if (DEFINED ENV{QTC_${_orca_build_plugin_var}})
    set(_orca_build_plugin_default "$ENV{QTC_${_orca_build_plugin_var}}")
  endif()
  if (_arg_INTERNAL_ONLY)
    set(${_orca_build_plugin_var} "${_orca_build_plugin_default}")
  else()
    set(${_orca_build_plugin_var} "${_orca_build_plugin_default}" CACHE BOOL "Build plugin ${name}.")
  endif()

  if ((${_arg_CONDITION}) AND ${_orca_build_plugin_var})
  ...
  if (EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${name}.json.in")
    ...
    string(REPLACE "\\'" "'" plugin_json_in ${plugin_json_in})
    string(REPLACE "$$ORCA_VERSION" "\${IDE_VERSION}" plugin_json_in ${plugin_json_in})
    string(REPLACE "$$ORCA_COMPAT_VERSION" "\${IDE_VERSION_COMPAT}" plugin_json_in ${plugin_json_in})
    string(REPLACE "$$ORCA_COPYRIGHT_YEAR" "\${IDE_COPYRIGHT_YEAR}" plugin_json_in ${plugin_json_in})
  ...
  add_library(Orca::${target_name} ALIAS ${target_name})
  ...
  if (NOT _arg_SKIP_INSTALL)
    ...
    orca_separate_debug_info(${target_name} "${plugin_dir}")
...
function(add_qtc_executable name)
  ...
  string(TOUPPER "ORCA_BUILD_EXECUTABLE_${name}" _orca_build_executable_var)
  if (DEFINED _arg_BUILD_DEFAULT)
    set(_orca_build_executable_default ${_arg_BUILD_DEFAULT})
  else()
    set(_orca_build_executable_default ${ORCA_BUILD_EXECUTABLES_BY_DEFAULT})
  endif()
  if (DEFINED ENV{QTC_${_orca_build_executable_var}})
    set(_orca_build_executable_default "$ENV{QTC_${_orca_build_executable_var}}")
  endif()
  set(${_orca_build_executable_var} "${_orca_build_executable_default}" CACHE BOOL "Build executable ${name}.")

  if ((${_arg_CONDITION}) AND ${_orca_build_executable_var})
  ...
  if (_arg_QTC_RUNNABLE)
      # Used by Orca to select the default target in the project
  ...
  if (NOT _arg_SKIP_INSTALL)
    ...
    orca_separate_debug_info(${name} "${_DESTINATION}")
  endif()
endfunction()
...
function(qtc_add_public_header header)
  ...
  qtc_source_dir(orca_source_dir)
  get_filename_component(source_dir ${header} DIRECTORY)
  file(RELATIVE_PATH include_dir_relative_path ${orca_source_dir} ${source_dir})
