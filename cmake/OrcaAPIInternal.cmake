...
if (APPLE)
  ...
  set(_IDE_HEADER_INSTALL_PATH "${_IDE_DATA_PATH}/Headers/orca")
...
elseif(WIN32)
  ...
  set(_IDE_LIBRARY_PATH "${_IDE_LIBRARY_BASE_PATH}/orca")
  set(_IDE_PLUGIN_PATH "${_IDE_LIBRARY_BASE_PATH}/orca/plugins")
  set(_IDE_LIBEXEC_PATH "bin")
  set(_IDE_DATA_PATH "share/orca")
  set(_IDE_DOC_PATH "share/doc/orca")
  set(_IDE_BIN_PATH "bin")
  set(_IDE_LIBRARY_ARCHIVE_PATH "${_IDE_BIN_PATH}")

  set(_IDE_HEADER_INSTALL_PATH "include/orca")
  ...
  set(_IDE_LIBRARY_PATH "${_IDE_LIBRARY_BASE_PATH}/orca")
  set(_IDE_PLUGIN_PATH "${_IDE_LIBRARY_BASE_PATH}/orca/plugins")
  set(_IDE_LIBEXEC_PATH "${CMAKE_INSTALL_LIBEXECDIR}/orca")
  set(_IDE_DATA_PATH "${CMAKE_INSTALL_DATAROOTDIR}/orca")
  set(_IDE_DOC_PATH "${CMAKE_INSTALL_DATAROOTDIR}/doc/orca")
...
function(finalize_test_setup test_name)
  ...
  if (WIN32)
    ...
    list(APPEND env_path $<TARGET_FILE_DIR:Qt6::Test>)
    if (TARGET libclang)
    ...
    if (NOT ${target_type} STREQUAL "OBJECT_LIBRARY")
      ...
      if (NOT TARGET ${PROJECT_NAME}PchGui AND NOT TARGET ${PROJECT_NAME}PchConsole)
        ...
        _add_pch_target(${PROJECT_NAME}PchGui
          "${Orca_SOURCE_DIR}/src/shared/orca_gui_pch.h" Qt6::Widgets)
        _add_pch_target(${PROJECT_NAME}PchConsole
          "${Orca_SOURCE_DIR}/src/shared/orca_pch.h" Qt6::Core)
      endif()

      unset(PCH_TARGET)
      if ("Qt6::Widgets" IN_LIST dependencies)
        set(PCH_TARGET ${PROJECT_NAME}PchGui)
      elseif ("Qt6::Core" IN_LIST dependencies)
...
function(extend_qtc_target target_name)
  ...
  if (APPLE AND ORCA_BUILD_WITH_PCH)
    foreach(source IN LISTS _arg_SOURCES)
